...
const Layout = (props: { children: any; title?: string; user?: User | null; hideNav?: boolean; announcement?: string; isAdmin?: boolean }) => {
  ...
  // Allow Right Click on Inputs and Video
  ...
  <html lang="my">
    <head>
      ...
      {/* 🔥 HLS.JS (For m3u8 Support in Native Player) 🔥 */}
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 REVERT TO NATIVE PLAYER + HLS SUPPORT + EXTRA DL BUTTON 🔥
             window.loadPlayer = async function(content, type, movieId, title, poster) {
                ...
                const dlBtnContainer = document.getElementById('extra-dl-btn-container');
                ...
                // Create Extra Download Button
                if(dlBtnContainer) {
                    dlBtnContainer.innerHTML = '<a href="'+finalUrl+'" download target="_blank" class="w-full bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-3 px-4 rounded border border-zinc-700 flex items-center justify-center gap-2"><i class="fa-solid fa-download"></i> Download Video</a>';
                    dlBtnContainer.style.display = 'block';
                }

                // If embed/iframe
                if (type === 'embed' || finalUrl.includes('<iframe')) { 
                    let htmlContent = finalUrl.includes('<iframe') ? finalUrl : '<iframe src="'+finalUrl+'" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>';
                    container.innerHTML = htmlContent; 
                    container.style.display = 'block';
                    if(loader) loader.style.display = 'none';
                    if(dlBtnContainer) dlBtnContainer.style.display = 'none'; // No DL for embed
                    return;
                }

                // Native Player with HLS check
                container.innerHTML = '<video id="native-vid" controls autoplay class="w-full h-full" poster="'+poster+'"><source src="'+finalUrl+'" type="video/mp4"></video>';
                container.style.display = 'block';
                const video = document.getElementById('native-vid');
                ...
                video.addEventListener('error', (e) => {
                    if(loader) loader.style.display = 'none';
                    container.innerHTML = '<div class="flex flex-col items-center justify-center h-full bg-[#111] text-center p-6 border border-red-900/50 rounded"><i class="fa-solid fa-triangle-exclamation text-4xl text-yellow-500 mb-3"></i><p class="text-white font-bold mb-2">Browser Cannot Play This Format</p><p class="text-xs text-gray-400 mb-4">This video is likely .MKV which browsers don\'t support.</p><a href="'+finalUrl+'" target="_blank" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded font-bold transition flex items-center gap-2"><i class="fa-solid fa-download"></i> Download to Watch</a></div>';
                });
           ...
           <div class="p-4">
               ...
               {premium && (
                   <div class="flex flex-col gap-2 mb-6">
                        {/* 🔥 EXTRA DOWNLOAD BUTTON CONTAINER (Injected by JS) 🔥 */}
                        <div id="extra-dl-btn-container" class="hidden"></div>
...
app.post("/admin/config/vip", adminAuth, async (c) => { const body = await c.req.parseBody(); const days = parseInt(String(body['days'])); const targetDate = Date.now() + (days * 86400000); const current = await getConfig(); await kv.set(["config"], { ...current, globalVipExpiry: targetDate }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Global VIP Event Started"); });
app.post("/admin/config/vip-clear", adminAuth, async (c) => { const current = await getConfig(); await kv.set(["config"], { ...current, globalVipExpiry: 0 }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Global Event Cleared"); });

// RE-INDEX ACTION
app.post("/admin/config/reindex", adminAuth, async (c) => { await reIndexDatabase(); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Database Optimized!"); });
