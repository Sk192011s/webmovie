...
if (!ADMIN_PASS) {
  ADMIN_PASS = "Soekyawwin@93"; 
  console.log("⚠️ WARNING: ADMIN_PASSWORD not set. Using default: Soekyawwin@93");
}
...
interface Movie {
  ...
  category: "Movies" | "Series" | "Animation" | "Adult" | "All Uncensored"; description: string; tags: string;
...
async function saveMovieDB(movie: Movie) {
    ...
    // အရင်ရှိပြီးသားလား စစ်မယ် (ရှိပြီးသား Edit လုပ်တာဆိုရင် အရေအတွက် မတိုးဘူး)
    const old = await kv.get<Movie>(["movies", movie.id]);
    
    const op = kv.atomic();
    
    if (!old.value) {
        // ဇာတ်ကားအသစ်ဆိုရင် အရေအတွက် +1 တိုးမယ်
        op.sum(["counts", movie.category], 1n);
    } else if (old.value.category !== movie.category) {
        // Category ပြောင်းလိုက်ရင် (ဥပမာ Movies ကနေ Series ပြောင်းရင်)
        op.sum(["counts", old.value.category], -1n); // အဟောင်းထဲကနှုတ်
        op.sum(["counts", movie.category], 1n);      // အသစ်ထဲမှာပေါင်း
        
        // အဟောင်း Index တွေဖျက်
...
async function deleteMovieDB(id: string) {
    ...
    // ဇာတ်ကားဖျက်ရင် အရေအတွက် -1 လျှော့မယ်
...
async function reIndexDatabase() {
    // ၁. ရှိသမျှ Counter အဟောင်းတွေကို အရင် Reset ချမယ် (သုည ပြန်လုပ်မယ်)
    ...
    // ၂. ဇာတ်ကားတွေကို ပြန် Save ပြီး Index တွေ ပြန်စီမယ်
    const iter = kv.list<Movie>({ prefix: ["movies"] });
    // ရေတွက်ဖို့အတွက် ယာယီ variable
    const countMap: Record<string, number> = {};

    for await (const res of iter) {
        const m = res.value;
        await saveMovieDB(m); // Index တွေ ပြန်စီတာပါ

        // လက်ရှိ Category ကို မှတ်မယ်
        const cat = m.category;
        countMap[cat] = (countMap[cat] || 0) + 1;
    }
    
    // ၃. ရေတွက်ရသမျှကို Database ထဲ အတည်ပြု သိမ်းမယ်
    ...
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        /* 🔥 APP STYLE BOTTOM NAV (Centered Icons) */
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.switchSeason = function(seasonId) {
                 ...
                 // အခုနှိပ်လိုက်တဲ့ဟာက ဖွင့်နေပြီးသားလား စစ်မယ်
                 const isAlreadyActive = !targetGrid.classList.contains('hidden');

                 // ၁. အရင်ဆုံး အားလုံးကို ပိတ်လိုက်မယ် (Reset)
                 ...
                 // ၂. အကယ်၍ အရင်က ပိတ်နေတာဆိုရင် အခု ပြန်ဖွင့်မယ် (Toggle On)
                 // (အရင်က ပွင့်နေပြီးသားဆိုရင် အပေါ်ကအဆင့်မှာ ပိတ်သွားပြီမို့ ဘာမှဆက်မလုပ်တော့ဘူး -> Toggle Off)
                 if (!isAlreadyActive) {
...
app.get("/category/:cat", async (c) => { 
    ...
    // 🔥 အရင်က Loop ပတ်ရေတာကို ဖြုတ်ပြီး၊ DB ကနေ တိုက်ရိုက်ဖတ်လိုက်ပါပြီ (Instant Load)
        ...
        <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
            <div class="px-4 py-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <span class="bg-yellow-500 text-black text-[10px] px-2 py-1 rounded font-bold tracking-wider">
                        {totalCount}
                    </span>
                </div>
                    ...
                    {movies.map(m => (
                        <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'mb-4' : ''}`}>
                            <div class={`relative overflow-hidden ${isUncensored ? "aspect-video" : "aspect-[2/3]"}`}>
                                <img src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                            </div>
