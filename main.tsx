...
// Line 276 ဝန်းကျင်မှာ ဒါလေးထည့်ပါ
...
console.log("Playing:", finalUrl);
        ...
        // Variable အဟောင်းတွေကို ဖျက်ပြီး ဒါတွေကို အစားထိုးပါ
        let currentCursor = null; 
        let isInitialized = false; // ပထမဆုံးအကြိမ် စစ်ဆေးပြီးပြီလား
        let isLoading = false; 
        let hasMore = true; // နောက်ထပ် ကျန်သေးလား

        async function loadMoreMovies(category) { 
            // ၁။ ပထမဆုံးအကြိမ်ဆိုရင် Server ကပေးလိုက်တဲ့ cursor ကိုယူမယ်
            if (!isInitialized) {
                if (typeof window.initialCursor === "string") {
                    currentCursor = window.initialCursor;
                    // Server က Cursor အလွတ်ပေးလိုက်ရင် နောက်ထပ်မရှိတော့ဘူးလို့ မှတ်မယ်
                    if (!currentCursor) hasMore = false; 
                }
                isInitialized = true;
            }

            // ၂။ Loading ဖြစ်နေရင် (သို့) နောက်ထပ်မရှိတော့ရင် (သို့) Cursor မရှိရင် အလုပ်မလုပ်ဘူး
            // ဒီအချက်က ဇာတ်ကားနည်းတဲ့အခါ ထပ်မတက်အောင် တားပေးပါလိမ့်မယ်
            if(isLoading || !hasMore || !currentCursor) return; 
            
            isLoading = true; 
            const grid = document.getElementById('movie-grid');
            
            // Loading Skeleton ပြမယ်
            const skeletons = [];
            for(let i=0; i<6; i++) {
            ...
            try { 
                ...
                // ၃။ နောက်ထပ် ဆွဲစရာကျန်သေးလား update လုပ်မယ်
                if (data.nextCursor) {
                    currentCursor = data.nextCursor;
                } else {
                    currentCursor = null; 
                    hasMore = false; // ကုန်သွားပြီ၊ ထပ်မဆွဲနဲ့တော့
                }

                if(data.movies.length === 0) return; 

                // ဇာတ်ကားတွေကို Grid ထဲထည့်မယ်
...
// Line 661 ဝန်းကျင်က /api/list အဟောင်းကိုဖျက်ပြီး ဒါထည့်ပါ
app.get("/api/list", async (c) => { 
    const cat = c.req.query("cat") || "Movies"; 
    const cursor = c.req.query("cursor") || ""; 
    const limit = 15; 
    
    // Function အသစ်ကို ခေါ်လိုက်ပြီ
...
// Line 674 ဝန်းကျင်က /category/:cat အဟောင်းကိုဖျက်ပြီး ဒါထည့်ပါ
app.get("/category/:cat", async (c) => { 
    const user = await getCurrentUser(c); 
    const cat = c.req.param("cat"); 
    const config = await getConfig(); 
    
    // Function အသစ်သုံးပြီး nextCursor ကိုပါ ယူမယ်
    ...
    <Layout 
        ...
        globalExpiry={config.globalVipExpiry}
    >
    {/* ဒီနေရာမှာ Cursor ကို JavaScript ထဲ လှမ်းထည့်ပေးလိုက်တယ် */}
