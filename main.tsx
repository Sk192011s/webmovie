...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        // URL ကနေ Fetch မလား၊ Text အတိုင်းလား စစ်မယ်
        if (inputData.startsWith("http://") || inputData.startsWith("https://")) {
            try {
                const res = await fetch(inputData);
                movies = await res.json();
            } catch (err) {
                return c.redirect(ADMIN_ROUTE + "/dashboard?error=Failed to fetch JSON from URL");
            }
        } else {
            movies = JSON.parse(inputData);
        }

        if (!Array.isArray(movies)) throw new Error("Format must be an array []");

        // UI မှာ အသစ်ဆုံးရောက်အောင် ပြောင်းပြန်လှန်မယ်
        movies.reverse();

        // ID ထပ်မထွက်အောင် လုပ်ပေးမယ့် Function
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // ----------------------------------------------------
                // 🔥 LOGIC: JSON ပုံစံကို မူတည်ပြီး Data ခွဲထုတ်ခြင်း
                // ----------------------------------------------------
                
                let title = "", poster = "", cover = "", desc = "", duration = "";
                let finalLink = "", category = "";

                // (A) JAV JSON Format ဖြစ်မဖြစ် စစ်မယ် (photo1 သို့မဟုတ် name ပါရင် Jav)
                if (m.name || m.photo1 || m.dapp) {
                    title = m.name || "";
                    poster = m.photo1 || "";
                    cover = m.photo2 || m.photo1 || ""; // photo2 မရှိရင် poster ပဲသုံးမယ်
                    desc = m.info || "";
                    category = "Jav";

                    // 🔥 LINK SELECTION (HD > SD)
                    // dapp (HD) ရှိပြီး mediafire ဖြစ်ရင် ယူမယ်
                    if (m.dapp && m.dapp.includes("mediafire.com")) {
                        finalLink = m.dapp;
                    } 
                    // HD မရှိရင် download (SD) ကို စစ်မယ်
                    else if (m.download && m.download.includes("mediafire.com")) {
                        finalLink = m.download;
                    }
                } 
                // (B) MYANMAR / Standard JSON Format (Title ပါရင်)
                else if (m.title) {
                    title = m.title || "";
                    poster = m.image || "";
                    cover = m.image || "";
                ...
                // ခေါင်းစဉ်မရှိရင် (သို့) MediaFire Link လုံးဝမပါရင် ကျော်သွားမယ် (Auto ပယ်သည်)
                if (!title || !finalLink) return;

                const uniqueKey = finalLink; // Link ကို ID အဖြစ်သုံးမယ် (ထပ်မသွားအောင်)
                const id = await createUniqueId(uniqueKey);
                
                // Server 1 & Download 1 အတွက် Proxy ခံမယ်
                const proxyLink = "https://proxy.avotc.tk/" + finalLink; 
                ...
                const movieData: Movie = {
                    ...
                    posterUrl: poster,
                    coverUrl: cover, 
                    category: category, // Jav သို့မဟုတ် Myanmar and Asian
                    description: desc,
                    tags: title,
                    ...
                    // Streaming Links
                    streamUrl: proxyLink,        // Server 1 (Direct)
                    streamUrl2: finalLink,       // Server 2 (Original)
                    
                    // 🔥 Download Links (Proxy ခံထားလို့ Direct DL ဖြစ်သွားမယ်)
                    downloadUrl: proxyLink,      // Download 1
                    downloadUrl2: finalLink,     // Download 2 (Backup)
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Auto Filtered)");
