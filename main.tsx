...
// 🔥 MEDIAFIRE STREAMER (Advanced Proxy Tunneling Fix)
app.get("/api/mediafire-stream", async (c) => {
    const url = c.req.query("link");
    if (!url) return c.text("Missing Link", 400);

    // 1. မူရင်း MediaFire Link ကို သန့်ရှင်းရေးလုပ်မယ်
    let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();
    
    // 2. Deno ကနေ MediaFire ကို တိုက်ရိုက်မခေါ်ဘဲ Proxy ကနေတဆင့် ခေါ်မယ်
    // (ဒါမှ Link ထုတ်တဲ့ IP နဲ့ ကြည့်တဲ့ IP တူမှာပါ)
    const proxyFetchUrl = "https://proxy.avotc.tk/" + cleanUrl;
    ...
    try {
        ...
        if (match && match[1]) {
            ...
            // 4. ရလာတဲ့ Link မှာ Proxy မပါရင် ထည့်ပေးမယ်
            if (!directLink.includes("proxy.avotc.tk")) {
                 directLink = "https://proxy.avotc.tk/" + directLink;
            }

            // 5. User ဆီ ပြန်ပို့မယ် (IP တူသွားလို့ VPN မလိုတော့ပါ)
            return c.redirect(directLink, 302);
        } else {
            // Link ရှာမတွေ့ရင် Proxy Webpage ဆီပဲ ပို့မယ် (လက်နဲ့နှိပ်ဒေါင်းလို့ရအောင်)
            return c.redirect(proxyFetchUrl, 302);
        }
    } catch (e) {
