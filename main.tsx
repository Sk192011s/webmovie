    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        async function loadMoreMovies(category) { 
            ...
            // Skeleton (အဖြူကွက်လပ်) ဖန်တီးခြင်း
            for(let i=0; i<6; i++) {
                ...
                // 🔥 (၁) Skeleton အတွက် 4K Porns ထည့်ခြင်း
            ...
            try { 
                ...
                data.movies.forEach(m => { 
                    ...
                    // 🔥 (၂) Badge အတွက် 4K Porns ထည့်ခြင်း
                    const badge = (m.category === "4K Movies" || m.category === "4K Porns") ? "4K" : "HD";

                    // 🔥 (၃) Card ပုံစံအတွက် 4K Porns ထည့်ခြင်း (Wide Card)
                    if(category === "All Uncensored" || category === "Myanmar and Asian" || category === "4K Porns") { 
...
app.get("/", async (c) => {
  ...
  // 🔥 Category တွေသတ်မှတ်တဲ့နေရာ (4K Porns ပါ ထည့်ထားသည်)
  const sections = [
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
      ...
      {/* 🔥 ပြင်ထားသော Main Section (See All + Wide Card + 4K Badge) */}
      <div class="px-4 space-y-10 pb-8">
          {sections.map(section => { 
              ...
              // 🔥 4K Porns ကိုပါ Wide Card (ပုံအကြီး) နဲ့ပြဖို့ Condition ထည့်ထားသည်
              if(cat === "All Uncensored" || cat === "Myanmar and Asian" || cat === "4K Porns") { 
                  return (
                    <div>
                        {/* 🔥 See All Header (အသစ်ပြန်ထည့်ပေးထားသည်) */}
                        <div class="flex justify-between items-end mb-4">
                            <h2 class="text-lg font-bold text-white border-l-4 border-yellow-500 pl-3">{cat}</h2>
                            <a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1 hover:text-white transition">{t.see_all} <i class="fa-solid fa-chevron-right text-[10px]"></i></a>
                        </div>
              ...
              // 🔥 ကျန်တဲ့ Category တွေအတွက် မူလပုံစံ (Portrait)
              return (
                <div>
                    <div class="flex justify-between items-end mb-4">
                        <h2 class="text-lg font-bold text-white border-l-4 border-yellow-500 pl-3">{cat}</h2>
                        ...
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform movie-card group">
                                <div class="aspect-[2/3] w-full relative overflow-hidden">
                                    ...
                                    {/* 🔥 4K Badge Logic for Portrait */}
                                    <div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow">
                                        {m.category === "4K Movies" || m.category === "4K Porns" ? "4K" : "HD"}
                                    </div>
                                </div>
