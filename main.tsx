...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    // 👇👇👇 ဒီ ၂ ကြောင်း ထည့်လိုက်ရင် List မှာ ကားပျောက်တာ လုံးဝ မဖြစ်တော့ပါ 👇👇👇
    c.header('Cache-Control', 'private, no-cache, no-store, must-revalidate');
    c.header('Expires', '-1');
    // 👆👆👆

    const iter = kv.list<Movie>({ prefix: ["movies"] });
    // ... ကျန်တဲ့ ကုဒ်တွေ မူလအတိုင်း ...
                        ...
                        <div class="lg:col-span-1 bg-[#111] p-5 rounded-2xl border border-zinc-800 h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl">
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="bg-zinc-800 p-3 rounded-xl mb-2 border border-zinc-700">
                                    ...
                                    <div class="space-y-2">
                                        <input id="gen-url" placeholder="Paste Link here and replace '01' with ***" class="input-box text-[10px] bg-black border-zinc-600 h-8" />
                                        
                                        <div class="flex gap-2">
                                            <input id="gen-season" placeholder="Season (e.g. 1)" value="1" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-1/3 text-center" />
                                            <input id="gen-start" type="number" placeholder="Start (1)" value="1" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-1/3 text-center" />
                                            <input id="gen-end" type="number" placeholder="End (16)" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-1/3 text-center" />
                                        </div>

                                        <button type="button" onclick="generateEpisodes()" class="w-full bg-green-700 hover:bg-green-600 text-white text-xs font-bold py-2 rounded transition">
                                            Generate Links
                                        </button>
                                    </div>

                                    {/* JAVASCRIPT LOGIC FOR GENERATOR */}
                                    <script dangerouslySetInnerHTML={{__html: `
                                        function generateEpisodes() {
                                            ...
                                            if (!urlTpl || !end) {
                                                alert("Please enter URL (with ***) and End Episode number!");
                                                return;
                                            }
                                            ...
                                            for (let i = start; i <= end; i++) {
                                                // 1 to 9 ကို 01, 09 ဖြစ်အောင်လုပ်မယ် (Padding)
                                                const numStr = i < 10 ? '0' + i : '' + i;
                                                
                                                // Link ထဲက *** နေရာမှာ ဂဏန်းအစားထိုးမယ်
                                                const finalUrl = urlTpl.replace('***', numStr);
                                                
                                                // Format: Season | Ep.XX | URL
                                ...
                                <div class="p-3 bg-black/40 rounded-xl border border-zinc-800">
                                    <label class="text-[10px] text-yellow-500 uppercase font-bold mb-1 block">Series Episodes</label>
                                    {/* ... အောက်က Textarea အဟောင်းအတိုင်း ဆက်ရှိပါစေ ... */}
                    ...
                    <div class="max-w-xl mx-auto bg-[#111] p-6 rounded-2xl border border-zinc-800 space-y-8 shadow-xl">
                        ...
                        <div class="border-t border-zinc-800 pt-8">
                            ...
                            <form action="/admin/config/popup" method="post" class="space-y-4">
                                ...
                                {/* 👇 စာရှည်ရှည်ရေးဖို့ Message Box အသစ် 👇 */}
                                <div>
                        ...
                        {/* 3. GLOBAL VIP EVENT (VIP အလကားပေးခြင်း) */}
                        <div class="border-t border-zinc-800 pt-8">
                        ...
                        {/* 4. DATABASE RE-INDEX (Database ပြန်စစ်ခြင်း) */}
                        <div class="border-t border-zinc-800 pt-8">
