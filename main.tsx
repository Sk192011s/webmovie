    ...
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        <div class="px-4 pb-4">
             <form action="/search" method="get" class="relative">
                 {/* 🔥 ICON ကို BUTTON ပြောင်းလိုက်ပါပြီ */}
      ...
      <div id="video-player" class="w-full h-full hidden"></div>
           ...
           <div class="p-6">
               ...
               {movie.category === "Series" && episodes.length > 0 && premium && ( 
                   <div class="mb-8">
                       {/* Ungrouped Episodes (Season မခွဲထားသော အပိုင်းများ ရှိလျှင်) */}
                       {ungrouped.length > 0 && (
                           <div class="grid grid-cols-3 md:grid-cols-4 gap-2 mb-3">
                               {ungrouped.map(ep => (
                                   <button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn bg-zinc-800 hover:bg-yellow-500 hover:text-black text-xs py-2 px-1 rounded-lg truncate text-center border border-zinc-700 transition-colors font-bold">
                                       {ep.name}
                                   </button>
                               ))}
                           </div>
                       ...
                       {/* 🔥 SEASONS GRID (3 COLUMNS) 🔥 */}
                       {/* 🔥 SEASONS GRID (3 COLUMNS) 🔥 */}
                       <div class="grid grid-cols-3 gap-2">
                           {Object.keys(seasons).map(season => { 
                               const safeId = season.replace(/\s+/g, '-'); 
                               return (
                                   <div class="border border-zinc-800 rounded-lg bg-[#1f1f1f] h-fit overflow-hidden">
                                       {/* Season Header Button */}
                                       <button onclick={`toggleSeason('${safeId}')`} class="w-full flex justify-between items-center p-2 text-xs font-bold text-gray-300 hover:bg-zinc-800 transition">
                                           <span class="truncate">{season}</span>
                                           <i id={`icon-${safeId}`} class="fa-solid fa-chevron-down text-[10px] transition-transform"></i>
                                       </button>
                                       
                                       {/* Episodes List (Grid 3 Columns inside Season) */}
                                       <div id={`season-${safeId}`} class="hidden p-1.5 border-t border-zinc-800 bg-black/40">
                                            {/* 🔥 ဒီနေရာမှာ grid-cols-3 ပြောင်းလိုက်ပါပြီ */}
                                            <div class="grid grid-cols-3 gap-1">
                                               {seasons[season].map(ep => (
                                                   <button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn w-full bg-zinc-800 hover:bg-yellow-500 hover:text-black text-[9px] py-1.5 px-0.5 rounded truncate text-center border border-zinc-700 transition-colors">
                                                       {ep.name}
                                                   </button>
                                               ))}
                                            </div>
