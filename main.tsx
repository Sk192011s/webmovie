    ...
    try {
        ...
        if (inputData.startsWith("http://") || inputData.startsWith("https://")) {
            try {
                ...
                // 🔥 SUPER FIX: JSON Error တက်စေမည့် အရာများကို ရှင်းမယ်
                
                // 1. Enter ခေါက်ထားတဲ့ စာကြောင်းတွေကို Space နဲ့ အစားထိုးမယ် (ဒါအရေးကြီးဆုံး)
                // (JSON ထဲမှာ Enter ခေါက်ထားရင် Parse မရလို့ပါ)
                rawText = rawText.replace(/[\r\n]+/g, ' ');

                // 2. နောက်ဆုံးပိတ် ကော်မာ (Trailing Comma) တွေကို ဖြုတ်မယ်
                rawText = rawText.replace(/,\s*([\]}])/g, '$1');

                // 3. Array [] မပါရင် ထည့်ပေးမယ်
                ...
                return c.redirect(ADMIN_ROUTE + "/dashboard?error=JSON Error: Description တွေမှာ Enter ခေါက်မရေးရပါ။");
            }
        } else {
            // Textarea အတွက်လည်း အလားတူ ရှင်းမယ်
            let rawText = inputData.replace(/[\r\n]+/g, ' '); // Enter ဖျောက်
            rawText = rawText.replace(/,\s*([\]}])/g, '$1'); // ကော်မာပို ဖျောက်
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Auto Fixed JSON)");
