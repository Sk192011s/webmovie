    ...
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
      ...
      <div class="px-4 space-y-10 pb-8">
          {sections.map(section => { 
              ...
              if(cat === "All Uncensored" || cat === "Myanmar and Asian") { 
                  return (
                    <div>
                        ...
                        <div class="h-scroll-section">
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded-xl overflow-hidden active:scale-95 transition-transform movie-card group">
                                    <div class="aspect-video w-full relative overflow-hidden"><img src={m.coverUrl || m.posterUrl} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" /><div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center"><i class="fa-solid fa-circle-play text-4xl text-white drop-shadow-lg"></i></div></div>
...
app.get("/category/:cat", async (c) => { 
    ...
    const isUncensored = cat === "All Uncensored" || cat === "Myanmar and Asian"; 
    const lang = getLang(c);
        ...
        <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
            <div class="p-4">
                ...
                <div class="grid grid-cols-3 gap-3">
                    {results.map(m => {
                        const isWide = m.category === "All Uncensored" || m.category === "Myanmar and Asian";
                        return (
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex gap-2 w-full">
                                        <select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">
                                            {["Movies","Series","4K Movies","Animation","Jav","All Uncensored", "Myanmar and Asian"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}
                                        </select>
