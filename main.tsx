...
// =======================
// 4. UI LAYOUT (MODERN SEARCH & PREMIUM PROFILE)
// =======================
  ...
  <html lang={l}>
    <head>
      ...
      <style>{`
        ...
        /* 🔥 BACKGROUND: Solid Dark Gray (No Gradient) */
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        #instant-results { overflow-y: auto; flex-grow: 1; margin-top: 10px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; padding-bottom: 50px; align-content: start; }
        ...
        /* Removed dark gradient overlay to show original color */
                     ...
                     searchTimeout = setTimeout(async () => {
                         try {
                             ...
                             if(data.results.length === 0) {
                                 ...
                                 // 🔥 UPDATE: Vertical Grid with Title on Bottom Overlay
                                 resultsContainer.innerHTML = data.results.map(m => \`
                                     <a href="/movie/\${m.id}" class="block relative rounded-xl overflow-hidden aspect-[2/3] group shadow-lg">
                                         <img src="\${m.posterUrl}" class="w-full h-full object-cover transition duration-500 group-hover:scale-110" />
                                         <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-100"></div>
                                         <div class="absolute bottom-0 left-0 right-0 p-2.5">
                                             <p class="text-[11px] font-bold text-white leading-tight line-clamp-2 drop-shadow-md">\${m.title}</p>
                                         </div>
                                     </a>
                                 \`).join('');
                             }
    ...
    <body>
      ...
      {/* 🔥 NEW SEARCH OVERLAY (FULL SCREEN) */}
      <div id="search-overlay">
          ...
          <form action="/search" method="get" class="relative mb-6">
              ...
              <input id="search-input-main" name="q" placeholder="Start typing..." autocomplete="off" class="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white text-lg focus:border-purple-500 outline-none transition" />
          </form>
...
// 🔥 NEW API FOR LIVE SEARCH
app.get("/api/search", async (c) => {
    const query = c.req.query("q")?.toLowerCase() || "";
    if (query.length < 1) return c.json({ results: [] });
    
    // We reuse the existing search logic but return JSON
    const results = await searchMoviesDB(query);
    
    // Return only necessary data to be light
...
app.get("/", async (c) => {
  ...
  // 🔥 FIX: Browser Cache ဖြုတ်ထားသည် (Logout ပြဿနာရှင်းရန်)
...
app.get("/movie/:id", async (c) => {
    ...
    if (canWatch) {
        // 🔥 UPDATE: Just store the RAW URL. Don't resolve here. It's too slow.
        // The API will resolve it when the player loads.
...
// 🔥 AUTH ROUTES (Myanmar Text Hardcoded)
app.get("/login", (c) => {
    const lang = getLang(c);
    const t = i18n['my'];
    return c.html(
      <Layout hideNav={true} lang={lang} activeTab="me">
        <div class="min-h-[80vh] flex items-center justify-center bg-black p-4">
            <div class="w-full max-w-sm">
                ...
                <form action="/login" method="post" class="bg-[#1f1f1f] p-8 rounded-2xl border border-zinc-800 space-y-5 shadow-2xl">
...
app.post("/login", async (c) => { 
    ...
    // 🔥 SECURITY: Login Rate Limit Check
    ...
    return c.html(
        <Layout hideNav={true} lang={lang}>
            <div class="min-h-screen flex items-center justify-center bg-black p-4">
                <div class="w-full max-w-sm bg-[#1f1f1f] p-8 rounded-2xl border border-zinc-800 shadow-xl">
    ...
    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-6 pb-24 pt-4">
            <div class="relative w-full aspect-[1.8/1] rounded-3xl overflow-hidden shadow-2xl group border border-yellow-500/30">
                {/* Gold Gradient Background */}
                <div class="absolute inset-0 bg-gradient-to-br from-yellow-600 via-yellow-400 to-yellow-700"></div>
                <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"></div>
                <div class="absolute -right-10 -bottom-10 opacity-20 transform rotate-12"><i class="fa-solid fa-crown text-9xl text-white"></i></div>
                
                <div class="relative z-10 h-full flex flex-col justify-between p-6">
                     <div class="flex justify-between items-start">
                         <div class="flex items-center gap-4">
                             <div class="w-16 h-16 bg-black/20 backdrop-blur-sm rounded-full flex items-center justify-center text-3xl font-black border-2 border-white/40 text-white shadow-lg">{user.username[0].toUpperCase()}</div>
                             <div>
                                <h2 class="text-2xl font-black tracking-tight leading-none drop-shadow-md text-white">{user.username}</h2>
                                <span class="text-[10px] font-bold bg-black/40 text-yellow-300 px-3 py-1 rounded-full inline-block mt-1.5 uppercase tracking-widest border border-yellow-300/30 backdrop-blur-sm shadow-sm">{statusText}</span>
                             </div>
                         </div>
                         <i class="fa-solid fa-rss text-2xl text-white/50"></i>
                     </div>
                     <div class="font-mono text-white/90">
                        <div class="flex justify-between items-end">
                            <div>
                                <p class="text-[9px] uppercase opacity-80 font-bold mb-1 tracking-widest">Membership ID</p>
                                <p class="text-lg font-bold tracking-widest drop-shadow-sm">{memberId}</p>
                            </div>
                            <button onclick={`copyToClip('${memberId}')`} class="w-8 h-8 flex items-center justify-center bg-black/20 rounded-lg hover:bg-black/30 transition text-sm backdrop-blur text-white"><i class="fa-regular fa-copy"></i></button>
                        </div>
                     </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                 <a href="/favorites" class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-white/5 transition active:scale-95 border border-white/5 bg-[#1e293b]/50"><div class="w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center text-red-400 text-lg shadow-inner"><i class="fa-solid fa-heart"></i></div><span class="block text-xl font-black text-white leading-none">{favCount}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Saved</span></a>
                 <div class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2 border border-white/5 bg-[#1e293b]/50"><div class="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 text-lg shadow-inner"><i class="fa-solid fa-coins"></i></div><span class="block text-xl font-black text-white leading-none">{userCoins}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Kyats</span></div>
                 <div class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2 border border-white/5 bg-[#1e293b]/50"><div class={`w-10 h-10 rounded-full flex items-center justify-center text-lg shadow-inner ${premium ? 'bg-green-500/20 text-green-400' : 'bg-gray-700/20 text-gray-500'}`}><i class="fa-solid fa-clock"></i></div><span class={`block text-xl font-black leading-none ${premium ? 'text-white' : 'text-gray-500'}`}>{premium ? finalDays : "0"}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Days Left</span></div>
            </div>

            <div class="glass-panel p-6 rounded-3xl relative overflow-hidden group border border-purple-500/20 bg-[#1e293b]/30">
                 <div class="absolute -right-6 -top-6 opacity-10 group-hover:opacity-20 transition duration-500"><i class="fa-solid fa-ticket text-9xl text-purple-500"></i></div>
                 ...
                 <form action="/profile/redeem" method="post" class="relative z-10 flex gap-3">
                    <input name="key" placeholder="Enter VIP or Coin Code..." required class="input-box font-mono tracking-wide bg-black/40 border-white/10 focus:border-purple-500" />
                    <button class="bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold px-6 rounded-xl hover:brightness-110 transition shadow-lg shadow-purple-900/30"><i class="fa-solid fa-arrow-right"></i></button>
                 </form>
            </div>
            
            <div class="glass-panel p-6 rounded-3xl relative overflow-hidden border border-blue-500/20 bg-[#1e293b]/30">
                 ...
                 <form action="/profile/topup" method="post" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <label class="cursor-pointer relative group">
                            ...
                            <div class="p-4 rounded-2xl bg-black/40 border border-white/10 peer-checked:border-orange-500 peer-checked:bg-orange-500/10 transition text-center hover:bg-white/5 group-active:scale-95 h-full flex flex-col justify-center items-center">
                        ...
                        <label class="cursor-pointer relative group">
                            ...
                            <div class="p-4 rounded-2xl bg-black/40 border border-white/10 peer-checked:border-yellow-400 peer-checked:bg-yellow-400/10 transition text-center hover:bg-white/5 group-active:scale-95 h-full flex flex-col justify-center items-center">
                    ...
                    <input type="number" name="amount" placeholder="Amount (Ks)" required class="input-box bg-black/40 border-white/10 focus:border-blue-500" />
                    <input name="transactionId" placeholder="Transaction ID / Slip Link" required class="input-box bg-black/40 border-white/10 focus:border-blue-500" />
                ...
                <div id="tab-movies" class="tab-content">
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-2 bg-[#111] p-5 rounded-2xl border border-zinc-800 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw] shadow-xl">
                            ...
                            {/* 🔥 ADMIN: Bulk Import Feature Added */}
