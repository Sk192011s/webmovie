    ...
    try {
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // Format Logic (Jav / USA / Myanmar)
                if (m.name || m.photo1 || m.dapp) { // JAV
                ...
                } else if (m.profileImg && m.files) { // USA (New)
                ...
                // Priority: Proxy > R2 > Deno > MediaFire
                ...
                if (target.includes("mediafire.com") && !target.includes("proxy.avotc.tk")) {
                    if (target.includes("premium")) {
                        // Premium -> Direct Proxy
                        server1 = addProxy(target);
                        dl1 = addProxy(target);
                    } else {
                        // Standard -> API Scraper (Tunneling)
                        server1 = addProxy(target); // Play will likely use API if this fails, but better to set fallback
                        dl1 = "/api/mediafire-stream?link=" + encodeURIComponent(target);
                        // Note: For Player, we rely on the client or API. Let's set Server 1 to API for maximum safety
                        server1 = "/api/mediafire-stream?link=" + encodeURIComponent(target); 
                    }
                } else {
                    // Direct / R2 / Already Proxied
                    server1 = addProxy(target);
                    dl1 = addProxy(target);
                }
                
                server2 = target; // Backup is raw
                dl2 = target;
