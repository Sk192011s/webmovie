      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 🔥 FIX: MEDIAFIRE အတွက် EXTERNAL API ပြန်သုံးမယ် (Deno IP Block ကို ရှောင်ရန်)
                if (finalUrl.includes('mediafire.com')) {
                    finalUrl = 'https://mmaoapi.vercel.app/mediafire?link=' + finalUrl;
                } 
                else if (type === 'direct') { 
                    try { 
                ...
                // Native Video Tag
                const videoHtml = '<video id="main-video" controls autoplay playsinline class="w-full h-full" style="background-color:black;" poster="'+poster+'"><source src="'+finalUrl+'" type="video/mp4"></video>';
                ...
                if(btnElement && btnElement.dataset.next) {
                    const nextUrl = btnElement.dataset.next;
                    ...
                    document.getElementById('next-ep-btn').onclick = function() {
                        const nextEl = document.querySelector('[data-url="'+nextUrl+'"]');
...
// 🔥 SELF-HOSTED MEDIAFIRE EXTRACTOR (No 3rd Party API)
app.get("/api/mediafire", async (c) => {
    const url = c.req.query("link");
    if (!url) return c.text("Missing Link", 400);

    try {
        // 1. MediaFire Page ကို Browser အနေနဲ့ လှမ်းခေါ်မယ်
        ...
        // 2. HTML ထဲက Download Button ရဲ့ Link ကို Regex နဲ့ ရှာမယ်
        // MediaFire မှာ Download Link က id="downloadButton" ထဲမှာ ရှိတတ်တယ်
        ...
        if (match && match[1]) {
            // 3. တွေ့ရင် Video File အစစ်ဆီ Redirect လုပ်ပေးလိုက်မယ်
            return c.redirect(match[1]);
        } else {
            return c.text("MediaFire Extraction Failed: Link not found", 404);
        }
    } catch (e) {
        console.error(e);
...
// 🔥 BULK IMPORT (Supports JSON Text OR Direct URL)
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        // 🔥 URL ဖြစ်နေရင် Deno ကိုယ်တိုင် သွားဆွဲခိုင်းမယ် (Copy ကူးစရာမလိုတော့)
        if (inputData.startsWith("http://") || inputData.startsWith("https://")) {
            ...
            // URL မဟုတ်ရင် စာသားအတိုင်း Parse လုပ်မယ်
