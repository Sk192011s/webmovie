...
app.get("/profile", async (c) => { 
    ...
    return c.html(
     ...
     <form action="/profile/topup" method="post" class="space-y-4">
        ...
        {/* 🔥 PURPOSE SELECTION (အသစ်ထည့်လိုက်သည့်အပိုင်း) */}
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <label class="cursor-pointer relative">
                    ...
                    <div class="py-3 px-2 rounded-xl bg-black/50 border border-zinc-700 peer-checked:border-yellow-500 peer-checked:bg-yellow-900/20 transition text-center hover:bg-zinc-800">
                        ...
                        <span class="font-bold text-[10px] text-gray-300 peer-checked:text-white">Coins (4K) ဖြည့်ရန်</span>
                    </div>
                </label>
            </div>
        </div>
    ...
    {plans.map(p => (
        <div class={`relative p-5 rounded-2xl border-2 flex flex-col justify-between transition active:scale-95 shadow-lg
            ${p.popular 
                ? 'bg-gradient-to-br from-yellow-900/40 to-black border-yellow-500 shadow-yellow-900/20' // 🔥 Popular: ရွှေရောင် နောက်ခံ/ဘောင်
                : 'bg-zinc-900 border-zinc-700 hover:border-zinc-600' // 🔥 Normal: မီးခိုးရောင် အဖျော့/ဘောင်
            }`}>
...
app.post("/profile/topup", async (c) => {
    const user = await getCurrentUser(c);
    if (!user) return c.redirect("/login");
    const { amount, method, transactionId, purpose } = await c.req.parseBody(); // 🔥 purpose ကို ထည့်ယူသည်
    ...
    const topup: TopupRequest = {
        ...
        status: "pending",
        timestamp: Date.now(),
        purpose: String(purpose || "VIP Plan") // 🔥 Database ထဲထည့်သိမ်းသည်
    };
                ...
                <div class={`p-4 rounded-xl border flex justify-between items-center ${t.status === 'pending' ? 'bg-black border-blue-500/30' : 'bg-zinc-900 border-zinc-800 opacity-60'}`}>
                    ...
                    <div class="flex gap-2 items-center">
                        {t.status === 'pending' ? (
                            ...
                            // 🔥 Pending မဟုတ်ရင် (Approved/Rejected) ဖျက်လို့ရတဲ့ ခလုတ်
...
app.post("/admin/topup/approve", adminGuard, async (c) => {
    ...
    if (user) {
        ...
        // 🔥 LOGIC: VIP Plan လား Coin လား ခွဲခြားခြင်း
        if (topup.purpose === "VIP Plan") {
            
            // ဈေးနှုန်းနှင့် ရက် သတ်မှတ်ချက်များ
            ...
            if (daysToAdd) {
                // ✅ ပမာဏ ကိုက်ညီလျှင် VIP ရက်တိုးပေးမည်
                const now = Date.now();
                let currentExpiry = user.expiryDate ? new Date(user.expiryDate).getTime() : 0;
                
                // သက်တမ်းကုန်နေရင် လက်ရှိအချိန်ကနေ စရေမယ်
                ...
                // ⚠️ ပမာဏ မကိုက်ညီလျှင် (ဥပမာ 1000 Ks) Coin အဖြစ်ပဲ ထည့်ပေးလိုက်မည် (ပိုက်ဆံမဆုံးရှုံးစေရန်)
                user.coins = (user.coins || 0) + topup.amount;
                actionMessage = `Approved ${topup.amount}Ks (Plan Mismatch) for ${topup.username}`;
            }

        } else {
            // ✅ "Coins (4K)" ကိုရွေးထားလျှင် ပိုက်ဆံပဲ ပေါင်းပေးမည်
            user.coins = (user.coins || 0) + topup.amount;
...
// 🔥 1. DELETE SINGLE TOPUP (တစ်ခုချင်းဖျက်ရန်)
...
// 🔥 2. CLEAR HISTORY (Pending မဟုတ်သော အဟောင်းများအားလုံး ဖျက်ရန်)
app.post("/admin/topup/clear-history", adminGuard, async (c) => {
    const iter = kv.list<TopupRequest>({ prefix: ["topups"] });
    let count = 0;
    for await (const res of iter) {
        // Pending ဖြစ်နေရင် မဖျက်ဘူး (အလုပ်မလုပ်ရသေးလို့)
