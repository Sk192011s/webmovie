...
`;
  ...
  <html lang={l}>
    <head>
      ...
      {/* 🔥 UX: Plyr Player for Better Experience */}
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 UX: UPDATED PLAYER (PLYR) & NEXT EPISODE LOGIC
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                if(btnElement && btnElement.dataset.next) {
                    ...
                    // We render a hidden button or overlay for Next Episode
                    nextEpBtn = '<div id="next-ep-overlay" class="absolute bottom-16 right-4 z-[50] hidden"><button onclick="loadPlayer(\\''+nextUrl+'\\', \\''+type+'\\', \\''+movieId+'\\', \\''+title+'\\', \\''+poster+'\\', document.querySelector(\\'[data-url=\\\\\\"'+nextUrl+'\\\\\\"]\\'))" class="bg-white text-black font-bold px-4 py-2 rounded-full shadow-lg hover:scale-105 transition flex items-center gap-2 text-xs"><i class="fa-solid fa-forward-step"></i> Next Episode</button></div>';
                }

                container.innerHTML = '<div class="relative w-full h-full"><video id="plyr-player" controls playsinline data-poster="'+poster+'" class="w-full h-full"><source src="'+finalUrl+'" type="video/mp4"></video>' + nextEpBtn + '</div>';
                ...
                // Show Next Button on End
                player.on('ended', () => {
                    const nextOverlay = document.getElementById('next-ep-overlay');
                    if(nextOverlay) nextOverlay.classList.remove('hidden');
                });
    ...
    return c.html(
     ...
     <form action="/profile/topup" method="post" class="space-y-4">
        ...
        <div class="pt-2 space-y-3">
            <input type="text" name="method" value="KPay / Wave" readonly class="input-box bg-black/30" />
