...
// 🔥 ADMIN လမ်းကြောင်း (ကြိုက်ရာပြောင်းပါ)
...
// Update: globalVipExpiry ထည့်ထားသည်
...
// Update: Config ကိုပါ ထည့်စစ်ပေးမယ့် Logic
function isPremium(user: User | null, config: AppConfig) {
  const now = Date.now();
  // 1. Check Global Event
  if (config.globalVipExpiry && config.globalVipExpiry > now) {
      return true;
  }
  // 2. Check User Personal VIP
    ...
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        /* SPINNER FIX: Only show when active */
      ...
      <script dangerouslySetInnerHTML={{__html: `
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.showLoader = () => { loader.classList.add('active'); setTimeout(() => loader.classList.remove('active'), 5000); }; // Auto hide after 5s safety
             ...
             document.body.addEventListener('click', (e) => {
                ...
                if (link) {
                    ...
                    if (href && href.startsWith('/') && !href.includes('#') && target !== '_blank') window.showLoader();
                }
             });
             window.addEventListener('pageshow', window.hideLoader); // Fix for back button
...
app.get("/movie/:id", async (c) => {
    ...
    // UPDATED: Check global config VIP
...
app.get("/profile", async (c) => { 
    ...
    const config = await getConfig(); // Get global config
    const premium = isPremium(user, config); 
    ...
    if (premium) {
        if(isGlobal && globalDaysLeft > personalDaysLeft) statusText = `VIP Event Active (${globalDaysLeft} days left)`;
        else statusText = `VIP Active (${personalDaysLeft} days left)`;
    }
...
app.post(ADMIN_ROUTE + "/login", async (c) => { const { password } = await c.req.parseBody(); if (password === ADMIN_PASS) { setCookie(c, "admin_session", String(password), { path: "/", httpOnly: true, secure: !c.req.url.includes("localhost"), sameSite: "Lax" }); return c.redirect(ADMIN_ROUTE + "/dashboard"); } return c.redirect(ADMIN_ROUTE); });
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-zinc-900 min-h-screen">
                ...
                {/* TAB 1: MOVIES - FIXED LAYOUT & OVERLAP */}
                <div id="tab-movies" class="tab-content active">
                    <div class="grid lg:grid-cols-3 gap-6">
                        {/* Form is only sticky on LARGE screens (lg:sticky), regular flow on mobile */}
                ...
                {/* TAB 3: USERS */}
                <div id="tab-users" class="tab-content"><div class="max-w-xl mx-auto bg-[#1f1f1f] p-8 rounded border border-zinc-700 shadow-lg"><h2 class="font-bold mb-6 text-blue-500 text-xl"><i class="fa-solid fa-users-gear mr-2"></i> User Management</h2><div class="bg-black/50 p-6 rounded border border-zinc-800"><h3 class="font-bold text-white mb-4">Reset User Password</h3><form action="/admin/user/reset" method="post" class="space-y-4"><div><label class="block text-xs text-gray-400 mb-1">Target Username</label><input name="username" placeholder="Enter username..." required class="input-box" /></div><div><label class="block text-xs text-gray-400 mb-1">New Password</label><input name="newpass" placeholder="Enter new password..." required class="input-box" /></div><button class="bg-blue-600 text-white font-bold w-full py-3 rounded hover:bg-blue-700 transition">Reset Password</button></form></div></div></div>
                ...
                {/* TAB 5: CONFIG - ADDED GLOBAL VIP */}
                <div id="tab-config" class="tab-content">
                    <div class="max-w-xl mx-auto bg-[#1f1f1f] p-6 rounded border border-zinc-700 space-y-6">
                        <div>
                            <h2 class="font-bold mb-4 text-gray-300">Announcement Bar</h2>
                            <form action="/admin/config" method="post" class="space-y-4">
                                <div><label class="block text-xs text-gray-400 mb-1">Message</label><input name="text" placeholder="Enter message..." value={config.announcement} class="input-box" /></div>
                                <label class="flex items-center gap-2 p-3 bg-black rounded border border-zinc-800 cursor-pointer"><input type="checkbox" name="show" checked={config.showAnnouncement} class="accent-yellow-500 w-5 h-5" /><span class="font-bold text-sm">Show Bar</span></label>
                                <button class="btn-primary w-full">Save Message</button>
                            </form>
                        </div>
                        <div class="border-t border-zinc-800 pt-6">
                            <h2 class="font-bold mb-2 text-green-500"><i class="fa-solid fa-gift mr-2"></i> Global VIP Event</h2>
                            <p class="text-xs text-gray-500 mb-4">Set a specific number of days. Everyone (old & new users) will get VIP until that date. (Fixed Expiry Date)</p>
                            <div class="bg-black p-4 rounded border border-zinc-800 mb-4">
                                <span class="text-sm text-gray-400">Current Event Ends:</span> 
                                <span class="block text-xl font-bold text-white mt-1">{vipDate}</span>
                            </div>
                            <form action="/admin/config/vip" method="post" class="flex gap-2">
                                <input type="number" name="days" placeholder="Days from now (e.g. 30)" required class="input-box" />
                                <button class="bg-green-600 text-white font-bold px-4 rounded hover:bg-green-700">Set Event</button>
                            </form>
                            <form action="/admin/config/vip-clear" method="post" class="mt-2">
                                <button class="text-xs text-red-500 hover:text-red-400">Clear Global Event</button>
                            </form>
                        </div>
                    </div>
                </div>
...
// Admin Actions
app.post("/admin/config", adminAuth, async (c) => { const body = await c.req.parseBody(); const current = await getConfig(); await kv.set(["config"], { ...current, announcement: body['text'], showAnnouncement: body['show'] === 'on' }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Message Saved"); });
app.post("/admin/config/vip", adminAuth, async (c) => { 
    const body = await c.req.parseBody(); 
    const days = parseInt(String(body['days']));
    const targetDate = Date.now() + (days * 86400000); // Current Time + Days
...
app.post("/admin/config/vip-clear", adminAuth, async (c) => { 
    const current = await getConfig(); 
