        ...
        <div class="max-w-4xl mx-auto">
           ...
           <div class="p-6">
               ...
               <div class="flex flex-wrap items-center gap-2 mb-6">
                   ...
                   {/* 🔥 File Size ပြမည့်နေရာ (ရှိမှပြမယ်) */}
                   {movie.fileSize && (
                       <span class="bg-blue-900/30 text-blue-400 border border-blue-500/30 text-xs px-3 py-1 rounded-full flex items-center gap-1 font-bold">
                           <i class="fa-solid fa-file-arrow-down"></i> {movie.fileSize}
                       </span>
                   )}

                   {/* 🔥 Duration ပြမည့်နေရာ (ရှိမှပြမယ်) */}
                   {movie.duration && (
                       <span class="bg-purple-900/30 text-purple-400 border border-purple-500/30 text-xs px-3 py-1 rounded-full flex items-center gap-1 font-bold">
                   ...
                   {moviePrice > 0 ? (
                       isPurchased ? 
                       <span class="bg-green-900/30 text-green-400 px-3 py-1 rounded-full border border-green-500/30 font-bold text-xs"><i class="fa-solid fa-check"></i> Owned</span> :
                       <span class="bg-red-900/30 text-red-400 px-3 py-1 rounded-full border border-red-500/30 font-bold text-xs">{moviePrice} Ks</span>
                   ) : (
                       <span class="bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 px-3 py-1 rounded-full font-bold text-xs">VIP</span>
                   )}
               </div>
    ...
    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            ...
            {/* 🔥 My Purchased Movies Section (Updated Layout) */}
            {purchasedMovies.length > 0 && (
                <div class="pt-4 border-t border-zinc-800">
                    ...
                    {/* 🔥 Grid 2 Columns for Landscape Covers */}
                    <div class="grid grid-cols-2 gap-3">
                        {purchasedMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group relative">
                                {/* 🔥 Aspect Video (16:9) + Cover Image */}
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                {/* 🔥 ဒီနေရာမှာ required ကို ဖြုတ်လိုက်ပါပြီ */}
    ...
    {/* 🔥 Duration အကွက်အသစ် */}
    <input name="duration" placeholder="Duration (e.g. 25m)" value={editMovie?.duration} class="input-box w-full bg-black border-zinc-700" />
    <input type="number" name="price" placeholder="Price (Ks)" value={editMovie?.price || 0} class="input-box w-full bg-black border-yellow-500/50 text-yellow-500 font-bold" />
</div>
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    let description = body["description"] as string; // 🔥 Description ကို variable အဖြစ်ထုတ်လိုက်သည်
    let price = parseInt(String(body["price"] || "0"));

    // 🔥 4K Porns (Auto System)
    if (category === "4K Porns" && body["mode"] === "create") {
        
        // ၁. ဈေးနှုန်းသတ်မှတ်ခြင်း
        price = 50; 

        // ၂. Description အလိုအလျောက်ဖြည့်ခြင်း (အစ်ကို ဘာမှမရေးခဲ့ရင် ဒါတွေဝင်သွားမယ်)
        ...
        // ၃. Title Auto Increment (Live + Drafts စစ်ခြင်း)
        if (!title) {
            let maxNum = 0;
            // Live Movies စစ်သည်
    ...
    // Title မပါရင် Error ပြန်ပို့ (4K မဟုတ်သော အခြေအနေများ)
    if (!title) {
    ...
    // 🔥 ပြင်ဆင်ထားသော title, description နှင့် price ကို ဒီမှာ ထည့်သိမ်းလိုက်သည်
    const movie = { 
        ...body, 
        title: title, 
        description: description, // Update description here
        id: id, 
    ...
    if (saveType === "draft") { await kv.set(["drafts", id], movie); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts&success=Saved to Drafts (" + title + ")"); } else { await kv.delete(["drafts", id]); await saveMovieDB(movie as Movie); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Published Live!"); }
});
