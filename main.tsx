    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 SIMPLE PLAYER (Native Browser Loader + Proxy Support)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // Loader variable ကို ဖယ်လိုက်ပါပြီ
                ...
                // Loader.style.display ကုဒ်များကို ဖယ်လိုက်ပါပြီ (Browser မူရင်းအတိုင်းလည်ပါစေ)
                ...
                if (type === 'embed' || (type !== 'direct' && finalUrl.includes('<iframe'))) { 
                    const src = finalUrl.includes('<iframe') ? finalUrl : `<iframe src="${finalUrl}" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>`;
                    container.innerHTML = src;
                } else { 
                    // Video Tag (Background အမည်းထားပေးမယ်)
                    ...
                    if(video) { 
                        // 🔥 Iframe Fallback (မလာရင် Iframe ပြောင်းဖွင့်မယ်)
                        video.addEventListener('error', (e) => { 
                            console.log("Video failed, trying iframe mode...");
                            container.innerHTML = `<iframe src="${finalUrl}" width="100%" height="100%" frameborder="0" allowfullscreen class="w-full h-full bg-black"></iframe>`;
                        });
...
// 🔥 OPTIMIZED MOVIE DETAIL ROUTE (4 Related, Cached Config, Proxy Player)
           ...
           <div class="w-full aspect-video bg-black relative shadow-lg group rounded-xl overflow-hidden border border-zinc-800">
                {canWatch ? (
                    <>
                        ...
                        <div class="max-w-4xl mx-auto">
           {/* 🔥 VIDEO PLAYER (Loader ဖြုတ်ထားသည် - Browser မူရင်းအတိုင်းပြမည်) */}
           <div class="w-full aspect-video bg-black relative shadow-lg group rounded-xl overflow-hidden border border-zinc-800">
                {canWatch ? (
                    <>
                        <div id="video-cover" class="absolute inset-0 z-20 cursor-pointer group">
