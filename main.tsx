...
// 🔥 ဒီကုဒ် ပျောက်နေလို့ 404 ဖြစ်နေတာပါ (Copy & Paste လုပ်ပါ)
app.post("/admin/movie/save", adminGuard, async (c) => {
    ...
    const movie: Movie = {
        ...
        coverUrl: String(body.posterUrl),
        category: String(body.category) as any,
        description: String(body.description || ""), 
        tags: String(body.title).toLowerCase(), // Search အတွက်
    ...
    // Episode List (Textarea to Array)
    const epText = String(body.episodeList || "");
    if(epText.trim()){
        movie.episodes = epText.split("\n").map(line => {
            const parts = line.split("|").map(s=>s.trim());
            if(parts.length >= 2) return { name: parts[0], url: parts[1] }; // Simple Format
            return null;
