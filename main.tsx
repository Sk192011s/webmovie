...
// 🔥 OLD CODE FUNCTION (Server-Side Resolve)
// ဒါက Video မလာတဲ့ ပြဿနာကို ဖြေရှင်းပေးမယ့် အဓိကသော့ချက် function ပါ
    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 HYBRID PLAYER (Server Resolve for Old Style, Client Fallback for Speed)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 2. Reset Player (Browser Native Loader)
                ...
                // 3. Resolve URL (Token ဖြည်ပြီး Link အစစ်ကိုရယူမည်)
                // /stream/ လမ်းကြောင်း မသုံးတော့ဘဲ API ကနေ Link အစစ်တောင်းမည် (Old Code Style)
                if (type === 'direct') { 
                    try { 
                ...
                // 4. Player Construction (Proxy/Embed -> Iframe, MP4 -> Video Tag)
                if (type === 'embed' || finalUrl.includes('<iframe') || finalUrl.includes('proxy.avotc.tk')) { 
                    const src = finalUrl.includes('<iframe') ? finalUrl : '<iframe src="'+finalUrl+'" width="100%" height="100%" frameborder="0" allowfullscreen referrerpolicy="no-referrer"></iframe>';
                    ...
                    if(video) { 
                        // Error တက်ရင် Iframe ပြောင်းဖွင့်မည့် Fallback (Security)
                        video.addEventListener('error', (e) => { 
                            console.log("Video Failed, Fallback to Iframe");
                            container.innerHTML = '<iframe src="'+finalUrl+'" width="100%" height="100%" frameborder="0" allowfullscreen class="w-full h-full bg-black" referrerpolicy="no-referrer"></iframe>';
                        });
...
// 🔥 SERVER-SIDE RESOLVER ENDPOINT (Used by Client Logic)
app.get("/api/resolve-url", async (c) => { 
    ...
    // Server-Side Resolve Logic (Old Code Style)
    // Client က ဒီ API ကိုခေါ်မှ Link အစစ်ကို ရှာပေးမယ်
    let url = entry.value as string;
    if(isValidUrl(url)) {
        try { url = await resolveRedirect(url); } catch(e) {}
    }
...
app.get("/dl/:token", async (c) => { const token = c.req.param("token"); const entry = await kv.get(["stream_tokens", token]); if (!entry.value) return c.text("Download Link Expired", 403); 
    // Download Link တွေအတွက် Server-Side Resolve လုပ်ပြီးမှ Redirect ပေးမယ် (Old Code Style)
    let url = entry.value as string;
    if(isValidUrl(url)) { try { url = await resolveRedirect(url); } catch(e) {} }
...
app.get("/movie/:id", async (c) => {
    ...
    if (canWatch) {
        // Token ထဲမှာ မူရင်း Link ကိုပဲ သိမ်းထားမယ် (Page Load မြန်အောင်)
        // Video ဖွင့်ခါနီးမှ Client က API ကိုလှမ်းခေါ်ပြီး Link အစစ်ကို တောင်းလိမ့်မယ်
