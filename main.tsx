...
app.get("/movie/:id", async (c) => {
    ...
    // Logic: ဈေးရှိရင် ဝယ်ပြီးမှကြည့်ရမယ်
    if (moviePrice > 0) {
      ...
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        ...
        <div id="buy-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
             <div class="bg-[#1f1f1f] p-6 rounded-2xl border border-yellow-500/30 w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  ...
                  <p class="text-gray-400 text-xs mb-6 bg-black/40 p-2 rounded border border-zinc-800">
                      <i class="fa-solid fa-check text-green-500 mr-1"></i> Lifetime Stream and Download
                  </p>
        ...
        {/* 🔥 VIP Modal (Regular Movies - Price 0) */}
        <div id="vip-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
             <div class="bg-[#1f1f1f] p-6 rounded-2xl border border-yellow-500/30 w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  ...
                  <p class="text-gray-300 text-sm leading-relaxed mb-4 font-bold">
                      Moviesများ ကြည့်ရန် ဒေါင်းရန်အတွက် Accountဖွင့်ပါ
                  </p>
                  <p class="text-yellow-500 text-xs mb-6 bg-yellow-500/10 p-3 rounded-xl border border-yellow-500/20 font-bold leading-relaxed">
                      LuGyi Apk Vipမန်ဘာများဖြစ်လျှင် VIP Keyအခမဲ့လာယူပါ
                  </p>
                  ...
                  <div class="flex gap-3 h-12"> 
                      ...
                      <a href={user ? "/profile" : "/login"} class="flex-1 h-full flex items-center justify-center rounded-xl bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold hover:brightness-110 transition shadow-lg">
                          {user ? "Enter Key" : "Login"}
                      </a>
                  </div>
        ...
        <div class="max-w-4xl mx-auto">
                ...
                {canWatch ? (
                    ...
                    <div class="absolute inset-0 z-20">
                        ...
                        <div class="absolute inset-0 bg-black/20 flex items-center justify-center">
                            <i class="fa-solid fa-lock text-5xl text-white/50 drop-shadow-lg"></i>
                        </div>
                    </div>
                )}
           </div>
           ...
           <div class="p-6">
               ...
               <div class="flex items-center gap-3 text-xs text-gray-400 mb-6">
                   ...
                   {moviePrice > 0 ? (
                       isPurchased ? 
                       <span class="bg-green-900/30 text-green-400 px-3 py-1 rounded-full border border-green-500/30 font-bold"><i class="fa-solid fa-check"></i> Owned</span> :
                       <span class="bg-blue-900/30 text-blue-400 px-3 py-1 rounded-full border border-blue-500/30 font-bold">{moviePrice} Ks</span>
                   ) : (
                       // 🔥 ဒီနေရာမှာ Free အစား VIP လို့ ပြောင်းလိုက်ပါပြီ
                       <span class="bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 px-3 py-1 rounded-full font-bold">VIP</span>
                   )}
               </div>
               ...
               <div class="flex flex-col gap-3 mb-8">
                   ...
                   {movie.category !== "Series" && !(movie.category === "Animation" && episodes.length > 0) && (
                       <div class={`grid gap-2 ${movie.streamUrl2 ? "grid-cols-2" : "grid-cols-1"}`}>
                            <button 
                                onclick={canWatch 
                                    ? `loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`
                                    : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)} 
                                class="srv-btn w-full bg-white text-black font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition hover:brightness-110 shadow-lg text-xs"
                            >
                                <i class={`fa-solid ${canWatch ? 'fa-play' : 'fa-lock'}`}></i> {t.server1} {canWatch ? "" : (moviePrice > 0 ? "(Buy)" : "")}
                            </button>

                            {movie.streamUrl2 && (
                                <button 
                                    onclick={canWatch 
                                        ? `loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl2}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`
                                        : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)} 
                                    class="srv-btn w-full bg-zinc-800 text-white font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition hover:bg-zinc-700 border border-zinc-700 text-xs"
                                >
                                    <i class={`fa-solid ${canWatch ? 'fa-server' : 'fa-lock'}`}></i> {t.server2}
                                </button>
                            )}
                       </div>
                   )}
                   ...
                   {movie.category !== "Series" && (movie.downloadUrl || movie.downloadUrl2) && (
                         <div class="grid grid-cols-2 gap-2 mt-1">
                            {movie.downloadUrl && (
                                <button 
                                    onclick={canWatch 
                                        ? `confirmDownload('${secureDownloadUrl}', '${movie.title.replace(/'/g, "\\'")}', '${movie.fileSize || ""}')`
                                        : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)}
                                    class="w-full bg-zinc-800 text-white font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 text-xs"
                                >
                                    <i class={`fa-solid ${canWatch ? 'fa-download' : 'fa-lock'}`}></i> DL 1
                                </button>
                            )}
                            {movie.downloadUrl2 && (
                                <button 
                                    onclick={canWatch 
                                        ? `confirmDownload('${secureDownloadUrl2}', '${movie.title.replace(/'/g, "\\'")}', '${movie.fileSize || ""}')`
                                        : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)}
                                    class="w-full bg-zinc-800 text-white font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 text-xs"
                                >
                                    <i class={`fa-solid ${canWatch ? 'fa-download' : 'fa-lock'}`}></i> DL 2
                                </button>
                            )}
                         </div>
                    )}
                                   ...
                                   <div id={`ep-grid-${safeId}`} class="season-content hidden mt-3 bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3 animate-fade-in">
                                       <div class="grid grid-cols-4 gap-2 max-h-60 overflow-y-auto custom-scroll pr-1">
                                           {seasons[season].map(ep => (
                                               <button 
                                                   onclick={canWatch 
                                                    ? `loadPlayer('${ep.url.trim()}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`
                                                    : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)}
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="grid grid-cols-2 gap-2"><input name="fileSize" placeholder="File Size (e.g. 1.2 GB)" value={editMovie?.fileSize} class="input-box w-full bg-black border-zinc-700" /><input type="number" name="price" placeholder="Price (Ks) - 0 for Free" value={editMovie?.price || 0} class="input-box w-full bg-black border-yellow-500/50 text-yellow-500 font-bold" /></div>
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    // Form မှ Data များရယူခြင်း
    let category = body["category"] as string;
    let title = body["title"] as string;
    let price = parseInt(String(body["price"] || "0"));

    // 🔥 4K Porns အတွက် Auto System (အသစ်ထည့်ထားသောအပိုင်း)
    // Create Mode (အသစ်တင်ချိန်) ဖြစ်မှသာ အလုပ်လုပ်မည်
    if (category === "4K Porns" && body["mode"] === "create") {
        
        // ၁. ဈေးနှုန်းကို 50Ks အလိုအလျောက်သတ်မှတ်
        price = 50; 

        // ၂. Title ကို နံပါတ်စဉ် အလိုအလျောက်တပ် (1, 2, 3...)
        // နောက်ဆုံးတင်ထားသော 4K Porn ကားကို လှမ်းစစ်သည်
        const latestMovies = await getMoviesByCategory("4K Porns", 1);
        
        if (latestMovies.length > 0) {
            // နောက်ဆုံးကားရဲ့ နာမည်ကို နံပါတ်ပြောင်းပြီး ၁ ပေါင်းသည်
            const lastNum = parseInt(latestMovies[0].title);
            if (!isNaN(lastNum)) {
                title = String(lastNum + 1);
            } else {
                // အရင်ကားက နံပါတ်မဟုတ်ရင် (ဥပမာ "Test") ၁ ကနေ ပြန်စမည်
                title = "1"; 
            }
        } else {
            // လုံးဝ ပထမဆုံးကားဆိုရင် ၁ ပေးမည်
            title = "1";
        }
    }
    ...
    // 🔥 ပြင်ဆင်ထားသော title နှင့် price ကို ဒီမှာ ထည့်သိမ်းလိုက်သည်
    const movie = { ...body, title: title, id: id, createdAt: timeToSave, episodes, price: price }; 
