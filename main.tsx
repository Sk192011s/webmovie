...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        // Sorting Logic (Myanmar = Reverse)
        if (movies.length > 0) {
            const firstSample = movies[0];
            const isJav = firstSample.name || firstSample.photo1 || firstSample.dapp; 
            const isUSA = firstSample.profileImg && firstSample.files; 
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // 🔥 LINK COLLECTION (အားလုံးကို စုမည်)
                let allLinks = [];
                ...
                // ---------------------------------------------------------
                // 🔥 NEW SELECTION LOGIC (Proxy First -> Then Raw)
                // ---------------------------------------------------------
                ...
                // 2. မူရင်း Source (Raw) အကောင်းဆုံးတစ်ခု ရှာမယ် (Priority: R2 > Deno > Vercel > MediaFire)
                ...
                // ဘာ Link မှ မရှိရင် ကျော်မယ်
                if (!preProxiedLink && !bestRawLink) return;

                // ID Generation (Link ပေါ်မူတည်ပြီး)
                ...
                if (preProxiedLink) {
                    // ✅ CASE A: Proxy Link ပါပြီးသား
                    server1 = preProxiedLink;
                    dl1 = preProxiedLink;
                    
                    // Server 2 အတွက် Raw Link ကိုသုံးမယ် (မရှိရင် Proxy Link ပဲ ပြန်သုံးမယ်)
                    server2 = bestRawLink || preProxiedLink;
                    dl2 = bestRawLink || preProxiedLink;

                } else {
                    // ✅ CASE B: Proxy Link မပါဘူး (Raw Link ပဲရှိတယ်) -> ကိုယ်တိုင် Proxy ခံမယ်
                    ...
                    if (bestRawLink.includes("mediafire.com")) {
                        ...
                        if (bestRawLink.includes("premium")) {
                            ...
                            server1 = PROXY_BASE + bestRawLink;
                            // MediaFire Standard (Non-premium) needs API for DL
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Proxy Priority)");
