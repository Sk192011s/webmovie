...
// 🔥 PERFORMANCE: IMAGE PROXY (To make images load faster & lighter)
function optimizeImg(url: string, width: number = 400) {
    if(!url) return "";
    if(url.includes("base64") || url.startsWith("/public")) return url;
    // Using wsrv.nl as a free reliable image proxy (converts to WebP automatically)
    return `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=${width}&output=webp&il`;
}
    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 UX: PLAYER WITH SELF-HOSTED MEDIAFIRE & NATIVE CONTROLS
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 🔥 MEDIAFIRE CHECKER (SELF-HOSTED)
                if (finalUrl.includes('mediafire.com')) {
                    // ကိုယ့် Server က Extract လုပ်ပေးမယ့် လမ်းကြောင်း
