...
// 🔥 4. BULK IMPORT (Supports URL + Custom JSON Keys)
    ...
    try {
        ...
        for (const m of movies) {
            if (m.title) {
                ...
                // Data Mapping (မင်းရဲ့ JSON Format နဲ့ ကိုက်အောင်ညှိထားသည်)
                const poster = m.posterUrl || m.image || ""; 
                const stream = m.streamUrl || m.link || "";  
                const desc = m.description || m.info || "";  
                const cat = m.category || "Myanmar and Asian"; 

                const movieData = {
                    id,
                    title: m.title,
                    posterUrl: poster,
                    coverUrl: m.coverUrl || poster, 
                    category: cat,
                    description: desc,
                    tags: m.tags || m.title,
        ...
        await logAdminAction("bulk_import", `Imported ${count} movies`);
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + count + " Movies");
    } catch (e) {
        return c.redirect(ADMIN_ROUTE + "/dashboard?error=Invalid JSON or URL");
    }
});
