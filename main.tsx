...
// =======================
// 2. MIDDLEWARE (IP SECURITY)
// =======================

// GLOBAL IP CHECK (Runs on EVERY request)
app.use('*', async (c, next) => {
    ...
    if (banned.value) {
        return c.text("Access Denied: Your IP has been banned by Admin.", 403);
    }
...
async function getCurrentUser(c: any) {
  ...
  if (!user.value || user.value.sessionId !== token) return null;
  return user.value;
}
...
async function addLoginLog(username: string, ip: string) { 
    await kv.set(["login_logs", Date.now()], { username, ip, timestamp: Date.now() }, { expireIn: 86400 * 3 }); 
}
...
// =======================
// 5. STATIC & API ROUTES (PRIORITY 1)
// =======================
...
app.get("/api/list", async (c) => {
    ...
    const allMovies = []; for await (const res of iter) allMovies.push(res.value);
    // Sort Newest First
    allMovies.sort((a, b) => (Number(b.createdAt) || 0) - (Number(a.createdAt) || 0));
...
// =======================
// 6. AUTH & ADMIN ROUTES (PRIORITY 2)
// =======================
...
app.post("/login", async (c) => { const body = await c.req.parseBody(); const user = await kv.get<User>(["users", body["username"] as string]); const hashedInput = await hashPassword(body["password"] as string); if (user.value && user.value.passwordHash === hashedInput) { const sessionId = crypto.randomUUID(); user.value.sessionId = sessionId; await kv.set(["users", user.value.username], user.value); const maxAge = body["remember"] === "on" ? 60 * 60 * 24 * 7 : undefined; setCookie(c, "auth_session", `${user.value.username}:${sessionId}`, { path: "/", maxAge }); 
    // Log IP
    const ip = c.req.header('x-forwarded-for')?.split(',')[0].trim() || "unknown";
    await addLoginLog(user.value.username, ip);
...
app.post("/admin/movie/save", adminAuth, async (c) => { const body = await c.req.parseBody(); const epString = body["episodeList"] as string; const episodes: Episode[] = []; if(epString && epString.trim().length > 0) { epString.split('\n').forEach(line => { const parts = line.split('|'); if(parts.length >= 2) { episodes.push({ season: parts[0].trim(), name: parts[1].trim(), url: parts[2].trim() }); } }); } const movie = { ...body, id: body["id"], createdAt: Number(body["createdAt"]), episodes }; await kv.set(["movies", movie.id as string], movie); return c.redirect("/admin/dashboard"); });
...
// =======================
// 6. DYNAMIC & UI ROUTES (Priority 3)
// =======================
...
app.get("/movie/:id", async (c) => {
    if(await checkIP(c)) return c.text("Access Denied", 403);
...
app.get("/", async (c) => {
  ...
  const user = await getCurrentUser(c);
  // Important: Disable caching for Home Page HTML to update Login/Me status immediately
