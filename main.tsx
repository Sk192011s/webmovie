...
console.log("Playing:", finalUrl);
        ...
        async function loadMoreMovies(category) { 
            // Server က ပို့လိုက်တဲ့ cursor ရှိရင် ပထမဆုံးအကြိမ်မှာ ယူသုံးမယ်
            if (isFirstLoad && window.initialCursor) {
                currentCursor = window.initialCursor;
                isFirstLoad = false;
            }

            // cursor မရှိတော့ရင် (ဒါမှမဟုတ်) loading ဖြစ်နေရင် ဆက်မလုပ်ဘူး
            if(isLoading || (!currentCursor && !isFirstLoad)) return; 
            
            isLoading = true; 
            const grid = document.getElementById('movie-grid');
            
            // Loading Skeleton လေးတွေ ပြမယ်
            const skeletons = [];
            for(let i=0; i<6; i++) {
            ...
            try { 
                // page= အစား cursor= နဲ့ လှမ်းခေါ်မယ်
                const url = '/api/list?cat=' + category + (currentCursor ? '&cursor=' + currentCursor : '');
                const res = await fetch(url); 
                const data = await res.json(); 
                
                skeletons.forEach(s => s.remove());
                
                // နောက်ထပ် ဆွဲစရာကျန်သေးလား update လုပ်မယ်
                if (data.nextCursor) {
                    currentCursor = data.nextCursor;
                } else {
                    currentCursor = null; // ကုန်သွားပြီ
                }
        ...
        // Scroll ဆွဲရင် အလုပ်လုပ်မယ့် Listener
