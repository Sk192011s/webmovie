...
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    ...
    if (!apiKey) return c.json({ error: "API Key Not Found" });
    if (!title) return c.json({ error: "Title is required" });

    // AI ကို ခိုင်းမည့် ပုံစံ (Prompt)
    // မြန်မာလို လိုချင်ရင် "in Burmese" လို့ ထည့်ရေးထားတာပါ
    ...
    try {
        ...
        if (!result) return c.json({ error: "AI could not generate text." });
