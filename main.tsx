...
const Layout = (props: { children: any; title?: string; user?: User | null; hideNav?: boolean; announcement?: string; isAdmin?: boolean }) => {
  const protectCSS = props.isAdmin ? "" : `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             document.body.addEventListener('click', (e) => {
                ...
                if (link) {
                    ...
                    if (href && href.startsWith('/') && !href.includes('#') && !href.includes('/dl/') && target !== '_blank') window.showLoader();
                }
             });
           ...
           <div class="p-4">
               ...
               {premium && (
                   <div class="flex flex-col gap-2 mb-6">
                        {/* Player Buttons only for movies or ungrouped */}
    ...
    return c.html(
      <Layout user={user}>
        <div class="p-4 max-w-4xl mx-auto space-y-6">
            ...
            {/* 1. MEMBERSHIP CARD */}
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-300 via-yellow-500 to-amber-600 p-6 shadow-2xl text-black">
                <div class="absolute top-0 right-0 p-4 opacity-20"><i class="fa-solid fa-crown text-9xl"></i></div>
                <div class="relative z-10 flex flex-col h-full justify-between min-h-[180px]">
                    <div class="flex justify-between items-start">
                         <div class="flex items-center gap-4">
                             <div class="w-16 h-16 bg-black text-yellow-500 rounded-full flex items-center justify-center text-3xl font-black border-2 border-white shadow-lg">{user.username[0].toUpperCase()}</div>
                             <div>
                                 <h2 class="text-2xl font-black tracking-tight">{user.username}</h2>
                                 <div class="text-xs font-bold bg-black/20 px-2 py-0.5 rounded inline-block uppercase tracking-wider">{statusText}</div>
                             </div>
                         </div>
                         <i class="fa-solid fa-wifi text-2xl opacity-70"></i>
                    </div>
                    <div class="mt-6 flex justify-between items-end font-mono">
                        <div>
                            <p class="text-[10px] uppercase opacity-70 font-bold mb-1">Member ID</p>
                            <p class="text-lg font-bold tracking-widest">{memberId}</p>
                        </div>
                        <div class="text-right">
                             <p class="text-[10px] uppercase opacity-70 font-bold mb-1">Expires</p>
                             <p class="text-sm font-bold">{premium ? `${finalDays} Days Left` : "Not Active"}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                 <div class="bg-[#1f1f1f] p-4 rounded-xl border border-zinc-800 flex flex-col justify-center items-center gap-2">
                     ...
                     <div class="text-center">
                         <span class="block text-2xl font-bold text-white">{favCount}</span>
                         <span class="text-xs text-gray-500">Saved Movies</span>
                     </div>
                 </div>
                 <div class="bg-[#1f1f1f] p-4 rounded-xl border border-zinc-800 flex flex-col justify-center">
                     ...
                     <div class="w-full bg-black h-2 rounded-full overflow-hidden">
                         <div class={`h-full ${premium ? 'bg-yellow-500' : 'bg-gray-700'}`} style={{width: `${premium ? progress : 0}%`}}></div>
                     </div>
            ...
            <div class="bg-[#1f1f1f] p-6 rounded-xl border border-yellow-500/30 relative overflow-hidden group">
                 ...
                 <form action="/profile/redeem" method="post" class="relative z-10 flex gap-2">
                     <input name="key" placeholder="Enter VIP Code..." required class="bg-black border border-zinc-700 text-white px-4 py-3 rounded-lg w-full outline-none focus:border-yellow-500 transition" />
                     <button class="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold px-6 py-3 rounded-lg hover:brightness-110 shadow-lg whitespace-nowrap"><i class="fa-solid fa-check"></i></button>
                 </form>
            </div>
                    ...
                    {plans.map(p => (
                        <div class={`relative bg-[#1f1f1f] p-5 rounded-xl border ${p.popular ? 'border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.2)]' : 'border-zinc-800'} flex flex-col`}>
                            ...
                            <div class="flex justify-between items-baseline mb-4">
                                ...
                                <span class="text-yellow-500 font-mono font-bold text-xl">{p.price}</span>
                            </div>
                            <ul class="space-y-2 mb-6 flex-grow">
                                {p.features.map(f => (
                                    <li class="flex items-center gap-2 text-xs text-gray-400"><i class="fa-solid fa-check text-green-500"></i> {f}</li>
                                ))}
                            </ul>
                            <a href="https://t.me/your_telegram_username" target="_blank" class={`block text-center py-3 rounded-lg font-bold text-sm transition ${p.popular ? 'bg-yellow-500 text-black hover:bg-yellow-400' : 'bg-zinc-800 text-white hover:bg-zinc-700'}`}>Buy Now</a>
                        </div>
                    ))}
                </div>
            </div>
...
app.post("/admin/movie/delete/:id", adminAuth, async (c) => { await kv.delete(["movies", c.req.param("id")]); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Deleted"); });
...
app.post("/admin/restore", adminAuth, async (c) => { try { const body = await c.req.parseBody(); const file = body['file']; if (file instanceof File) { const text = await file.text(); const data = JSON.parse(text); for (const item of data) { await kv.set(item.key, item.value); } return c.redirect(ADMIN_ROUTE + "/dashboard?success=Data Restored"); } } catch(e) { return c.redirect(ADMIN_ROUTE + "/dashboard?error=Restore Failed"); } });
