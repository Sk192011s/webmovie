...
import { csrf } from "hono/csrf"; // 🔥 Added CSRF Middleware
...
interface AppConfig { announcement: string; showAnnouncement: boolean; globalVipExpiry?: number; 
    popupImage?: string; popupMessage?: string; popupBtnText?: string; popupLink?: string; popupTarget?: string; showPopup?: boolean;
}
            ...
            try { 
                ...
                data.movies.forEach(m => { 
                    ...
                    if(category === "All Uncensored" || category === "Myanmar and Asian") { 
                        ...
                        el.innerHTML = '<div class="relative aspect-video overflow-hidden"><img src="'+(m.coverUrl || m.posterUrl)+'" loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" /><div class="absolute top-2 right-2 bg-yellow-500 text-black text-[9px] font-bold px-1.5 py-0.5 rounded shadow z-10">'+badge+'</div></div><div class="p-3"><h3 class="text-sm font-bold truncate text-white">'+m.title+'</h3></div>';
                    } else { 
                        el.className = 'movie-card block bg-[#1f1f1f] rounded overflow-hidden group relative'; 
                        el.innerHTML = '<div class="relative aspect-[2/3] overflow-hidden"><img src="'+m.posterUrl+'" loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" /><div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow z-10">'+badge+'</div></div><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white">'+m.title+'</h3></div>';
                    } 
                    grid.appendChild(el); 
                }); 
                        ...
                        <div class="h-scroll-section">
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded-xl overflow-hidden active:scale-95 transition-transform movie-card group">
                                    <div class="aspect-video w-full relative overflow-hidden"><img src={m.coverUrl || m.posterUrl} loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" /><div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center"><i class="fa-solid fa-circle-play text-4xl text-white drop-shadow-lg"></i></div></div>
                    ...
                    <div class="h-scroll-section custom-scroll">
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform movie-card group">
                                <div class="aspect-[2/3] w-full relative overflow-hidden"><img src={m.posterUrl} loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" /><div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow">
    ...
    {movies.map(m => (
        <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'mb-4' : ''}`}>
            ...
            <div class={`relative overflow-hidden ${isUncensored ? "aspect-video" : "aspect-[2/3]"}`}>
                <img src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                        ...
                        return (
                            <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isWide ? 'col-span-3' : ''}`}>
                                <div class={`${isWide ? "aspect-video" : "aspect-[2/3]"} relative overflow-hidden`}>
                                    <img src={isWide ? (m.coverUrl || m.posterUrl) : m.posterUrl} loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
           ...
           <div class="p-6">
               ...
               {premium && (
                   <div class="flex flex-col gap-3 mb-8">
                        {/* 🔥 PLAYER BUTTONS (Auto Full Width if Single Server) */}
               {movie.category !== "Series" && !(movie.category === "Animation" && episodes.length > 0) && (
                   // 👇 ဒီနေရာလေး ပြင်လိုက်ပါတယ် (Server 2 ရှိမှ ၂ ကွက်ခွဲမယ်၊ မရှိရင် ၁ ကွက်တည်း အပြည့်ပြမယ်)
                           ...
                           {related.map(m => {
                               ...
                               return (
                                   <a href={`/movie/${m.id}`} class={`h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden flex-shrink-0 movie-card group ${isUncensored ? 'wide' : 'w-28'}`}>
                                       <div class={`${isUncensored ? "aspect-video" : "aspect-[2/3]"} w-full relative overflow-hidden`}>
                                           <img src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                                       </div>
                                       <div class="p-2"><h3 class="text-[11px] font-bold truncate text-white">{m.title}</h3></div>
                                   </a>
