    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 UX: PLAYER WITH CLIENT-SIDE SCRAPER (No Backend/External API Needed)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // Loader ပြမယ် (MediaFire ဖြည်နေတုန်း စောင့်ခိုင်းရန်)
                container.innerHTML = '<div class="flex items-center justify-center h-full bg-black"><div class="spinner w-10 h-10 border-4 border-purple-500 border-t-transparent rounded-full animate-spin"></div></div>';

                var finalUrl = content.trim();

                // 🔥 MEDIAFIRE LOGIC (Client-Side Scraper)
                if (finalUrl.includes('mediafire.com')) {
                    try {
                        console.log("Extracting MediaFire via Proxy...");
                        // Public Proxy 'AllOrigins' ကိုသုံးပြီး HTML လှမ်းဆွဲမယ်
                        var proxyUrl = 'https://api.allorigins.win/get?url=' + encodeURIComponent(finalUrl);
                        var res = await fetch(proxyUrl);
                        var data = await res.json();
                        var html = data.contents; // HTML စာသားရပြီ

                        // Regex နဲ့ Video Link အစစ်ကို ရှာမယ်
                        var match = html.match(/aria-label="Download file"\s+href="([^"]+)"/) || 
                                    html.match(/id="downloadButton"\s+href="([^"]+)"/) ||
                                    html.match(/href="([^"]+)"\s+id="downloadButton"/);

                        if (match && match[1]) {
                            finalUrl = match[1]; // Direct Video Link ရပြီ!
                            console.log("MediaFire Direct Link Found:", finalUrl);
                        } else {
                            throw new Error("Extraction Failed");
                        }
                    } catch (e) {
                        console.error("MediaFire Error:", e);
                        // Error တက်ရင် မူရင်း Link အတိုင်းထားလိုက်မယ် (အောက်မှာ Download Button ပြဖို့)
                    }
                } 
                // Direct Link Logic
                else if (type === 'direct') { 
                ...
                videoHtml += '<p class="text-gray-400 mb-4 text-[10px]">MediaFire link needs direct access.</p>';
                videoHtml += '<a href="' + finalUrl + '" target="_blank" class="bg-blue-600 px-6 py-3 rounded-xl text-white font-bold inline-block shadow-lg hover:bg-blue-500 transition"><i class="fa-solid fa-download"></i> Click to Download / Watch</a></div>';
                videoHtml += '</div>';
...
// 🔥 3. SELF-HOSTED MEDIAFIRE API
app.get("/api/mediafire", async (c) => {
    const link = c.req.query("link");
    if (!link) return c.text("Link Required", 400);
    ...
    try {
        ...
        // Download Link ရှာဖွေခြင်း
        const match = html.match(/aria-label="Download file"\s+href="([^"]+)"/) || 
                      html.match(/id="downloadButton"\s+href="([^"]+)"/) ||
                      html.match(/href="([^"]+)"\s+id="downloadButton"/);

        if (match && match[1]) {
            return c.redirect(match[1]);
        } else {
            return c.text("Error: Cannot extract link", 404);
        }
    } catch (e) {
