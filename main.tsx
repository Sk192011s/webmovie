...
// Check Rate Limit (3 attempts -> 1 minute block)
function checkRateLimit(ip: string): boolean {
  ...
  if (record.count >= 3) return false; // Max 3 attempts
  record.count++;
  return true;
}
...
async function getCurrentUser(c: any) {
  ...
  if (user.isBanned) return null; // Ban ထားရင် အလိုလို Logout ဖြစ်စေမည်
  return user;
}
...
// 🔥 LOGIN ROUTE (Rate Limit UI + IP Track + Ban Check)
app.post("/login", async (c) => { 
    const ip = getClientIp(c);

    // 1. Rate Limit Check (Red Shield UI)
...
app.post("/signup", async (c) => { 
    ...
    const newUser: User = { 
        ...
        favorites: [], 
        sessionId: "",
        ip: clientIp,           // Register IP
        lastLoginIp: clientIp,  // Login IP
...
app.post("/admin/user/toggle-ban", adminGuard, async (c) => { 
    ...
    if (user) { 
        ...
        if(user.isBanned) user.sessionId = "banned"; // Kill session
        await kv.set(["users", String(username)], user); 
        return c.redirect(ADMIN_ROUTE + "/dashboard?tab=users&success=User Status Updated"); 
    } 
