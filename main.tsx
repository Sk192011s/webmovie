      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                if(container) container.innerHTML = '<div class="w-full h-full flex items-center justify-center bg-black"><div class="spinner border-t-purple-500"></div><span class="ml-3 text-xs text-purple-400 font-bold animate-pulse">Bypassing MediaFire (No VPN)...</span></div>'; 
                if(cover) cover.style.display = 'none';
                if(container) container.style.display = 'block'; 

                var finalUrl = content.trim();

                // 🔥 CLIENT-SIDE EXTRACTION (VPN မလိုအောင် Browser ကနေပဲ တိုက်ရိုက်လုပ်မယ်)
                if (finalUrl.includes('mediafire.com')) {
                    try {
                        // 1. Proxy သုံးပြီး Browser ကနေ တိုက်ရိုက်လှမ်းခေါ်မယ်
                        // (Proxy URL ပြောင်းချင်ရင် ဒီမှာပြောင်းပါ)
                        var proxyUrl = "https://proxy.avotc.tk/"; 
                        var targetUrl = proxyUrl + finalUrl;

                        var res = await fetch(targetUrl);
                        var html = await res.text();

                        // 2. Download Link ကို ရှာမယ်
                        var match = html.match(/aria-label="Download file"\s+href="([^"]+)"/);
                        if (!match) match = html.match(/id="downloadButton"\s+href="([^"]+)"/);
                        if (!match) match = html.match(/href="([^"]+)"\s+id="downloadButton"/);

                        if (match && match[1]) {
                            // 3. Link ရရင် ရှေ့က Proxy ပြန်ခံပြီး Play မယ်
                            finalUrl = proxyUrl + match[1];
                            console.log("Client Extracted:", finalUrl);
                        } else {
                            throw new Error("Cannot extract link");
                        }
                    } catch (e) {
                        console.error("Client Extraction Failed", e);
                        // မရရင် Server API ကို Fallback အနေနဲ့ သုံးမယ်
...
app.get("/api/mediafire-stream", async (c) => {
    ...
    try {
        let cleanUrl = url.replace("https://proxy.avotc.tk/", "");
        ...
        if (match && match[1]) {
            // Direct Link ရရင် Proxy မခံဘဲ ပြန်ပေးမယ် (Browser က Proxy ခံထားပြီးသားမို့)
            return c.redirect(match[1], 302);
        } else {
            return c.redirect(cleanUrl, 302); // မရရင် Webpage ပြန်ပို့
        }
    } catch (e) {
