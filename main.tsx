...
const i18n: any = {
    en: {
        ...
        search_ph: "Search movies...", featured: "Featured", play: "Play Now",
        see_all: "View All", views: "Views", dl_help: "Download Guide",
        server1: "Stream Server 1", server2: "Stream Server 2", share: "Share",
        unlock: "Unlock VIP Access", vip_only: "VIP Exclusive",
...
// =======================
// 6. UI LAYOUT (🔥 UPDATED PREMIUM UI)
// =======================
...
`;
  ...
  <html lang={l}>
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        /* Glass Effect */
        .glass-nav { background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255, 255, 255, 0.05); }
        ...
        .movie-card img { transition: opacity 0.3s; }
        .movie-card:hover img { opacity: 0.8; }
    ...
    <body>
      ...
      {!props.hideNav && (
        <nav class="fixed top-0 w-full z-40 glass-nav px-4 py-3 shadow-lg">
          <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="flex gap-4 items-center text-xs font-bold text-gray-300">
              {props.isAdmin ? (
                 <>
                 ...
                 <>
                   <a href="/" class="hover:text-yellow-500 transition">{t.home}</a>
                   <a href="/favorites" class="hover:text-yellow-500 transition">{t.saved}</a>
                   <a href="/request" class="hover:text-yellow-500 transition">{t.request}</a>
                   {props.user ? <a href="/profile" class="text-yellow-500">{t.me}</a> : <a href="/login">{t.login}</a>}
      ...
      {props.announcement && (
          <div class="fixed top-[53px] w-full z-30 bg-yellow-500 text-black text-xs font-bold px-4 py-2 flex items-center gap-2 overflow-hidden shadow-md">
              <i class="fa-solid fa-bullhorn animate-pulse"></i>
              <marquee scrollamount="5">{props.announcement}</marquee>
          </div>
      )}

      <main class="flex-grow w-full pb-10 pt-[70px]">
        {props.children}
      </main>
    </body>
  </html>
...
app.get("/", async (c) => {
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang}>
        <div class="p-3 bg-black/50 backdrop-blur-md sticky top-[60px] z-30">
             <form action="/search" method="get" class="relative">
                 <i class="fa-solid fa-magnifying-glass absolute left-3 top-3.5 text-gray-500"></i>
                 <input name="q" placeholder={t.search_ph} class="w-full bg-[#1a1a1a] border border-zinc-800 rounded-xl py-3 pl-10 pr-4 text-sm text-white focus:border-yellow-500 outline-none transition shadow-sm" />
             </form>
        </div>
      ...
      <div class="px-4 space-y-10 pb-8">
          {sections.map(section => { 
              ...
              if(cat === "All Uncensored") { 
                  return (
                    <div>
                        <div class="flex justify-between items-end mb-4"><h2 class="text-xl font-bold text-white border-l-4 border-yellow-500 pl-3">{cat}</h2><a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1 hover:text-white transition">{t.see_all} <i class="fa-solid fa-chevron-right text-[10px]"></i></a></div>
...
app.get("/movie/:id", async (c) => {
    ...
    <>
      ...
      <div id="video-player" class="w-full h-full hidden"></div>
           ...
           <div class="p-6">
               <div class="flex justify-between items-start mb-3"><h1 class="text-3xl font-black text-white leading-tight">{movie.title}</h1>{user && (<form action="/api/fav" method="post"><input type="hidden" name="movieId" value={movie.id} /><button class="text-2xl p-2 transition hover:scale-110"><i class={`fa-solid fa-heart ${isFav ? 'text-red-600' : 'text-zinc-600'}`}></i></button></form>)}</div>
                   ...
                   <div class="flex flex-col gap-3 mb-8">
                        ...
                        {movie.category !== "Series" && (
                            <div class="flex gap-2 overflow-x-auto pb-2">
                                <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn flex-1 bg-white text-black font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition hover:brightness-110 whitespace-nowrap shadow-lg">
                                    <i class="fa-solid fa-play"></i> {movie.streamUrl2 ? t.server1 : t.play}
                                </button>
                                {movie.streamUrl2 && (
                                    <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken2 : playerUrl2}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn flex-1 bg-zinc-800 text-white font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap border border-zinc-700">
                                        <i class="fa-solid fa-server"></i> {t.server2}
                                    </button>
                                )}
                                <button onclick={`shareMovie('${movie.title}')`} class="bg-zinc-800 text-white font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700">
                        ...
                        {movie.category !== "Series" && (
                             <div class="flex gap-2">
                                {secureDownloadUrl && (<button onclick={`confirmDownload('${secureDownloadUrl}', '${movie.title.replace(/'/g, "\\'")}', '${movie.fileSize || ""}')`} class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs"><i class="fa-solid fa-download"></i> DL {movie.downloadUrl2 ? "1" : ""}</button>)}
                       ...
                       <button onclick="toggleHelp()" class="text-xs text-yellow-500 hover:text-yellow-400 flex items-center gap-1 mt-1 font-bold"><i class="fa-solid fa-circle-question"></i> {t.dl_help}</button>
        ...
        <div class="p-4 max-w-4xl mx-auto space-y-6">
            <div class="relative w-full aspect-[1.7/1] rounded-2xl bg-gradient-to-br from-yellow-400 via-yellow-600 to-yellow-700 p-6 shadow-2xl text-black flex flex-col justify-between overflow-hidden">
                ...
                <div class="relative z-10 flex justify-between items-end font-mono mt-4">
                    ...
                    <div class="text-right"><p class="text-[9px] uppercase opacity-70 font-bold mb-0.5">Expires</p><p class="text-sm font-bold">{premium ? `${finalDays} Days Left` : "Not Active"}</p></div>
                </div>
            </div>
