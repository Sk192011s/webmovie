...
// 🔥 UX: PLAYER WITH SELF-HOSTED API (NO 3RD PARTY)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 🔥 MEDIAFIRE LOGIC: ကိုယ့် Server က API ကို သုံးမည်
                if (finalUrl.includes('mediafire.com')) {
                    // /api/mediafire လမ်းကြောင်းကို လှမ်းခေါ်လိုက်သည်
                ...
                // Video Player
                const videoHtml = `
                    <video id="main-video" controls autoplay playsinline class="w-full h-full" style="background-color:black;" poster="${poster}">
                        <source src="${finalUrl}" type="video/mp4">
                        <p class="text-white text-center pt-10">Browser doesn't support video.</p>
                    </video>
                `;
                ...
                if(video) { 
                    // Error တက်ရင် Download Link ပြပေးမယ့် စနစ်
                    video.addEventListener('error', () => {
                       container.innerHTML = `<div class="flex items-center justify-center h-full bg-black text-white flex-col gap-3">
                            <p class="text-red-500 font-bold">Playback Error</p>
                            <a href="${finalUrl}" target="_blank" class="bg-blue-600 px-4 py-2 rounded-lg text-sm font-bold">Click to Download / Watch Directly</a>
                       </div>`;
                    });
...
// 🔥 SELF-HOSTED MEDIAFIRE EXTRACTOR (Ultimate Fix)
app.get("/api/mediafire", async (c) => {
    const link = c.req.query("link");
    if (!link) return c.text("Link Required", 400);

    try {
        // 1. MediaFire Page ကို Browser ဖွင့်သလို လှမ်းခေါ်မယ်
        ...
        // 2. Download Button ကို Regex နဲ့ ရှာမယ်
        // (MediaFire က ပုံစံခဏခဏပြောင်းလို့ ပုံစံ ၃ မျိုးနဲ့ ရှာခိုင်းထားတယ်)
        ...
        if (match && match[1]) {
            // 3. တွေ့ရင် Video ဖိုင်အစစ်ဆီ Redirect လုပ်ပေးမယ်
            return c.redirect(match[1]);
        } else {
            return c.text("Error: Cannot extract link from MediaFire", 404);
        }
    } catch (e) {
