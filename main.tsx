    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 UX: PLAYER CONNECTED TO BACKEND API
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 🔥 MEDIAFIRE: Backend API ကို လှမ်းခေါ်မယ်
                if (finalUrl.includes('mediafire.com')) {
                    finalUrl = '/api/mediafire?link=' + encodeURIComponent(finalUrl);
                } 
                else if (type === 'direct') { 
                    try { 
...
// 🔥 3. MEDIAFIRE EXTRACTOR (Logic from Redirector Code)
app.get("/api/mediafire", async (c) => {
    const url = c.req.query("link");
    if (!url) return c.text("Link Required", 400);

    try {
        // 1. MediaFire ကို Browser အနေနဲ့ လှမ်းခေါ်မယ် (User-Agent ထည့်ထားသည်)
        ...
        // 2. မင်းပေးတဲ့ ကုဒ်ထဲက Regex ပုံစံတွေအတိုင်း ရှာမယ် (၃ မျိုး စစ်ထားတယ်)
        let match = html.match(/aria-label="Download file"\s+href="([^"]+)"/);
        if (!match) match = html.match(/id="downloadButton"\s+href="([^"]+)"/);
        if (!match) match = html.match(/class="input popsok"\s+href="([^"]+)"/);
        if (!match) match = html.match(/href="((https?:\/\/download)[^"]+)"/); // Extra backup

        if (match && match[1]) {
            // 3. Link အစစ်ရပြီ၊ Redirect လုပ်ပေးလိုက်မယ်
            return c.redirect(match[1], 302);
        } else {
            return c.text("MediaFire Link Not Found (Might be deleted/blocked)", 404);
        }
    } catch (e) {
