...
// 🔥 OLD CODE FUNCTION RESTORED (Server-Side Redirect Resolver)
async function resolveRedirect(url: string) {
  if (!isValidUrl(url)) return url;
  // Proxy Link တွေအတွက် 5 စက္ကန့် Timeout ပေးပြီး စစ်မယ်
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 5000);
  try { 
      const res = await fetch(url, { method: "HEAD", redirect: "follow", signal: controller.signal }); 
      clearTimeout(timeoutId); 
      return res.url; // Link အစစ်ကို ပြန်ပေးမယ်
  } catch { return url; } // Error တက်ရင် မူရင်း Link ပဲ ပြန်ပေးမယ်
}
...
// 🔥 OPTIMIZED MOVIE DETAIL ROUTE (Server-Side Resolve for Direct Links)
app.get("/movie/:id", async (c) => {
    ...
    if (canWatch) {
        // Server 1 - Resolve Real URL if direct link
        let realUrl1 = initialStreamUrl;
        if (movie.linkType === 'direct') {
            realUrl1 = await resolveRedirect(initialStreamUrl);
        }
        const token = crypto.randomUUID(); 
        await kv.set(["stream_tokens", token], realUrl1, { expireIn: 3600 * 3 }); 
        playerUrl = `/stream/${token}`; 
        playbackToken = token; 

        // Server 2 - Resolve Real URL if direct link
        if (movie.streamUrl2) { 
            let realUrl2 = movie.streamUrl2;
            if (movie.linkType === 'direct') {
                realUrl2 = await resolveRedirect(movie.streamUrl2);
            }
            const token2 = crypto.randomUUID(); 
            await kv.set(["stream_tokens", token2], realUrl2, { expireIn: 3600 * 3 }); 
