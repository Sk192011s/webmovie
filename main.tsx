...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    // Episode List ခွဲထုတ်ခြင်း (မူလအတိုင်း)
    ...
    // 🔥 အဓိက ပြင်ဆင်ရမည့်နေရာ 🔥
    const existing = await kv.get(["movies", body["id"]]);
    const isUpdateRequested = body["updateTime"] === "on"; // Checkbox စစ်မယ်

    let timeToSave;
    
    // (က) ဇာတ်ကားသစ် ဖြစ်နေရင် -OR- Checkbox ခြစ်ထားရင် => လက်ရှိအချိန်ကို ယူမယ်
    if (!existing.value || isUpdateRequested) {
        timeToSave = Date.now();
    } else {
        // (ခ) မဟုတ်ရင် => မူလအချိန်အတိုင်း ထားမယ်
    ...
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Saved"); 
});
