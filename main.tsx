...
app.get("/api/mediafire-stream", async (c) => {
    ...
    try {
        ...
        if (match && match[1]) {
            // Direct Link အစစ်ကိုပဲ ပြန်ပေးမယ် (Proxy မခံတော့ဘူး)
            // VPN (US) ရှိရင် သေချာပေါက် အလုပ်လုပ်ပါမယ်
            return c.redirect(match[1], 302);
        } else {
            // ရှာမတွေ့ရင် MediaFire Webpage ဆီပဲ ပို့မယ်
