    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 🔥 FINAL FIX PLAYER (Supports Proxy & 2-Step Links)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 3. Resolve URL (ဒီနေရာက အရေးကြီးဆုံးပါ)
                // Browser ကို Redirect မလုပ်ခိုင်းဘဲ၊ Link စာသားအစစ်ကို အရင်လှမ်းတောင်းလိုက်ပါတယ်
                if (type === 'direct') { 
                    try { 
                        const res = await fetch('/api/resolve-url?token=' + content); 
                        const data = await res.json(); 
                        if (data.url) {
                            finalUrl = data.url; // Proxy Link အစစ်ကို ဒီမှာ ရပါပြီ
                        }
                    } catch (e) { 
                ...
                if (type === 'embed' || (type !== 'direct' && finalUrl.includes('<iframe'))) { 
                    ...
                    // Direct Link / Proxy Link / 2-Step Link အကုန်လုံး ဒီထဲမှာ အလုပ်လုပ်ပါမယ်
