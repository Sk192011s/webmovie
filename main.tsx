                                    ...
                                    async function generateAIDesc() {
                                        ...
                                        try {
                                            ...
                                            if(data.error) {
                                                ...
                                                box.value = data.desc; // ဖြည့်လိုက်ပြီ
                                            }
                                        } catch(e) {
...
// 🔥 AI Description Generator Route (Stable Model: gemini-flash-latest)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    ...
    const prompt = `Write a short, engaging plot summary (under 100 words) for the ${type || "movie"} titled "${title}". Write it in Burmese language (Myanmar Font). Do not include spoilers.`;

    try {
        // 👇 ဒီနေရာမှာ 'gemini-flash-latest' ကို ပြောင်းလိုက်ပါတယ် (ဒါက Free Tier ပိုများပါတယ်)
        ...
        if (data.error) {
            ...
            // Error အသေးစိတ်ကို ပြန်ပို့ပေးမယ့် ကုဒ်
            return c.json({ error: "AI Error: " + (data.error.message || "Quota Limit or API Error") });
        }
