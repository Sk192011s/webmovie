...
app.get("/api/list", async (c) => { 
    ...
    const limit = 15; 
    const { movies, nextCursor } = await getMoviesByCategoryWithCursor(cat, limit, cursor); 
...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        if (movies.length > 0) {
            ...
            // Jav/USA (Format A & B) က JSON အောက်ဆုံးကောင် Web မှာ ထိပ်ရောက်မယ်
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // (B) NEW FORMAT (USA / JAV)
                    ...
                    // 1. JSON ထဲက "category" field ကို အရင်စစ်မယ်
                    if (m.category && m.category.toLowerCase().includes("jav")) {
                        category = "Jav";
                    }
                    // 2. Title ထဲမှာ Jav Code (ABC-123) ပါမပါ စစ်မယ်
                    else if (/[a-zA-Z]{2,}-[0-9]{3,}/.test(title)) {
                        category = "Jav";
                    } 
                    // 3. ဘာမှမကိုက်ရင် USA ထားမယ်
                    else {
                        category = "USA";
                    }
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Auto Jav/USA Split)");
