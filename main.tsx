    ...
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        /* Special Class for UNCENSORED items (Landscape) */
        .h-scroll-item.wide { min-width: 200px; width: 200px; }
      ...
      <script dangerouslySetInnerHTML={{__html: `
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.toggleSeason = function(id) { const el = document.getElementById('season-' + id); const icon = document.getElementById('icon-' + id); el.classList.toggle('hidden'); icon.classList.toggle('rotate-180'); }
            window.loadPlayer = async function(content, type, movieId, title, poster) {
                const container = document.getElementById('video-player'); const cover = document.getElementById('video-cover'); const loader = document.getElementById('video-player-loader');
                if(cover) cover.style.display = 'none'; if(loader) loader.style.display = 'flex';
        ...
        async function loadMoreMovies(category) {
            ...
            try {
                ...
                data.movies.forEach(m => {
                    ...
                    // Conditional Styling for All Uncensored
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined}>
      ...
      <div class="px-3 py-6 space-y-8">{sections.map(cat => { 
          ...
          // SPECIAL LAYOUT FOR "All Uncensored"
...
// JSON API (Modified for Cover URL support)
...
// Category Page (Conditional Grid)
app.get("/category/:cat", async (c) => {
    ...
    return c.html(<Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined}>
        <div class="px-3 py-6">
            <div class="flex justify-between items-center mb-4"><h1 class="text-xl font-bold text-white flex items-center gap-2"><a href="/" class="text-gray-400"><i class="fa-solid fa-arrow-left"></i></a> {cat}</h1><span class="bg-red-600 text-[10px] px-2 py-1 rounded text-white font-bold tracking-wider">{filtered.length} ITEMS</span></div>
            
            {/* Conditional Grid: 1 Column for Uncensored, 3 Columns for Others */}
            <div id="movie-grid" class={isUncensored ? "space-y-4" : "grid grid-cols-3 gap-2"}>
                {initialMovies.map(m => (
                     <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded overflow-hidden ${isUncensored ? 'mb-4' : ''}`}>
                        <img src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} class={isUncensored ? "aspect-video object-cover w-full" : "aspect-[2/3] object-cover w-full"} />
                        <div class={isUncensored ? "p-3" : "p-1.5"}>
                            <h3 class={isUncensored ? "text-sm font-bold truncate text-white" : "text-[10px] font-bold truncate text-white"}>{m.title}</h3>
                        </div>
                     </a>
                ))}
            </div>
            <div id="loading-indicator" class="text-center py-4 hidden"><i class="fa-solid fa-circle-notch fa-spin text-red-600 text-xl"></i></div>
        </div>
    </Layout>);
});
...
app.get("/movie/:id", async (c) => { const id = c.req.param("id"); const movie = await getMovie(id); const user = await getCurrentUser(c); const config = await getConfig(); if (!movie) return c.text("Not Found", 404); const premium = isPremium(user); const isFav = user?.favorites?.includes(id); const displayImage = movie.coverUrl || movie.posterUrl; const allMovies = await getMovies(); const related = allMovies.filter(m => m.category === movie.category && m.id !== movie.id).slice(0, 6); let initialStreamUrl = movie.streamUrl; let episodes = movie.episodes || []; if (movie.category === "Series" && episodes.length > 0) initialStreamUrl = episodes[0].url; const seasons: Record<string, Episode[]> = {}; const ungrouped: Episode[] = []; if(episodes) { episodes.forEach(ep => { if(ep.season) { if(!seasons[ep.season]) seasons[ep.season] = []; seasons[ep.season].push(ep); } else { ungrouped.push(ep); } }); } let playerUrl = ""; let secureDownloadUrl = ""; let secureDownloadUrl2 = ""; let playerUrl2 = ""; let playbackToken = ""; let playbackToken2 = ""; if (premium) { if (movie.linkType === "embed" || initialStreamUrl.includes("<iframe")) { playerUrl = initialStreamUrl; } else { let realUrl = initialStreamUrl; if (movie.linkType === "direct") realUrl = await resolveRedirect(initialStreamUrl); const token = crypto.randomUUID(); await kv.set(["stream_tokens", token], realUrl, { expireIn: 3600 * 3 }); playerUrl = `/stream/${token}`; playbackToken = token; } if (movie.streamUrl2) { const token2 = crypto.randomUUID(); await kv.set(["stream_tokens", token2], movie.streamUrl2, { expireIn: 3600 * 3 }); playerUrl2 = `/stream/${token2}`; playbackToken2 = token2; } if (movie.downloadUrl) { const dlToken = crypto.randomUUID(); await kv.set(["stream_tokens", dlToken], movie.downloadUrl, { expireIn: 3600 * 3 }); secureDownloadUrl = `/dl/${dlToken}`; } if (movie.downloadUrl2) { const dlToken2 = crypto.randomUUID(); await kv.set(["stream_tokens", dlToken2], movie.downloadUrl2, { expireIn: 3600 * 3 }); secureDownloadUrl2 = `/dl/${dlToken2}`; } } return c.html( <Layout user={user} title={movie.title} announcement={config.showAnnouncement ? config.announcement : undefined}> <div class="max-w-4xl mx-auto"> <div class="w-full aspect-video bg-black relative shadow-lg group"> {premium ? ( <> <div id="video-cover" class="absolute inset-0 z-20"> <img src={displayImage} class="w-full h-full object-cover" /> </div> <div id="video-player-loader" class="video-loader"><div class="spinner"></div></div> <div id="video-player" class="w-full h-full hidden"></div> </> ) : ( <div class="absolute inset-0 flex flex-col items-center justify-center bg-zinc-900 z-30"> <img src={displayImage} class="absolute inset-0 w-full h-full object-cover opacity-20 blur-sm" /> <div class="relative z-40 text-center"> <i class="fa-solid fa-lock text-4xl text-red-600 mb-2"></i> <h2 class="text-lg font-bold text-white">VIP Only</h2> <a href={user ? "/profile" : "/login"} class="mt-4 bg-white text-black px-6 py-2 rounded font-bold text-sm inline-block">Unlock</a> </div> </div> )} </div> <div class="p-4"> <div class="flex justify-between items-start mb-2"> <h1 class="text-2xl font-bold text-white leading-tight">{movie.title}</h1> {user && ( <form action="/api/fav" method="post"> <input type="hidden" name="movieId" value={movie.id} /> <button class="text-2xl p-2"><i class={`fa-solid fa-heart ${isFav ? 'text-red-600' : 'text-gray-600'}`}></i></button> </form> )} </div> <div class="flex items-center gap-2 text-xs text-gray-400 mb-4"> <span class="bg-gray-800 px-2 py-0.5 rounded">{movie.year}</span> <span class="text-red-500 font-bold border border-red-500/50 px-2 py-0.5 rounded">{movie.category}</span> </div> {premium && movie.category !== "Series" && ( <div class="flex flex-col gap-2 mb-6"> <div class="flex gap-2 overflow-x-auto"> <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="flex-1 bg-white text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-gray-200 whitespace-nowrap"> <i class="fa-solid fa-play"></i> {movie.streamUrl2 ? "Server 1" : "Play"} </button> {movie.streamUrl2 && ( <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken2 : playerUrl2}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="flex-1 bg-gray-200 text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-white whitespace-nowrap"> <i class="fa-solid fa-server"></i> Server 2 </button> )} <button onclick={`shareMovie('${movie.title}')`} class="bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700"> <i class="fa-solid fa-share-nodes"></i> </button> </div> <div class="flex gap-2"> {secureDownloadUrl && ( <a href={secureDownloadUrl} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs"> <i class="fa-solid fa-download"></i> DL {movie.downloadUrl2 ? "1" : ""} </a> )} {secureDownloadUrl2 && ( <a href={secureDownloadUrl2} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs"> <i class="fa-solid fa-download"></i> DL 2 </a> )} </div> <button onclick="toggleHelp()" class="text-xs text-yellow-500 hover:text-yellow-400 flex items-center gap-1 mt-2"><i class="fa-solid fa-circle-question"></i> ဒေါင်းလုဒ်လုပ်နည်း</button> <div id="download-help" class="hidden bg-zinc-900 border border-yellow-600/50 rounded-lg p-3 text-xs text-gray-300 space-y-2 mt-1"> <p><strong class="text-yellow-500">နည်းလမ်း (၁) - Direct Download</strong><br/>Download (သို့) DL ခလုတ်ပါလျှင် နှိပ်၍ ဒေါင်းပါ။ မပါလျှင် Video Play ပြီးမှ ဒေါင်းပါ။</p> <hr class="border-zinc-700"/> <p><strong class="text-yellow-500">နည်းလမ်း (၂) - Video Player မှတဆင့်</strong><br/>1. Video ကို Play နှိပ်ပါ။<br/>2. Video ဖွင့်လာလျှင် ညာဘက်အောက်ထောင့်က အစက် ၃ စက် (<i class="fa-solid fa-ellipsis-vertical"></i>) ကိုနှိပ်ပါ။<br/>3. 'Download' ကို ရွေးချယ်ပါ။</p> </div> </div> )} {movie.category === "Series" && episodes.length > 0 && premium && ( <div class="mb-6 space-y-2"> {ungrouped.length > 0 && ( <div class="grid grid-cols-3 md:grid-cols-4 gap-2"> {ungrouped.map(ep => ( <button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="bg-zinc-800 hover:bg-red-600 text-xs py-3 px-1 rounded truncate text-center border border-zinc-700 transition-colors">{ep.name}</button> ))} </div> )} {Object.keys(seasons).map(season => { const safeId = season.replace(/\s+/g, '-'); return ( <div class="border border-zinc-800 rounded bg-[#1f1f1f]"> <button onclick={`toggleSeason('${safeId}')`} class="w-full flex justify-between items-center p-3 text-sm font-bold text-gray-300 hover:bg-zinc-800 transition"><span>{season}</span><i id={`icon-${safeId}`} class="fa-solid fa-chevron-down transition-transform"></i></button> <div id={`season-${safeId}`} class="hidden p-2 grid grid-cols-3 gap-2 border-t border-zinc-800"> {seasons[season].map(ep => (<button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="bg-zinc-800 hover:bg-red-600 text-xs py-2 px-1 rounded truncate text-center border border-zinc-700 transition-colors">{ep.name}</button>))} </div> </div> ) })} </div> )} <p class="text-sm text-gray-300 leading-relaxed mb-8">{movie.description}</p> {related.length > 0 && ( <div class="pt-4 border-t border-zinc-800"> <h3 class="font-bold text-white mb-4">You May Also Like</h3> <div class="h-scroll-section custom-scroll"> {related.map(m => ( <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded overflow-hidden"> <img src={m.posterUrl} /> <div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white">{m.title}</h3></div> </a> ))} </div> </div> )} </div> </div> </Layout> ); });
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-[#1f1f1f] p-4 rounded border border-zinc-700 sticky top-4">
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-2 text-sm">
                                <input type="hidden" name="id" value={editMovie?.id || crypto.randomUUID()} /><input type="hidden" name="createdAt" value={editMovie?.createdAt || Date.now()} /><input name="title" placeholder="Title" value={editMovie?.title} required class="input-box" /><div class="flex gap-2"><select name="category" class="input-box"><option>Movies</option><option>Series</option><option>Adult</option><option>All Uncensored</option></select><input name="year" value={editMovie?.year || "2025"} class="input-box w-20" /></div>
                                <input name="posterUrl" placeholder="Poster URL" value={editMovie?.posterUrl} required class="input-box" /><input name="coverUrl" placeholder="Cover URL" value={editMovie?.coverUrl} required class="input-box border-yellow-500/50" />
                                ...
                                <div class="p-2 bg-black rounded border border-zinc-800"><label class="text-xs text-yellow-500 mb-1 block">Series Episodes (Season | Ep | URL)</label><textarea name="episodeList" placeholder="Season 1 | Ep 1 | https://..." rows={5} class="input-box font-mono text-xs whitespace-nowrap overflow-x-auto">{epString}</textarea></div>
