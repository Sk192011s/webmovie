    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 RESOLVE FIRST PLAYER (Fixes Redirect Issues)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // Loader ပြန်ထည့်ပေးလိုက်ပါတယ် (Link ဖြေရှင်းနေတုန်း မငြိမ်နေအောင်ပါ)
                if(loader) loader.style.display = 'flex';

                let finalUrl = content;

                // 3. Resolve URL (Token ကို Link အစစ်ဖြစ်အောင် အရင်ပြောင်းမယ်)
                // /stream/ လမ်းကြောင်းကို လုံးဝ မသုံးတော့ပါ (Redirect ရှောင်ရန်)
                if (type === 'direct') { 
                    try { 
                        // Server ကို Token ပေးပြီး Link အစစ် တောင်းမယ်
                        const res = await fetch('/api/resolve-url?token=' + content); 
                        const data = await res.json(); 
                        if (data.url) {
                            finalUrl = data.url; 
                            console.log("Resolved URL:", finalUrl); // Debug အတွက်
                        }
                    } catch (e) { 
                ...
                // Proxy Link တွေ၊ iframe ပါတာတွေ၊ embed တွေကို Iframe နဲ့ဖွင့်မယ်
                if (type === 'embed' || finalUrl.includes('<iframe') || finalUrl.includes('proxy.avotc.tk')) { 
                    ...
                    // Iframe က Load ဖြစ်မဖြစ် မသိနိုင်လို့ 2 စက္ကန့်နေရင် Loader ဖျောက်မယ်
                    setTimeout(() => { if(loader) loader.style.display = 'none'; }, 2000);

                } else { 
                    // ကျန်တာတွေကို Video Tag နဲ့ဖွင့်မယ် (Link အစစ်ရောက်လာပြီမို့ ပွင့်ကိုပွင့်ရပါမယ်)
                    ...
                    if(video) { 
                        ...
                        video.addEventListener('error', (e) => { 
                            console.log("Video Tag Failed, Fallback to Iframe...");
                            // Video Tag နဲ့ဖွင့်မရရင် Iframe နဲ့ ထပ်စမ်းမယ် (နောက်ဆုံး လက်နက်)
...
// 🔥 OPTIMIZED MOVIE DETAIL ROUTE (4 Related, Cached Config, Proxy Player, No Lock Icon)
           ...
           <div class="w-full aspect-video bg-black relative shadow-lg group rounded-xl overflow-hidden border border-zinc-800">
                {canWatch ? (
                    <>
                        ...
                        {/* No custom loader here - relies on browser native loader or iframe fallback */}
                    </>
                ) : (
