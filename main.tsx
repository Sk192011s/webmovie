...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        if (movies.length > 0) {
            ...
            // Jav/USA (Format 1 & 3) -> အောက်ဆုံးကောင် ထိပ်ရောက် (Do Nothing)
            // Myanmar (Format 2) -> အပေါ်ဆုံးကောင် ထိပ်ရောက် (Reverse)
            
            const isJav = firstSample.name || firstSample.photo1 || firstSample.dapp; // Format 1
            const isUSA = firstSample.profileImg && firstSample.files; // Format 3 (New)
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                if (m.name || m.photo1 || m.dapp) {
                    ...
                    desc = m.info || "";
                    category = "Jav"; // Default

                    // Title Check for Jav vs USA (Legacy check)
                ...
                // (B) USA Format (NEW JSON)
                    ...
                    // Files string ကို Parse လုပ်မည်
                    let allLinks = [];
                    try {
                        const filesObj = JSON.parse(m.files);
                        if (filesObj.stream && Array.isArray(filesObj.stream)) {
                            allLinks.push(...filesObj.stream.map(s => s.url));
                        }
                        if (filesObj.download && Array.isArray(filesObj.download)) {
                            allLinks.push(...filesObj.download.map(d => d.url));
                        }
                    } catch(e) {
                    ...
                    // 1. R2 / Cloudflare
                    let target = allLinks.find(u => u.includes("r2") || u.includes("pub") || u.includes("cloudflare"));
                    
                    // 2. Vercel / Deno
                    if (!target) target = allLinks.find(u => u.includes("vercel.app") || u.includes("deno.dev"));
                    
                    // 3. MediaFire
                    if (!target) target = allLinks.find(u => u.includes("mediafire.com"));

                    if (target) finalLink = target;
                }
                ...
                // 🔥 LINK GENERATION (MediaFire vs Direct)
                let proxyStreamLink = "";
                let directDlLink = "";

                if (finalLink.includes("mediafire.com")) {
                    // MediaFire ဆိုရင် API Logic သုံးမယ်
                    proxyStreamLink = "https://proxy.avotc.tk/" + finalLink; 
                    directDlLink = "/api/mediafire-stream?link=" + encodeURIComponent(finalLink);
                } else {
                    // R2, Vercel, Deno ဆိုရင် Proxy ခံပြီး တန်းသုံးမယ်
                    proxyStreamLink = "https://proxy.avotc.tk/" + finalLink;
                    directDlLink = "https://proxy.avotc.tk/" + finalLink;
                }
                ...
                const movieData: Movie = {
                    ...
                    streamUrl: proxyStreamLink,   // Server 1
                    streamUrl2: finalLink,        // Server 2 (Original)
                    
                    downloadUrl: directDlLink,    // Download 1
                    downloadUrl2: finalLink,      // Download 2 (Original)
