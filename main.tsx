      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                if(movieId) {
                    try {
                        const progRes = await fetch('/api/watch-progress?movieId=' + movieId);
                        const progData = await progRes.json();
                        if(progData.time) savedTime = progData.time;
                    } catch(e) {}
                }
                ...
                if(video) { 
                    video.addEventListener('loadedmetadata', () => {
                         if(savedTime > 0) {
                             ...
                             showToast('Resuming playback...', 'success');
                         }
                    });
                    ...
                    video.addEventListener('timeupdate', () => {
                        ...
                        if (now - lastSave > 5000) {
                            ...
                            fetch('/api/watch-progress', {
                                ...
                                body: JSON.stringify({ movieId, time: video.currentTime })
                            });
                        }
                    });
...
app.get("/profile", async (c) => { 
    ...
    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            ...
            {/* 🔥 Slip Upload / Manual Topup */}
            <div class="bg-[#1f1f1f] p-5 rounded-2xl border border-blue-900/30 relative overflow-hidden">
                 <h3 class="font-bold text-blue-400 text-sm mb-3 flex items-center gap-2"><i class="fa-solid fa-money-bill-transfer"></i> Manual Top-up (KPay/Wave)</h3>
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <label class="cursor-pointer relative">
            ...
            <div class="p-3 rounded-xl bg-black/50 border border-zinc-700 peer-checked:border-orange-500 peer-checked:bg-orange-500/10 transition text-center hover:bg-zinc-800">
                <i class="fa-solid fa-wallet text-orange-500 mb-1 text-lg block"></i>
        ...
        <label class="cursor-pointer relative">
            ...
            <div class="p-3 rounded-xl bg-black/50 border border-zinc-700 peer-checked:border-yellow-400 peer-checked:bg-yellow-400/10 transition text-center hover:bg-zinc-800">
                <i class="fa-solid fa-money-bill-wave text-yellow-400 mb-1 text-lg block"></i>
    ...
    <!-- Amount Input -->
    <input type="number" name="amount" placeholder="ပမာဏ ထည့်ပါ (Amount in Ks)" required class="input-box bg-black/50 text-sm focus:border-blue-500 transition" />
</div>
                    <input name="transactionId" placeholder="Transaction ID (e.g. 01234...) or Link" required class="input-box bg-black/50 text-sm" />
                    <button class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition text-xs">Submit Slip</button>
                 </form>
            </div>
