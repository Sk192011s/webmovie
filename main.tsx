...
// 🔥 MEDIAFIRE STREAMER (Premium + Standard Support)
app.get("/api/mediafire-stream", async (c) => {
    ...
    try {
        ...
        // 1. Link ကို လှမ်းခေါ်ကြည့်မယ် (Redirect တွေကို လိုက်သွားမယ်)
        const res = await fetch(cleanUrl, {
            ...
            redirect: "follow" // 🔥 အရေးကြီးဆုံး: Redirect ဆုံးတဲ့ထိ လိုက်မယ်
        });

        // 2. Content-Type ကို စစ်မယ် (Video ဖိုင်လား Webpage လား)
        const contentType = res.headers.get("content-type") || "";

        if (contentType.includes("video") || contentType.includes("application/octet-stream")) {
            // ✅ Premium Link (Direct File) ဖြစ်နေရင်
            // Download ဆက်မလုပ်ဘဲ ရပ်လိုက်မယ် (Server ဝန်မပိအောင်)
            try { res.body?.cancel(); } catch(e) {}

            // Final URL (Redirect ဆုံးတဲ့ Link) ကိုယူပြီး Proxy ခံမယ်
            const finalDirectLink = res.url;
            return c.redirect("https://proxy.avotc.tk/" + finalDirectLink, 302);
        }

        // 3. Webpage ဖြစ်နေရင် (Standard Link) HTML ဖတ်ပြီး ခလုတ်ရှာမယ်
        ...
        if (match && match[1]) {
            // Standard Link ရရင်လည်း Proxy ခံပြီးပေးမယ်
            return c.redirect("https://proxy.avotc.tk/" + match[1], 302);
        } else {
            // ရှာမတွေ့ရင် Webpage ဆီပဲ Redirect လုပ်မယ်
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // (B) NEW FORMAT (USA / JAV)
                    ...
                    if (m.category && m.category.toLowerCase().includes("jav")) category = "Jav";
                    else if (/[a-zA-Z]{2,}-[0-9]{3,}/.test(title)) category = "Jav";
                    else category = "USA";
                ...
                if (preProxiedLink) {
                    ...
                    if (bestRawLink.includes("mediafire.com")) {
                        // 🔥 MEDIAFIRE LOGIC (Premium or Standard)
                        // Premium ဖြစ်စေ၊ Standard ဖြစ်စေ API ဆီပို့မယ် (API က Auto ခွဲခြားပြီး Proxy လုပ်ပေးမှာမို့)
                        ...
                        // R2 / Deno / Vercel -> Proxy ခံပြီး တန်းသုံးမယ်
                        server1 = PROXY_BASE + bestRawLink;
                        dl1 = PROXY_BASE + bestRawLink;
                    }
