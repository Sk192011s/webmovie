...
// Get Latest Movies (Paginated via Index)
...
// Get Category Movies (Paginated via Index)
...
// Search (Still scan but optimized to specific fields)
async function searchMoviesDB(query: string) {
    ...
    for await (const res of iter) {
        ...
        if (m.title.toLowerCase().includes(q) || (m.tags && m.tags.toLowerCase().includes(q))) {
            results.push(m);
            if (results.length >= 50) break; // Limit search results to 50 for speed
        }
    }
    return results;
}

// Re-Index All (Maintenance)
...
const Layout = (props: { children: any; title?: string; user?: User | null; hideNav?: boolean; announcement?: string; isAdmin?: boolean }) => {
  ...
  const protectJS = props.isAdmin ? "" : `document.addEventListener('contextmenu', event => event.preventDefault()); window.addEventListener('dragstart', event => event.preventDefault());`;
...
app.get("/category/:cat", async (c) => { 
    ...
    const config = await getConfig(); 
    // Fetch Only 50 Latest items for now (Better than all)
...
app.get("/favorites", async (c) => { const user = await getCurrentUser(c); if(!user) return c.redirect("/login"); 
    // Optimization: Fetch saved IDs only
    const favs = [];
    if(user.favorites) {
...
app.get("/movie/:id", async (c) => {
    ...
    // Optimized Related: Fetch just 6 from same category (Index)
