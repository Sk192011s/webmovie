...
// 🔥 AI Description Generator (Auto-Retry for Jav/Explicit Content)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    ...
    // Function to call AI (AI ခေါ်မည့် ဖန်ရှင်)
    ...
    // ---------------------------------------------------------
    // အဆင့် (၁) - Channel Admin Style (အမိုက်စား ရေးခိုင်းမည်)
    // ---------------------------------------------------------
    const stylePrompt = `
    Role: Popular Movie Channel Admin.
    Task: Rewrite/Translate this plot into engaging Burmese (Myanmar Unicode).
    Style: Natural storytelling, suspenseful, exciting. Avoid textbook endings like "သည်/မည်". Use "တယ်/မယ်/နော်".
    Input: "${currentDesc || title}"
    Output: ONLY Burmese text.
    `;

    let result = await askGemini(stylePrompt);

    // ---------------------------------------------------------
    // အဆင့် (၂) - မရခဲ့ရင် (Jav/Explicit ဖြစ်နေရင်) ရိုးရိုး ဘာသာပြန်ခိုင်းမည်
    // ---------------------------------------------------------
    if (!result) {
        console.log("Style mode failed. Retrying with direct translation...");
        const rawPrompt = `
        Task: Translate the following text to Burmese (Myanmar Unicode) directly.
        Input: "${currentDesc || title}"
        Requirement: Translate accurately. Do not summarize. Output ONLY Burmese.
        `;
        result = await askGemini(rawPrompt);
    }

    // နောက်ဆုံးထိ မရမှ Error ပြမယ်
    if (!result) return c.json({ error: "AI blocked this content (Too Explicit)." });
