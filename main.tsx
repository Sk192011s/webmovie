...
async function saveMovieDB(movie: Movie) {
    ...
    if (!old.value) {
        op.sum(["counts", movie.category], 1n);
    } else if (old.value.category !== movie.category) {
        op.sum(["counts", old.value.category], -1n); 
        op.sum(["counts", movie.category], 1n);      
...
// 🔥 SAFE DELETE FUNCTION (Error Fix)
async function deleteMovieDB(id: string) {
    try {
        // ၁. ဖျက်မယ့်ကား ရှိမရှိ အရင်စစ်မယ်
        const movieRes = await kv.get<Movie>(["movies", id]);
        
        // မရှိရင် ဘာမှဆက်မလုပ်ဘူး (Error မတက်တော့ဘူး)
        if (!movieRes.value) return;

        const m = movieRes.value;
        const op = kv.atomic();

        // ၂. ဇာတ်ကားနဲ့ ပတ်သက်တာတွေ အကုန်ဖျက်မယ်
        op.delete(["movies", id]);
        op.delete(["idx_time", m.createdAt, id]);
        op.delete(["idx_cat", m.category, m.createdAt, id]);

        // ၃. Counter လျှော့မယ် (Error မတက်အောင် စစ်မယ်)
        if (m.category) {
            // Deno KV မှာ sum က error တက်လွယ်လို့ သတိထားရတယ်
            // ဒီနေရာမှာ Error တက်လည်း ကိစ္စမရှိအောင် Try Catch ခံစရာမလိုဘဲ
            // atomic operation မို့လို့ တခါတည်း ပို့လိုက်မယ်
            op.sum(["counts", m.category], -1n);
        }
        ...
        if (!result.ok) {
            console.error("Delete Commit Failed for:", id);
        }

    } catch (e) {
        // Error တက်ရင် Console မှာပဲပြမယ်၊ Server ကို 500 မပြစေဘူး
