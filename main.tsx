...
// 🔥 AI Description Generator (Natural Movie Review Style)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    ...
    // 🔥 PROMPT ပြင်ဆင်ချက် (လူရေးသလို ဖြစ်အောင် ခိုင်းခြင်း)
    const baseInstruction = `
    Role: You are a popular Movie Channel Admin who writes engaging movie reviews/synopses for Myanmar audience.
    Target Language: Burmese (Myanmar Unicode).
    
    Style Guidelines:
    1. ❌ AVOID rigid/textbook endings like "သည်", "မည်", "ဖြစ်သည်", "လုပ်ဆောင်သည်".
    2. ✅ USE natural storytelling style (Spoken/Narrative style). Use endings like "တယ်", "မယ်", "ပါလိမ့်မယ်", "...", "နော်", "ပါ".
    3. Tone: Dramatic, Suspenseful, and Exciting. Make the reader want to watch the movie immediately.
    4. Structure: Start with an interesting hook. Explain the conflict. End with a suspenseful question or recommendation (Don't spoil the ending).
    `;

    let contentToProcess = "";
    
    if (currentDesc && String(currentDesc).trim().length > 10) {
        // Translate Mode
        contentToProcess = `Rewrite and Translate this plot into natural Burmese movie synopsis style:\n"${currentDesc}"`;
    } else {
        // Generate Mode
        contentToProcess = `Find the plot for "${title}" (${year}) and write a natural Burmese movie synopsis.`;
    }

    const finalPrompt = baseInstruction + "\n\nTask: " + contentToProcess;
    ...
    try {
        ...
        if (data.error) {
            ...
            return c.json({ error: "AI Error: " + (data.error.message || "Unknown API Error") });
        }

        const result = data?.candidates?.[0]?.content?.parts?.[0]?.text;

        if (!result) return c.json({ error: "AI returned empty text." });
        
        return c.json({ desc: result });

    } catch (e) {
        return c.json({ error: "Connection Error: " + e.message });
    }
});
