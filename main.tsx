...
app.use("*", secureHeaders({
  xFrameOptions: "SAMEORIGIN",       // သူများ Website က Iframe ထဲထည့်ပြီး ခိုးပြတာ ကာကွယ်မယ်
  xContentTypeOptions: "nosniff",    // File Type တွေကို Browser က မှားယွင်းစွာ အလုပ်မလုပ်အောင် ကာကွယ်မယ်
  xXssProtection: "1; mode=block",   // Cross-Site Scripting (XSS) ကို ကာကွယ်မယ်
}));
// 👆 ပြီးပါပြီ
...
// ENV မရှိရင် ဆက်မလုပ်ဘဲ App ကို ရပ်လိုက်မည့် Code
if (!SALT || !ADMIN_PASS) {
  console.error("❌ ERROR: You MUST set 'SECRET_SALT' and 'ADMIN_PASSWORD' in Environment Variables.");
  Deno.exit(1); 
}

// 👇 ဒီစာကြောင်း ပျောက်သွားလို့ Error တက်နေတာပါ (ပြန်ထည့်ပေးလိုက်ပါ)
const ADMIN_ROUTE = "/soekyawwin"; 

const ADMIN_SESSION_EXPIRE = 24 * 60 * 60 * 1000; 
// const loginAttempts = ... (ဒါက ခုနက ဖျက်ခိုင်းထားလို့ မပါတော့တာ မှန်ပါတယ်)
...
async function checkRateLimit(ip: string): Promise<boolean> {
  ...
  // ၅ ကြိမ်ထက်ကျော်ရင် Block မယ် (false ပြန်မယ်)
  if (count >= 5) return false;

  // အကြိမ်ရေ ၁ တိုးမယ်၊ ၅ မိနစ် (5 * 60 * 1000) ပြည့်ရင် အလိုလို ပြန် reset ကျမယ်
...
// =======================
// 🔥 SAVE FUNCTION (Time Change Logic Added)
async function saveMovieDB(movie: Movie) {
    ...
    // ၁. အရင်က ရှိပြီးသားလား စစ်မယ်
    const oldRes = await kv.get<Movie>(["movies", movie.id]);
    const old = oldRes.value;
    
    // ၂. အဟောင်းရှိပြီးသားဆိုရင် ရှင်းလင်းရေး အရင်လုပ်မယ်
    if (old) {
        // (က) Category ပြောင်းသွားရင် အဟောင်းဖျက်၊ Counter လျှော့
        if (old.category !== movie.category) {
            ...
            try {
                ...
                // အသစ်အတွက် Counter တိုးတာကို အောက်မှာ ဆက်လုပ်မယ်
            } catch (e) {}
        }

        // (ခ) 🔥 အချိန်ပြောင်းသွားရင် (Move to Top) နေရာဟောင်းက Index ကို ဖျက်ရမယ်
        if (old.createdAt !== movie.createdAt) {
             await kv.delete(["idx_time", old.createdAt, old.id]);
             // Category မပြောင်းပေမယ့် အချိန်ပြောင်းရင် Category Index မှာလည်း အချိန်ပါလို့ ဖျက်ပေးရမယ်
             if (old.category === movie.category) {
                 await kv.delete(["idx_cat", old.category, old.createdAt, old.id]);
             }
        }
    ...
    // ၃. Counter အသစ်တိုးခြင်း (Category ပြောင်းခြင်း သို့မဟုတ် အသစ်ဖြစ်ခြင်း)
...
// 🔥 MANUAL FORCE DELETE (100% WORKING)
async function deleteMovieDB(id: string) {
    // ၁. ဖျက်မယ့်ကား ဒေတာကို အရင်ဆွဲထုတ်
    const res = await kv.get<Movie>(["movies", id]);
    
    // မရှိရင် ဘာမှဆက်မလုပ်ဘူး
    if (!res.value) return;

    const m = res.value;

    // ၂. တစ်ခုချင်းစီ သီးသန့် လိုက်ဖျက်မယ် (Atomic မသုံးတော့ဘူး)
    // ဒါမှ တစ်ခု Error တက်လည်း ကျန်တာတွေ ပျက်သွားမှာ
    
    // (က) ဇာတ်ကားအဓိကဖိုင် ဖျက်မယ်
    ...
    // ၃. Counter လျှော့မယ် (Manual Calculation)
    // ဒါ Error တက်ရင်လည်း ကိစ္စမရှိဘူး၊ ဇာတ်ကားက ပျက်သွားပြီ
    try {
        const countKey = ["counts", m.category];
        const countRes = await kv.get(countKey);
        
        if (countRes.value) {
            // လက်ရှိဂဏန်းကို ယူ၊ ၁ နှုတ်၊ ပြီးရင် ပြန်သိမ်း
...
`;
  ...
  <html lang={l}>
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        .bottom-nav { 
            ...
            /* အရောင်ခွဲလိုက်ပါပြီ (အမဲရောင် အတိမဟုတ်ဘဲ Dark Grey) */
            background: #151515; 
            
            /* အပေါ်မျဉ်းကို ပိုထင်ရှားအောင် လုပ်လိုက်ပါတယ် */
            border-top: 1px solid #333; 
            
            /* အပေါ်ကို အရိပ်ထွက်အောင် လုပ်လိုက်ပါတယ် (Layer ခွဲသလို ဖြစ်သွားမယ်) */
        ...
        /* 🔥 SCROLLBAR HIDDEN (အတန်းပျောက်ကုဒ်) */
        .h-scroll-section { 
            display: flex; 
            overflow-x: auto; 
            gap: 12px; 
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 Tab Switcher Logic (Toggle ON/OFF)
    ...
    <body>
      ...
      {/* 👇 မိတ်ဆွေပြထားတဲ့ Download Modal အဟောင်း */}
      ...
      {/* 👆 အဟောင်းပြီးသွားပြီ (ဒီအောက်မှာ အသစ်ထည့်ပါ) */}


      {/* 👇 ဒီနေရာမှာ Download Guide Modal (Code No.1) ကို ထည့်ပါ */}
...
app.get("/api/cron/reindex", async (c) => {
    // Secret Key စစ်မယ် (Browser က လျှောက်ဝင်လို့ မရအောင်)
    const key = c.req.query("key");
    
    // ⚠️ ဒီနေရာမှာ မိတ်ဆွေကြိုက်တဲ့ Key နာမည် ပြောင်းထည့်ပါ
    if (key !== "GoldFlix_Cron_Key_999") {
        return c.text("Unauthorized", 401);
    }

    // Database Re-Sync လုပ်မယ်
...
app.get("/", async (c) => {
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        ...
        {/* 👇👇👇 POPUP LOGIC WITH SESSION CHECK 👇👇👇 */}
        {shouldShowPopup && (
          <div id="promo-popup" class="fixed inset-0 z-[200] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
              <div class="relative w-full max-w-sm bg-[#1f1f1f] rounded-2xl overflow-hidden shadow-2xl border border-yellow-500/30 modal-enter flex flex-col max-h-[85vh]">
                  ...
                  <div class="overflow-y-auto custom-scroll flex-grow">
                      ...
                      <div class="p-5 bg-[#1f1f1f]">
                          {/* 👇 စာသားအသစ် ပြမည့်နေရာ (ရှိမှပြမယ်) 👇 */}
                          {config.popupMessage && (
                          ...
                          {/* Action Button (မြှားဖြုတ်လိုက်ပါပြီ) */}
              ...
              <script dangerouslySetInnerHTML={{__html: `
                  ...
                  // ၁. ဒီနေ့အတွက် "မကြည့်တော့ပါ" နှိပ်ထားလား စစ်မယ်
                  const isHiddenToday = localStorage.getItem('popup_hidden_date') === today;
                  
                  // ၂. ဒီ Browser ဖွင့်ထားတဲ့ Session အတွင်း ပိတ်ဖူးလား စစ်မယ်
                  const isHiddenSession = sessionStorage.getItem('popup_closed_session');

                  // နှစ်ခုလုံး မငြိမှ Popup ပေါ်မယ်
                  ...
                  window.closePopup = function(hideToday) {
                      ...
                      if(hideToday) {
                          // "Don't show again" နှိပ်ရင် တနေကုန် ပိတ်မယ်
                          localStorage.setItem('popup_hidden_date', today);
                      } else {
                          // "X" နှိပ်ရင် Tab မပိတ်မချင်း ပြန်မပေါ်တော့ဘူး (Refresh လုပ်လည်း မပေါ်တော့ဘူး)
                    ...
                    <div class="h-scroll-section custom-scroll">
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform movie-card group">
                                <div class="aspect-[2/3] w-full relative overflow-hidden"><img src={m.posterUrl} loading="lazy"class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" /><div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow">
        ...
        <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
            <div class="p-4">
                ...
                <div class="grid grid-cols-3 gap-3">
                    {results.map(m => {
                        // Uncensored ဆိုရင် Cover ပုံသုံးမယ်၊ မဟုတ်ရင် Poster သုံးမယ်
                        ...
                        return (
                            <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'col-span-3' : ''}`}>
                                <div class={`${isUncensored ? "aspect-video" : "aspect-[2/3]"} relative overflow-hidden`}>
                                    <img src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} loading="lazy" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
      ...
      <div id="video-player" class="w-full h-full hidden"></div>
           ...
           <div class="p-6">
               <div class="flex justify-between items-start mb-3">
                   ...
                   {/* 🔥 ALIGNMENT FIX: Fixed Height & Flex Center */}
                   <div class="flex items-center gap-2 flex-shrink-0 h-10">
                       
                       {/* Share Button (Icon Only, Moved Here) */}
               ...
               {premium && (
                   <div class="flex flex-col gap-3 mb-8">
                        ...
                        {/* 🔥 FIX: Series သို့မဟုတ် Animation (အပိုင်းပါလျှင်) အပေါ်ခလုတ်ဖျောက်မည် */}
                        {movie.category !== "Series" && !(movie.category === "Animation" && episodes.length > 0) && (
                            <div class="grid grid-cols-2 gap-2">
                                <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`} class="srv-btn w-full bg-white text-black font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition hover:brightness-110 shadow-lg text-xs">
                                    {/* 🔥 FIX: Play အစား Server 1 လို့ အမြဲပေါ်ချင်ရင် t.server1 ထည့်လိုက်ပါ */}
                        ...
                        )}                      {/* 👇👇👇 ဒီခလုတ်လေး လွတ်နေလို့ပါ၊ ဒါလေး ကူးပြီး ထည့်လိုက်ပါ 👇👇👇 */}
...
{/* 👆👆👆 ပြီးပါပြီ 👆👆👆 */}                        
               ...
               {(movie.category === "Series" || movie.category === "Animation") && episodes.length > 0 && premium && ( 
                   <div class="mb-8">
                       ...
                       {/* 2. EPISODES AREA (No default background here) */}
                       <div id="episodes-container">
                           {Object.keys(seasons).map((season, idx) => {
                               const safeId = season.replace(/\s+/g, '-');
                               return (
                                   // 🔥 STYLE MOVED HERE: Background & Border only appear when this div is shown
    ...
    {ep.name}
               ...
               {/* 👇👇👇 UPDATED RELATED MOVIES SECTION 👇👇👇 */}
               {related.length > 0 && (
      ...
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            ...
            <div class="grid grid-cols-2 gap-3">
                 ...
                 {/* Days Left (No more progress bar) */}
...
// ADMIN DASHBOARD (FULL FIX VERSION)
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    // Cache Fix (List မပျောက်အောင်)
    c.header('Cache-Control', 'private, no-cache, no-store, must-revalidate');
    c.header('Expires', '-1');
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                {/* MAGIC TOOLS: TOOL 1 + TOOL 2 (FULL) */}
                                <div class="bg-zinc-800 p-3 rounded-xl mb-2 border border-zinc-700">
                                    ...
                                    {/* JAVASCRIPT LOGIC (BINARY BUSTER + DECODE FIX ✅) */}
                                    <script dangerouslySetInnerHTML={{__html: `
                                        // FUNCTION FOR TOOL 1
                                        ...
                                        // FUNCTION FOR TOOL 2 (ULTIMATE FIX 🚀)
                                        function extractLinks() {
                                            try {
                                                ...
                                                // 1. AGGRESSIVE CLEANING (အမှိုက်ရှင်းခြင်း)
                                                // Binary Code တွေကို Enter (New Line) အဖြစ် ပြောင်းပစ်မယ်
                                                // ဒါမှ Link တွေက တစ်ကြောင်းစီ ကွဲထွက်လာမှာ
                                                rawText = rawText.replace(/[\\x00-\\x1F\\x7F]/g, "\\n");

                                                // 2. EXTRACT (Link ရှာခြင်း)
                                                // http နဲ့စမယ်... Extension (.txt, .mp4) တွေ့တဲ့အထိ ယူမယ်
                                                // Non-greedy match (*?) ကို သုံးထားလို့ Extension တွေ့တာနဲ့ ချက်ချင်းရပ်မယ် (နောက်က အမှိုက်တွေ မပါတော့ဘူး)
                                                ...
                                                // 3. DECODE & NORMALIZE (Link တွေကို ပုံစံတူအောင် ညှိမယ်)
                                                // %20 တွေကို Space ပြောင်းမယ်, Duplicate ဖယ်မယ်
                                                ...
                                                // 4. SMART SORT (နံပါတ်စဉ်လိုက် စီမယ်)
                                                ...
                                                uniqueLinks.forEach(url => {
                                                    // အနောက်မှာ အစက် (.) ပါနေရင် ဖြုတ်မယ်
                                                    let cleanUrl = url;
                                                    if (cleanUrl.endsWith('.')) cleanUrl = cleanUrl.slice(0, -1);
                                                    
                                                    // အရမ်းတိုလွန်းတဲ့ Link (အမှား) တွေကို ဖယ်မယ်
                                                ...
                                                alert(\`Success! Extracted & Fixed \${addedCount} links!\`);
                                ...
                                <div class="p-3 bg-black/40 rounded-xl border border-zinc-800 mt-2">
                                    <label class="text-[10px] text-pink-500 uppercase font-bold mb-1 block">Cast / Actors</label>
                                    <textarea name="castList" placeholder="Name | Photo URL" rows={4} class="input-box w-full font-mono text-xs whitespace-pre overflow-x-auto bg-[#111]">
                                        {editMovie?.cast?.map(c => `${c.name} | ${c.img}`).join('\n') || ""}
                                    </textarea>
                                </div>
                                ...
                                <label class="flex items-center gap-2 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700 cursor-pointer hover:bg-zinc-800 transition my-2">
                                    ...
                                    <span class="text-xs font-bold text-gray-300">Move to Top (New Episode Updated)</span>
                                </label>

                                <button class={`w-full py-3 text-sm uppercase tracking-widest shadow-lg font-bold rounded-lg transition ${editMovie ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/20' : 'btn-primary'}`}>
                                    {editMovie ? "Update Existing Movie" : "Save New Movie"}
                                </button>
                            </form>
                        </div>
                        ...
                        <div class="lg:col-span-2 bg-[#111] p-5 rounded-2xl border border-zinc-800 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw] shadow-xl">
                            ...
                            <div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scroll">
                                {movies.map(m => (
                                    <div class={`movie-item flex gap-4 p-3 rounded-xl items-center group relative border transition ${editMovie?.id === m.id ? 'bg-blue-900/10 border-blue-500' : 'bg-black border-zinc-800/50 hover:border-zinc-600'}`} data-title={m.title}>
...
// ၁။ ဒါက မူလရှိပြီးသား ကုဒ် (Announcement အတွက်) -> ဒီအတိုင်းထားပါ၊ မဖျက်ပါနဲ့
...
app.post("/admin/config/popup", adminGuard, async (c) => { 
    ...
    await kv.set(["config"], { 
        ...
        popupMessage: body['popupMessage'], // 👈 ဒါလေး ထပ်ထည့်လိုက်ပါတယ်
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    // 🔥🔥🔥 OVERWRITE PROTECTION LOGIC (HERE) 🔥🔥🔥
    let id = body["id"] as string;
    const mode = body["mode"] as string; // Form က ပို့လိုက်တဲ့ mode ကို ယူမယ်
    
    // Database မှာ ဒီ ID ရှိမရှိ စစ်မယ်
    const existing = await kv.get(["movies", id]);
    const isUpdateRequested = body["updateTime"] === "on";

    // အကယ်၍ "အသစ်တင်တာ (create)" ဖြစ်နေပေမယ့် "ID က ရှိပြီးသား" ဖြစ်နေရင်
    // ဒါဟာ Browser Cache ကြောင့် မှားနေတာ ဖြစ်လို့ ID အသစ် ထုတ်ပေးလိုက်မယ်
    if (mode === "create" && existing.value) {
        console.log("⚠️ Potential Overwrite Detected! Generating new ID for safe creation.");
        id = crypto.randomUUID(); // ID အသစ် ပြောင်းလိုက်ပြီ (အဟောင်း မပျက်တော့ဘူး)
    }

    let timeToSave;
    // အသစ်တင်တာ (သို့) ID အသစ်ပြောင်းသွားတာ (သို့) Checkbox ခြစ်ထားတာ ဆိုရင် အချိန်အသစ်ယူမယ်
    if ((mode === "create") || isUpdateRequested || !existing.value) {
        timeToSave = Date.now();
    } else {
        timeToSave = Number(body["createdAt"]);
    }

    const movie = { 
        ...body, 
        id: id, // စစ်ဆေးပြီးသား ID ကို သုံးမယ်
        createdAt: timeToSave, 
        episodes 
    }; 
    
    await saveMovieDB(movie as Movie); 
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Saved Successfully"); 
});
