                   ...
                   <div class="mb-8">
                       ...
                       <div class="grid grid-cols-3 gap-2">
                           {Object.keys(seasons).map(season => { 
                               ...
                               return (
                                   <div class="border border-zinc-800 rounded-lg bg-[#1f1f1f] h-fit">
                                       {/* Season Header Button */}
                                       <button onclick={`toggleSeason('${safeId}')`} class="w-full flex justify-between items-center p-2.5 text-xs font-bold text-gray-300 hover:bg-zinc-800 transition">
                                           <span class="truncate">{season}</span>
                                           <i id={`icon-${safeId}`} class="fa-solid fa-chevron-down text-[10px] transition-transform"></i>
                                       </button>
                                       
                                       {/* Episodes List (Inside the small box) */}
                                       <div id={`season-${safeId}`} class="hidden p-2 border-t border-zinc-800 bg-black/30">
                                            <div class="flex flex-col gap-2">
                                               {seasons[season].map(ep => (
                                                   <button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn w-full bg-zinc-800 hover:bg-yellow-500 hover:text-black text-[10px] py-2 px-1 rounded truncate text-center border border-zinc-700 transition-colors">
                                                       {ep.name}
                                                   </button>
                                               ))}
                                            </div>
