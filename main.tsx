...
app.get("/movie/:id", async (c) => {
    ...
    return c.html(
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        ...
        {/* 🔥 Modal Design Fix (ခလုတ်ညီအောင် ပြင်ထားသည်) */}
        <div id="buy-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
             <div class="bg-[#1f1f1f] p-6 rounded-2xl border border-yellow-500/30 w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  ...
                  <div class="flex gap-3 h-12"> {/* Height သတ်မှတ်လိုက်သည် */}
                      ...
                      {user ? (
                          <form action="/api/buy-movie" method="post" class="flex-1 h-full"> {/* Form ကို height အပြည့်ပေးသည် */}
                              <input type="hidden" name="movieId" value={movie.id} />
                              <button class="w-full h-full rounded-xl bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold hover:brightness-110 transition shadow-lg">
                                  Buy Now
                              </button>
                          </form>
                      ) : (
        ...
        <div class="max-w-4xl mx-auto">
           ...
           <div class="p-6">
               ...
               {/* 🔥 Purchased Notification (ဝယ်ပြီးကြောင်းပြသည့် Noti Bar) */}
               {isPurchased && (
                   <div class="bg-gradient-to-r from-green-900/40 to-green-600/10 border border-green-500/30 p-3 rounded-xl mb-4 flex items-center gap-3">
                       ...
                       <div>
                           <h3 class="text-green-400 font-bold text-sm">ဝယ်ယူပြီးပါပြီ (Purchased)</h3>
                           <p class="text-green-500/60 text-[10px]">You have lifetime access to this video.</p>
                       </div>
                   </div>
               )}
