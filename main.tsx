...
// 1. SECURITY & CONFIG
// =======================
// crypto.randomUUID() ကို ဖြုတ်ပြီး ဒီလို စာသားအသေ ထည့်လိုက်ပါ
const SALT = Deno.env.get("SECRET_SALT") || "MY_SUPER_SECRET_FIXED_SALT_2025"; // Auto generate if missing
...
function checkRateLimit(ip: string): boolean {
  ...
  // ၁ မိနစ် ကျော်သွားပြီလား စစ်မယ် (ကျော်ရင် Reset ချမယ်)
  if (now - record.time > 60 * 1000) { 
    loginAttempts.set(ip, { count: 1, time: now });
    return true;
  }

  // ၃ ကြိမ်ထက် များနေရင် (၃ ကြိမ်မြောက်မှာတင် ပိတ်မယ်)
...
interface User { 
    ...
    favorites: string[]; 
    sessionId?: string;
    ip?: string;          // <--- (အသစ်တိုး) မှတ်ပုံတင်တုန်းက IP
    lastLoginIp?: string; // <--- (အသစ်တိုး) နောက်ဆုံးဝင်ခဲ့တဲ့ IP
}
...
// AUTH
app.get("/login", (c) => c.html(<Layout hideNav={true}><div class="min-h-screen flex items-center justify-center bg-black p-4"><div class="w-full max-w-sm"><h1 class="text-3xl font-black text-red-600 mb-8 text-center italic">GOLD FLIX</h1><form action="/login" method="post" class="bg-[#1f1f1f] p-6 rounded-lg border border-zinc-800 space-y-4 shadow-xl"><h2 class="text-xl font-bold text-white mb-2">Sign In</h2><input name="username" placeholder="Username" required class="input-box" /><input type="password" name="password" placeholder="Password" required class="input-box" /><label class="flex items-center text-gray-400 text-xs"><input type="checkbox" name="remember" class="mr-2 accent-red-600" /> Remember Me </label><button class="btn-primary w-full mt-2">Login</button><p class="text-xs text-gray-500 text-center mt-4">အကောင့်မရှိဘူးလား? <a href="/signup" class="text-white font-bold">ဖွင့်ရန်</a></p></form></div></div></Layout>));
// LOGIN ROUTE (UI Design + IP Tracking Combined)
app.post("/login", async (c) => { 
    // 🔥 1. IP ကို အရင်ဖမ်းမယ် (Deno Deploy အတွက် အတိအကျဖြစ်အောင် split လုပ်ထားပါတယ်)
    const ip = c.req.header("x-forwarded-for")?.split(",")[0] || "unknown";
    
    // 🔥 2. Rate Limit UI (အစ်ကို့ရဲ့ ဒီဇိုင်းအလန်းလေး)
            ...
            <Layout hideNav={true} title="Security Alert">
                <div class="min-h-screen flex items-center justify-center bg-black p-4 relative overflow-hidden">
                    ...
                    <div class="relative z-10 bg-[#1f1f1f] p-8 rounded-2xl border border-red-500/30 shadow-2xl max-w-sm w-full text-center">
                        <div class="w-20 h-20 bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-6 border border-red-500/20">
                        ...
                        <p class="text-gray-400 text-sm mb-6 leading-relaxed">
                            မှားယွင်းစွာ ဝင်ရောက်မှု အကြိမ်အရေအတွက် များနေပါသည်။ လုံခြုံရေးအရ သင့်ကို ခေတ္တပိတ်ပင်ထားပါသည်။
                        </p>
                        
                        <div class="bg-black/50 rounded-lg p-3 border border-red-900/50 mb-6">
                            <p class="text-yellow-500 text-xs font-bold uppercase tracking-wider">Please Wait</p>
                            <p class="text-white font-mono text-lg font-bold">1 Minute</p>
                        </div>

                        <a href="/login" class="block w-full bg-zinc-700 hover:bg-zinc-600 text-white font-bold py-3 rounded-lg transition border border-zinc-600">
                            Back to Login
                        </a>
                    </div>
                </div>
    ...
    if (user && user.passwordHash === hashedInput) { 
        ...
        // 🔥 3. ဒီနေရာမှာ IP ကို Database ထဲ ထည့်သိမ်းလိုက်ပါပြီ 🔥
        user.lastLoginIp = ip; 
...
app.post("/signup", async (c) => { 
    ...
    const newUser: User = { 
        ...
        favorites: [], 
        sessionId: "",
        ip: ip,            // <--- IP သိမ်းလိုက်ပြီ
        lastLoginIp: ip    // <--- IP သိမ်းလိုက်ပြီ
    }; 
...
// ==========================================
// ADMIN DASHBOARD (UPDATED WITH USER STATS)
// ==========================================
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    // 5. 🔥 FETCH USERS (အသစ်ထည့်လိုက်သော အပိုင်း) 🔥
                    ...
                    <div class="max-w-4xl mx-auto">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                             <div class="bg-gradient-to-br from-blue-900/40 to-blue-900/10 border border-blue-500/30 p-6 rounded-xl flex items-center gap-4">
                                 ...
                                 <div>
                                     <h3 class="text-xs uppercase text-blue-400 font-bold tracking-wider">Total Users</h3>
                                     <p class="text-3xl font-black text-white">{totalUsers}</p>
                                 </div>
                             </div>
                             {/* User List Box with IP */}
                             <div class="bg-[#1f1f1f] border border-zinc-700 rounded-xl overflow-hidden h-64 flex flex-col">
                                 <h3 class="bg-black/50 px-4 py-2 text-xs font-bold text-gray-400 border-b border-zinc-800 flex justify-between">
                                     <span>Username</span>
                                     <span>IP Address / Status</span>
                                 </h3>
                                         ...
                                         return (
                                            <div class="flex justify-between items-center bg-black/50 p-2 rounded text-xs border border-zinc-800/50 hover:border-zinc-600 transition">
                                                <div class="flex items-center gap-2">
                                                    <div class={`w-2 h-2 rounded-full ${isVip ? 'bg-green-500' : 'bg-gray-500'}`}></div>
                                                    <span class="text-gray-300 font-bold truncate">{u.username}</span>
                                                </div>
                                                <div class="text-right">
                                                    {/* IP ပြမယ့်နေရာ */}
                                                    <span class="block font-mono text-[10px] text-gray-500">{u.lastLoginIp || u.ip || "Unknown"}</span>
                                                    <span class={isVip ? "text-yellow-500 font-bold text-[10px]" : "text-gray-600 text-[10px]"}>{isVip ? "VIP Member" : "Free User"}</span>
                                                </div>
                                            </div>
                                         )
                                     })}
