...
const AuthSchema = z.object({
    username: z.string().min(3, "Username must be at least 3 chars").max(20).regex(/^[a-zA-Z0-9_]+$/, "Only letters, numbers and _ allowed"),
    password: z.string().min(6, "Password must be at least 6 chars")
});
const RequestSchema = z.object({
    movieName: z.string()
        .trim()
        .min(2, "Movie name too short")
        .max(100, "Movie name too long (Max 100 chars)") 
        .regex(/^[a-zA-Z0-9\s\-\.\(\)\[\]]+$/, "Special characters or JSON not allowed") 
});
const TopupSchema = z.object({
    amount: z.coerce.number().min(500, "Minimum amount is 500").max(1000000, "Amount too high"),
    transactionId: z.string()
        .trim()
        .min(4, "Transaction ID too short")
        .max(30, "Transaction ID too long") 
        .regex(/^[a-zA-Z0-9]+$/, "Only numbers and letters allowed"), 
...
const i18n: any = {
    en: {
        ...
        see_all: "View All", views: "Views", dl_help: "Download အညွန်း",
        ...
        dl_btn: "Download", next_ep: "Next Episode"
    },
    my: {
...
interface Movie {
  ...
  category: "Movies" | "Series" | "USA" | "Jav" | "All Uncensored" | "Myanmar and Asian" | "4K Porns";
...
interface MovieSummary { id: string; title: string; posterUrl: string; coverUrl: string; category: string; createdAt: number; isDeleted?: boolean; }
interface User {
    ...
    securityQ?: string;
    securityA?: string;
    notifications?: { id: string; msg: string; date: number; read: boolean }[]; 
}
...
function optimizeImg(url: string, width: number = 400) {
    if (!url) return "";
    return url.trim();
}
...
async function saveMovieDB(movie: Movie) {
    const summary: MovieSummary = {
        ...
        coverUrl: movie.coverUrl, category: movie.category, createdAt: movie.createdAt, isDeleted: false
    };
...
async function softDeleteMovieDB(id: string) {
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    const res = await kv.get<Movie>(["movies", id]);
    if (!res.value) return;
    const m = res.value;
    m.isDeleted = true;
    await kv.set(["movies", id], m);
...
async function restoreMovieDB(id: string) {
    const res = await kv.get<Movie>(["movies", id]);
    if (!res.value) return;
    const m = res.value;
    m.isDeleted = false;
...
async function permanentDeleteMovieDB(id: string) {
    const res = await kv.get<Movie>(["movies", id]);
    if (!res.value) return;
    const m = res.value;
...
async function getLatestMovies(limit: number = 20) {
    ...
    const iter = kv.list<MovieSummary>({ prefix: ["idx_time"] }, { reverse: true, limit: limit + 10 }); 
    const movies = []; 
    for await (const res of iter) {
        if(!res.value.isDeleted) movies.push(res.value);
...
async function getMoviesByCategory(cat: string, limit: number = 20) {
    const iter = kv.list<MovieSummary>({ prefix: ["idx_cat", cat] }, { reverse: true, limit: limit + 10 });
    const movies = []; 
    for await (const res of iter) {
        if(!res.value.isDeleted) movies.push(res.value);
        if(movies.length >= limit) break;
...
async function getMoviesByCategoryWithCursor(cat: string, limit: number = 20, cursor: string = "") {
    const options: any = { reverse: true, limit: limit };
    if (cursor) options.cursor = cursor;
    const iter = kv.list<MovieSummary>({ prefix: ["idx_cat", cat] }, options);
    const movies = [];
    for await (const res of iter) {
        if(!res.value.isDeleted) movies.push(res.value);
    }
    return { movies, nextCursor: iter.cursor };
}
async function searchMoviesDB(query: string) {
    const words = tokenize(query);
    if (words.length === 0) return [];
    const searchWord = words.reduce((a, b) => a.length > b.length ? a : b, "");
    const startKey = ["idx_search", searchWord];
    const endKey = ["idx_search", searchWord + "\uffff"]; 
    const iter = kv.list({ start: startKey, end: endKey }, { limit: 100 }); 
    const movieIds = new Set<string>();
    for await (const entry of iter) {
        movieIds.add(entry.key[2] as string); 
    }
    ...
    for (let i = 0; i < uniqueIds.length; i += 10) {
        ...
        for (const r of res) {
            if (r.value && !(r.value as Movie).isDeleted) results.push(r.value as Movie);
        }
    }
...
async function reIndexDatabase() {
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    ...
    for await (const res of iter) {
        if (!res.value.isDeleted) {
            tasks.push(saveMovieDB(res.value));
...
`;
  ...
  const notiCount = props.user?.notifications ? props.user.notifications.filter(n => !n.read).length : 0;
  let daysLeft = 0;
  const now = Date.now();
  ...
  <html lang={l}>
    <head>
      ...
      <meta property="og:image" content={optimizeImg(props.coverUrl || "https://cdn-icons-png.flaticon.com/512/2503/2503508.png")} />
      <script src="https://cdn.tailwindcss.com"></script>
      <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
      <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet" />
      <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;700;900&display=swap" rel="stylesheet" />
      <style>{`
        :root { --glass-bg: rgba(255, 255, 255, 0.08); --glass-border: rgba(255, 255, 255, 0.1); --primary: #8b5cf6; --accent: #06b6d4; --plyr-color-main: #8b5cf6; }
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        .plyr--video { border-radius: 12px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.5); --plyr-color-main: #c084fc; }
      `}</style>
      <script dangerouslySetInnerHTML={{__html: `
        ${protectJS}
        // 🔥 IMAGE SYSTEM FIX (Gray Box Solver)
        
        // 1. ပုံအောင်မြင်ရင် Skeleton ဖျက်မယ်
        ...
        // 2. ပုံ Error တက်ရင်လည်း Skeleton ကို အတင်းဖျက်မယ် + မူရင်း Link ပြန်ချိတ်မယ်
        document.addEventListener('error', function(e) {
            if (e.target.tagName === 'IMG') {
                const img = e.target;
                
                // Error တက်တာနဲ့ "loaded" လို့သတ်မှတ်ပြီး Skeleton ဖျက်မယ် (ဒါမှ အကွက်လွတ်မဖြစ်မှာ)
                img.classList.add('loaded');
                if(img.parentElement && img.parentElement.classList.contains('img-skeleton')) {
                    img.parentElement.classList.remove('img-skeleton');
                }

                // Proxy (wsrv.nl) ကြောင့် Error တက်တာလား စစ်မယ်
                if (img.src.includes('wsrv.nl')) {
                    try {
                        const urlObj = new URL(img.src);
                        const originalUrl = urlObj.searchParams.get('url');
                        
                        // Proxy ဖြုတ်ပြီး မူရင်း Link နဲ့ ပြန်စမ်းမယ် (Retry)
                        if (originalUrl && !img.dataset.retried) {
                            console.log('Proxy failed, retrying original:', originalUrl);
                            img.dataset.retried = "true"; // Loop မဖြစ်အောင် တားမယ်
                            img.src = originalUrl;
                            return; // မူရင်း Link နဲ့ ပြန်စမ်းနေတုန်းမို့ Placeholder မပြသေးဘူး
                        }
                    } catch(err) {}
                } 
                
                // မူရင်း Link ပါ ပျက်နေရင် (သို့) Proxy မဟုတ်ရင် Error Icon ပြမယ်
                if (!img.src.includes("flaticon")) {
                    img.src = "https://cdn-icons-png.flaticon.com/512/2748/2748558.png"; 
                    img.style.objectFit = "contain"; // Icon လှအောင်
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                var finalUrl = content.trim();
                if (finalUrl.includes('mediafire.com')) {
                    finalUrl = '/api/mediafire-stream?link=' + encodeURIComponent(finalUrl);
                } 
                else if (type === 'direct') { 
    try { 
        var res = await fetch('/api/resolve-url?token=' + content); 
...
console.log("Playing:", finalUrl);
                var videoHtml = '<video id="main-video" controls autoplay playsinline class="w-full h-full" style="background-color:black;" poster="' + poster + '">';
                videoHtml += '<source src="' + finalUrl + '" type="video/mp4">';
                videoHtml += '<p class="text-white text-center pt-10">Browser does not support video.</p>';
                videoHtml += '</video>';
                videoHtml += '<div id="error-msg" class="hidden absolute inset-0 flex items-center justify-center bg-black/90 text-white text-xs text-center p-4 z-50">';
                videoHtml += '<div><p class="text-red-500 font-bold mb-2">Video Playback Failed</p>';
                videoHtml += '<a href="' + finalUrl + '" target="_blank" class="bg-blue-600 px-4 py-2 rounded-lg text-white font-bold inline-block">Click to Download</a></div>';
                videoHtml += '</div>';
                ...
                if(btnElement && btnElement.dataset.next) {
                    var nextUrl = btnElement.dataset.next;
                    var nextBtn = document.createElement('div');
                    nextBtn.className = "absolute bottom-16 right-4 z-[50] hidden";
                    nextBtn.innerHTML = '<button id="next-ep-btn" class="bg-white text-black font-bold px-4 py-2 rounded-full shadow-lg hover:scale-105 transition flex items-center gap-2 text-xs"><i class="fa-solid fa-forward-step"></i> Next Episode</button>';
                    container.appendChild(nextBtn);
                    document.getElementById('next-ep-btn').onclick = function() {
                        var nextEl = document.querySelector('[data-url="'+nextUrl+'"]');
                        loadPlayer(nextUrl, type, movieId, title, poster, nextEl);
                    };
                    if(video) {
                        video.addEventListener('ended', function() { nextBtn.classList.remove('hidden'); });
                    }
                }
        ...
        async function loadMoreMovies(category) { 
            if (!isInitialized) {
                if (typeof window.initialCursor === "string") {
                    currentCursor = window.initialCursor;
            ...
            for(let i=0; i<6; i++) {
                ...
                if(category === "All Uncensored" || category === "Myanmar and Asian" || category === "4K Porns") { 
                    el.classList.remove('aspect-[2/3]'); 
            ...
            try { 
                const url = '/api/list?cat=' + category + '&cursor=' + currentCursor;
                ...
                data.movies.forEach(m => { 
                    ...
                    let badge = "HD";
                    if (m.category === "4K Movies" || m.category === "4K Porns") badge = "4K";
                    else if (m.category === "Myanmar and Asian") badge = "SD";
        ...
        window.addEventListener('scroll', () => { 
            if(window.location.pathname.startsWith('/category/') && (window.innerHeight + window.scrollY) >= document.body.offsetHeight - 500) { 
                const cat = window.location.pathname.split('/').pop().replace(/%20/g, ' '); 
                loadMoreMovies(cat); 
            }
        });
      `}} />
    ...
    <div class="px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/10 text-[10px] font-bold text-purple-400 tracking-wide backdrop-blur-md shadow-[0_0_10px_rgba(168,85,247,0.3)]">P - {daysLeft} Day</div>
            ...
            {!props.isAdmin && (
                <nav class="bottom-nav">
                    ...
                    <a href={props.user ? "/profile" : "/login"} class={`nav-item ${active === 'me' ? 'active' : ''} relative`}>
                        <i class="fa-solid fa-user"></i><span>{t.me}</span>
                        {notiCount > 0 && <span class="absolute top-1 right-6 w-2 h-2 bg-red-500 rounded-full"></span>}
                    </a>
                </nav>
            )}
        </>
...
const app = new Hono();
// 🔥 MISSING API FIX: Load More အတွက် လိုအပ်သော လမ်းကြောင်း
app.get("/api/list", async (c) => { 
    const cat = c.req.query("cat") || "Movies"; 
    const cursor = c.req.query("cursor"); // Pagination အတွက်
    
    // Database ထဲကနေ နောက်ထပ်ကားတွေ ဆွဲထုတ်မယ်
    const { movies, nextCursor } = await getMoviesByCategoryWithCursor(cat, 15, cursor); 
    
    // ပုံတွေကို Optimize လုပ်ပြီး JSON ပြန်ထုတ်မယ်
    const cleanMovies = movies.map(m => ({
        id: m.id,
        title: m.title,
        posterUrl: optimizeImg(m.posterUrl, 300),
        coverUrl: optimizeImg(m.coverUrl, 600),
        category: m.category
    })); 
    
    return c.json({ movies: cleanMovies, nextCursor }); 
});
app.get("/api/search", async (c) => {
    ...
    const cleanResults = results.map(m => ({
        id: m.id,
        title: m.title,
        posterUrl: optimizeImg(m.posterUrl, 200)
    }));
...
app.use("*", async (c, next) => {
    ...
    const limitKey = ["rate_limit_protection", ip];
    const limitRes = await kv.get<{ count: number }>(limitKey);
    const currentCount = limitRes.value?.count || 0;
    if (currentCount > 1000) return c.text("Too Many Requests! Please wait a minute.", 429);
    await kv.set(limitKey, { count: currentCount + 1 }, { expireIn: 60 * 1000 });
    await next();
});
app.use("*", async (c, next) => {
...
app.use("*", secureHeaders({
  ...
  contentSecurityPolicy: { defaultSrc: ["'self'"], scriptSrc: ["'self'", "'unsafe-inline'", "https://cdn.tailwindcss.com", "https://cdnjs.cloudflare.com", "https://cdn.jsdelivr.net", "https://cdn.plyr.io"], styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://cdnjs.cloudflare.com", "https://cdn.plyr.io"], fontSrc: ["'self'", "https://fonts.gstatic.com", "https://cdnjs.cloudflare.com"], imgSrc: ["'self'", "data:", "https:", "http:"], mediaSrc: ["'self'", "https:", "http:"], connectSrc: ["'self'", "https:", "http:"], frameSrc: ["'self'", "https:", "http:"], }
}));
app.use("*", compress());
      ...
      {config.showCustomBanner && config.customBannerImage ? (
            <div class="px-4 mb-8 mt-4">
                <a href={config.customBannerLink || "#"} class="block rounded-lg overflow-hidden shadow-2xl relative aspect-video group">
                    <img src={optimizeImg(config.customBannerImage)} class="w-full h-full object-cover" />
                </a>
            </div>
      ) : (
                  ...
                  <div class="slider-container relative z-0 group rounded-lg overflow-hidden shadow-2xl aspect-video">
                      {sliderMovies.map((m, idx) => (
                          <div class={`slide ${idx === 0 ? 'active' : ''} absolute inset-0 transition-opacity duration-1000`}>
                              <img src={optimizeImg(m.coverUrl)} class="w-full h-full object-cover" />
                            ...
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block group relative mb-4">
                                    <div class="aspect-video w-full relative overflow-hidden rounded-lg shadow-lg img-skeleton">
                                        <img src={optimizeImg(m.coverUrl || m.posterUrl, 600)} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" />
                                        <div class="absolute top-2 right-2 bg-pink-600 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow z-10 backdrop-blur-sm">{m.category === "4K Movies" || m.category === "4K Porns" ? "4K" : (m.category === "Myanmar and Asian" ? "SD" : "HD")}</div>
                        ...
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block w-28 flex-shrink-0 group relative mb-4">
                                <div class="aspect-[2/3] w-full relative overflow-hidden rounded-lg shadow-lg img-skeleton">
                                    <img src={optimizeImg(m.posterUrl, 300)} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" />
                                    <div class="absolute top-2 right-2 bg-purple-600 text-white text-[8px] font-bold px-1.5 py-0.5 rounded shadow backdrop-blur-sm">{m.category === "4K Movies" || m.category === "4K Porns" ? "4K" : (m.category === "Myanmar and Asian" ? "SD" : "HD")}</div>
                                </div>
...
app.get("/api/mediafire-stream", async (c) => {
    ...
    try {
        // ၁။ မူရင်း MediaFire Link သီးသန့်ကို အရင်ယူမယ်
        let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();
        if (!cleanUrl.startsWith("http")) {
            cleanUrl = "https://" + cleanUrl;
        }

        // ၂။ Browser မှာရိုက်သလိုမျိုး proxy.avotc.tk နဲ့တွဲပြီး Page ကိုလှမ်းခေါ်မယ်
        // Deno Server -> Proxy -> MediaFire Web Page
        const proxyWebUrl = "https://proxy.avotc.tk/" + cleanUrl;

        const res = await fetch(proxyWebUrl, {
            headers: {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
                "Referer": "https://proxy.avotc.tk/"
            },
            redirect: "follow"
        });

        // ၃။ Proxy ကပြန်ပေးတဲ့ HTML (Web Page) ကို ဖတ်မယ်
        const html = await res.text();

        // ၄။ Download Link ကို ရှာမယ်
        // Proxy ဖြတ်ထားတဲ့ Page မို့ Link တွေက ပုံစံပြောင်းကောင်းပြောင်းနေနိုင်တယ်
        let match = html.match(/aria-label="Download file"\s+href="([^"]+)"/);
        if (!match) match = html.match(/id="downloadButton"\s+href="([^"]+)"/);
        if (!match) match = html.match(/href="([^"]+)"\s+id="downloadButton"/);

        if (match && match[1]) {
            let finalLink = match[1];

            // ၅။ ရလာတဲ့ Link မှာ proxy.avotc.tk ပါပြီးသားလား စစ်မယ်
            if (!finalLink.includes("proxy.avotc.tk")) {
                // မပါသေးရင် (MediaFire Link အတိုင်းပဲဖြစ်နေရင်) ရှေ့ကတပ်ပေးမယ်
                finalLink = "https://proxy.avotc.tk/" + finalLink;
            }

            // ရလာတဲ့ Proxy Download Link ကို User ဆီ ပို့လိုက်မယ်
            return c.redirect(finalLink, 302);
        } else {
            // ရှာမတွေ့ရင်တောင် Browser မှာလိုပဲ Proxy Link နဲ့တွဲပြီး ပြန်လွှတ်မယ်
            // User က Download ကို နှိပ်ရင် Proxy နဲ့ပဲ ဆက်သွားပါလိမ့်မယ်
            return c.redirect(proxyWebUrl, 302);
        }

    } catch (e) {
        console.error("Proxy Fetch Error:", e);
        // Error တက်ရင်လည်း Proxy Link နဲ့တွဲပြီး Redirect လုပ်ပေးမယ်
        return c.redirect("https://proxy.avotc.tk/" + decodeURIComponent(url).replace("https://proxy.avotc.tk/", ""), 302);
    }
});
app.get("/category/:cat", async (c) => { 
    ...
    const config = await getConfig(); 
    const { movies, nextCursor } = await getMoviesByCategoryWithCursor(cat, 15); 
    ...
    >
    {}
    <script dangerouslySetInnerHTML={{__html: `window.initialCursor = "${nextCursor}";`}} />
    ...
    {movies.map(m => (
        <a href={`/movie/${m.id}`} class="block group relative mb-2">
            <div class={`relative overflow-hidden rounded-lg shadow-lg img-skeleton ${isUncensored ? "aspect-video" : "aspect-[2/3]"}`}>
                <img src={optimizeImg(isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl, isUncensored ? 600 : 300)} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" />
                <div class="absolute top-2 right-2 bg-purple-600 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow z-10 backdrop-blur-sm">{m.category === "4K Movies" || m.category === "4K Porns" ? "4K" : (m.category === "Myanmar and Asian" ? "SD" : "HD")}</div>
            </div>
                        ...
                        return (
                            <a href={`/movie/${m.id}`} class={`block group relative ${isWide ? 'col-span-3' : ''}`}>
                                <div class={`${isWide ? "aspect-video" : "aspect-[2/3]"} relative overflow-hidden rounded-lg shadow-lg img-skeleton`}>
                                    <img src={optimizeImg(isWide ? (m.coverUrl || m.posterUrl) : m.posterUrl, isWide ? 600 : 300)} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" />
...
app.post("/request", async (c) => { 
    ...
    if (!parsed.success) {
        return c.redirect("/request?error=" + encodeURIComponent(parsed.error.issues[0].message));
    }
    const { movieName } = parsed.data; 
...
app.get("/dl/:token", async (c) => {
    ...
    // MediaFire Link ဖြစ်မှသာ Direct Link ရှာတဲ့အလုပ်ကို လုပ်မယ်
    if (url.includes("mediafire.com")) {
        try {
            // ၁။ Database ထဲက Link က Proxy ခံထားတာလား စစ်မယ်
            const isProxy = url.includes("proxy.avotc.tk");
            
            // ၂။ Link အတိုင်း (Proxy ဆို Proxy, ရိုးရိုးဆို ရိုးရိုး) Page ကိုလှမ်းခေါ်မယ်
            const res = await fetch(url, {
                headers: {
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
                    "Referer": isProxy ? "https://proxy.avotc.tk/" : "https://www.mediafire.com/"
                },
                redirect: "follow"
            });
            ...
            // ၃။ Direct Download Link ကို Regex နဲ့ ရှာမယ်
            let match = html.match(/aria-label="Download file"\s+href="([^"]+)"/);
            if (!match) match = html.match(/id="downloadButton"\s+href="([^"]+)"/);
            if (!match) match = html.match(/href="([^"]+)"\s+id="downloadButton"/);

            if (match && match[1]) {
                let directLink = match[1];

                // ၄။ မူရင်းက Proxy ဖြစ်ရင် ရလာတဲ့ Link ကို Proxy ပြန်ခံပေးမယ်
                if (isProxy && !directLink.includes("proxy.avotc.tk")) {
                    directLink = "https://proxy.avotc.tk/" + directLink;
                }
                
                // ၅။ မူရင်းက ရိုးရိုးဖြစ်ရင် ရိုးရိုးအတိုင်းပဲ ပြန်ပေးမယ် (Proxy ပါနေရင် ဖြုတ်မယ်)
                if (!isProxy && directLink.includes("proxy.avotc.tk")) {
                    directLink = directLink.replace("https://proxy.avotc.tk/", "");
                }

                // Direct Link ကို Redirect လုပ်မယ် (User က နှိပ်လိုက်တာနဲ့ Download တန်းကျမယ်)
                return c.redirect(directLink, 302);
            }
        } catch (e) {
            console.error("DL Parse Error:", e);
            // Error တက်ရင် မူရင်း Page Link အတိုင်းပဲ လွှတ်ပေးလိုက်မယ်
        }
    }

    // MediaFire မဟုတ်ရင် (သို့) ရှာမတွေ့ရင် မူရင်း Link အတိုင်းသွားမယ်
...
app.get("/movie/:id", async (c) => {
    ...
    if (!movie || movie.isDeleted) return c.text("Not Found", 404);
    let related = [];
    try {
    ...
    const displayImage = optimizeImg(movie.coverUrl || movie.posterUrl, 800); 
    ...
    >
        ...
        <div id="vip-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-md p-4">
             <div class="glass-panel p-6 rounded-lg w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  ...
                  <p class="text-gray-300 text-sm leading-relaxed mb-4 font-bold">Moviesများ ကြည့်ရန် ဒေါင်းရန် VIP Accဖြစ်ရန်လိုပါသည်</p>
                  <div class="flex gap-3 h-12"> 
...
app.post("/login", async (c) => { 
    ...
    if (!parsed.success) return c.redirect("/login?error=" + parsed.error.issues[0].message);
    const user = await getUser(String(body["username"])); 
    const hashedInput = await hashPassword(String(body["password"])); 
    if (user && user.passwordHash === hashedInput) { 
        ...
        setCookie(c, "auth_session", `${user.username}:${sessionId}`, { path: "/", maxAge, httpOnly: true, secure: true, sameSite: "Strict" }); 
        return c.redirect("/"); 
    } 
...
app.post("/signup", async (c) => { 
    ...
    if (await isIpBanned(clientIp)) return c.text("Banned", 403);
    const body = await c.req.parseBody();
    const parsed = AuthSchema.safeParse(body);
    if (!parsed.success) return c.redirect("/signup?error=" + parsed.error.issues[0].message);
    ...
    const newUser: User = { 
        ...
        securityQ: String(question), securityA: answerHash, notifications: []
    }; 
...
app.get("/profile", async (c) => { 
    ...
    if (user.purchased && user.purchased.length > 0) {
        ...
        purchasedMovies.push(...results.filter(m => m && !m.isDeleted)); 
    }
    const plans = [
      ...
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            ...
            {user.notifications && user.notifications.length > 0 && (
                <div class="bg-black/50 border border-zinc-800 p-4 rounded-xl">
                    <h3 class="font-bold text-white text-xs mb-3 flex items-center gap-2"><i class="fa-solid fa-bell text-yellow-500"></i> Notifications</h3>
                    <div class="space-y-2 max-h-40 overflow-y-auto custom-scroll">
                        {user.notifications.slice().reverse().map(n => (
                            <div class="text-xs p-2 bg-zinc-800/50 rounded flex justify-between items-start">
                                <span class="text-gray-300">{n.msg}</span>
                                <span class="text-[9px] text-gray-600 whitespace-nowrap ml-2">{new Date(n.date).toLocaleDateString()}</span>
                            </div>
                        ))}
                    </div>
                    <form action="/profile/clear-notis" method="post" class="mt-2 text-right">
                        <button class="text-[9px] text-red-400 hover:text-white underline">Clear All</button>
                    </form>
                </div>
            )}
        ...
        <div class="grid grid-cols-1 gap-3 mb-3">
            ...
            <label class="cursor-pointer relative group">
                ...
                <div class="p-4 rounded-xl bg-black/50 border border-zinc-700 peer-checked:border-blue-500 peer-checked:bg-blue-900/10 transition hover:bg-zinc-800 group-active:scale-95 flex items-center gap-4">
                    <img src={optimizeImg("https://szenulkqzclwvudsgnfx.supabase.co/storage/v1/object/public/lugyiapp/img_1768294652912_e9ozhgi.png")} class="w-12 h-12 rounded-lg object-cover shadow-sm border border-white/10 flex-shrink-0" />
                    <div class="flex-grow">
            ...
            <label class="cursor-pointer relative group">
                ...
                <div class="p-4 rounded-xl bg-black/50 border border-zinc-700 peer-checked:border-yellow-400 peer-checked:bg-yellow-900/10 transition hover:bg-zinc-800 group-active:scale-95 flex items-center gap-4">
                    <img src={optimizeImg("https://szenulkqzclwvudsgnfx.supabase.co/storage/v1/object/public/lugyiapp/img_1768294676568_j1jfvuu.jpg")} class="w-12 h-12 rounded-lg object-cover shadow-sm border border-white/10 flex-shrink-0" />
                    <div class="flex-grow">
                            ...
                            {purchasedMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group relative border border-zinc-800/50">
                                    <div class="aspect-video relative overflow-hidden">
                                        <img src={optimizeImg(m.coverUrl || m.posterUrl, 300)} loading="lazy" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
...
app.post("/profile/topup", async (c) => {
    ...
    if (!parsed.success) return c.redirect("/profile?error=" + encodeURIComponent(parsed.error.issues[0].message));
    const { amount, method, transactionId, purpose } = parsed.data; 
...
app.post(ADMIN_ROUTE + "/login", async (c) => { 
    const ip = getClientIp(c);
    if (!await checkAdminRateLimit(ip)) return c.text("Too many failed attempts. Try again in 15 minutes.", 429);
    const { password } = await c.req.parseBody(); 
    if (password === ADMIN_PASS) { 
        const sessionId = crypto.randomUUID();
        await kv.set(["admin_sessions", sessionId], "active", { expireIn: ADMIN_SESSION_EXPIRE });
        setCookie(c, "admin_session_id", sessionId, { path: "/", httpOnly: true, secure: true, sameSite: "Strict" }); 
        await logAdminAction("login", "Admin Logged In");
        return c.redirect(ADMIN_ROUTE + "/dashboard"); 
    } 
    await recordAdminFail(ip);
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    if (adminQuery) {
        ...
        const iter = kv.list({ prefix: ["idx_time"] }, { reverse: true, limit: limit, cursor: cursor });
        for await (const res of iter) {
    ...
    const iterDrafts = kv.list({ prefix: ["drafts"] });
    ...
    const catCounts = {};
    movies.forEach(m => { if(!m.isDeleted) catCounts[m.category] = (catCounts[m.category] || 0) + 1; });
    return c.html(
        <Layout title="Admin Dashboard" isAdmin={true}>
            <div class="p-4 bg-black min-h-screen font-sans text-sm">
                {}
                <div class="flex justify-between items-center mb-6 bg-[#111] p-4 rounded-xl border border-zinc-800 shadow-sm">
                    <h1 class="font-bold text-blue-500 text-lg flex items-center gap-2">
                        <i class="fa-solid fa-shield-cat"></i> Dashboard
                    </h1>
                    <div class="flex gap-2">
                        <a href="/admin/backup" class="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1.5 rounded-lg text-xs border border-zinc-700 font-bold"><i class="fa-solid fa-download"></i> Backup</a>
                        <form action="/admin/restore" method="post" enctype="multipart/form-data" class="inline">
                            <label class="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1.5 rounded-lg text-xs border border-zinc-700 cursor-pointer font-bold">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl flex flex-col justify-center items-center">
                            ...
                            <div class="grid grid-cols-2 gap-6 w-full text-center">
                                ...
                                <div class="bg-black p-4 rounded-xl border border-zinc-800"><p class="text-3xl font-black text-yellow-500">{movies.filter(m=>!m.isDeleted).length}</p><p class="text-xs text-gray-500 uppercase">Active Movies</p></div>
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-lg border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title" value={editMovie?.title} class="input-box w-full bg-black border-zinc-700 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">{["Movies","Series","4K Movies","USA","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-24 text-center flex-shrink-0 bg-black border-zinc-700" /></div></div>
                                ...
                                <div class="grid grid-cols-3 gap-2"><input name="fileSize" placeholder="Size (1.2 GB)" value={editMovie?.fileSize} class="input-box w-full bg-black border-zinc-700" /><input name="duration" placeholder="Time (25m)" value={editMovie?.duration} class="input-box w-full bg-black border-zinc-700" /><input type="number" name="price" placeholder="Price (Ks)" value={editMovie?.price || 0} class="input-box w-full bg-black border-yellow-500/50 text-yellow-500 font-bold" /></div>
                        ...
                        <div class="lg:col-span-2 bg-[#111] p-5 rounded-lg border border-zinc-800 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw] shadow-xl">
                            ...
                            <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-3">
                                <h2 class="font-bold text-sm text-green-500 flex-shrink-0">
                                    <i class="fa-solid fa-circle text-[8px] animate-pulse"></i> Live Movies ({movies.length})
                                </h2>
                                <form action={ADMIN_ROUTE + "/dashboard"} method="get" class="flex gap-2 w-full md:w-auto">
                                    <input type="hidden" name="tab" value="movies" />
                                    <input name="q" value={adminQuery} placeholder="Search Database..." class="bg-black border border-zinc-700 rounded-lg px-4 py-2 text-xs w-full md:w-48 outline-none focus:border-blue-500 transition text-white" />
                                    <button class="bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-500 transition shadow-lg">
                                        <i class="fa-solid fa-magnifying-glass"></i>
                                    </button>
                                    {adminQuery && (
                                        <a href={ADMIN_ROUTE + "/dashboard?tab=movies"} class="bg-red-500/20 text-red-500 border border-red-500/30 px-3 py-2 rounded-lg text-xs font-bold hover:bg-red-500 hover:text-white transition flex items-center">
                            ...
                            <details class="mb-4 p-3 bg-zinc-900 rounded-xl border border-zinc-700 cursor-pointer text-xs">
                                ...
                                <form action="/admin/movie/bulk-import" method="post" class="mt-3">
                                    <textarea name="json" placeholder='Paste your JSON URL here...' class="w-full h-32 bg-black border border-zinc-700 rounded-lg p-2 font-mono text-xs"></textarea>
                            ...
                            <div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scroll">
                                {movies.map(m => (
                                    <div class={`movie-item flex gap-4 p-3 rounded-xl items-center border transition ${m.isDeleted ? 'bg-red-900/20 border-red-500/50 opacity-70' : 'bg-black border-zinc-800/50 hover:border-zinc-600'}`}>
                                        <img src={optimizeImg(m.posterUrl, 100)} class="w-10 h-14 object-cover rounded-lg flex-shrink-0" />
                                        <div class="flex-grow min-w-0">
                                            <div class="flex items-center gap-2">
                                                <div class="font-bold text-sm truncate text-gray-200">{m.title}</div>
                                                {m.isDeleted && <span class="text-[9px] bg-red-600 text-white px-2 rounded font-bold">TRASH</span>}
                                            </div>
                                        ...
                                        <div class="flex gap-2">
                                            {!m.isDeleted ? (
                                                <>
                                                    ...
                                                    <form action={`/admin/movie/soft-delete/${m.id}`} method="post" onsubmit="return confirm('Move to Trash?')">
                                                        <button class="text-red-500 text-xs border border-red-500/20 bg-red-500/10 px-3 py-1.5 rounded hover:bg-red-600 hover:text-white transition"><i class="fa-solid fa-trash"></i></button>
                                                    </form>
                                                </>
                                            ) : (
                                                <>
                                                    <form action={`/admin/movie/restore/${m.id}`} method="post">
                                                        <button class="text-green-500 text-xs border border-green-500/20 bg-green-500/10 px-3 py-1.5 rounded hover:bg-green-600 hover:text-white transition"><i class="fa-solid fa-rotate-left"></i> Restore</button>
                                                    </form>
                                                    <form action={`/admin/movie/permanent-delete/${m.id}`} method="post" onsubmit="return confirm('PERMANENTLY DELETE? Cannot undo!')">
                                                        <button class="text-red-500 text-xs border border-red-500/20 bg-red-500/10 px-3 py-1.5 rounded hover:bg-red-600 hover:text-white transition">Delete Forever</button>
                                                    </form>
                                                </>
                                            )}
                                        </div>
                ...
                <div id="tab-drafts" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl min-h-[50vh]">
                        <h2 class="font-bold text-yellow-500 text-lg flex items-center gap-2 mb-6"><i class="fa-solid fa-file-pen"></i> Drafts ({drafts.length})</h2>
                        {drafts.length > 0 && <form action="/admin/drafts/publish-all" method="post" onsubmit="return confirm('Publish ALL?')"><button class="bg-green-600 text-white px-4 py-2 rounded mb-4 text-xs font-bold shadow-lg">Publish All ({drafts.length})</button></form>}
                        {drafts.length === 0 ? <p class="text-gray-500 text-center">No Drafts</p> : (
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {drafts.map(d => (
                                    <div class="flex gap-4 p-4 rounded-xl bg-black border border-yellow-500/20">
                                        <img src={d.posterUrl} class="w-16 h-24 object-cover rounded-lg" />
                                        <div class="flex flex-col justify-between flex-grow">
                                            <h3 class="font-bold text-white text-md truncate">{d.title}</h3>
                                            <div class="flex gap-2 mt-2">
                                                <a href={`${ADMIN_ROUTE}/dashboard?edit=${d.id}&type=draft`} class="flex-1 text-center bg-zinc-800 text-xs py-2 rounded">Edit</a>
                                                <form action={`/admin/draft/publish/${d.id}`} method="post" class="flex-1"><button class="w-full bg-blue-600 text-xs py-2 rounded">Publish</button></form>
                                                <form action={`/admin/draft/delete/${d.id}`} method="post" onsubmit="return confirm('Delete?')"><button class="px-3 bg-red-900/20 text-red-500 border border-red-500/20 rounded"><i class="fa-solid fa-trash"></i></button></form>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                ...
                <div id="tab-keys" class="tab-content">
                    <div class="max-w-2xl mx-auto space-y-4">
                        <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
                            <h2 class="font-bold mb-4 text-yellow-500"><i class="fa-solid fa-crown"></i> Generate VIP Key</h2>
                            <form action="/admin/key/create" method="post" class="flex gap-3"><input type="hidden" name="type" value="vip" /><input type="number" name="days" placeholder="Days (e.g. 30)" required class="input-box flex-grow bg-black border-zinc-700" /><button class="btn-primary w-32 shadow-lg">Gen VIP</button></form>
                        </div>
                        <div class="bg-[#111] p-6 rounded-lg border border-blue-900/30 shadow-xl">
                            <h2 class="font-bold mb-4 text-blue-400"><i class="fa-solid fa-coins"></i> Generate Money Key</h2>
                            <form action="/admin/key/create" method="post" class="flex gap-3"><input type="hidden" name="type" value="coin" /><input type="number" name="value" placeholder="Amount (e.g. 1000)" required class="input-box flex-grow bg-black border-zinc-700 text-blue-400 font-bold" /><button class="bg-blue-600 text-white font-bold px-4 rounded-lg w-32 hover:bg-blue-500 transition shadow-lg">Gen Ks</button></form>
                        </div>
                        <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
                            <h2 class="font-bold mb-4 text-gray-300">Active Keys</h2>
                            <div class="space-y-3 max-h-[500px] overflow-y-auto custom-scroll">{keys.map(k => (<div class="flex justify-between items-center p-3 bg-black rounded-xl border border-zinc-800/50"><span class={`font-mono font-bold ${k.type==='coin'?'text-blue-400':'text-yellow-500'}`}>{k.code} ({k.type==='coin' ? k.value + ' Ks' : k.days + ' Days'})</span><div class="flex gap-2"><button onclick={`copyToClip('${k.code}')`} class="text-xs bg-zinc-800 px-2 py-1 rounded"><i class="fa-solid fa-copy"></i></button><form action={`/admin/key/delete/${k.code}`} method="post"><button class="text-xs bg-red-900/20 text-red-500 px-2 py-1 rounded"><i class="fa-solid fa-trash"></i></button></form></div></div>))}</div>
                        </div>
                    </div>
                </div>
                {}
                <div id="tab-topups" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
                        <div class="flex justify-between items-center mb-6"><h2 class="font-bold text-blue-400 flex items-center gap-2"><i class="fa-solid fa-money-bill-transfer"></i> Top-up Requests</h2><form action="/admin/topup/clear-history" method="post" onsubmit="return confirm('Clear History?')"><button class="text-red-500 text-xs border border-red-500/30 px-3 py-1.5 rounded hover:bg-red-500/10">Clear History</button></form></div>
                        <div class="space-y-4">
                            {topups.map(t => (
                                <div class={`p-4 rounded-xl border flex justify-between items-center ${t.status === 'pending' ? 'bg-black border-blue-500/30' : 'bg-zinc-900 border-zinc-800 opacity-60'}`}>
                                    <div>
                                        <div class="flex items-center gap-2 mb-1"><span class="font-bold text-white text-lg">{t.amount} Ks</span><span class="text-xs bg-zinc-800 px-2 py-0.5 rounded text-gray-400 uppercase">{t.method}</span><span class="text-[10px] bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded uppercase">{t.purpose}</span>{t.status === 'pending' && <span class="text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded animate-pulse">PENDING</span>}</div>
                                    ...
                                    <div class="flex gap-2 items-center">
                                        {t.status === 'pending' ? (<><form action="/admin/topup/approve" method="post"><input type="hidden" name="id" value={t.id} /><button class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold">Approve</button></form><form action="/admin/topup/reject" method="post"><input type="hidden" name="id" value={t.id} /><button class="bg-red-900/30 text-red-500 border border-red-500/30 px-3 py-1.5 rounded-lg text-xs font-bold">Reject</button></form></>) : (<form action={`/admin/topup/delete/${t.id}`} method="post"><button class="text-gray-500 hover:text-red-500"><i class="fa-solid fa-xmark"></i></button></form>)}
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div id="tab-users" class="tab-content">
                    <div class="max-w-4xl mx-auto space-y-6">
                        {}
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gradient-to-br from-blue-900/20 to-black border border-blue-500/20 p-6 rounded-lg flex items-center gap-5 shadow-lg"><div class="w-14 h-14 bg-blue-500/10 rounded-full flex items-center justify-center text-blue-400 text-3xl shadow-inner"><i class="fa-solid fa-users"></i></div><div><h3 class="text-xs uppercase text-blue-400 font-bold tracking-widest mb-1">Total Users</h3><p class="text-4xl font-black text-white tracking-tight">{totalUsers}</p></div></div>
                            <div class="bg-[#111] border border-zinc-800 rounded-lg overflow-hidden h-64 flex flex-col shadow-lg"><h3 class="bg-black/50 px-5 py-3 text-xs font-bold text-gray-400 border-b border-zinc-800 flex justify-between backdrop-blur-sm"><span>User Control</span><span>Status</span></h3><div class="overflow-y-auto custom-scroll p-3 space-y-2">{userList.map(u => { const isVip = u.expiryDate && new Date(u.expiryDate) > new Date(); return (<div class="flex justify-between items-center bg-black/40 p-2.5 rounded-xl border border-zinc-800/30 hover:border-zinc-700 transition"><div class="flex items-center gap-3"><div class={`w-2.5 h-2.5 rounded-full shadow ${u.isBanned ? 'bg-red-600' : (isVip ? 'bg-green-500' : 'bg-gray-500')}`}></div><span class={`font-bold block text-xs ${u.isBanned ? 'text-red-500 line-through' : 'text-gray-300'}`}>{u.username}</span></div><form action="/admin/user/toggle-ban" method="post"><input type="hidden" name="username" value={u.username} /><button class={`px-3 py-1 rounded-lg text-[9px] font-bold border transition ${u.isBanned ? 'bg-green-500/10 text-green-500 border-green-500/50' : 'bg-red-500/10 text-red-500 border-red-500/50'}`}>{u.isBanned ? "UNBAN" : "BAN"}</button></form></div>) })}</div></div>
                        </div>
                        {}
                        <div class="bg-red-900/10 border border-red-500/30 p-6 rounded-lg shadow-xl relative overflow-hidden">
                            <div class="absolute -right-6 -top-6 text-red-500/10 text-9xl"><i class="fa-solid fa-biohazard"></i></div>
                            <h2 class="font-bold text-red-500 mb-2 relative z-10 flex items-center gap-2"><i class="fa-solid fa-triangle-exclamation"></i> Delete ALL Users</h2>
                            <p class="text-xs text-gray-400 mb-4 relative z-10">Database ထဲရှိ User အကောင့်အားလုံးကို ဖျက်ပစ်မည်။ (Admin Panel ဝင်ခွင့် မပျက်ပါ။)</p>
                            <form action="/admin/user/delete-all-force" method="post" class="relative z-10" onsubmit="return confirm('Warning: User အားလုံး ပျက်သွားပါလိမ့်မည်။ သေချာပါသလား?')">
                                <button class="bg-red-600 hover:bg-red-500 text-white font-bold px-6 py-4 rounded-xl w-full transition shadow-lg shadow-red-900/20 uppercase tracking-widest">
                                    <i class="fa-solid fa-trash-can"></i> Confirm Delete All
                                </button>
                            </form>
                        </div>
                        {}
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="bg-[#111] p-6 rounded-lg border border-yellow-500/20 shadow-xl h-fit relative overflow-hidden"><div class="absolute top-0 right-0 p-4 opacity-5"><i class="fa-solid fa-crown text-8xl text-yellow-500"></i></div><h2 class="font-bold mb-5 text-yellow-500 text-lg flex items-center gap-2 relative z-10"><i class="fa-solid fa-circle-plus"></i> Manual VIP Top-up</h2><form action="/admin/user/add-vip" method="post" class="space-y-4 relative z-10"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Username</label><input name="username" placeholder="Enter username..." required class="input-box bg-black border-zinc-700" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Duration (Days)</label><input type="number" name="days" placeholder="e.g. 30" required class="input-box bg-black border-zinc-700" /></div><button class="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold w-full py-3 rounded-xl hover:brightness-110 transition shadow-lg shadow-yellow-500/20">Add VIP Time</button></form></div>
                            <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl h-fit relative overflow-hidden"><div class="absolute top-0 right-0 p-4 opacity-5"><i class="fa-solid fa-lock text-8xl text-blue-500"></i></div><h2 class="font-bold mb-5 text-blue-500 text-lg flex items-center gap-2 relative z-10"><i class="fa-solid fa-user-lock"></i> Reset Password</h2><form action="/admin/user/reset" method="post" class="space-y-4 relative z-10"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Username</label><input name="username" placeholder="Enter username..." required class="input-box bg-black border-zinc-700" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">New Password</label><input name="newpass" placeholder="Enter new password..." required class="input-box bg-black border-zinc-700" /></div><button class="bg-blue-600 text-white font-bold w-full py-3 rounded-xl hover:bg-blue-500 transition shadow-lg shadow-blue-500/20">Reset Password</button></form></div>
                        </div>
                        <div class="bg-[#111] p-6 rounded-lg border border-purple-500/30 shadow-xl h-fit relative overflow-hidden">
                            <div class="absolute top-0 right-0 p-4 opacity-5"><i class="fa-solid fa-gift text-8xl text-purple-500"></i></div>
                            <h2 class="font-bold mb-5 text-purple-500 text-lg flex items-center gap-2 relative z-10"><i class="fa-solid fa-hand-holding-heart"></i> Gift Days to ALL Users</h2>
                            <p class="text-xs text-gray-500 mb-4 relative z-10">User အားလုံး (Free ရော VIP ရော) ကို ရက်ထပ်ပေါင်းပေးမည်။</p>
                            <form action="/admin/user/give-all" method="post" class="space-y-4 relative z-10" onsubmit="return confirm('သေချာပါသလား? User အကုန်လုံး ရက်တိုးသွားပါလိမ့်မည်။')">
                                <div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Add Days (ရက်အရေအတွက်)</label><input type="number" name="days" placeholder="e.g. 2" required class="input-box bg-black border-zinc-700" /></div>
                                <button class="bg-gradient-to-r from-purple-600 to-purple-500 text-white font-bold w-full py-3 rounded-xl hover:brightness-110 transition shadow-lg shadow-purple-500/20">Add Days to Everyone</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div id="tab-config" class="tab-content">
                    <div class="max-w-xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 space-y-8 shadow-xl">
                        ...
                        {}
                        <div class="border-t border-zinc-800 pt-8"><h2 class="font-bold mb-4 text-blue-400 flex items-center gap-2"><i class="fa-solid fa-panorama"></i> Home Main Banner</h2><p class="text-xs text-gray-500 mb-4">"ON" လုပ်ထားလျှင် Auto Slider ပျောက်ပြီး ဒီပုံကိုပဲ ပြပါမည်။ (ကြော်ငြာအတွက်)</p><form action="/admin/config/banner" method="post" class="space-y-4"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Banner Image URL</label><input name="bannerImage" placeholder="https://..." value={config.customBannerImage || ""} class="input-box bg-black border-zinc-700" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Link URL (Optional)</label><input name="bannerLink" placeholder="Click နှိပ်ရင် ရောက်မည့် Link..." value={config.customBannerLink || ""} class="input-box bg-black border-zinc-700" /></div><label class="flex items-center gap-3 p-4 bg-black rounded-xl border border-zinc-800 cursor-pointer hover:border-zinc-600 transition"><input type="checkbox" name="showBanner" checked={config.showCustomBanner} class="accent-blue-500 w-5 h-5" /><span class="font-bold text-sm text-gray-300">Show Custom Banner (Hide Movie Slider)</span></label><button class="btn-primary w-full shadow-lg bg-blue-600 hover:bg-blue-500">Save Banner Settings</button></form></div>
                        {}
...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        let inputData = String(json).trim();
        let movies = [];
        ...
        if (inputData.startsWith("http://") || inputData.startsWith("https://")) {
            try {
                ...
                return c.redirect(ADMIN_ROUTE + "/dashboard?error=JSON Error: Format မမှန်ပါ။");
            }
        } else {
            let rawText = inputData.replace(/[\r\n]+/g, ' ');
            rawText = rawText.replace(/,\s*([\]}])/g, '$1');
            if (rawText.startsWith('{') && !rawText.startsWith('[')) rawText = `[${rawText}]`;
            try {
                movies = JSON.parse(rawText);
            } catch(e) {
                return c.redirect(ADMIN_ROUTE + "/dashboard?error=Invalid JSON Text");
            }
        }
        ...
        // 🔥 SORTING LOGIC
        if (movies.length > 0) {
            const firstSample = movies[0];
            const isJavFormat = firstSample.name || firstSample.photo1 || firstSample.dapp; 
            const isNewFormat = firstSample.profileImg && firstSample.files; 
            
            // Myanmar Format (Title only) ဆိုမှ Reverse လှန်မယ်
            if (!isJavFormat && !isNewFormat) {
                movies.reverse();
            }
        }

        async function createUniqueId(text) {
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                let title = "", poster = "", cover = "", desc = "", duration = "";
                let category = "";
                let allLinks = [];

                // (A) JAV Format (Old)
                if (m.name || m.photo1 || m.dapp) {
                    title = m.name || "";
                    poster = m.photo1 || "";
                    cover = m.photo2 || m.photo1 || ""; 
                    desc = m.info || "";
                    category = "Jav"; 
                    
                    if (!/[a-zA-Z]{2,}-[0-9]{3,}/.test(title)) category = "USA";

                    if (m.dapp) allLinks.push(m.dapp);
                    if (m.download) allLinks.push(m.download);
                } 
                
                // (B) NEW FORMAT (Myanmar / USA / Jav)
                else if (m.profileImg && m.files) {
                    title = m.title || "";
                    poster = m.profileImg || "";
                    cover = m.coverImg || m.profileImg || "";
                    desc = m.content || "";
                    
                    // 🔥 CATEGORY DETECTION (Strict Check)
                    // JSON ထဲက Category စာသားကို အရင်ဆုံး ယုံကြည်မယ်
                    const jsonCat = (m.category || "").toLowerCase();

                    if (jsonCat.includes("myanmar") || jsonCat.includes("asian")) {
                        category = "Myanmar and Asian";
                    } 
                    else if (jsonCat.includes("jav") || /[a-zA-Z]{2,}-[0-9]{3,}/.test(title)) {
                        category = "Jav";
                    } 
                    else if (jsonCat.includes("usa")) {
                        category = "USA";
                    }
                    else {
                        // ဘာမှမပါရင် Title ကိုကြည့်ပြီးမှန်းမယ်
                        if (/[\u1000-\u109F]/.test(title)) category = "Myanmar and Asian";
                        else category = "USA";
                    }

                    try {
                        const filesObj = typeof m.files === 'string' ? JSON.parse(m.files) : m.files;
                        if (filesObj.stream) allLinks.push(...filesObj.stream.map(s => s.url));
                        if (filesObj.download) allLinks.push(...filesObj.download.map(d => d.url));
                    } catch(e) {}
                }

                // (C) MYANMAR Format (Old)
                else if (m.title) {
                    title = m.title || "";
                    poster = m.image || "";
                    cover = m.image || "";
                    ...
                    duration = m.time || "";
                    category = "Myanmar and Asian";
                    if (m.link) allLinks.push(m.link);
                }

                if (!title || allLinks.length === 0) return;

                if (desc) desc = desc.replace(/(bluetv\s*pro|bluetv)/gi, "Gold Flix");

                allLinks = allLinks.filter(u => u && typeof u === 'string');

                // ---------------------------------------------------------
                // 🔥 PROXY & PRIORITY LOGIC
                // ---------------------------------------------------------

                let server1 = "", server2 = "", dl1 = "", dl2 = "";
                const PROXY_BASE = "https://proxy.avotc.tk/";
                
                // Helper
                const addProxy = (u) => u.includes("proxy.avotc.tk") ? u : PROXY_BASE + u;

                // 1. Proxy ပါပြီးသား Link ရှိမရှိ ရှာမယ်
                const preProxiedLink = allLinks.find(u => u.includes("proxy.avotc.tk"));

                // 2. မူရင်း Source (Raw) အကောင်းဆုံးတစ်ခု ရှာမယ်
                let bestRawLink = allLinks.find(u => u.includes("r2") || u.includes("pub") || u.includes("cloudflare"));
                if (!bestRawLink) bestRawLink = allLinks.find(u => u.includes("deno.dev") || u.includes("vercel.app"));
                if (!bestRawLink) bestRawLink = allLinks.find(u => u.includes("mediafire.com"));

                if (!preProxiedLink && !bestRawLink) return;

                const uniqueKey = preProxiedLink || bestRawLink;
                const id = await createUniqueId(uniqueKey);

                if (preProxiedLink) {
                    server1 = preProxiedLink;
                    dl1 = preProxiedLink;
                    server2 = bestRawLink || preProxiedLink;
                    dl2 = bestRawLink || preProxiedLink;
                } else {
                    // Raw Link Only
                    if (bestRawLink.includes("mediafire.com")) {
                        if (bestRawLink.includes("premium")) {
                            server1 = addProxy(bestRawLink);
                            dl1 = addProxy(bestRawLink);
                        } else {
                            server1 = "/api/mediafire-stream?link=" + encodeURIComponent(bestRawLink);
                            dl1 = "/api/mediafire-stream?link=" + encodeURIComponent(bestRawLink);
                        }
                    } else {
                        server1 = addProxy(bestRawLink);
                        dl1 = addProxy(bestRawLink);
        ...
        await logAdminAction("bulk_import", `Imported ${totalCount} movies`);
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Category Fix)");

    } catch (e) {
        console.error(e);
        return c.redirect(ADMIN_ROUTE + "/dashboard?error=Error: " + e.message);
    }
});
...
app.post("/admin/draft/publish/:id", adminGuard, async (c) => {
    ...
    if (!res.value) return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts&error=Draft Not Found");
    await saveMovieDB(res.value); 
...
app.post("/admin/movie/soft-delete/:id", adminGuard, async (c) => { 
    await softDeleteMovieDB(c.req.param("id"));
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=Moved to Trash"); 
});
app.post("/admin/movie/restore/:id", adminGuard, async (c) => { 
    await restoreMovieDB(c.req.param("id"));
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=Restored Successfully"); 
});
app.post("/admin/movie/permanent-delete/:id", adminGuard, async (c) => { 
    await permanentDeleteMovieDB(c.req.param("id"));
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=Permanently Deleted"); 
});
...
app.post("/admin/user/reset", adminGuard, async (c) => {
    ...
    const user = await getUser(String(username));
    if (!user) return c.redirect(ADMIN_ROUTE + "/dashboard?tab=users&error=User Not Found");
    user.passwordHash = await hashPassword(String(newpass));
    await kv.set(["users", user.username], user);
...
app.post("/admin/user/give-all", adminGuard, async (c) => {
    ...
    for await (const res of iter) {
        ...
        user.notifications.push({
            id: crypto.randomUUID(),
            msg: `Gift: You received ${addDays} FREE VIP Days!`,
            date: Date.now(),
            read: false
        });
...
app.post("/admin/topup/approve", adminGuard, async (c) => {
    ...
    if (user) {
        ...
        if (topup.purpose === "VIP Plan") {
            const plans: Record<number, number> = { 700: 30, 1500: 90, 2200: 150, 5000: 365 };
        ...
        user.notifications.push({
            id: crypto.randomUUID(),
            msg: `Topup Approved: ${actionMessage}`,
            date: Date.now(),
            read: false
        });
        await kv.set(["users", user.username], user);
...
app.post("/admin/topup/reject", adminGuard, async (c) => {
    ...
    if (topup) {
        ...
        const user = await getUser(topup.username);
        if(user) {
            if(!user.notifications) user.notifications = [];
            user.notifications.push({ id: crypto.randomUUID(), msg: `Topup Rejected: ${topup.amount}Ks (Check Transaction ID)`, date: Date.now(), read: false });
            await kv.set(["users", user.username], user);
        }
    }
...
app.get("/admin/backup", adminGuard, async (c) => { 
    const stream = new ReadableStream({ 
        async start(controller) { 
            const encoder = new TextEncoder(); 
            const prefixes = [["config"],["movies"],["users"],["keys"],["drafts"],["requests"],["topups"],["banned_ips"],["counts"],["idx_time"],["idx_cat"]];
    ...
    return new Response(stream, { headers: { "Content-Type": "application/x-ndjson", "Content-Disposition": `attachment; filename="goldflix_full_backup_${Date.now()}.ndjson"`, "Cache-Control": "no-cache" } }); 
});
app.post("/admin/user/delete-all-force", adminGuard, async (c) => {
    ...
    await logAdminAction("nuke_all_users", `Deleted ALL ${count} users.`);
    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=users&success=Deleted All " + count + " Users!");
});
...
app.get("/admin/force-fix-all", adminGuard, async (c) => {
    ...
    for await (const res of iter) {
        ...
        if (!m.createdAt) { m.createdAt = Date.now() - (count * 1000); needsUpdate = true; }
        if (typeof m.id !== 'string') { m.id = String(m.id); needsUpdate = true; }
        await saveMovieDB(m); 
        count++;
        if(needsUpdate) updated++;
    }
    return c.html(`<div style="font-family:sans-serif; padding:20px; background:#111; color:white; text-align:center;"><h1 style="color:#22c55e;">✅ System Repair Complete!</h1><p>Scanned: <b>${count}</b> Movies</p><p>Repaired & Timestamped: <b>${updated}</b> Movies</p><hr style="border-color:#333;"/><a href="${ADMIN_ROUTE}/dashboard" style="display:inline-block; padding:10px 20px; background:blue; color:white; text-decoration:none; border-radius:5px;">Go to Dashboard</a></div>`);
});
app.get("/admin/fix-counts-now", adminGuard, async (c) => {
    ...
    for await (const res of iter) {
        if(!res.value.isDeleted) {
            const cat = res.value.category || "Movies";
...
app.get("/admin/danger/nuke-myanmar", adminGuard, async (c) => {
    const iter = kv.list<Movie>({ prefix: ["movies"] });
    let count = 0;
    for await (const res of iter) {
        if (res.value.category === "Myanmar and Asian") {
            await kv.delete(res.key); 
            count++;
        }
    }
    return c.text(`Deleted ${count} movies from Myanmar and Asian. Now please go to Admin > Config > Re-Sync Database.`);
});
app.get("/admin/fix-myanmar-ghosts", adminGuard, async (c) => {
    const targetCat = "Myanmar and Asian";
    const iter = kv.list({ prefix: ["idx_cat", targetCat] });
    let removedCount = 0;
    for await (const res of iter) {
        const summary = res.value; 
        const movieId = summary.id;
        const movieData = await kv.get(["movies", movieId]);
        if (!movieData.value) {
            await kv.delete(res.key);
            await kv.delete(["idx_time", summary.createdAt, movieId]);
            const words = tokenize(summary.title);
            for (const w of words) {
                await kv.delete(["idx_search", w, movieId]);
            }
            removedCount++;
        }
    }
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    return c.text(`Scanned '${targetCat}'. Found and removed ${removedCount} ghost images.`);
});
app.get("/admin/force-fix-counts", adminGuard, async (c) => {
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    for(const cat of cats) await kv.delete(["counts", cat]);
    const iter = kv.list({ prefix: ["movies"] });
    const tally = {}; 
    ...
    let msg = `✅ Fixed Counts for ${totalMovies} Total Movies:\n\n`;
    for (const [cat, count] of Object.entries(tally)) {
        await kv.set(["counts", cat], new Deno.KvU64(BigInt(count)));
        msg += `- ${cat}: ${count}\n`;
    }
    return c.text(msg);
});
app.get("/admin/emergency-rebuild", adminGuard, async (c) => {
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    for(const cat of cats) await kv.delete(["counts", cat]);
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    RAM_CACHE.featuredMovies = { data: [], timestamp: 0 };
    const iter = kv.list({ prefix: ["movies"] });
    let count = 0;
    const BATCH_SIZE = 20; 
    let tasks = [];
    ...
    return c.text(`✅ Rebuild Complete! Processed ${count} movies. \nGo to Home Page now.`);
});
app.get("/admin/fix-usa-category", adminGuard, async (c) => {
    const iter = kv.list({ prefix: ["movies"] });
    let toSeries = 0;
    let toMovies = 0;
    for await (const res of iter) {
        const m = res.value;
        if (m && !m.isDeleted && m.category === "USA") {
            if ((m.episodes && m.episodes.length > 0) || m.title.toLowerCase().includes("season")) {
                m.category = "Series";
                toSeries++;
            } 
            else {
    ...
    return c.text(`✅ Migration Complete! \n- Moved to Series: ${toSeries}\n- Moved to Movies: ${toMovies}\n\nAnimation category is now empty. You can rename it to 'USA' in code.`);
});
// 🔥 UNDO FIX: လွန်ခဲ့သော မိနစ် ၂၀ အတွင်းက မှားဝင်သွားသော USA ကားများကို Myanmar ထဲ ပြောင်းမည်
app.get("/admin/undo-last-20mins", adminGuard, async (c) => {
    const iter = kv.list({ prefix: ["movies"] });
    let count = 0;
    
    // လွန်ခဲ့သော မိနစ် ၂၀ (20 Minutes Ago)
    const TIME_LIMIT = Date.now() - (20 * 60 * 1000);

    for await (const res of iter) {
        const m = res.value;
        
        // ၁. USA Category ဖြစ်ရမယ်
        // ၂. CreatedAt (တင်တဲ့အချိန်) က လွန်ခဲ့တဲ့ မိနစ် ၂၀ အတွင်း ဖြစ်ရမယ်
        if (m && !m.isDeleted && m.category === "USA" && m.createdAt > TIME_LIMIT) {
            
            m.category = "Myanmar and Asian";
            await saveMovieDB(m);
            count++;
        }
    }
    ...
    return c.text(`✅ Successfully moved ${count} movies from 'USA' to 'Myanmar' (Imported in last 20 mins).`);
});
