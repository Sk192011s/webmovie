...
// Database ကို ခဏခဏ မခေါ်ဘဲ RAM ပေါ်ကနေ ယူသုံးမယ့် စနစ်
const RAM_CACHE = {
...
const i18n: any = {
    ...
    my: {
        home: "ပင်မ", saved: "သိမ်းဆည်း", request: "တောင်းဆို", login: "အကောင့်ဝင်ရန်", me: "မိမိ",
        ...
        dl_btn: "ဒေါင်းလုပ်",
        create_acc: "အကောင့်သစ် ဖွင့်မည်",
...
// 🔥 SECURITY: Login Rate Limit (Brute Force Protection)
async function checkLoginRateLimit(ip: string): Promise<boolean> {
    ...
    // 5 ကြိမ်မှားရင် 15 မိနစ် ပိတ်မယ်
...
async function recordLoginFail(ip: string) {
    ...
    await kv.set(key, { count }, { expireIn: 15 * 60 * 1000 }); // 15 Minutes Ban
}
...
async function checkRateLimit(ip: string): Promise<boolean> {
  ...
  const res = await kv.get<{ count: number }>(key);
  const count = res.value?.count || 0;
  if (count >= 20) return false; // General Request Limit (20 req / 1 min)
...
// နှိပ်ပြီးသား Link တွေကို KV ထဲမှာ 1 Hour မှတ်ထားမယ်
...
async function saveMovieDB(movie: Movie) {
    ...
    // Clear RAM Cache to show new movie immediately
...
async function deleteMovieDB(id: string) {
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 }; // Clear Cache
...
// 🔥 PERFORMANCE: RAM Cached Latest Movies
async function getLatestMovies(limit: number = 20) {
    // Only cache if limit is standard (e.g., homepage request)
...
// 🔥 SYSTEM: Cron Job for Daily Maintenance (Auto-runs)
if (typeof Deno.cron !== "undefined") {
    Deno.cron("Daily Cleanup", "0 0 * * *", async () => {
        console.log("Running Daily Cleanup Task...");
        // Clear Expired Link Caches or Old Logs if needed
        // Here we just re-index for safety
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 3. Resolve URL (Token ဖြည်ပြီး Link အစစ်ယူမယ်)
                // Old code logic: fetch resolved url from api
                if (type === 'direct') { 
                    try { 
                ...
                // 4. Force Video Tag (Embed/Iframe စစ်ဆေးချက်များ ဖြုတ်ထားသည်)
                // ဘာ Link လာလာ <video> tag ထဲပဲ အတင်းထည့်ဖွင့်ပါမယ် - Old Code ပုံစံအတိုင်း
                ...
                if(video) { 
                    ...
                    // Error တက်ရင် Console မှာပြမယ် (Iframe မပြောင်းတော့ဘူး)
                    video.addEventListener('error', (e) => { 
                        console.log("Video Failed to Load");
                        if(loader) loader.style.display = 'none'; // Loader ဆက်မလည်အောင် ပိတ်ပေးမယ်
                    });
        ...
        for (const m of movies) {
            if (m.title && m.posterUrl) {
                ...
                const movieData = {
                    ...
                    linkType: "direct",
                    createdAt: Date.now() + count // slightly stagger creation time
                } as Movie;
