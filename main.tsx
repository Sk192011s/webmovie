...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        if (movies.length > 0) {
            ...
            // Myanmar Format ဆိုမှ Reverse လှန်မယ် (အပေါ်ဆုံးကောင် ထိပ်ရောက်ဖို့)
            if (!isJav && !isUSA) {
                movies.reverse();
            }
        }
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                // 🔥 LINK GENERATION LOGIC (UPDATED)
                let proxyStreamLink = "";
                let directDlLink = "";

                if (finalLink.includes("mediafire.com")) {
                    // Check if it contains "premium" (Pro Link)
                    if (finalLink.includes("premium")) {
                        // Pro Link -> Direct Proxy Use (No Scraper)
                        proxyStreamLink = "https://proxy.avotc.tk/" + finalLink; 
                        directDlLink = "https://proxy.avotc.tk/" + finalLink;
                    } else {
                        // Standard Link -> Use API Scraper
                        proxyStreamLink = "https://proxy.avotc.tk/" + finalLink; 
                        directDlLink = "/api/mediafire-stream?link=" + encodeURIComponent(finalLink);
                    }
                } 
                else {
                    // Other Direct Links (R2/Vercel) -> Proxy Direct
                    proxyStreamLink = "https://proxy.avotc.tk/" + finalLink;
                    directDlLink = "https://proxy.avotc.tk/" + finalLink;
                }
                ...
                const movieData: Movie = {
                    ...
                    streamUrl: proxyStreamLink,   
                    streamUrl2: finalLink,        
                    downloadUrl: directDlLink,    
                    downloadUrl2: finalLink,      
                    linkType: "direct", 
                    createdAt: itemTime,
                    isDeleted: false
                };
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Premium Detection Active)");
