...
// 🔥 CACHE VARIABLES (Global Scope for Config)
let configCache: AppConfig | null = null;
let lastConfigTime = 0;

async function getConfig() {
    // ၁ မိနစ်မပြည့်သေးရင် RAM (Cache) ထဲကပဲ ယူသုံးမယ် (Database မခေါ်ဘူး)
    if (configCache && (Date.now() - lastConfigTime < 60000)) {
        return configCache;
    }

    // ၁ မိနစ်ကျော်သွားရင် Database ခေါ်မယ်
    const res = await kv.get<AppConfig>(["config"]);
    const config = res.value || { 
        announcement: "Welcome to Gold Flix!", 
        showAnnouncement: true, 
        globalVipExpiry: 0 
    };

    // ရလာတာကို Cache ထဲထည့်မှတ်ထားမယ်
    configCache = config;
...
app.get("/movie/:id", async (c) => {
    ...
    // 🔥 OPTIMIZATION 1: getConfig() က အခုဆိုရင် Cache ထဲကနေပဲ ယူတော့မှာမို့ အရမ်းမြန်သွားပါပီ
    ...
    // 🔥 OPTIMIZATION 2: Related Movies ကို 6 ကားအစား 4 ကားပဲ ယူလိုက်ပါတယ် (Page Load ပိုမြန်စေရန်)
    let related = [];
    try {
        const allRelated = await getMoviesByCategory(movie.category, 5); // ၅ ကားယူပြီး
        related = allRelated.filter(m => m.id !== movie.id).slice(0, 4); // ကိုယ့်ကားဖယ်၊ ၄ ကားပဲယူ
    } catch (e) {}
           ...
           <div class="w-full aspect-video bg-black relative shadow-lg group">
                {canWatch ? (
                    ...
                    <div class="absolute inset-0 z-20">
                        ...
                        <div class="absolute inset-0 bg-black/20 flex items-center justify-center"><i class="fa-solid fa-lock text-5xl text-white/50 drop-shadow-lg"></i></div>
                    </div>
                )}
           </div>
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    try {
        ...
        // ... (အပေါ်ပိုင်း ကုဒ်များ) ...
        let title = body["title"] as string;
        let description = body["description"] as string; // မူရင်းစာကြောင်း
        
        // 🔥 AUTO CLEANER (အညွှန်းသန့်ရှင်းရေးလုပ်မည်)
        if (description) {
            description = description
                .replace(/&amp;nbsp;/g, " ")  // &amp;nbsp; ကို Space ပြောင်းမယ်
                .replace(/&nbsp;/g, " ")      // &nbsp; ကို Space ပြောင်းမယ်
                .replace(/&amp;/g, "&")       // &amp; ကို & ပြောင်းမယ်
                .replace(/<[^>]*>?/gm, "")    // HTML Tag (p, br, div) တွေကို ဖျက်မယ်
                .replace(/\s+/g, " ")         // Space တွေအရမ်းများနေရင် တစ်ခုတည်းဖြစ်အောင်လုပ်မယ်
                .trim();                      // ရှေ့နောက်က မလိုတဲ့ Space တွေဖျက်မယ်
        }

        let duration = body["duration"] as string;
        // ... (အောက်ပိုင်း ကုဒ်များ) ...
        let duration = body["duration"] as string; 
