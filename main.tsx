...
// 🔥 BULK IMPORT (Updated for Custom JSON)
    ...
    try {
        ...
        for (const m of movies) {
            ...
            if (m.title) {
                ...
                // 🔥 DATA MAPPING (မင်းရဲ့ JSON နဲ့ ကိုက်အောင် ညှိပေးထားသည်)
                const poster = m.posterUrl || m.image || ""; // 'image' key ပါရင်ယူမယ်
                const stream = m.streamUrl || m.link || "";  // 'link' key ပါရင်ယူမယ်
                const desc = m.description || m.info || "";  // 'info' key ပါရင်ယူမယ်
                const cat = m.category || "Myanmar and Asian"; // Category မပါရင် ဒါ Auto ထည့်မယ်

                const movieData = {
                    id,
                    title: m.title,
                    posterUrl: poster,
                    coverUrl: m.coverUrl || poster, // Cover မရှိရင် Poster ပဲသုံးမယ်
                    category: cat,
                    description: desc,
                    tags: m.tags || m.title,
                    year: m.year || new Date().getFullYear().toString(),
                    streamUrl: stream,
                    linkType: "direct", // MediaFire link ဖြစ်တဲ့အတွက် direct ထားလိုက်မယ်
