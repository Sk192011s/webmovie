...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    const existing = await kv.get(["movies", id]);
    
    // ID Overwrite Protection
    if (body["mode"] === "create" && existing.value && saveType === "publish") {
    ...
    if (saveType === "draft") {
        // Draft သီးသန့်နေရာမှာ သိမ်းမယ် (Public မှာ မပေါ်ဘူး)
        await kv.set(["drafts", id], movie);
        return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts&success=Saved to Drafts");
    } else {
        // အကယ်၍ Draft ထဲမှာရှိနေရင် ဖျက်မယ် (Real DB ထဲရောက်သွားပြီမို့လို့)
        await kv.delete(["drafts", id]);
        
        // အချိန်ကို လက်ရှိအချိန် ပြောင်းမယ် (Latest မှာ ပေါ်အောင်)
