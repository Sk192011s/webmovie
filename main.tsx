...
if (!SALT || !ADMIN_PASS) {
  ...
  // Deno Deploy won't exit, but logs will show error
}
...
const i18n: any = {
    en: {
        ...
        see_all: "View All", views: "Views", dl_help: "ဒေါင်းလော့ လုပ်နည်း",
...
interface Movie {
  ...
  category: "Movies" | "Series" | "Animation" | "Jav" | "All Uncensored" | "Myanmar and Asian"; 
...
async function deleteMovieDB(id: string) {
    ...
    try {
        ...
        console.log("Counter error ignored.");
    }
}
  ...
  <html lang={l}>
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        // 🔥 (၁) ပုံပေါ်လာရင် အလုပ်လုပ်မည့် Function (အသစ်ထည့်သည်)
        window.imgLoaded = function(img) {
            img.classList.add('loaded');
            // Skeleton loader ကို ဖြုတ်မည်
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 (၂) Cache ဖြစ်ပြီး ပုံပြီးနေရင် ချက်ချင်းဖော်မည် (အသစ်ထည့်သည်)
             setTimeout(() => {
                 document.querySelectorAll('img.img-fade').forEach(img => {
                     if(img.complete && img.naturalHeight !== 0) {
                         window.imgLoaded(img);
                     }
                 });
             }, 100);
             ...
             window.switchSeason = function(seasonId) {
                 ...
                 if (!isAlreadyActive) {
                     ...
                     if(targetBtn) {
                         targetBtn.classList.remove('bg-zinc-800', 'text-gray-400');
                         targetBtn.classList.add('bg-yellow-500', 'text-black');
                     }
                 }
             }
        ...
        async function loadMoreMovies(category) { 
            ...
            for(let i=0; i<6; i++) {
                ...
                if(category === "All Uncensored" || category === "Myanmar and Asian") { el.classList.remove('aspect-[2/3]'); el.classList.add('aspect-video'); }
                grid.appendChild(el);
                skeletons.push(el);
            }
            page++; 
            try { 
                ...
                data.movies.forEach(m => { 
                    ...
                    const badge = m.category === "4K Movies" ? "4K" : "HD";

                    if(category === "All Uncensored" || category === "Myanmar and Asian") { 
                        // 🔥 Load More အတွက် ပုံကုဒ်ပြင်ထားသည်
      ...
      <div id="help-modal" class="fixed inset-0 z-[100] bg-black/80 hidden items-center justify-center backdrop-blur-sm">
           <div class="bg-[#1f1f1f] p-6 rounded-xl border border-zinc-700 w-11/12 max-w-sm text-center relative shadow-2xl modal-enter">
              ...
              <div class="text-left text-xs text-gray-300 space-y-3 mb-6 bg-black/40 p-3 rounded-lg border border-zinc-800">
                   <p>
                       <strong class="text-yellow-500">နည်းလမ်း ၁ (Direct)</strong><br/>
                       Movieမှာ <span class="text-white font-bold bg-zinc-700 px-1 rounded">DL</span> ခလုတ်များ ပါရှိပါက  နှိပ်၍ တိုက်ရိုက်ဒေါင်းယူနိုင်ပါသည်။ မပါလျှင် နည်းလမ်း၂ကို သုံးပါ
                   </p>
                   <hr class="border-zinc-700"/>
                   <p>
                       <strong class="text-yellow-500">နည်းလမ်း ၂ (Player)</strong><br/>
                       ၁။ Video ကို ခနPlay လိုက်ပါ။<br/>
                       ၂။Videoလာလျှင် ညာဘက်အောက်ထောင့်က အစက် ၃ စက် (<i class="fa-solid fa-ellipsis-vertical"></i>) ကိုနှိပ်ပါ။<br/>
                       ၃။ <span class="text-white font-bold">'Download'</span> ကို ရွေးချယ်ပီးဒေါင်းနိူင်ပါသည်။
 ဒေါင်းနေချိန်Videoဖွင့်ကြည့်ရန်မလိုပါ၊ ခဏပဲလိုအပ်ပါသည်။<br/>
 Chrome Browser Apkကိုသာ အသုံးပြုပါ။
                   </p>
              </div>
            ...
            <header class="top-header">
                ...
                <div class="flex items-center gap-3">
                    ...
                    {active === 'home' && !props.isAdmin && (
                        <div class="px-2.5 py-1 rounded-lg border border-yellow-500/30 bg-yellow-500/10 text-[10px] font-bold text-yellow-500 tracking-wide shadow-[0_0_10px_rgba(234,179,8,0.1)]">
                            P - {daysLeft} Day
                        </div>
                    )}
                    {props.isAdmin ? (
...
app.use("*", async (c, next) => {
    // ၁. Admin Route နဲ့ Login/Static ဖိုင်တွေကို မပိတ်ဘူး (မို့ရင် Admin ပါ ပြန်ဝင်မရ ဖြစ်သွားမယ်)
    ...
    if (config.maintenanceMode) {
        ...
        // ၄. Admin မဟုတ်ရင် Maintenance စာမျက်နှာပြမယ်
        if (!isAdmin) {
            return c.html(
                <html lang="en">
                <head>
                ...
                <body class="bg-black text-white h-screen flex flex-col items-center justify-center p-6 text-center">
                    <i class="fa-solid fa-person-digging text-6xl text-yellow-500 mb-6 animate-bounce"></i>
                    <h1 class="text-3xl font-black text-white mb-2 uppercase tracking-widest">Website ပြုပြင်နေပါသည်</h1>
                    <p class="text-gray-400 max-w-md mx-auto leading-relaxed">
                        Gold Flix ကို ပိုမိုကောင်းမွန်အောင် ပြုပြင်မွမ်းမံမှုများ ပြုလုပ်နေပါသည်။ 
                        <br/><span class="text-yellow-500 font-bold">ခေတ္တစောင့်ဆိုင်းပေးပါ။</span>
                    </p>
                    <div class="mt-8 p-4 bg-[#111] rounded-xl border border-zinc-800">
                        <p class="text-xs text-gray-500 font-mono">Status: <span class="text-red-500 font-bold">System Upgrade In Progress...</span></p>
                    </div>
                </body>
                </html>
            );
...
// 🔥 SECURITY: Update Headers (CSP added)
app.use("*", secureHeaders({
  xFrameOptions: "DENY",      
  xContentTypeOptions: "nosniff",    
  xXssProtection: "1; mode=block",
  // Allowing unsafe-inline because frontend uses it heavily (for simplicity).
  // Production grade would separate JS files.
  contentSecurityPolicy: {
     defaultSrc: ["'self'"],
     scriptSrc: ["'self'", "'unsafe-inline'", "https://cdn.tailwindcss.com", "https://cdnjs.cloudflare.com"],
     styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://cdnjs.cloudflare.com"],
     fontSrc: ["'self'", "https://fonts.gstatic.com", "https://cdnjs.cloudflare.com"],
     imgSrc: ["'self'", "data:", "https:", "http:"], // Allow images from any source for posters
     mediaSrc: ["'self'", "https:", "http:"], // Allow videos
     connectSrc: ["'self'", "https:", "http:"],
     frameSrc: ["'self'", "https:", "http:"], // For embeds
  }
}));
app.use("*", compress());
// 🔥 SECURITY: CSRF Protection
app.use("*", csrf({ origin: (origin) => true })); // Allows all origins for now, tighten for prod if needed

app.notFound((c) => {
  return c.html(
    <Layout title="Not Found" hideNav={true}>
      <div class="min-h-screen flex flex-col items-center justify-center text-center p-6 bg-black text-white">
        <i class="fa-solid fa-compass-slash text-6xl text-zinc-700 mb-4"></i>
        <h1 class="text-4xl font-black text-yellow-500 mb-2">404</h1>
        <p class="text-gray-400 mb-8 font-bold">ဒီလမ်းကြောင်းမှာ ဘာမှမရှိပါဘူး။</p>
        <a href="/" class="btn-primary rounded-xl text-sm shadow-lg shadow-yellow-500/20">
            <i class="fa-solid fa-house mr-2"></i> Back Home
        </a>
      </div>
    </Layout>
  );
...
app.onError((err, c) => {
  console.error(err); 
  return c.html(
    <Layout title="Error" hideNav={true}>
      <div class="min-h-screen flex flex-col items-center justify-center text-center p-6 bg-black text-white">
        <i class="fa-solid fa-triangle-exclamation text-6xl text-red-600 mb-4 animate-pulse"></i>
        <h1 class="text-2xl font-bold text-white mb-2">Something went wrong!</h1>
        <p class="text-gray-500 text-sm mb-8">စနစ်ပိုင်းဆိုင်ရာ ချို့ယွင်းချက် ဖြစ်ပေါ်နေပါသည်။</p>
        <a href="/" class="bg-zinc-800 border border-zinc-700 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-zinc-700 transition">
            Try Again
        </a>
      </div>
    </Layout>
...
app.get("/sitemap.xml", async (c) => {
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        ${movies.map(m => `
        <url>
            <loc>https://${c.req.header("host")}/movie/${m.id}</loc>
...
app.get("/api/fix-categories", async (c) => {
    const pass = c.req.query("pass");
    if (pass !== ADMIN_PASS) return c.text("Unauthorized", 401);
    ...
    await kv.delete(["counts", "Adult"]);
    return c.text(`ပြီးပါပြီ! ဇာတ်ကား ${count} ကားကို Adult မှ Jav သို့ ပြောင်းလိုက်ပါပြီ။`);
});
...
app.get("/", async (c) => {
  ...
  const sections = [
    ...
    { name: "4K Movies", data: await getMoviesByCategory("4K Movies", 8) },
    { name: "Animation", data: await getMoviesByCategory("Animation", 8) },
    { name: "Jav", data: await getMoviesByCategory("Jav", 8) },
    { name: "All Uncensored", data: await getMoviesByCategory("All Uncensored", 8) },
    { name: "Myanmar and Asian", data: await getMoviesByCategory("Myanmar and Asian", 8) },
    { name: "4K Porns", data: await getMoviesByCategory("4K Porns", 8) }
];
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        {shouldShowPopup && (
          <div id="promo-popup" class="fixed inset-0 z-[200] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
              <div class="relative w-full max-w-sm bg-[#1f1f1f] rounded-2xl overflow-hidden shadow-2xl border border-yellow-500/30 modal-enter flex flex-col max-h-[85vh]">
                  <button onclick="closePopup(false)" class="absolute top-2 right-2 z-10 bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 transition backdrop-blur-md">
                      <i class="fa-solid fa-xmark"></i>
                  </button>
                  <div class="overflow-y-auto custom-scroll flex-grow">
                      <a href={config.popupLink || "#"}>
                          <img src={config.popupImage} class="w-full h-auto object-cover" />
                      </a>
                      <div class="p-5 bg-[#1f1f1f]">
                          {config.popupMessage && (
                              <p class="text-gray-300 text-sm mb-4 leading-relaxed whitespace-pre-wrap font-medium text-center">
                                  {config.popupMessage}
                              </p>
                          )}
                          <a href={config.popupLink || "#"} class="block w-full bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold text-center py-3.5 rounded-xl shadow-lg hover:brightness-110 transition uppercase tracking-wider text-sm">
                              {config.popupBtnText || "View Details"}
                          </a>
                          <button onclick="closePopup(true)" class="block w-full text-center text-xs text-gray-500 font-bold mt-3 py-2 rounded-lg hover:bg-zinc-800 hover:text-gray-300 transition">
              ...
              <script dangerouslySetInnerHTML={{__html: `
                  ...
                  const isHiddenToday = localStorage.getItem('popup_hidden_date') === today;
                  const isHiddenSession = sessionStorage.getItem('popup_closed_session');
                  if (!isHiddenToday && !isHiddenSession) {
                      setTimeout(() => {
                  ...
                  window.closePopup = function(hideToday) {
                      ...
                      if(hideToday) {
                          ...
                          sessionStorage.setItem('popup_closed_session', 'true');
                      }
                  }
              `}} />
          </div>
        ...
        <div class="sticky top-[50px] z-30 bg-black/95 backdrop-blur-md px-4 py-3 border-b border-zinc-800 transition-all shadow-lg">
             <form action="/search" method="get" class="relative">
                 <button type="submit" class="absolute left-4 top-3.5 text-gray-500 hover:text-yellow-500 transition z-10">
                  ...
                  {sliderMovies.map((m, idx) => (
                      <div class={`slide ${idx === 0 ? 'active' : ''}`}>
                          ...
                          <div class="absolute bottom-6 left-4 right-4">
                              ...
                              <a href={`/movie/${m.id}`} class="mt-3 inline-flex items-center gap-2 bg-white/90 backdrop-blur text-black px-4 py-1.5 rounded-lg font-bold text-xs hover:scale-105 transition transform shadow-lg">
                                  <i class="fa-solid fa-play"></i> {t.play}
                              </a>
                          </div>
                      </div>
                  ))}
          ...
          <div class="h-scroll-section">
              {catMovies.map(m => (
                  <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded-xl overflow-hidden active:scale-95 transition-transform movie-card group">
                      ...
                      {/* 🔥 Lazy Loading & Fade Effect ပါဝင်သော Cover Photo */}
                      <div class="aspect-video w-full relative overflow-hidden img-skeleton">
                          <img 
                              src={m.coverUrl || m.posterUrl} 
                              loading="lazy" 
                              decoding="async" 
                              onload="window.imgLoaded(this)"
                              class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" 
                          />
                          {/* 4K Badge (ညာဘက်ထောင့်) */}
                          <div class="absolute top-2 right-2 bg-yellow-500 text-black text-[9px] font-bold px-1.5 py-0.5 rounded shadow z-10">
                              { (m.category === "4K Movies" || m.category === "4K Porns") ? "4K" : "HD" }
                          </div>
                    ...
                    <div class="h-scroll-section custom-scroll">
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform movie-card group">
                                <div class="aspect-[2/3] w-full relative overflow-hidden"><img src={m.posterUrl} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" /><div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow">
    {m.category === "4K Movies" ? "4K" : "HD"}
</div></div>
...
app.get("/category/:cat", async (c) => { 
    ...
    return c.html(<Layout ...>
        ...
        <div id="movie-grid" class={isUncensored ? "space-y-4" : "grid grid-cols-3 gap-3"}>
            {movies.map(m => (
                <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'mb-4' : ''}`}>
                    
                    {/* 🔥 2. Skeleton & Fade Effect ထည့်ပါ */}
                    <div class={`relative overflow-hidden ${isUncensored ? "aspect-video" : "aspect-[2/3]"} img-skeleton`}>
                        <img 
                            src={isUncensored ? (m.coverUrl || m.posterUrl) : m.posterUrl} 
                            loading="lazy" 
                            decoding="async" 
                            onload="window.imgLoaded(this)"
                            class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" 
                        ...
                        {/* 🔥 3. Badge Logic (4K Porns ပါထည့်ပါ) */}
                        <div class="absolute top-2 right-2 bg-yellow-500 text-black text-[9px] font-bold px-1.5 py-0.5 rounded shadow z-10">
                            {(m.category === "4K Movies" || m.category === "4K Porns") ? "4K" : "HD"}
                        </div>
                    </div>

                    <div class={isUncensored ? "p-3" : "p-2"}>
                        <h3 class={isUncensored ? "text-sm font-bold truncate text-white" : "text-[11px] font-bold truncate text-white"}>{m.title}</h3>
                    </div>
                </a>
            ))}
        </div>
            ...
            <div class="p-4">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <form action="/search" method="get" class="flex-grow relative">
                        <button type="submit" class="absolute left-3 top-3 text-gray-500 hover:text-yellow-500 transition z-10">
                            <i class="fa-solid fa-magnifying-glass"></i>
                        </button>
...
app.post("/report", async (c) => {
    const user = await getCurrentUser(c);
    const { movieId, movieName } = await c.req.parseBody();
    const reqId = crypto.randomUUID();
    const reportData: UserRequest = { 
        id: reqId, 
        username: user ? user.username : "Guest", 
        movieName: `[BROKEN LINK] ${movieName}`, 
        timestamp: Date.now() 
    }; 
    await kv.set(["requests", reqId], reportData);
    return c.redirect(`/movie/${movieId}?success=Report Sent! Admin will fix it soon.`);
});
...
app.get("/movie/:id", async (c) => {
    ...
    // 🔥 Optimization 1: Database မေးတာတွေကို ပြိုင်တူမေးလိုက်မယ် (မြန်ဆန်စေသည်)
    ...
    const movie = movieRes; // movieRes ကို ပြန်ထုတ်သုံး

    if (!movie) return c.text("Not Found", 404);

    // 🔥 Optimization 2: Related Movies ကိုလည်း ပြိုင်တူရှာမယ်
    // (အရင်က ဒီနေရာမှာ DB ထပ်ခေါ်နေလို့ ကြာတာပါ)
    ...
    if (premium) {
        // 🔥 Optimization 3: resolveRedirect() ကို ဖြုတ်လိုက်ပါပြီ။
        // စာမျက်နှာဖွင့်ဖွင့်ချင်း Link သွားစစ်နေရင် အရမ်းကြာလို့ပါ။ 
        // Player ဖွင့်မှ JS နဲ့ Auto စစ်ပါလိမ့်မယ်။
        
        if (movie.linkType === "embed" || initialStreamUrl.includes("<iframe")) { 
            playerUrl = initialStreamUrl; 
        } else { 
            // Link အစစ်မစစ်ဘဲ Token တန်းထုတ်ပေးလိုက်မယ် (Page Load အရမ်းမြန်သွားမယ်)
        ...
        if (movie.downloadUrl2) { 
            const dlToken2 = crypto.randomUUID(); 
            await kv.set(["stream_tokens", dlToken2], movie.downloadUrl2, { expireIn: 3600 * 3 }); 
            secureDownloadUrl2 = `/dl/${dlToken2}`; 
        }
    }
           ...
           <div class="p-6">
               <div class="flex justify-between items-start mb-3">
                   ...
                   <div class="flex items-center gap-2 flex-shrink-0 h-10">
                       <button onclick={`shareMovie('${movie.title}')`} class="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700 transition">
                           <i class="fa-solid fa-share-nodes text-lg"></i>
                       </button>
                       {user && (
                           <form action="/api/fav" method="post" class="m-0 p-0 flex">
                               <input type="hidden" name="movieId" value={movie.id} />
                               <button class="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-800 transition hover:bg-zinc-700">
                                   <i class={`fa-solid fa-heart text-lg ${isFav ? 'text-red-600' : 'text-zinc-400'}`}></i>
                               </button>
                           </form>
                       )}
                   </div>
               </div>
               <div class="flex items-center gap-3 text-xs text-gray-400 mb-6"><span class="bg-zinc-800 px-3 py-1 rounded-full border border-zinc-700">{movie.year}</span><span class="text-yellow-500 font-bold border border-yellow-500/30 bg-yellow-500/10 px-3 py-1 rounded-full">{movie.category}</span><span class="bg-zinc-800 px-3 py-1 rounded-full border border-zinc-700">
                   {movie.fileSize || (movie.category === "4K Movies" ? "4K" : "HD")}
               </span></div>
               {premium && (
...
<button onclick="openHelpModal()" class="w-fit mx-auto text-xs text-yellow-500 hover:text-yellow-400 flex items-center gap-2 mt-4 font-bold transition-colors bg-yellow-500/10 px-4 py-2 rounded-full border border-yellow-500/20">
   <i class="fa-solid fa-circle-question"></i> {t.dl_help}
</button>

<form action="/report" method="post" class="mt-4 text-center">
    <input type="hidden" name="movieId" value={movie.id} />
    <input type="hidden" name="movieName" value={movie.title} />
    <button class="text-[10px] text-red-500 hover:text-red-400 font-bold underline decoration-dotted opacity-80 hover:opacity-100 transition">
        <i class="fa-solid fa-flag"></i> Report Broken Link
    </button>
</form>
                   </div>
               )}
               {(movie.category === "Series" || movie.category === "Animation") && episodes.length > 0 && premium && ( 
                   <div class="mb-8">
                       <h3 class="font-bold text-white mb-3 flex items-center gap-2">
                           <i class="fa-solid fa-layer-group text-yellow-500"></i> Seasons & Episodes
                       </h3>
                       <div class="flex overflow-x-auto gap-3 pb-2 scrollbar-hide">
                           {Object.keys(seasons).map((season, idx) => {
                               ...
                               return (
                                   <button 
                                       id={`btn-${safeId}`}
                                       onclick={`switchSeason('${safeId}')`}
                                       class="season-tab-btn flex-shrink-0 px-5 py-2 rounded-full text-xs font-bold transition border border-zinc-800 bg-zinc-800 text-gray-400 hover:text-white hover:border-zinc-600"
                                   >
                                       {season}
                                   </button>
                               )
                                   ...
                                   <div id={`ep-grid-${safeId}`} class="season-content hidden mt-3 bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3 animate-fade-in">
                                       <div class="grid grid-cols-4 gap-2 max-h-60 overflow-y-auto custom-scroll pr-1">
                                           {seasons[season].map(ep => (
                                               <button 
    onclick={`loadPlayer('${ep.url.trim()}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`} 
    class="srv-btn bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] py-3 rounded-lg border border-zinc-700 transition-colors font-bold truncate">
    {ep.name}
</button>
                                           ))}
                                       </div>
               ...
               {related.length > 0 && (
                   <div class="pt-6 border-t border-zinc-800">
                       <h3 class="font-bold text-white mb-4 text-lg">You May Also Like</h3>
                       <div class="h-scroll-section custom-scroll">
                           {related.map(m => {
                               const isUncensored = m.category === "All Uncensored";
                               return (
                                   <a href={`/movie/${m.id}`} class={`h-scroll-item block relative bg-[#1f1f1f] rounded-xl overflow-hidden flex-shrink-0 movie-card group ${isUncensored ? 'wide' : 'w-28'}`}>
                                       <div class={`${isUncensored ? "aspect-video" : "aspect-[2/3]"} w-full relative overflow-hidden img-skeleton`}>
...
app.get("/profile", async (c) => { 
    ...
    const lang = getLang(c);
    const plans = [
        { name: "LuGyi Apk 1 Month + Gold 15 Day", price: "700 Ks", days: 30, features: ["Unlock All Movies", "Direct Download"] },
        { name: "LuGyi Apk 3 Month + Gold 45 Day", price: "1,500 Ks", days: 90, popular: true, features: ["Unlock All Movies", "Direct Download"] },
        { name: "LuGyi Apk 5 Month + Gold 75 Day", price: "2,200 Ks", days: 150, features: ["Unlock All Movies", "Direct Download"] },
        { name: "LuGyi Apk 1Year + Gold 180 Day", price: "5,000 Ks", days: 365, features: ["Unlock All Movies", "Direct Download"] }
    ]; 
    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            <div class="relative w-full aspect-[1.8/1] rounded-2xl bg-gradient-to-br from-[#FFD700] via-[#FDB931] to-[#9e7f13] p-6 shadow-[0_10px_30px_-10px_rgba(253,185,49,0.4)] text-black flex flex-col justify-between overflow-hidden relative group">
                ...
                <div class="relative z-10 flex justify-between items-start">
                     <div class="flex items-center gap-4">
                         <div class="w-14 h-14 bg-black/90 text-yellow-500 rounded-full flex items-center justify-center text-2xl font-black border-2 border-white/50 shadow-lg backdrop-blur-sm">
                            {user.username[0].toUpperCase()}
                         </div>
                         <div>
                            <h2 class="text-2xl font-black tracking-tighter leading-none">{user.username}</h2>
                            <span class="text-[10px] font-bold bg-black/20 px-2 py-0.5 rounded text-black/80 inline-block mt-1 uppercase tracking-widest border border-black/10">{statusText}</span>
                         </div>
                     </div>
                     <i class="fa-solid fa-wifi text-2xl opacity-60"></i>
                </div>
                <div class="relative z-10 font-mono">
                    <p class="text-[9px] uppercase opacity-70 font-bold mb-1">Membership ID</p>
                    <div class="flex items-center gap-2">
                        <p class="text-lg font-bold tracking-widest">{memberId}</p>
                        <button onclick={`copyToClip('${memberId}')`} class="w-6 h-6 flex items-center justify-center bg-black/10 rounded hover:bg-black/20 transition text-xs"><i class="fa-regular fa-copy"></i></button>
                    </div>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
                 <a href="/favorites" class="bg-[#1f1f1f] p-4 rounded-2xl border border-zinc-800 flex items-center gap-4 hover:bg-[#252525] transition active:scale-95">
                     <div class="w-10 h-10 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 text-lg">
                        <i class="fa-solid fa-heart"></i>
                     </div>
                     <div>
                        <span class="block text-2xl font-bold text-white leading-none">{favCount}</span>
                        <span class="text-[10px] text-gray-500 font-bold uppercase">Saved</span>
                     </div>
                 </a>
                 <div class="bg-[#1f1f1f] p-4 rounded-2xl border border-zinc-800 flex items-center gap-4">
                     <div class={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${premium ? 'bg-green-500/10 text-green-500' : 'bg-gray-700/20 text-gray-500'}`}>
                        <i class="fa-solid fa-clock"></i>
                     </div>
                     <div>
                        <span class={`block text-2xl font-bold leading-none ${premium ? 'text-white' : 'text-gray-500'}`}>
                            {premium ? finalDays : "0"}
                        </span>
                        <span class="text-[10px] text-gray-500 font-bold uppercase">Days Left</span>
                     </div>
                 </div>
            </div>
            <div class="bg-[#1f1f1f] p-5 rounded-2xl border border-zinc-800 relative overflow-hidden">
                 ...
                 <h3 class="font-bold text-gray-300 text-sm mb-3 flex items-center gap-2 relative z-10">
                    <i class="fa-solid fa-gift text-yellow-500"></i> VIP Redeem Code
                 </h3>
                 <form action="/profile/redeem" method="post" class="relative z-10 flex gap-2">
                    <input name="key" placeholder="Paste VIP Code here..." required class="bg-black/50 border border-zinc-700 text-white text-sm px-4 py-3 rounded-xl w-full outline-none focus:border-yellow-500 transition placeholder-gray-600 font-mono" />
            ...
            <div>
                <h3 class="font-bold text-white mb-4 flex items-center gap-2 text-sm uppercase tracking-wider text-gray-500">
                    <i class="fa-solid fa-gem text-blue-500"></i> LuGyi and Gold Plan
                </h3>
                <div class="grid grid-cols-2 gap-3">
                    {plans.map(p => (
                        <div class={`relative bg-[#1f1f1f] p-4 rounded-2xl border flex flex-col justify-between transition active:scale-95 ${p.popular ? 'border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'border-zinc-800'}`}>
                            {p.popular && <div class="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-yellow-500 text-black text-[9px] font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-widest">Best Value</div>}
                            <div class="text-center mb-3">
                                <h4 class="text-gray-400 font-bold text-xs uppercase mb-1">{p.name}</h4>
                                <span class="text-white font-black text-xl tracking-tight">{p.price}</span>
                            </div>
                            <div class="space-y-1.5 mb-4 px-2">
                                {p.features.slice(0,2).map(f => (
                                    <div class="flex items-center gap-1.5 text-[10px] text-gray-500 justify-center">
                                        <i class="fa-solid fa-check text-green-500"></i> {f}
                                    </div>
                                ))}
                            </div>
                            <a href="https://t.me/LuGyiandYoteshinMovies" target="_blank" class={`block text-center py-2.5 rounded-xl font-bold text-xs transition ${p.popular ? 'bg-yellow-500 text-black hover:bg-yellow-400' : 'bg-zinc-800 text-white hover:bg-zinc-700'}`}>
                                Buy Now
                            </a>
                        </div>
                    ))}
            ...
            <a href="/logout" class="flex items-center justify-center w-full gap-2 text-red-500 font-bold text-xs py-3.5 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500 hover:text-white transition active:scale-95 shadow-sm mt-4">
        ...
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-black min-h-screen font-sans text-sm">
                ...
                <div class="flex justify-between items-center mb-6 bg-[#111] p-4 rounded-xl border border-zinc-800 shadow-sm">
                    ...
                    <div class="flex gap-2">
                        ...
                        <form action="/admin/restore" method="post" enctype="multipart/form-data" class="inline">
                            <label class="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1.5 rounded-lg text-xs border border-zinc-700 cursor-pointer font-bold"><i class="fa-solid fa-upload"></i> Restore<input type="file" name="file" class="hidden" onchange="this.form.submit()" /></label>
                        </form>
                    </div>
                </div>

                <div class="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
                    <button id="btn-movies" onclick="openTab('movies')" class="tab-btn active px-5 py-2.5 bg-[#111] rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-zinc-800">Movies</button>
                    <button id="btn-drafts" onclick="openTab('drafts')" class="tab-btn px-5 py-2.5 bg-[#111] rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-zinc-800 flex items-center gap-2">
                        Drafts <span class="bg-yellow-500 text-black px-1.5 rounded-full text-[9px]">{drafts.length}</span>
                    </button>
                    <button id="btn-keys" onclick="openTab('keys')" class="tab-btn px-5 py-2.5 bg-[#111] rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-zinc-800">VIP Keys</button>
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <div class="flex justify-between items-center mb-4 border-b border-white/10 pb-3">
                                <div>
                                    <h2 class={`font-bold text-sm ${editMovie ? "text-blue-400" : "text-yellow-500"}`}>
                                        {editMovie ? "✏️ EDITING MODE" : "✨ ADD NEW MOVIE"}
                                    </h2>
                                    {editMovie && <p class="text-[10px] text-gray-400 mt-1">Editing: <span class="text-white font-bold">{editMovie.title}</span></p>}
                                </div>
                                {editMovie && (
                                    <a href={ADMIN_ROUTE + "/dashboard"} class="bg-red-600 text-white text-[10px] font-bold px-3 py-1.5 rounded hover:bg-red-500 transition shadow">
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex gap-2 w-full">
                                        <select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">
                                            {["Movies","Series","4K Movies","Animation","Jav","All Uncensored", "Myanmar and Asian" "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}
                                        </select>
                                ...
                                <div class="bg-zinc-800 p-3 rounded-xl mb-4 border border-zinc-700">
                                    <div class="flex justify-between items-center mb-2">
                                        <label class="text-[10px] text-green-400 uppercase font-bold"><i class="fa-solid fa-wand-magic-sparkles"></i> Magic Tools</label>
                                        <button type="button" onclick="clearEpisodes()" class="text-[10px] text-red-400 hover:text-white underline">Clear All</button>
                                    </div>
                                    
                                    <div class="flex gap-2 mb-3">
                                        <input id="gen-season" placeholder="Season (e.g. 1)" value="1" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-1/4 text-center" />
                                        <input id="gen-start" type="number" placeholder="Start Ep (1)" value="1" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-1/4 text-center" />
                                    </div>

                                    <div class="border-t border-zinc-700 pt-2 mb-3">
                                        <p class="text-[9px] text-gray-400 mb-1">Tool 1: Sequence (Use *** or $$)</p>
                                        <div class="flex gap-2">
                                            <input id="gen-url" placeholder="Link template..." class="input-box text-[10px] bg-black border-zinc-600 h-8 flex-grow" />
                                            <input id="gen-end" type="number" placeholder="End Ep" class="input-box text-[10px] bg-black border-zinc-600 h-8 w-16 text-center" />
                                            <button type="button" onclick="generateEpisodes()" class="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold px-3 rounded transition">Gen</button>
                                        </div>
                                    </div>

                                    <div class="border-t border-zinc-700 pt-2">
                                        <p class="text-[9px] text-gray-400 mb-1">Tool 2: Bulk Extractor (Paste Database/Messy text)</p>
                                        <textarea id="bulk-text" placeholder="Paste text here..." class="input-box text-[10px] bg-black border-zinc-600 h-16 w-full mb-2"></textarea>
                                        <button type="button" onclick="extractLinks()" class="w-full bg-green-700 hover:bg-green-600 text-white text-xs font-bold py-2 rounded transition">
                                            Force Extract Links
                                        </button>
                                    </div>
                                    ...
                                    <script dangerouslySetInnerHTML={{__html: `
                                        function generateEpisodes() {
                                            const urlTpl = document.getElementById('gen-url').value;
                                            const season = document.getElementById('gen-season').value;
                                            const start = parseInt(document.getElementById('gen-start').value) || 1;
                                            const end = parseInt(document.getElementById('gen-end').value);
                                            const textarea = document.getElementsByName('episodeList')[0];

                                            if (!urlTpl || !end) { alert("URL & End Number Required!"); return; }
                                            
                                            let placeholder = "";
                                            if (urlTpl.includes("***")) placeholder = "***";
                                            else if (urlTpl.includes("$$")) placeholder = "$$";
                                            else if (urlTpl.includes("XX")) placeholder = "XX";
                                            
                                            if (!placeholder) { alert("Use *** or $$ in URL!"); return; }

                                            let result = textarea.value ? textarea.value + "\\n" : "";
                                            for (let i = start; i <= end; i++) {
                                                const numStr = i < 10 ? '0' + i : '' + i;
                                                const finalUrl = urlTpl.replace(placeholder, numStr);
                                                result += \`Season \${season} | Ep.\${i} | \${finalUrl}\\n\`;
                                            }
                                        ...
                                        function extractLinks() {
                                            try {
                                                let rawText = document.getElementById('bulk-text').value;
                                                const season = document.getElementById('gen-season').value;
                                                let epCount = parseInt(document.getElementById('gen-start').value) || 1;
                                                const textarea = document.getElementsByName('episodeList')[0];

                                                if (!rawText) { alert("Paste some text first!"); return; }

                                                rawText = rawText.replace(/[\\x00-\\x1F\\x7F]/g, "\\n");

                                                const urlRegex = /(https?:\\/\\/[^\\n\\r]+?\\.(?:txt|mp4|mkv|m3u8|avi|mov|flv|webm|ts|mpg|mpeg))/gi;
                                                
                                                const foundLinks = rawText.match(urlRegex);
                                                ...
                                                let processedLinks = foundLinks.map(link => {
                                                    try {
                                                ...
                                                uniqueLinks.sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
                                                ...
                                                uniqueLinks.forEach(url => {
                                                    ...
                                                    if (cleanUrl.length < 15) return;

                                                    result += \`Season \${season} | Ep.\${epCount} | \${cleanUrl}\\n\`;
                                                    epCount++;
                                                    addedCount++;
                                                });
                                                ...
                                                alert(\`Success! Extracted \${addedCount} links!\`);
                                        ...
                                        function clearEpisodes() { 
                                            if(confirm('Clear all episodes?')) {
                                                document.getElementsByName('episodeList')[0].value = ''; 
                                            }
                                        }
                                    `}} />
                                </div>
                                ...
                                <div class="p-3 bg-black/40 rounded-xl border border-zinc-800">
                                    ...
                                    <textarea name="episodeList" placeholder="S1 | Ep.1 | https://..." rows={3} class="input-box w-full font-mono text-xs whitespace-pre overflow-x-auto bg-[#111]">{epString}</textarea>
                                </div>
                                ...
                                <div class="p-3 bg-black/40 rounded-xl border border-zinc-800">
                                    <label class="text-[10px] text-gray-500 uppercase font-bold">Backup Server</label>
                                ...
                                <div class="relative">
                                    <div class="flex justify-between items-end mb-1">
                                        <label class="text-[10px] text-gray-500 uppercase font-bold">Synopsis / Description</label>
                                        <button type="button" onclick="generateAIDesc()" class="text-[10px] bg-purple-600 hover:bg-purple-500 text-white px-2 py-1 rounded flex items-center gap-1 transition">
                                            <i class="fa-solid fa-robot"></i> Auto Gen (AI)
                                        </button>
                                    </div>
                                    <textarea id="desc-box" name="description" placeholder="Description..." class="input-box w-full h-24 bg-black border-zinc-700">{editMovie?.description}</textarea>
                                    
                                    {/* AI Loading Indicator */}
                                    <div id="ai-loader" class="absolute inset-0 bg-black/80 hidden items-center justify-center rounded-lg z-10">
                                        <div class="text-purple-500 font-bold text-xs animate-pulse">Thinking...</div>
                                    </div>
                                </div>

                                {/* Script for AI (Smart Translate Mode) */}
                                <script dangerouslySetInnerHTML={{__html: `
                                    async function generateAIDesc() {
                                        const title = document.getElementsByName('title')[0].value;
                                        const type = document.getElementsByName('category')[0].value;
                                        const year = document.getElementsByName('year')[0].value;
                                        const box = document.getElementById('desc-box');
                                        const currentDesc = box.value; // 🔥 လက်ရှိ box ထဲက စာကို ယူမယ်

                                        if(!title) { alert("Please enter Movie Title first!"); return; }

                                        const loader = document.getElementById('ai-loader');
                                        loader.classList.remove('hidden'); loader.classList.add('flex');

                                        try {
                                            const res = await fetch('/admin/api/generate-desc', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                                                // 🔥 currentDesc ကိုပါ ထည့်ပို့လိုက်မယ်
                                                body: 'title=' + encodeURIComponent(title) + '&type=' + encodeURIComponent(type) + '&year=' + encodeURIComponent(year) + '&currentDesc=' + encodeURIComponent(currentDesc)
                                            });
                                            const data = await res.json();
                                            if(data.error) { alert(data.error); } else { box.value = data.desc; }
                                        } catch(e) { alert("Error generating description"); } 
                                        finally { loader.classList.add('hidden'); loader.classList.remove('flex'); }
                                    }
                                `}} />
                                
                                <label class="flex items-center gap-2 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700 cursor-pointer hover:bg-zinc-800 transition my-2">
                                    <input type="checkbox" name="updateTime" class="accent-yellow-500 w-4 h-4" />
                                    <span class="text-xs font-bold text-gray-300">Move to Top / ထိပ်ဆုံးသို့ရွှေ့မည်</span>
                                </label>

                                <div class="grid grid-cols-2 gap-3 pt-2">
                                    <button type="submit" name="saveType" value="draft" class="py-3 rounded-xl font-bold text-xs bg-zinc-800 text-gray-300 hover:bg-zinc-700 border border-zinc-700 transition">
                                        <i class="fa-solid fa-floppy-disk"></i> Save Draft
                                    </button>
                                    
                                    <button type="submit" name="saveType" value="publish" class="py-3 rounded-xl font-bold text-xs bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-900/20 transition">
                                        {editMovie ? "Update Live" : "Publish Now"}
                                    </button>
                                </div>
                            </form>
                        </div>
                                ...
                                {movies.map(m => (
                                    <div class="movie-item flex gap-4 p-3 rounded-xl items-center border bg-black border-zinc-800/50 hover:border-zinc-600 transition" data-title={m.title}>
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <form action={`/admin/movie/delete/${m.id}`} method="post" onsubmit="return confirm('Delete?')"><button class="text-red-500 text-xs border border-red-500/20 bg-red-500/10 px-3 py-1.5 rounded hover:bg-red-500 hover:text-white transition">Del</button></form>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div id="tab-drafts" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-2xl border border-zinc-800 shadow-xl min-h-[50vh]">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="font-bold text-yellow-500 text-lg flex items-center gap-2"><i class="fa-solid fa-file-pen"></i> Drafts ({drafts.length})</h2>
                                    ...
                                    <div class="flex gap-4 p-4 rounded-xl bg-black border border-yellow-500/20 relative group">
                                        ...
                                        <div class="flex flex-col justify-between flex-grow">
                                            ...
                                            <div class="flex gap-2 mt-3">
                                                ...
                                                <form action="/admin/movie/save" method="post" class="flex-1">
                                                    <input type="hidden" name="id" value={d.id} />
                                                    {Object.entries(d).map(([k,v]) => {
                                                        if(k==='episodes'||k==='id') return null;
                                                        return <input type="hidden" name={k} value={String(v)} />
                                                    })}
                                                </form>

                                                <form action={`/admin/draft/delete/${d.id}`} method="post" onsubmit="return confirm('Delete draft?')">
                                                    <button class="px-3 py-2 bg-red-900/20 text-red-500 border border-red-900/50 rounded hover:bg-red-600 hover:text-white transition"><i class="fa-solid fa-trash"></i></button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div id="tab-config" class="tab-content">
                    ...
                    <div class="bg-red-900/10 border border-red-500/30 p-4 rounded-xl flex justify-between items-center">
            <div>
                <h3 class="font-bold text-red-500 flex items-center gap-2"><i class="fa-solid fa-screwdriver-wrench"></i> Maintenance Mode</h3>
                <p class="text-[10px] text-gray-400">Turn ON to hide website from users.</p>
            </div>
            <form action="/admin/config/maintenance" method="post">
                <input type="hidden" name="status" value={config.maintenanceMode ? "off" : "on"} />
                <button class={`px-4 py-2 rounded-lg font-bold text-xs transition ${config.maintenanceMode ? 'bg-red-600 text-white animate-pulse' : 'bg-zinc-800 text-gray-400'}`}>
                    {config.maintenanceMode ? "🔴 ON (Hidden)" : "⚪️ OFF (Visible)"}
                </button>
            </form>
            </div>                     
                        <div>
                            <h2 class="font-bold mb-4 text-gray-300 flex items-center gap-2"><i class="fa-solid fa-bullhorn"></i> Announcement</h2>
                            <form action="/admin/config" method="post" class="space-y-4">
                                <div><input name="text" placeholder="Enter message..." value={config.announcement} class="input-box bg-black border-zinc-700" /></div>
                                <label class="flex items-center gap-3 p-4 bg-black rounded-xl border border-zinc-800 cursor-pointer hover:border-zinc-600 transition">
                                    <input type="checkbox" name="show" checked={config.showAnnouncement} class="accent-yellow-500 w-5 h-5" />
                                    <span class="font-bold text-sm text-gray-300">Show Announcement Bar</span>
                                </label>
                                <button class="btn-primary w-full shadow-lg">Save Changes</button>
                            </form>
                        </div>

                        <div class="border-t border-zinc-800 pt-8">
                            <h2 class="font-bold mb-4 text-yellow-500 flex items-center gap-2">
                                <i class="fa-solid fa-image"></i> Popup Ads / Promotion
                            </h2>
                            <form action="/admin/config/popup" method="post" class="space-y-4">
                                <div>
                                    <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Image URL</label>
                                    <input name="popupImage" placeholder="https://..." value={config.popupImage || ""} class="input-box bg-black border-zinc-700" />
                                </div>
                                <div>
                                    <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Message / Description (Optional)</label>
                                    <textarea name="popupMessage" placeholder="Write your promotion text here..." class="input-box bg-black border-zinc-700 h-20 text-xs">{config.popupMessage || ""}</textarea>
                                </div>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Button Text</label>
                                        <input name="popupBtnText" placeholder="e.g. Buy VIP Now" value={config.popupBtnText || "Check it out"} class="input-box bg-black border-zinc-700" />
                                    </div>
                                    <div>
                                        <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Link URL</label>
                                        <input name="popupLink" placeholder="/profile" value={config.popupLink || "/profile"} class="input-box bg-black border-zinc-700" />
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Target Audience</label>
                                    <select name="popupTarget" class="input-box bg-black border-zinc-700">
                                        <option value="all" selected={config.popupTarget !== "free"}>Show to EVERYONE (VIP + Free)</option>
                                        <option value="free" selected={config.popupTarget === "free"}>Show to FREE Users Only (Hide for VIP)</option>
                                    </select>
                                </div>
                                <label class="flex items-center gap-3 p-4 bg-black rounded-xl border border-zinc-800 cursor-pointer hover:border-zinc-600 transition">
                                    <input type="checkbox" name="showPopup" checked={config.showPopup} class="accent-yellow-500 w-5 h-5" />
                                    <span class="font-bold text-sm text-gray-300">Active Popup (Enable)</span>
                                </label>
                                <button class="btn-primary w-full shadow-lg">Save Popup Settings</button>
                            </form>
                        </div>

                        <div class="border-t border-zinc-800 pt-8">
                            <h2 class="font-bold mb-2 text-green-500 flex items-center gap-2"><i class="fa-solid fa-gift"></i> Global VIP Event</h2>
                            <p class="text-xs text-gray-500 mb-4">Give VIP access to ALL users until a specific date.</p>
                            <div class="bg-black p-4 rounded-xl border border-zinc-800 mb-4 flex justify-between items-center"><span class="text-xs font-bold text-gray-500 uppercase">Status</span> <span class="text-lg font-black text-white">{vipDate}</span></div>
                            <form action="/admin/config/vip" method="post" class="flex gap-3">
                                <input type="number" name="days" placeholder="Days" required class="input-box bg-black border-zinc-700 w-24 text-center" />
                                <button class="bg-green-600 text-white font-bold px-4 py-2 rounded-xl hover:bg-green-500 flex-grow shadow-lg shadow-green-900/20">Start Event</button>
                            </form>
                            <form action="/admin/config/vip-clear" method="post" class="mt-3 text-right">
                                <button class="text-xs text-red-500 hover:text-red-400 font-bold bg-red-500/10 px-3 py-1.5 rounded-lg border border-red-500/20 hover:bg-red-500 hover:text-white transition">End Event</button>
                            </form>
                        </div>

                        <div class="border-t border-zinc-800 pt-8">
                            <h2 class="font-bold mb-2 text-purple-500 flex items-center gap-2"><i class="fa-solid fa-database"></i> Database</h2>
                            <p class="text-xs text-gray-500 mb-4">Re-sync search index if movies are missing.</p>
                            <form action="/admin/config/reindex" method="post">
                                <button class="bg-purple-900/30 text-purple-400 border border-purple-500/30 font-bold px-6 py-3 rounded-xl hover:bg-purple-600 hover:text-white w-full transition">Re-Sync Database</button>
                            </form>
                        </div>
...
app.post("/admin/config/popup", adminGuard, async (c) => { 
    ...
    await kv.set(["config"], { 
        ...
        popupMessage: body['popupMessage'], 
        popupBtnText: body['popupBtnText'], 
        popupLink: body['popupLink'],
        popupTarget: body['popupTarget'], 
        showPopup: body['showPopup'] === 'on' 
    }); 
...
app.post("/admin/config/maintenance", adminGuard, async (c) => { 
    const body = await c.req.parseBody(); 
    const current = await getConfig(); 
    // Status က 'on' ဖြစ်နေရင် true (ပိတ်မည်), မဟုတ်ရင် false (ဖွင့်မည်)
    await kv.set(["config"], { 
        ...current, 
        maintenanceMode: body['status'] === 'on' 
    }); 
...
// 🔥 AI Model Checker (Model တွေကို စစ်ဆေးမည့် လမ်းကြောင်း)
app.get("/admin/check-ai", adminGuard, async (c) => {
    const apiKey = Deno.env.get("GEMINI_API_KEY");
    if (!apiKey) return c.json({ error: "API Key မရှိပါ" });

    try {
        // Google ဆီကို "ငါ့ Key နဲ့ ဘယ် Model တွေ သုံးလို့ရလဲ" လို့ လှမ်းမေးတာပါ
...
// 🔥 AI Description Generator (Smart Jav Admin Style)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    const { title, type, year, currentDesc } = await c.req.parseBody();
    const apiKey = Deno.env.get("GEMINI_API_KEY");

    if (!apiKey) return c.json({ error: "Server Error: GEMINI_API_KEY not set." });
    ...
    // ---------------------------------------------------------
    // အဆင့် (၁) - Normal Admin Style (ပုံမှန်ကားများအတွက်)
    // ---------------------------------------------------------
    let prompt = `
    Role: Movie Channel Admin.
    Task: Rewrite the input text into engaging Burmese (Myanmar Unicode).
    Style: Use natural, spoken Burmese (e.g., "တယ်", "မယ်", "နော်"). Avoid formal "သည်/မည်".
    Tone: Exciting, Suspenseful.
    Input: "${currentDesc || title}"
    ...
    // ---------------------------------------------------------
    // အဆင့် (၂) - Block ခံရရင် "Jav/Romance Admin Style" သုံးမည်
    // ---------------------------------------------------------
    if (!result) {
        console.log("Blocked. Switching to Soft Romance Admin Style...");
        prompt = `
        Role: Adult Romance Novelist / Channel Admin.
        Task: Rewrite the explicit plot below into a "Romantic/Erotic Drama" summary in Burmese.
        
        Rules:
        1. Euphemism: Replace explicit/dirty words with soft, romantic metaphors (e.g., instead of specific body parts, use "ရင်ခုန်စရာ အထိအတွေ့", "အချစ်နယ်ကျွံ", "သာယာမှု").
        2. Style: Maintain the "Channel Admin" storytelling style (engaging, informal).
        3. Goal: Make it sound like a hot romance novel, not a medical report.
    ...
    // ---------------------------------------------------------
    // အဆင့် (၃) - ဒါမှမရရင်တော့ ရိုးရိုးပဲ ဘာသာပြန်တော့မယ်
    // ---------------------------------------------------------
    if (!result) {
        prompt = `Translate to Burmese directly. Input: "${currentDesc || title}"`;
        result = await askGemini(prompt);
    }

    if (!result) return c.json({ error: "AI gave up. Text is too extreme." });
...
app.post("/admin/draft/delete/:id", adminGuard, async (c) => { 
    await kv.delete(["drafts", c.req.param("id")]); 
    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts"); 
});
...
// 🔥 SECURITY & PERFORMANCE: Stream JSON response for backups to avoid OOM
