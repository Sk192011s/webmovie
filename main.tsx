...
app.get("/", async (c) => {
  ...
  return c.html(
    ...
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
      ...
      {config.showPopup && config.popupImage && (
          <div id="promo-popup" class="fixed inset-0 z-[200] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
              <div class="relative w-full max-w-sm bg-[#1f1f1f] rounded-2xl overflow-hidden shadow-2xl border border-yellow-500/30 modal-enter">
                  ...
                  {/* Close Button */}
                  <button onclick="closePopup()" class="absolute top-2 right-2 z-10 bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 transition backdrop-blur-md">
              ...
              <script dangerouslySetInnerHTML={{__html: `
                  ...
                  const lastSeen = localStorage.getItem('popup_seen_date');
                  const today = new Date().toDateString();

                  // ဒီနေ့ မကြည့်ရသေးရင် ပြမယ်
                  if (lastSeen !== today) {
                      setTimeout(() => {
                          popup.classList.remove('hidden');
                          popup.classList.add('flex');
                      }, 1500); // Website တက်ပြီး ၁.၅ စက္ကန့်နေမှ ပေါ်မယ်
                  }

                  window.closePopup = function(forever) {
      ...
      {sliderMovies.length > 0 && (
          // 🔥 FIX: အပြင်ကနေ div တစ်ခုခံပြီး px-4 (ဘေးကွာ) ပေးလိုက်ပါတယ်
          <div class="px-4 mb-6">
              <div class="slider-container rounded-2xl overflow-hidden shadow-2xl border border-zinc-800 relative z-0">
                  {sliderMovies.map((m, idx) => (
                      <div class={`slide ${idx === 0 ? 'active' : ''}`}>
                          ...
                          {/* Gradient ဖယ်လိုက်ပါပြီ */}
                          <div class="absolute bottom-6 left-4 right-4">
                              {/* Featured စာသား ဖယ်လိုက်ပါပြီ */}
                              {/* စာသားကို ပိုထင်ရှားအောင် Text Shadow အမည်းလေး နည်းနည်း ခံပေးထားပါတယ် */}
                            ...
                            <form action="/admin/config/popup" method="post" class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input name="popupLink" placeholder="/profile or https://t.me/..." value={config.popupLink || "/profile"} class="input-box bg-black border-zinc-700" />
                                    </div>
                                </div>
                                <label class="flex items-center gap-3 p-4 bg-black rounded-xl border border-zinc-800 cursor-pointer hover:border-zinc-600 transition">
                                    ...
                                    <span class="font-bold text-sm text-gray-300">Active Popup (Show to Users)</span>
                                </label>
...
// ၂။ 👇👇👇 ဒီကောင်ကို သူ့အောက်မှာ ထပ်ထည့်ပါ (Popup အတွက်) 👇👇👇
app.post("/admin/config/popup", adminGuard, async (c) => { 
    const body = await c.req.parseBody(); 
    const current = await getConfig(); 
    
    // Config ထဲမှာ Popup Data တွေ ပေါင်းသိမ်းမယ်
    await kv.set(["config"], { 
        ...current, 
