        ...
        async function loadMoreMovies(category) { 
            ...
            try { 
                ...
                data.movies.forEach(m => { 
                    ...
                    // 🔥 Badge စာသား သတ်မှတ်ခြင်း (4K လား HD လား)
                    ...
                    if(category === "All Uncensored" || category === "Myanmar and Asian") { 
                        ...
                        el.innerHTML = `
                            <div class="relative aspect-video overflow-hidden">
                                <img src="${m.coverUrl || m.posterUrl}" loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                                <div class="absolute top-2 right-2 bg-yellow-500 text-black text-[9px] font-bold px-1.5 py-0.5 rounded shadow z-10">${badge}</div>
                            </div>
                            <div class="p-3"><h3 class="text-sm font-bold truncate text-white">${m.title}</h3></div>
                        `;
                    } else { 
                        // Poster ပုံစံ (Normal)
                        ...
                        el.innerHTML = `
                            <div class="relative aspect-[2/3] overflow-hidden">
                                <img src="${m.posterUrl}" loading="lazy" decoding="async" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                                <div class="absolute top-1 right-1 bg-yellow-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded shadow z-10">${badge}</div>
                            </div>
    ...
    {movies.map(m => (
        <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'mb-4' : ''}`}>
            ...
            <div class={`relative overflow-hidden ${isUncensored ? "aspect-video" : "aspect-[2/3]"}`}>
                ...
                {/* 🔥 HD/4K BADGE (အကုန်လုံးအတွက် ပေါ်မည်) */}
