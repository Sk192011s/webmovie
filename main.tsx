...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    // 🔥 Search အတွက် Query ရယူခြင်း (ဒါမရှိရင် Search မရပါ)
    ...
    // 🔥 Search Logic (Database တစ်ခုလုံးရှာမည်)
    if (adminQuery) {
        movies = await searchMoviesDB(adminQuery); 
    } else {
        // Search မလုပ်ထားရင် ပုံမှန်အတိုင်း Latest ကားတွေပြမယ်
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-black min-h-screen font-sans text-sm">
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-lg border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title" value={editMovie?.title} class="input-box w-full bg-black border-zinc-700 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">{["Movies","Series","4K Movies","Animation","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-24 text-center flex-shrink-0 bg-black border-zinc-700" /></div></div>
                ...
                {/* Tab: Drafts (Simplified for length) */}
                <div id="tab-drafts" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl min-h-[50vh]">
                        ...
                        {drafts.length > 0 && <form action="/admin/drafts/publish-all" method="post" onsubmit="return confirm('Publish ALL?')"><button class="bg-green-600 text-white px-4 py-2 rounded mb-4 text-xs font-bold">Publish All</button></form>}
                ...
                <div id="tab-keys" class="tab-content">
                    <div class="max-w-2xl mx-auto space-y-4">
                        <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
                            ...
                            <form action="/admin/key/create" method="post" class="flex gap-3"><input type="hidden" name="type" value="vip" /><input type="number" name="days" placeholder="Days" required class="input-box flex-grow bg-black border-zinc-700" /><button class="btn-primary w-32">Gen VIP</button></form>
                        </div>
                        <div class="bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
                            <h2 class="font-bold mb-4 text-gray-300">Active Keys</h2>
                            <div class="space-y-3 max-h-[500px] overflow-y-auto custom-scroll">{keys.map(k => (<div class="flex justify-between items-center p-3 bg-black rounded-xl border border-zinc-800/50"><span class="font-mono font-bold text-yellow-500">{k.code} ({k.days} Days)</span><form action={`/admin/key/delete/${k.code}`} method="post"><button class="text-xs text-red-500"><i class="fa-solid fa-trash"></i></button></form></div>))}</div>
                        </div>
                    </div>
                </div>

                {/* Tab: Topups, Users, Requests, Logs, Config (Remaining Tabs) */}
                <div id="tab-topups" class="tab-content"><div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800"><h2 class="font-bold text-blue-400 mb-4">Topups</h2><div class="space-y-4">{topups.map(t => (<div class="p-4 rounded-xl border bg-black border-zinc-800"><div class="flex justify-between"><div><span class="font-bold text-white">{t.amount} Ks</span> <span class="text-xs text-gray-500">{t.method}</span></div>{t.status==='pending' && <div class="flex gap-2"><form action="/admin/topup/approve" method="post"><input type="hidden" name="id" value={t.id} /><button class="text-green-500 text-xs font-bold">Approve</button></form><form action="/admin/topup/reject" method="post"><input type="hidden" name="id" value={t.id} /><button class="text-red-500 text-xs font-bold">Reject</button></form></div>}</div><p class="text-xs text-gray-500">{t.username} | {t.transactionId}</p></div>))}</div></div></div>
                <div id="tab-users" class="tab-content"><div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800"><h2 class="font-bold mb-4 text-white">Users ({totalUsers})</h2><div class="overflow-y-auto h-[500px] custom-scroll space-y-2">{userList.map(u => (<div class="flex justify-between bg-black p-3 rounded border border-zinc-800"><span class={u.isBanned?"text-red-500":"text-gray-300"}>{u.username}</span><form action="/admin/user/toggle-ban" method="post"><input type="hidden" name="username" value={u.username} /><button class="text-xs border px-2 py-1 rounded">{u.isBanned?"Unban":"Ban"}</button></form></div>))}</div></div></div>
                <div id="tab-requests" class="tab-content"><div class="max-w-3xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800"><h2 class="font-bold mb-4 text-pink-500">Requests</h2><div class="space-y-2">{requests.map(r => (<div class="bg-black p-3 rounded flex justify-between"><span>{r.movieName}</span><form action={`/admin/request/delete/${r.id}`} method="post"><button class="text-red-500"><i class="fa-solid fa-check"></i></button></form></div>))}</div></div></div>
                <div id="tab-logs" class="tab-content"><div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800"><h2 class="font-bold mb-4 text-gray-300">Logs</h2><div class="space-y-1 h-[500px] overflow-y-auto bg-black p-3 rounded custom-scroll">{logs.map(l => (<div class="text-xs font-mono text-gray-400 border-b border-zinc-900 pb-1">[{new Date(l.timestamp).toLocaleString()}] {l.action} - {l.details}</div>))}</div></div></div>
                <div id="tab-config" class="tab-content"><div class="max-w-xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 space-y-6"><h2 class="font-bold text-white">Config</h2><form action="/admin/config/maintenance" method="post"><button class={`w-full py-3 rounded font-bold ${config.maintenanceMode?'bg-red-600 text-white':'bg-zinc-800 text-gray-400'}`}>{config.maintenanceMode?"🔴 Maintenance ON":"⚪️ Maintenance OFF"}</button></form><form action="/admin/config" method="post" class="space-y-2"><input name="text" value={config.announcement} class="input-box bg-black" /><label class="flex items-center gap-2"><input type="checkbox" name="show" checked={config.showAnnouncement} /> Show Announcement</label><button class="btn-primary w-full">Save</button></form><form action="/admin/config/reindex" method="post"><button class="w-full py-3 bg-purple-900/50 text-purple-400 border border-purple-500/30 rounded font-bold">Re-Sync Database</button></form></div></div>
