                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                {/* 👇👇👇 AUTO GENERATOR & BULK EXTRACTOR (UPDATED) 👇👇👇 */}
                                <div class="bg-zinc-800 p-3 rounded-xl mb-2 border border-zinc-700">
                                    ...
                                    {/* TOOL 2: BULK EXTRACTOR (NEW 🔥) */}
                                    <div class="border-t border-zinc-700 pt-2">
                                        <p class="text-[9px] text-gray-400 mb-1">Tool 2: Bulk Extractor (Paste messy text here)</p>
                                        <textarea id="bulk-text" placeholder="Paste any text with links here. We will extract only links..." class="input-box text-[10px] bg-black border-zinc-600 h-16 w-full mb-2"></textarea>
                                    ...
                                    <script dangerouslySetInnerHTML={{__html: `
                                        ...
                                        function extractLinks() {
                                            ...
                                            // Link တွေကို Regex နဲ့ ရှာမယ် (http သို့ https နဲ့စတာမှန်သမျှ ယူမယ်)
                                            ...
                                            foundLinks.forEach(url => {
                                                result += \`Season \${season} | Ep.\${epCount} | \${url}\\n\`;
                                                epCount++;
                                            });

                                            textarea.value = result.trim();
                                            document.getElementById('bulk-text').value = ''; // ပြီးရင် Box ကို ရှင်းပေးမယ်
                                            alert(\`Found & Added \${foundLinks.length} links!\`);
                                        }

                                        function clearEpisodes() { 
                                            if(confirm('Clear all episodes from list?')) {
