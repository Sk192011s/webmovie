...
// PROFILE UI (MODERN STREAMING APP STYLE)
app.get("/profile", async (c) => { 
    ...
    // Menu Items Data
    const menuItems = [
        { icon: "fa-solid fa-clock-rotate-left", label: "Watch History", link: "#", color: "text-blue-500" },
        { icon: "fa-solid fa-heart", label: "My Saved List", link: "/favorites", color: "text-red-500", badge: user.favorites?.length || 0 },
        { icon: "fa-solid fa-download", label: "Downloads", link: "#", color: "text-green-500" },
    ];

    const settingsItems = [
        { icon: "fa-solid fa-key", label: "Change Password", link: "#", color: "text-gray-400" },
        { icon: "fa-solid fa-language", label: "Language", link: lang === 'en' ? '/lang/my' : '/lang/en', color: "text-gray-400", value: lang === 'en' ? 'ENG' : 'MYN' },
    ];

    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="pb-24 bg-black min-h-screen">
            
            {/* 1. HEADER PROFILE (Clean & Centered) */}
            <div class="bg-gradient-to-b from-[#1a1a1a] to-black pt-10 pb-6 px-4 text-center border-b border-zinc-900">
                <div class="relative inline-block mb-4">
                    <div class="w-24 h-24 bg-zinc-800 rounded-full flex items-center justify-center text-4xl font-bold text-gray-500 overflow-hidden border-4 border-[#1f1f1f] shadow-2xl">
                        {/* Avatar Image (Cartoon style or Initials) */}
                        <img src={`https://ui-avatars.com/api/?name=${user.username}&background=Eab308&color=000&bold=true`} class="w-full h-full object-cover" />
                    </div>
                    {premium && <div class="absolute bottom-0 right-0 bg-yellow-500 text-black text-[10px] font-black px-2 py-0.5 rounded-full border-2 border-black">VIP</div>}
                </div>
                <h1 class="text-2xl font-bold text-white mb-1">{user.username}</h1>
                <p class="text-xs text-gray-500 font-mono tracking-widest mb-4">{memberId}</p>

                {/* Quick Stats Row */}
                <div class="flex justify-center gap-4 text-xs">
                    <div class="bg-[#1f1f1f] px-4 py-2 rounded-lg border border-zinc-800">
                        <span class="block text-yellow-500 font-bold text-lg">{premium ? finalDays : 0}</span>
                        <span class="text-gray-500">Days Left</span>
                    </div>
                    <div class="bg-[#1f1f1f] px-4 py-2 rounded-lg border border-zinc-800">
                         <span class="block text-white font-bold text-lg">{user.favorites?.length || 0}</span>
            ...
            <div class="px-4 mt-6 space-y-6">
                ...
                {/* 2. VIP BANNER (If not VIP) */}
                {!premium && (
                    <div class="bg-gradient-to-r from-yellow-600 to-yellow-800 rounded-xl p-4 flex justify-between items-center shadow-lg">
                        <div class="text-white">
                            <h3 class="font-bold text-sm">Upgrade to VIP</h3>
                            <p class="text-[10px] opacity-90">Unlock all movies & features</p>
                        </div>
                        <a href="https://t.me/LuGyiandYoteshinMovies" target="_blank" class="bg-black text-white text-xs font-bold px-4 py-2 rounded-lg">Buy Now</a>
                    </div>
                )}

                {/* 3. REDEEM INPUT (Simple) */}
                <form action="/profile/redeem" method="post" class="flex gap-2">
                    <input name="key" placeholder="Enter Voucher Code" class="bg-[#1f1f1f] border border-zinc-800 text-white text-sm px-4 py-3 rounded-xl w-full outline-none focus:border-yellow-500" />
                    <button class="bg-zinc-800 text-yellow-500 font-bold px-4 rounded-xl border border-zinc-700 hover:bg-zinc-700"><i class="fa-solid fa-check"></i></button>
                </form>

                {/* 4. MENU LIST (Standard App Style) */}
                <div>
                    <h3 class="text-gray-500 text-xs font-bold uppercase mb-3 ml-2">My Library</h3>
                    <div class="bg-[#1f1f1f] rounded-xl overflow-hidden border border-zinc-800">
                        {menuItems.map((item, idx) => (
                            <a href={item.link} class={`flex items-center justify-between p-4 hover:bg-[#2a2a2a] transition ${idx !== menuItems.length -1 ? 'border-b border-zinc-800' : ''}`}>
                                <div class="flex items-center gap-4">
                                    <i class={`${item.icon} ${item.color} w-5 text-center`}></i>
                                    <span class="text-sm text-gray-200 font-medium">{item.label}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    {item.badge > 0 && <span class="text-[10px] bg-red-600 text-white px-1.5 rounded-full font-bold">{item.badge}</span>}
                ...
                {/* 5. SETTINGS LIST */}
                <div>
                    <h3 class="text-gray-500 text-xs font-bold uppercase mb-3 ml-2">Settings</h3>
                    <div class="bg-[#1f1f1f] rounded-xl overflow-hidden border border-zinc-800">
                        {settingsItems.map((item, idx) => (
                            <a href={item.link} class={`flex items-center justify-between p-4 hover:bg-[#2a2a2a] transition ${idx !== settingsItems.length -1 ? 'border-b border-zinc-800' : ''}`}>
                                <div class="flex items-center gap-4">
                                    <i class={`${item.icon} ${item.color} w-5 text-center`}></i>
                                    <span class="text-sm text-gray-200 font-medium">{item.label}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    {item.value && <span class="text-xs text-gray-500 font-bold">{item.value}</span>}
                                    <i class="fa-solid fa-chevron-right text-gray-600 text-xs"></i>
                                </div>
                            </a>
                        ))}
                        <a href="/logout" class="flex items-center justify-between p-4 hover:bg-[#2a2a2a] transition border-t border-zinc-800">
                            <div class="flex items-center gap-4">
                                <i class="fa-solid fa-power-off text-red-500 w-5 text-center"></i>
                                <span class="text-sm text-red-500 font-bold">Log Out</span>
                            </div>
                        </a>
                    </div>
                </div>
                ...
                <div class="text-center pb-6">
                    <p class="text-[10px] text-gray-700">Gold Flix v2.5.0</p>
                </div>
