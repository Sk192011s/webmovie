...
// ... (အပေါ်မှာ hashPassword ရှိမယ်) ...

// 🔥 1. TEXT CLEANER (Description ထဲက အမှိုက်စာလုံးများ ရှင်းထုတ်ရန်)
...
// 🔥 2. REVISED TOKENIZER (Search အတွက် MIDA-428 ရှာလို့ရမယ့်ကောင်)
function tokenize(text: string): string[] {
    return text.toLowerCase()
        .replace(/[-_.:]+/g, " ") // Hyphen (-) တွေ့ရင် Space ခြားပေးမယ် (ဒါပါမှ ရှာရမှာ)
        .replace(/[^\p{L}\p{N}\s]/gu, "") // ကျန်တဲ့ သင်္ကေတတွေ ဖျက်မယ်
        .split(/\s+/) 
        .filter(w => w.length > 1);
}

// ... (အောက်မှာ checkLoginRateLimit ရှိမယ်) ...
...
async function searchMoviesDB(query: string) {
    ...
    // 🔥 OPTIMIZATION: 10 ခုစီ အသုတ်လိုက် ခေါ်မည် (Batch Fetching)
...
function isPremium(user: User | null, config: AppConfig) {
  // 🔥 FIX 1: အကောင့်မရှိရင် လုံးဝကြည့်မရအောင် ပိတ်လိုက်သည်
  if (!user) return false;

  const now = Date.now();
  // 🔥 FIX 2: Global Bonus ရှိရင် အကောင့်ရှိသူတိုင်း ကြည့်ခွင့်ရမယ်
  if (config.globalVipExpiry && config.globalVipExpiry > now) return true;
  
  // Bonus မရှိရင် ကိုယ့်သက်တမ်းနဲ့ကိုယ် ကြည့်ရမယ်
...
// 🔥 FIX: globalExpiry?: number ထပ်ဖြည့်ထားသည်
...
`;
  ...
  // 🔥 FIX: ရက်တွက်တဲ့ Logic အသစ် (Bonus နဲ့ Personal ဘယ်ဟာများလဲ ယှဉ်ယူမည်)
  let daysLeft = 0;
  const now = Date.now();
  
  // ၁. ကိုယ်ပိုင်ရက် တွက်ခြင်း
  ...
  // ၂. Global Bonus ရက် တွက်ခြင်း (Bonus က ပိုများရင် အဲ့ဒါကို ယူမယ်)
  ...
  <html lang={l}>
    ...
    // Loader ကို မခေါ်တော့ဘူး

    if(container) container.innerHTML = ''; 
    if(cover) cover.style.display = 'none';
    if(container) container.style.display = 'block'; 
    // Loader ပြခိုင်းတဲ့ code ကို ဖြုတ်လိုက်ပြီ
...
// 🔥 SECURITY: Bot Blocker & Rate Limiter
app.use("*", async (c, next) => {
    const ip = getClientIp(c);
    const userAgent = c.req.header("user-agent") || "";

    // 1. Bot တွေကို အရင်စစ်မယ် (နာမည်ပါရင် ကန်ထုတ်မယ်)
    if (userAgent.match(/curl|wget|python|java|libwww|httpclient|axios/i)) {
        return c.text("Access Denied (Bot Detected)", 403);
    }

    // 2. လူစစ်မယ် (တစ်မိနစ်ကို အကြိမ် ၁၀၀ ထက်ပိုခေါ်ရင် ပိတ်မယ်)
    // Deno KV ကိုသုံးပြီး မှတ်ထားမယ်
    ...
    if (currentCount > 100) {
        return c.text("Too Many Requests! Please wait a minute.", 429);
    }

    // Count တိုးမယ် (ExpireIn 1 min)
...
app.get("/api/list", async (c) => { 
    ...
    // 🔥 Fetch only needed amount + buffer (not 300)
    // Page 1 = Fetch 20, Page 2 = Fetch 40... (This is still offset based but better)
    // Best practice is using cursors but for simple implementation:
...
<div class="bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-xl mb-6 flex items-start gap-3 animate-pulse">
    ...
    <p class="text-xs text-gray-300 leading-relaxed">
        Movieများ ကြည့်မရ ဒေါင်းမရလျှင် <span class="text-yellow-500 font-bold border-b border-yellow-500 border-dotted">VPN</span> (1.1.1.1 သို့မဟုတ် အခြား VPN တစ်ခုခု) ခံပြီး ပြန်စမ်းကြည့်ပေးပါ။
    </p>
</div>
      ...
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-5 pb-20">
            ...
            <div>
                ...
                <div class="space-y-3">{plans.map(p => (<div class={`relative bg-[#1f1f1f] p-4 rounded-lg border flex flex-col justify-between transition active:scale-95 ${p.popular ? 'border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'border-zinc-800'}`}>{p.popular && <div class="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-yellow-500 text-black text-[9px] font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-widest">Best Value</div>}<div class="text-center mb-3"><h4 class="text-gray-400 font-bold text-xs uppercase mb-1">{p.name}</h4><span class="text-white font-black text-xl tracking-tight">{p.price}</span></div><div class="space-y-1.5 mb-4 px-2">{p.features.slice(0,2).map(f => (<div class="flex items-center gap-1.5 text-[10px] text-gray-500 justify-center"><i class="fa-solid fa-check text-green-500"></i> {f}</div>))}</div><a href="https://t.me/LuGyiandYoteshinMovies" target="_blank" class={`block text-center py-2.5 rounded-xl font-bold text-xs transition ${p.popular ? 'bg-yellow-500 text-black hover:bg-yellow-400' : 'bg-zinc-800 text-white hover:bg-zinc-700'}`}>Buy Now</a></div>))}</div>
            </div>
...
// 🔥 CHANGE PASSWORD ROUTES (ဒီနေရာမှာ စထည့်ပါ)
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    const limit = 50;
    const cursor = c.req.query("cursor"); // URL ကနေ cursor ယူမယ်

    let movies = [];
    let nextCursor = ""; // Next page အတွက် ပြင်ဆင်ခြင်း

    if (adminQuery) {
        movies = await searchMoviesDB(adminQuery);
    } else {
        // Cursor ထည့်ပေးလိုက်တယ်
        ...
        // Data 50 ခုပြည့်ရင် နောက်တစ်မျက်နှာအတွက် cursor မှတ်မယ်
    ...
    // Season ပါရင် Season ပါ ထည့်ပြမယ်
...
{/* 🔥 GIFT BONUS DAYS TO ALL USERS (NEW CODE) */}
...
app.post("/admin/movie/save", adminGuard, async (c) => {
    ...
    // 🔥 FIX: Save Type ကို စစ်ဆေးခြင်း
    const saveType = String(body.saveType); // "draft" or "publish"

    if (saveType === "draft") {
        // Draft ဖြစ်ရင် Drafts ထဲပဲ ထည့်မယ် (Live Database ကို မထိဘူး)
        await kv.set(["drafts", movie.id], movie);
        
        // အကယ်၍ Live ထဲမှာ ရှိပြီးသားကို Draft ပြန်ပြောင်းတာဆိုရင် Live ကနေ ဖျက်ရမယ်
        // (ဒါပေမဲ့ ပုံမှန်ကတော့ Draft သိမ်းရင် Draft ထဲပဲ သွားတာ များတယ်)
        
        return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts&success=Saved to Drafts");
    } else {
        // Publish ဆိုမှ Live Database ထဲထည့်မယ်
        await saveMovieDB(movie);
        
        // Publish လုပ်လိုက်ရင် Draft ထဲမှာ ရှိနေရင် ဖျက်လိုက်မယ် (မလိုတော့လို့)
        await kv.delete(["drafts", movie.id]);
        
        return c.redirect(ADMIN_ROUTE + "/dashboard?tab=movies&success=Published Successfully");
    }
});
// 🔥 1. PUBLISH ALL DRAFTS (Draft တွေအကုန် Live တင်မယ့် ခလုတ်အတွက်)
app.post("/admin/drafts/publish-all", adminGuard, async (c) => {
    ...
    for await (const res of iter) {
        ...
        await kv.delete(res.key);     // Draft ထဲကဖျက်
        count++;
    }
    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=movies&success=Published " + count + " Drafts");
});

// 🔥 2. PUBLISH SINGLE DRAFT (Draft တစ်ခုချင်းစီ တင်မယ့် ခလုတ်အတွက်)
app.post("/admin/draft/publish/:id", adminGuard, async (c) => {
    ...
    await saveMovieDB(res.value); // Live Database ထဲထည့်
    await kv.delete(["drafts", id]); // Draft ထဲကဖျက်

    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=movies&success=Draft Published Successfully");
});

// 🔥 3. DELETE DRAFT (Draft ဖျက်တဲ့ ခလုတ်အတွက် - ရှိပြီးသားဆို မထည့်ပါနဲ့)
...
// 🔥 KEY GENERATOR ROUTE (VIP & COINS)
app.post("/admin/key/create", adminGuard, async (c) => {
    const body = await c.req.parseBody();
    
    // Key အချက်အလက်တွေ တည်ဆောက်မယ်
    const keyData: VipKey = {
        code: crypto.randomUUID().slice(0, 8).toUpperCase(), // 8 လုံးပါ Code
        days: parseInt(String(body.days || "0")),            // VIP ရက် (VIP ဖြစ်မှထည့်မယ်)
        value: parseInt(String(body.value || "0")),          // Coin ပမာဏ (Coin ဖြစ်မှထည့်မယ်)
        type: String(body.type) as "vip" | "coin"            // အမျိုးအစား
    };
...
// 🔥 DELETE KEY ROUTE (Key ဖျက်ချင်ရင် သုံးဖို့)
...
// 🔥 USER MANAGEMENT ROUTES (Missing Routes)

// 1. User Password Reset (Admin ကနေ အတင်းပြောင်းမယ်)
app.post("/admin/user/reset", adminGuard, async (c) => {
    ...
    // Password အသစ်ကို Hash လုပ်ပြီး သိမ်းမယ်
    user.passwordHash = await hashPassword(String(newpass));
    await kv.set(["users", user.username], user);
...
// 2. Manual VIP Add (Admin ကနေ ရက်တိုးပေးမယ်)
...
// 3. Ban / Unban User (User ကို ပိတ်မယ်/ဖွင့်မယ်)
app.post("/admin/user/toggle-ban", adminGuard, async (c) => {
    ...
    if(user) {
        ...
        user.sessionId = ""; // Ban လိုက်ရင် Auto Logout ဖြစ်သွားမယ်
        await kv.set(["users", user.username], user);
    }
    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=users");
});
// 🔥 GIFT DAYS TO ALL USERS (Bulk Update)
app.post("/admin/user/give-all", adminGuard, async (c) => {
    ...
    // User အားလုံးကို Loop ပတ်ပြီး ရက်လိုက်တိုးမည်
    ...
    for await (const res of iter) {
        ...
        // သက်တမ်းကုန်နေရင် (သို့) Free ဖြစ်နေရင် လက်ရှိအချိန်ကနေ စပေါင်းမယ်
        if (currentExpiry < now) {
            currentExpiry = now;
        }
        
        // ရှိပြီးသားရက်အပေါ် ထပ်ပေါင်းမယ်
...
// 🔥 FIXED BACKUP (Include ALL Data Explicitly)
app.get("/admin/backup", adminGuard, async (c) => { 
    const stream = new ReadableStream({ 
        async start(controller) { 
            const encoder = new TextEncoder(); 
            // 🔥 ဒီနေရာမှာ movies, users အကုန်ထည့်ပေးလိုက်ပြီ
            const prefixes = [
                ["config"],
                ["movies"], 
                ["users"], 
            ...
            for (const prefix of prefixes) {
                // Prefix တစ်ခုချင်းစီ လိုက်ဆွဲမယ်
...
app.post("/admin/restore", adminGuard, async (c) => { 
    try { 
        ...
        if (file instanceof File) { 
            ...
            // တစ်ကြောင်းချင်းစီ ပြန်ခွဲပြီး Database ထဲထည့်မယ်
            const lines = text.split("\n");
            let count = 0;

            for (const line of lines) {
                if (!line.trim()) continue; // လွတ်နေတဲ့ စာကြောင်းကျော်မယ်
                try {
...
// 🔥 Fix Category Counts Route
app.get("/admin/fix-counts-now", adminGuard, async (c) => {
    // 1. Reset all counts to 0 first (Safeguard)
    const cats = ["Movies","Series","4K Movies","Animation","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
