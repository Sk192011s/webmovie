...
app.get("/api/mediafire-stream", async (c) => {
    ...
    // Cache မမှတ်ရန် (အမြဲတမ်း Link အသစ်ရရန်)
    c.header("Cache-Control", "no-cache, no-store, must-revalidate");

    try {
        let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();

        // 🔥 FAST TRACK 1: Premium (သို့) Direct Link ဖြစ်နေရင် စစ်ဆေးစရာမလို၊ တန်းလွှတ်မယ်
        // (ဒါဆိုရင် စောင့်စရာမလိုဘဲ ချက်ချင်း Video တက်လာမယ်)
        if (cleanUrl.includes("file_premium") || cleanUrl.endsWith(".mp4") || cleanUrl.endsWith(".mkv")) {
            const finalLink = "https://proxy.avotc.tk/" + cleanUrl;
            return c.redirect(finalLink, 302);
        }

        // 🔥 STANDARD TRACK: Webpage ဖတ်ပြီး Link ရှာမယ်
        // Mobile User Agent သုံးမယ် (စာမျက်နှာ ပိုပေါ့ပြီး ပိုမြန်အောင်)
        ...
        if (match && match[1]) {
            ...
            // ရှာမတွေ့ရင် Proxy Page ကိုပဲ ပို့မယ် (Fail Safe)
            return c.redirect(proxyFetchUrl, 302);
        }

    } catch (e) {
        console.error("Fast API Error:", e);
        // Error တက်ရင် Proxy Link အတိုင်းပြန်ပို့မယ်
