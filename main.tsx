...
// 🔥 MIGRATION: Animation ကားတွေကို Series နဲ့ Movies ထဲ ခွဲထည့်ပေးမည့် ကုဒ်
app.get("/admin/fix-animation-category", adminGuard, async (c) => {
    const iter = kv.list({ prefix: ["movies"] });
    let toSeries = 0;
    let toMovies = 0;
    ...
    for await (const res of iter) {
        const m = res.value;
        // Animation Category ဖြစ်မှ အလုပ်လုပ်မယ်
        if (m && !m.isDeleted && m.category === "Animation") {
