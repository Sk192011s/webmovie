...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    try {
        ...
        // Data များ ရယူခြင်း (Duration ပါ ထည့်ယူထားသည်)
        let category = body["category"] as string;
        let title = body["title"] as string;
        let description = body["description"] as string;
        let duration = body["duration"] as string; // 🔥 Duration ကို ဒီမှာ ကြေငြာမှ အောက်မှာသုံးလို့ရမယ်
        let price = parseInt(String(body["price"] || "0"));

        // 🔥 4K Porns Auto System (Title, Price, Description)
        if (category === "4K Porns" && body["mode"] === "create") {
            
            // ၁. ဈေးနှုန်း
            price = 50; 

            // ၂. Description Auto Fill
            ...
            // ၃. Title Auto Increment (Drafts + Live)
            if (!title) {
                let maxNum = 0;
                // Live Check
        ...
        // 🔥 Saving Movie Object (duration ပါ ထည့်သွင်းသိမ်းဆည်းခြင်း)
        const movie = { 
            ...body, 
            title: title, 
            ...
            episodes, 
            price: price,
            duration: duration // 🔥 ဒီနေရာမှာ duration ထည့်ရပါမယ်
        }; 
