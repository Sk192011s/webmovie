                     ...
                     searchTimeout = setTimeout(async () => {
                         try {
                             ...
                             if(data.results.length === 0) {
                                 ...
                                 // 🔥 FIX: "padding-bottom: 150%" သုံးလိုက်တာပါ
                                 // ဒါက ဘယ်ဖုန်း၊ ဘယ် Browser မဆို 2:3 အချိုးကို အတင်းလုပ်ယူပေးလို့ ပုံမထပ်တော့ပါဘူး
                                 resultsContainer.innerHTML = data.results.map(m => \`
                                     <a href="/movie/\${m.id}" class="block rounded-xl overflow-hidden bg-zinc-800 relative group shadow-lg" style="width: 100%; padding-bottom: 150%; height: 0; position: relative;">
                                         <img src="\${m.posterUrl}" class="absolute inset-0 w-full h-full object-cover transition duration-300 group-hover:scale-110" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" />
        ...
        <div class="max-w-4xl mx-auto">
           ...
           <div class="p-6">
               ...
               <div class="flex flex-wrap items-center gap-2 mb-6">
                   {}
{movie.year && <span class="bg-zinc-800 text-gray-300 text-xs px-3 py-1 rounded-full border border-zinc-700">{movie.year}</span>}
...
app.get("/profile", async (c) => { 
    ...
    const plans = [
        { name: "1 Month", price: "1,000 Ks", days: 30, features: ["Watch All (Except 4K Porns)", "Direct Download"] },
        { name: "3 Month", price: "2,000 Ks", days: 90, popular: true, features: ["Watch All (Except 4K Porns)", "Direct Download"] },
        { name: "5 Month", price: "3,500 Ks", days: 150, features: ["Watch All (Except 4K Porns)", "Direct Download"] },
        { name: "1Year", price: "7,000 Ks", days: 365, features: ["Watch All (Except 4K Porns)", "Direct Download"] }
    ]; 
    return c.html(
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    if (editId) {
        ...
        if (!editMovie) {
            editMovie = await getMovie(editId);
        }
    } 
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-lg border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title" value={editMovie?.title} class="input-box w-full bg-black border-zinc-700 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">{["Movies","Series","4K Movies","USA","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select>{}
<input name="year" placeholder="Year" value={editMovie?.year || ""} class="input-box w-24 text-center flex-shrink-0 bg-black border-zinc-700" /></div></div>
    ...
    try {
        ...
        if (inputData.startsWith("http://") || inputData.startsWith("https://")) {
            try {
                ...
                return c.redirect(ADMIN_ROUTE + "/dashboard?error=JSON Error: " + err.message);
            }
        } else {
            let rawText = inputData.replace(/[\r\n]+/g, ' ').replace(/,\s*([\]}])/g, '$1');
            if (rawText.startsWith('{') && !rawText.startsWith('[')) rawText = `[${rawText}]`;
            try { movies = JSON.parse(rawText); } catch(e) { return c.redirect(ADMIN_ROUTE + "/dashboard?error=Invalid JSON Text"); }
        }
        if (!Array.isArray(movies)) throw new Error("Format must be array");
        if (movies.length > 0) {
            const first = movies[0];
            const isJav = first.name || first.photo1 || first.dapp; 
            const isUSA = first.profileImg && first.files; 
            if (!isJav && !isUSA) movies.reverse();
        }
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                let title = "", poster = "", cover = "", desc = "", duration = "", category = "";
                let allLinks = [];
                if (m.name || m.photo1 || m.dapp) { 
                    title = m.name || ""; poster = m.photo1 || ""; cover = m.photo2 || m.photo1 || ""; desc = m.info || ""; category = "Jav";
                    if (!/[a-zA-Z]{2,}-[0-9]{3,}/.test(title)) category = "USA";
                    if (m.dapp) allLinks.push(m.dapp); if (m.download) allLinks.push(m.download);
                } else if (m.profileImg && m.files) { 
                    title = m.title || ""; poster = m.profileImg || ""; cover = m.coverImg || m.profileImg || ""; desc = m.content || ""; category = "USA";
                    try {
                        const f = typeof m.files === 'string' ? JSON.parse(m.files) : m.files;
                        if (f.stream) allLinks.push(...f.stream.map(s => s.url));
                        if (f.download) allLinks.push(...f.download.map(d => d.url));
                    } catch(e) {}
                } else if (m.title) { 
                    title = m.title || ""; poster = m.image || ""; cover = m.image || ""; desc = m.info || ""; duration = m.time || ""; category = "Myanmar and Asian";
                ...
                let target = allLinks.find(u => u.includes("proxy.avotc.tk"));
                if (!target) target = allLinks.find(u => u.includes("r2") || u.includes("pub") || u.includes("cloudflare") || u.includes("deno.dev") || u.includes("vercel.app"));
                if (!target) target = allLinks.find(u => u.includes("mediafire.com"));
                if (!target) return;
                const id = await createUniqueId(target);
                if (target.includes("mediafire.com") && !target.includes("proxy.avotc.tk")) {
                    if (target.includes("premium")) {
                        server1 = addProxy(target);
                        dl1 = addProxy(target);
                    } else {
                        server1 = addProxy(target); 
                        dl1 = "/api/mediafire-stream?link=" + encodeURIComponent(target);
                        server1 = "/api/mediafire-stream?link=" + encodeURIComponent(target); 
                    }
                } else {
                    server1 = addProxy(target);
                    dl1 = addProxy(target);
                ...
                const movieData: Movie = {
                    id, title, posterUrl: poster, coverUrl: cover, category: category as any, description: desc, tags: title,
                    year: new Date().getFullYear().toString(), duration,
                    streamUrl: server1, streamUrl2: server2, downloadUrl: dl1, downloadUrl2: dl2,
                    linkType: "direct", createdAt: baseTime + i + idx, isDeleted: false
                };
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies");
    } catch (e) {
        return c.redirect(ADMIN_ROUTE + "/dashboard?error=" + e.message);
    }
});
app.post("/admin/movie/save", adminGuard, async (c) => {
    const body = await c.req.parseBody();
    let title = cleanText(String(body.title));
    let price = Number(body.price) || 0;
    const category = String(body.category);
    if (category === "4K Porns" && price === 0) {
        price = 50;
    }
    if (category === "4K Porns" && !title) {
        const countRes = await kv.get(["counts", "4K Porns"]);
        const nextNum = (Number(countRes.value) || 0) + 1;
        title = `4K Porn #${nextNum}`;
    }
    const newCreatedAt = body.updateTime === 'on' ? Date.now() : (Number(body.createdAt) || Date.now());
    const movie: Movie = {
...
// 🔥 DRAFTS PUBLISH ALL ROUTE (Fix 404)
app.post("/admin/drafts/publish-all", adminGuard, async (c) => {
    const iter = kv.list({ prefix: ["drafts"] });
    let count = 0;
    
    // Draft တစ်ခုချင်းစီကို Live ထဲပြောင်းထည့်မယ်
    for await (const res of iter) {
        await saveMovieDB(res.value); // Live Database ထဲထည့်
        await kv.delete(res.key);     // Draft ထဲက ဖျက်
        count++;
    }
    
    // ပြီးရင် Dashboard ပြန်ပို့မယ်
    return c.redirect(ADMIN_ROUTE + "/dashboard?tab=movies&success=Published " + count + " Drafts");
});
app.get("/admin/danger/nuke-all-movies", adminGuard, async (c) => {
    // 1. Movies data အားလုံး ဖျက်မယ်
    const iterMovies = kv.list({ prefix: ["movies"] });
    let movieCount = 0;
    for await (const res of iterMovies) {
        await kv.delete(res.key);
        movieCount++;
    }

    // 2. Time index အားလုံး ဖျက်မယ်
    const iterTime = kv.list({ prefix: ["idx_time"] });
    for await (const res of iterTime) await kv.delete(res.key);

    // 3. Category index အားလုံး ဖျက်မယ်
    const iterCat = kv.list({ prefix: ["idx_cat"] });
    for await (const res of iterCat) await kv.delete(res.key);

    // 4. Search index အားလုံး ဖျက်မယ်
    const iterSearch = kv.list({ prefix: ["idx_search"] });
    for await (const res of iterSearch) await kv.delete(res.key);

    // 5. Category counts ဖျက်မယ်
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    for (const cat of cats) await kv.delete(["counts", cat]);

    // 6. Drafts လည်း ဖျက်ချင်ရင်
    const iterDrafts = kv.list({ prefix: ["drafts"] });
    for await (const res of iterDrafts) await kv.delete(res.key);

    // 7. Link cache ဖျက်မယ်
    const iterCache = kv.list({ prefix: ["link_cache"] });
    for await (const res of iterCache) await kv.delete(res.key);

    // 8. Stream tokens ဖျက်မယ်
    const iterTokens = kv.list({ prefix: ["stream_tokens"] });
    for await (const res of iterTokens) await kv.delete(res.key);

    // 9. RAM Cache ရှင်းမယ်
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    RAM_CACHE.featuredMovies = { data: [], timestamp: 0 };

    await logAdminAction("nuke_all_movies", `Permanently deleted ${movieCount} movies and all indexes.`);

    return c.text(`✅ DONE! Deleted ${movieCount} movies + all indexes, drafts, and caches.`);
});
