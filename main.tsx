...
// CONFIG & ENV
// =======================
// Environment Variables (မရှိရင် Default တန်ဖိုးယူပါမယ်)
...
// =======================
// Global Cache Variable - လူဝင်တိုင်း KV ကိုမဖတ်ဘဲ ဒီကနေပဲယူမယ်
...
async function getMovies() {
  // Cache ရှိရင် KV ကို မဖတ်တော့ဘဲ Cache ကပဲ ပြန်ပေးမယ် (Performance အရမ်းမြန်သွားမယ်)
...
async function getMovie(id: string) { 
  // Cache ရှိရင် Cache ထဲက ရှာမယ်
  if (MOVIE_CACHE) {
    return MOVIE_CACHE.find(m => m.id === id);
  }
...
async function resolveRedirect(url: string) {
  // Timeout ထည့်ထားတယ် (5 စက္ကန့်ကျော်ရင် ဆက်မစောင့်တော့ဘူး)
    ...
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        /* WIDE COVER ITEM (For Uncensored) */
      ...
      <script dangerouslySetInnerHTML={{__html: `
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.toggleSeason = function(id) {
...
app.get("/.well-known/assetlinks.json", (c) => c.json([{
  ...
  "target": {
    ...
    "package_name": "dev.deno.goldflix_stream.twa", 
    "sha256_cert_fingerprints": ["29:7D:1A:43:86:09:03:FE:02:F9:69:46:5A:F8:B7:C0:9A:14:75:10:F6:F3:07:4F:2E:CF:0E:F1:3E:D4:5F:7D"]
  }
}]));
        ...
        <div id="movie-grid" class={isUncensored ? "space-y-4" : "grid grid-cols-3 gap-2"}>
            {initialMovies.map(m => (
                <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded overflow-hidden ${isUncensored ? 'mb-4' : ''}`}>
                    ...
                    <div class={isUncensored ? "p-3" : "p-1.5"}><h3 class={isUncensored ? "text-sm font-bold truncate text-white" : "text-[10px] font-bold truncate text-white"}>{m.title}</h3></div>
                </a>
            ))}
        </div>
                   ...
                   <div class="flex flex-col gap-2 mb-6">
                        <div class="flex gap-2 overflow-x-auto">
                            <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="flex-1 bg-white text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-gray-200 whitespace-nowrap">
                                <i class="fa-solid fa-play"></i> {movie.streamUrl2 ? "Server 1" : "Play"}
                            </button>
                            {movie.streamUrl2 && (
                                <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken2 : playerUrl2}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="flex-1 bg-gray-200 text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-white whitespace-nowrap">
                        ...
                        <div class="flex gap-2">
                             {secureDownloadUrl && (
                                <a href={secureDownloadUrl} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs">
                                    <i class="fa-solid fa-download"></i> DL {movie.downloadUrl2 ? "1" : ""}
                                </a>
                             )}
                             {secureDownloadUrl2 && (
                                <a href={secureDownloadUrl2} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs">
                                    <i class="fa-solid fa-download"></i> DL 2
                                </a>
                             )}
                        </div>
                   ...
                   <div class="mb-6 space-y-2">
                       {ungrouped.length > 0 && (
                            <div class="grid grid-cols-3 md:grid-cols-4 gap-2">
                                {ungrouped.map(ep => (
                                    <button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="bg-zinc-800 hover:bg-red-600 text-xs py-3 px-1 rounded truncate text-center border border-zinc-700 transition-colors">{ep.name}</button>
                                ))}
                            </div>
                       )}
                       {Object.keys(seasons).map(season => {
                           ...
                           return (
                               <div class="border border-zinc-800 rounded bg-[#1f1f1f]">
                                   <button onclick={`toggleSeason('${safeId}')`} class="w-full flex justify-between items-center p-3 text-sm font-bold text-gray-300 hover:bg-zinc-800 transition"><span>{season}</span><i id={`icon-${safeId}`} class="fa-solid fa-chevron-down transition-transform"></i></button>
                                   <div id={`season-${safeId}`} class="hidden p-2 grid grid-cols-3 gap-2 border-t border-zinc-800">
                                       {seasons[season].map(ep => (<button onclick={`loadPlayer('${ep.url}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}')`} class="bg-zinc-800 hover:bg-red-600 text-xs py-2 px-1 rounded truncate text-center border border-zinc-700 transition-colors">{ep.name}</button>))}
                                   </div>
                               </div>
                           )
                       })}
...
app.post("/login", async (c) => { 
    ...
    if (user && user.passwordHash === hashedInput) { 
        ...
        // Security Fix: httpOnly, secure, sameSite
        setCookie(c, "auth_session", `${user.username}:${sessionId}`, { 
            path: "/", 
            maxAge,
            httpOnly: true, // Client-side JS က ဖတ်လို့မရ
            secure: !c.req.url.includes("localhost"), // HTTPS only (Production)
...
const adminAuth = async (c: any, next: any) => { const session = getCookie(c, "admin_session"); if (session === ADMIN_PASS) await next(); else return c.redirect("/admin"); };
...
app.post("/admin/login", async (c) => { 
    ...
    if (password === ADMIN_PASS) { 
        setCookie(c, "admin_session", String(password), { 
            path: "/",
            httpOnly: true,
...
app.post("/admin/movie/save", adminAuth, async (c) => { 
    ...
    clearCache(); // Cache Clear (Important)
    return c.redirect("/admin/dashboard"); 
});
app.post("/admin/movie/delete/:id", adminAuth, async (c) => { await kv.delete(["movies", c.req.param("id")]); clearCache(); return c.redirect("/admin/dashboard"); });
...
app.post("/admin/restore", adminAuth, async (c) => { try { const body = await c.req.parseBody(); const file = body['file']; if (file instanceof File) { const text = await file.text(); const data = JSON.parse(text); for (const item of data) { await kv.set(item.key, item.value); } clearCache(); return c.redirect("/admin/dashboard?success=Data Restored"); } } catch(e) { return c.redirect("/admin/dashboard?error=Restore Failed"); } });
