...
// 6. UI LAYOUT
// =======================
const Layout = (props: { children: any; title?: string; user?: User | null; hideNav?: boolean; announcement?: string; isAdmin?: boolean; coverUrl?: string }) => {
...
app.get("/", async (c) => {
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined}>
      ...
      <div class="px-3 py-6 space-y-8">{sections.map(section => { if (section.data.length === 0) return null; const cat = section.name; const catMovies = section.data; if(cat === "All Uncensored") { return (<div><div class="flex justify-between items-end mb-3 px-1"><h2 class="text-lg font-bold text-white border-l-4 border-red-600 pl-2">{cat}</h2><a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1">See All <i class="fa-solid fa-chevron-right text-[10px]"></i></a></div><div class="h-scroll-section custom-scroll">{catMovies.map(m => (<a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded overflow-hidden active:scale-95 transition-transform"><img src={m.coverUrl || m.posterUrl} class="w-full h-full object-cover" /><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white leading-tight">{m.title}</h3></div></a>))}</div></div>) } return (<div><div class="flex justify-between items-end mb-3 px-1"><h2 class="text-lg font-bold text-white border-l-4 border-red-600 pl-2">{cat}</h2><a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1">See All <i class="fa-solid fa-chevron-right text-[10px]"></i></a></div><div class="h-scroll-section custom-scroll">{catMovies.map(m => (<a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded overflow-hidden active:scale-95 transition-transform"><img src={m.posterUrl} class="w-full h-full object-cover aspect-[2/3]" /><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white leading-tight">{m.title}</h3></div></a>))}</div></div>) })}</div>
    </Layout>
  );
});
...
app.get("/movie/:id", async (c) => {
    ...
    return c.html(
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined}>
...
app.post("/login", async (c) => { 
    ...
    if (await isIpBanned(ip)) {
        // 🔥 BANNED UI DESIGN 🔥
        return c.html(
            <Layout hideNav={true} title="Access Denied">
                <div class="min-h-screen flex items-center justify-center bg-black p-4">
                    <div class="bg-[#1f1f1f] p-8 rounded-2xl border border-red-600 text-center max-w-sm w-full shadow-2xl relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            ...
                            <h1 class="text-2xl font-black text-white mb-2 uppercase tracking-widest">Access Denied</h1>
                            <p class="text-gray-400 text-sm mb-6 leading-relaxed">
                                Your IP address <span class="font-mono text-red-400 bg-red-900/20 px-1 rounded">{ip}</span> has been permanently banned due to suspicious activity.
                            </p>
    ...
    if (!checkRateLimit(ip)) {
        return c.html(
            <Layout hideNav={true} title="Security Alert">
                <div class="min-h-screen flex items-center justify-center bg-black p-4 relative overflow-hidden">
                    ...
                    <div class="relative z-10 bg-[#1f1f1f] p-8 rounded-2xl border border-red-500/30 shadow-2xl max-w-sm w-full text-center">
                        ...
                        <h1 class="text-xl font-bold text-white mb-2">Security Alert</h1>
                        <p class="text-gray-400 text-sm mb-6 leading-relaxed">Too many failed attempts. Please wait.</p>
                        <div class="bg-black/50 rounded-lg p-3 border border-red-900/50 mb-6"><p class="text-yellow-500 text-xs font-bold uppercase tracking-wider">Please Wait</p><p class="text-white font-mono text-lg font-bold">1 Minute</p></div>
                ...
                <div class="min-h-screen flex items-center justify-center bg-black p-4">
                    <div class="bg-[#1f1f1f] p-8 rounded-2xl border border-red-600 text-center max-w-sm w-full shadow-2xl relative overflow-hidden">
                        ...
                        <div class="relative z-10">
                            ...
                            <h1 class="text-2xl font-black text-white mb-2 uppercase tracking-widest">Access Denied</h1>
                            <p class="text-gray-400 text-sm mb-6 leading-relaxed">
                                Registration blocked. Your IP address <span class="font-mono text-red-400 bg-red-900/20 px-1 rounded">{clientIp}</span> is banned.
                            </p>
                        </div>
                    </div>
                </div>
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-zinc-900 min-h-screen">
                ...
                {/* TAB 1: MOVIES (Mobile Responsive Fix) */}
...
// 🔥 TOGGLE BAN ACTION (NOW BANS IP TOO)
app.post("/admin/user/toggle-ban", adminGuard, async (c) => { 
    ...
    if (user) { 
        ...
        const targetIp = user.lastLoginIp || user.ip; // Get user's IP

        if (user.isBanned) {
            user.sessionId = "banned"; // Kill session
            if (targetIp && targetIp !== "Unknown-IP") {
                await kv.set(["banned_ips", targetIp], true); // 🔥 BAN IP
            }
        } else {
            // Unban user = Unban IP (Optional: You can remove this if you want strict IP bans)
            if (targetIp) {
                await kv.delete(["banned_ips", targetIp]); // 🔥 UNBAN IP
            }
        }
