...
console.log("Playing:", finalUrl);
             ...
             // 🔥 SAFE SEARCH CODE (No Crash, No Overlap)
             if(searchInput) {
                     ...
                     searchTimeout = setTimeout(async function() {
                         try {
                             ...
                             if(data.results.length === 0) {
                                 ...
                                 // 🔥 FIX: Template String (`...`) မသုံးဘဲ ရိုးရိုးပေါင်းရေးနည်း သုံးမယ် (JS Error မတက်အောင်)
                                 var html = '';
                                 data.results.forEach(function(m) {
                                     // Style ထဲမှာ aspect-ratio ထည့်လိုက်လို့ ပုံမထပ်တော့ဘူး
                                     html += '<a href="/movie/' + m.id + '" class="block rounded-xl overflow-hidden bg-zinc-800 relative group shadow-lg" style="aspect-ratio: 2/3; width: 100%; display: block;">';
                                     html += '<img src="' + m.posterUrl + '" class="absolute inset-0 w-full h-full object-cover transition duration-300 group-hover:scale-110" style="width:100%; height:100%;" />';
                                     html += '<div class="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition"></div>';
                                     html += '<div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/90 to-transparent">';
                                     html += '<p class="text-[10px] font-bold text-white truncate text-center">' + m.title + '</p>';
                                     html += '</div></a>';
                                 });
