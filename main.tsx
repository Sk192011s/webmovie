                                    ...
                                    async function generateAIDesc() {
                                        ...
                                        try {
                                            const res = await fetch('/admin/api/generate-desc', {
                                                ...
                                                // 🔥 Year ပါ ထည့်ပို့မယ်
                                                body: 'title=' + encodeURIComponent(title) + '&type=' + encodeURIComponent(type) + '&year=' + encodeURIComponent(year)
                                            });
                                            const data = await res.json();
...
// 🔥 AI Description Generator (Strict Plot Translation)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    const { title, type, year } = await c.req.parseBody(); 
    const apiKey = Deno.env.get("GEMINI_API_KEY");

    if (!apiKey) return c.json({ error: "Server Error: GEMINI_API_KEY not set." });
    if (!title) return c.json({ error: "Title is required" });

    // 🔥 PROMPT ပြင်လိုက်ပါပြီ (Critic မလုပ်ခိုင်းတော့ဘဲ ဇာတ်လမ်းအတိုချုပ်ကိုပဲ တောင်းပါမယ်)
    const prompt = `
    Task: Provide a factual plot summary for the ${type || "movie"} "${title}" (${year || "released recently"}).
    
    Instructions:
    1. Focus ONLY on the main plot points (what happens in the movie).
    2. Do NOT write a review, do NOT use flowery language like "Masterpiece" or "Great movie".
    3. If the movie involves specific events (like disappearances, crimes, etc.), mention them clearly.
    4. Translate the summary into natural Burmese (Myanmar Unicode).
    5. Length: Around 2 to 3 sentences.
    `;
