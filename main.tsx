...
const RAM_CACHE = {
    ...
    featuredMovies: { data: [], timestamp: 0 },
...
// 🔥 PERFORMANCE: Link Caching System
...
if (typeof Deno.cron !== "undefined") {
    Deno.cron("Daily Cleanup", "0 0 * * *", async () => {
        ...
        console.log("Database Re-Indexed via Cron.");
    });
}
...
// =======================
// 4. UI LAYOUT (NEW CYBER-GLASS UI)
// =======================
  ...
  <html lang={l}>
    <head>
      ...
      <style>{`
        ...
        body { background-color: #0f172a; background-image: radial-gradient(at 0% 0%, hsla(253,16%,7%,1) 0, transparent 50%), radial-gradient(at 50% 0%, hsla(225,39%,30%,1) 0, transparent 50%), radial-gradient(at 100% 0%, hsla(339,49%,30%,1) 0, transparent 50%); background-attachment: fixed; color: #e2e8f0; font-family: 'Inter', sans-serif; -webkit-tap-highlight-color: transparent; padding-bottom: 90px; }
        * { user-select: none; -webkit-user-select: none; }
        ...
        ${protectCSS}
        .glass-panel { background: var(--glass-bg); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid var(--glass-border); box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1); }
        ...
        .bottom-nav { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); width: 90%; max-width: 400px; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.5); display: flex; justify-content: space-around; padding: 12px 6px; z-index: 50; }
        ...
        .nav-item.active i { color: #c084fc; text-shadow: 0 0 10px rgba(192, 132, 252, 0.6); transform: translateY(-3px); }
        .top-header { position: fixed; top: 0; left: 0; width: 100%; z-index: 40; padding: 16px 20px; display: flex; justify-content: space-between; items-center; background: linear-gradient(to bottom, rgba(15,23,42,0.9), transparent); }
        ...
        /* 🔥 NEW LOADER */
        #page-loader { position: fixed; inset: 0; background: #0f172a; z-index: 9999; display: none; justify-content: center; align-items: center; }
        #page-loader.active { display: flex; }
        .spinner { width: 60px; height: 60px; border-radius: 50%; position: relative; animation: rotate 1s linear infinite; }
        .spinner::before, .spinner::after { content: ""; box-sizing: border-box; position: absolute; inset: 0px; border-radius: 50%; border: 5px solid #fff; animation: prixClipFix 2s linear infinite; }
        .spinner::after { border-color: #8b5cf6; animation: prixClipFix 2s linear infinite reverse; transform: rotate(0deg) scale(1.1); }
        ...
        .slide::after { content: ''; position: absolute; inset: 0; background: linear-gradient(to top, #0f172a 0%, transparent 60%); }
        .slide.active { opacity: 1; pointer-events: auto; }
        
        .modal-enter { animation: modalPop 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
        @keyframes modalPop { from { transform: scale(0.8); opacity: 0; } to { transform: scale(1); opacity: 1; } }
        
        img.img-fade { opacity: 0; transition: opacity 0.6s ease-in-out; }
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.switchSeason = function(seasonId) {
                 ...
                 document.querySelectorAll('.season-tab-btn').forEach(btn => { btn.classList.remove('bg-purple-600', 'text-white', 'shadow-lg'); btn.classList.add('bg-slate-800', 'text-gray-400'); });
                 if (!isAlreadyActive) { targetGrid.classList.remove('hidden'); if(targetBtn) { targetBtn.classList.remove('bg-slate-800', 'text-gray-400'); targetBtn.classList.add('bg-purple-600', 'text-white', 'shadow-lg'); } }
             }
             
             // 🔥 REVERTED TO OLD LOGIC: Fetch resolved URL & Play
        ...
        async function loadMoreMovies(category) { 
            ...
            for(let i=0; i<6; i++) {
                ...
                el.className = 'block rounded-xl overflow-hidden aspect-[2/3] img-skeleton';
            ...
            try { 
                ...
                data.movies.forEach(m => { 
                    ...
                    if(category === "All Uncensored" || category === "Myanmar and Asian" || category === "4K Porns") { 
                        el.className = 'movie-card block rounded-xl overflow-hidden mb-4 group relative glass-panel'; 
                        el.innerHTML = '<div class="relative aspect-video overflow-hidden img-skeleton"><img src="'+(m.coverUrl || m.posterUrl)+'" loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" /><div class="absolute top-2 right-2 bg-purple-600 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow z-10">'+badge+'</div></div><div class="p-3"><h3 class="text-sm font-bold truncate text-white">'+m.title+'</h3></div>';
                    } else { 
                        el.className = 'movie-card block rounded-xl overflow-hidden group relative glass-panel'; 
                        el.innerHTML = '<div class="relative aspect-[2/3] overflow-hidden img-skeleton"><img src="'+m.posterUrl+'" loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500 img-fade" /><div class="absolute top-2 right-2 bg-purple-600 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow z-10">'+badge+'</div></div><div class="p-2"><h3 class="text-[11px] font-bold truncate text-white">'+m.title+'</h3></div>';
                    } 
                    grid.appendChild(el); 
                }); 
    ...
    <body>
      ...
      <div id="dl-modal" class="fixed inset-0 z-[100] bg-black/80 hidden items-center justify-center backdrop-blur-md">
           <div class="glass-panel p-6 rounded-2xl w-11/12 max-w-sm text-center relative shadow-2xl modal-enter">
      ...
      {!props.hideNav && (
        <>
            <header class="top-header">
                <a href="/" class="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 tracking-tighter italic drop-shadow-sm">GOLD FLIX</a>
                <div class="flex items-center gap-3">
                    {active === 'home' && !props.isAdmin && (
                        <div class="px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/10 text-[10px] font-bold text-purple-400 tracking-wide backdrop-blur-md">P - {daysLeft} Day</div>
                    )}
                    {props.isAdmin ? (
                        <a href={ADMIN_ROUTE + "/dashboard"} class="text-xs font-bold bg-blue-600 text-white px-4 py-1.5 rounded-full shadow-lg hover:bg-blue-500 transition">ADMIN PANEL</a>
                    ) : (
      ...
      <main class={`flex-grow w-full ${props.announcement ? 'pt-[95px]' : 'pt-[70px]'}`}>
        {props.children}
      </main>
    </body>
  </html>
...
app.get("/", async (c) => {
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        {shouldShowPopup && (
          <div id="promo-popup" class="fixed inset-0 z-[200] bg-black/90 hidden items-center justify-center backdrop-blur-md p-4">
              <div class="relative w-full max-w-sm glass-panel p-0 rounded-2xl overflow-hidden shadow-2xl modal-enter flex flex-col max-h-[85vh]">
                  <button onclick="closePopup(false)" class="absolute top-3 right-3 z-10 bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 transition backdrop-blur-md"><i class="fa-solid fa-xmark"></i></button>
                  <div class="overflow-y-auto custom-scroll flex-grow"><a href={config.popupLink || "#"}><img src={config.popupImage} class="w-full h-auto object-cover" /></a><div class="p-5">{config.popupMessage && (<p class="text-gray-300 text-sm mb-4 leading-relaxed whitespace-pre-wrap font-medium text-center">{config.popupMessage}</p>)}<a href={config.popupLink || "#"} class="block w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-center py-3.5 rounded-xl shadow-lg hover:brightness-110 transition uppercase tracking-wider text-sm">{config.popupBtnText || "View Details"}</a><button onclick="closePopup(true)" class="block w-full text-center text-xs text-gray-500 font-bold mt-4 py-2 hover:text-white transition">Don't show again today</button></div></div>
              </div>
        ...
        <div class="px-4 mb-2 sticky top-[60px] z-30 transition-all">
             <form action="/search" method="get" class="relative group">
                 <button type="submit" class="absolute left-4 top-3.5 text-gray-400 group-hover:text-white transition z-10"><i class="fa-solid fa-magnifying-glass"></i></button>
                 <input name="q" placeholder={t.search_ph} class="w-full bg-[#1e293b]/80 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm text-white focus:border-purple-500 outline-none transition shadow-lg backdrop-blur-md" />
             </form>
        </div>
      {sliderMovies.length > 0 && (
          <div class="px-4 mb-8 mt-4">
              <div class="slider-container relative z-0 group">
                  {sliderMovies.map((m, idx) => (
                      <div class={`slide ${idx === 0 ? 'active' : ''}`}>
                          <img src={m.coverUrl} class="w-full h-full object-cover" />
                          <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#0f172a] via-[#0f172a]/80 to-transparent">
                              <h1 class="text-xl md:text-3xl font-black text-white drop-shadow-md truncate leading-tight mb-2">{m.title}</h1>
                              <a href={`/movie/${m.id}`} class="inline-flex items-center gap-2 bg-white text-black px-5 py-2 rounded-full font-bold text-xs hover:scale-105 transition transform shadow-[0_0_20px_rgba(255,255,255,0.3)]"><i class="fa-solid fa-play"></i> {t.play}</a>
                          </div>
                      </div>
                  ))}
              </div>
                  ...
                  return (
                    <div>
                        ...
                        <div class="h-scroll-section">
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative rounded-xl overflow-hidden active:scale-95 transition-transform movie-card group glass-panel">
                                    ...
                                    <div class="p-3"><h3 class="text-sm font-bold truncate text-white leading-tight">{m.title}</h3></div>
                                </a>
                            ))}
                        </div>
                    </div>
              ...
              return (
                <div>
                    ...
                    <div class="h-scroll-section custom-scroll">
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative rounded-xl overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform movie-card group glass-panel">
...
// 🔥 UPDATE: Resolve URL only when requested via API (Lazy Resolve)
app.get("/api/resolve-url", async (c) => { 
    ...
    // Check if it's safe and valid
    if(!isValidUrl(url)) return c.json({ error: "Unsafe URL detected" }, 403);

    // 🔥 RESOLVE NOW: This is where we pay the speed cost, only when user wants to play
    // But now we check the LINK CACHE first!
    try {
...
app.get("/category/:cat", async (c) => { 
    ...
    return c.html(<Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home"><div class="sticky top-[60px] z-30 px-4 py-3 bg-[#0f172a]/90 backdrop-blur-md transition-all">
             <form action="/search" method="get" class="relative">
                 <button type="submit" class="absolute left-4 top-3.5 text-gray-500 hover:text-white transition z-10"><i class="fa-solid fa-magnifying-glass"></i></button>
                 <input name="q" placeholder={i18n[lang].search_ph} class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm text-white focus:border-purple-500 outline-none transition shadow-sm" />
             </form>
    ...
    {movies.map(m => (
        <a href={`/movie/${m.id}`} class={`block rounded-xl overflow-hidden movie-card group glass-panel ${isUncensored ? 'mb-4' : ''}`}>
            ...
            <div class="p-4">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <form action="/search" method="get" class="flex-grow relative">
                        ...
                        <input name="q" value={query} placeholder="Search..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-10 pr-4 text-sm outline-none focus:border-purple-500 text-white" />
                    </form>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    {results.map(m => {
                        ...
                        return (
                            <a href={`/movie/${m.id}`} class={`block rounded-xl overflow-hidden movie-card group glass-panel ${isWide ? 'col-span-3' : ''}`}>
...
app.get("/favorites", async (c) => { const user = await getCurrentUser(c); if(!user) return c.redirect("/login"); const lang = getLang(c); const favs = []; if(user.favorites) { for(const id of user.favorites) { const m = await getMovie(id); if(m) favs.push(m); } } return c.html(<Layout user={user} title="Saved" lang={lang} activeTab="saved"><div class="p-4"><h1 class="text-2xl font-bold mb-6 flex items-center gap-2"><i class="fa-solid fa-heart text-red-500"></i> My Saved Movies</h1><div class="grid grid-cols-3 gap-3">{favs.map(m => (<a href={`/movie/${m.id}`} class="block rounded-xl overflow-hidden movie-card group glass-panel"><div class="aspect-[2/3] relative overflow-hidden"><img src={m.posterUrl} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" /></div><div class="p-2"><h3 class="text-[11px] font-bold truncate text-white">{m.title}</h3></div></a>))}</div>{favs.length===0 && <p class="text-gray-500 text-center mt-10">No saved movies.</p>}</div></Layout>); });
    ...
    return c.html(
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        ...
        <div id="buy-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-md p-4">
             <div class="glass-panel p-6 rounded-2xl w-full max-w-sm text-center relative shadow-2xl modal-enter">
        ...
        <div id="vip-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-md p-4">
             <div class="glass-panel p-6 rounded-2xl w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  <div class="w-20 h-20 bg-purple-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-purple-500/20">
                      <i class="fa-solid fa-crown text-4xl text-purple-500 animate-pulse"></i>
                  </div>
                  <h3 class="text-xl font-black text-white mb-2">VIP Access Required</h3>
                  <p class="text-gray-300 text-sm leading-relaxed mb-6 font-bold">Moviesများ ကြည့်ရန် ဒေါင်းရန်အတွက် Accountဖွင့်ပါ</p>
                  <div class="flex gap-3 h-12"> 
                  ...
                  <a href="https://t.me/iqowoq" target="_blank" class="block mt-6 text-xs text-blue-400 hover:text-white underline">Contact Admin on Telegram</a>
             </div>
        </div>

        <div class="max-w-4xl mx-auto">
           {/* 🔥 VIDEO PLAYER */}
           <div class="w-full aspect-video bg-black relative shadow-2xl group rounded-b-2xl overflow-hidden border-b border-white/10">
                {canWatch ? (
                    <>
                        <div id="video-cover" class="absolute inset-0 z-20 cursor-pointer group">
                            <img src={displayImage} class="w-full h-full object-cover transition duration-1000 group-hover:scale-105" />
                            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 bg-black/40 backdrop-blur-sm">
                                <div class="w-20 h-20 rounded-full bg-white/20 backdrop-blur flex items-center justify-center border border-white/30 shadow-2xl">
                                    <i class="fa-solid fa-play text-4xl text-white ml-1"></i>
                                </div>
                            </div>
                        </div>
                        <div id="video-player" class="w-full h-full hidden"></div>
                        <div id="video-player-loader" class="absolute inset-0 z-10 bg-slate-900 flex items-center justify-center hidden">
                    ...
                    <div class="absolute inset-0 z-20">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <i class="fa-solid fa-lock text-6xl text-white/50 mb-4"></i>
                            <p class="text-white/70 font-bold uppercase tracking-widest text-sm">Locked Content</p>
                        </div>
                    </div>
                )}
           </div>
           ...
           <div class="p-6">
               <div class="flex justify-between items-start mb-4">
                   <h1 class="text-2xl font-bold text-white leading-tight flex-grow pr-4">{movie.title}</h1>
                   <div class="flex items-center gap-2 flex-shrink-0 h-10">
                       <button onclick={`shareMovie('${movie.title}')`} class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 transition border border-white/10"><i class="fa-solid fa-share-nodes text-lg"></i></button>
                       {user && (<form action="/api/fav" method="post" class="m-0 p-0 flex"><input type="hidden" name="movieId" value={movie.id} /><button class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 transition hover:bg-white/10 border border-white/10"><i class={`fa-solid fa-heart text-lg ${user.favorites && user.favorites.includes(movie.id) ? 'text-red-500' : 'text-gray-400'}`}></i></button></form>)}
                   </div>
               </div>
               
               {isPurchased && (
                   <div class="bg-gradient-to-r from-green-900/40 to-emerald-900/20 border border-green-500/30 p-4 rounded-xl mb-6 flex items-center gap-4">
                       <div class="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center text-green-400"><i class="fa-solid fa-check text-xl"></i></div>
                       <div><h3 class="text-green-400 font-bold text-sm">Purchased</h3><p class="text-green-500/60 text-xs">You own this video forever.</p></div>
                   </div>
               )}

               <div class="flex flex-wrap items-center gap-2 mb-8">
                   <span class="bg-white/10 text-gray-300 text-xs px-3 py-1 rounded-full border border-white/10 font-mono">{movie.year}</span>
                   <span class="bg-purple-600/20 text-purple-400 border border-purple-500/30 text-xs px-3 py-1 rounded-full font-bold">{movie.category}</span>
                   ...
                   {movie.duration && (<span class="bg-pink-900/30 text-pink-400 border border-pink-500/30 text-xs px-3 py-1 rounded-full flex items-center gap-1 font-bold"><i class="fa-regular fa-clock"></i> {movie.duration}</span>)}
               ...
               <div class="flex flex-col gap-3 mb-8">
                   ...
                   {movie.category !== "Series" && (movie.downloadUrl || movie.downloadUrl2) && (
                    ...
                    <button onclick="openHelpModal()" class="w-full text-xs text-purple-400 hover:text-white flex items-center justify-center gap-2 mt-2 font-bold transition-colors py-3 rounded-xl border border-dashed border-purple-500/30 hover:border-purple-500 hover:bg-purple-500/10"><i class="fa-solid fa-circle-question"></i> {t.dl_help}</button>
                    {canWatch && <form action="/report" method="post" class="mt-2 text-center"><input type="hidden" name="movieId" value={movie.id} /><input type="hidden" name="movieName" value={movie.title} /><button class="text-[10px] text-red-500 hover:text-red-400 font-bold opacity-60 hover:opacity-100 transition"><i class="fa-solid fa-flag"></i> Report Broken Link</button></form>}
               </div>

               {(movie.category === "Series" || movie.category === "Animation") && episodes.length > 0 && (
                   <div class="mb-8">
                       <h3 class="font-bold text-white mb-4 flex items-center gap-2 text-lg"><i class="fa-solid fa-layer-group text-purple-500"></i> Seasons & Episodes</h3>
                       <div class="flex overflow-x-auto gap-3 pb-4 scrollbar-hide">{Object.keys(seasons).map((season, idx) => { const safeId = season.replace(/\s+/g, '-'); return (<button id={`btn-${safeId}`} onclick={`switchSeason('${safeId}')`} class="season-tab-btn flex-shrink-0 px-6 py-2.5 rounded-full text-xs font-bold transition border border-white/10 bg-slate-800 text-gray-400 hover:text-white hover:border-purple-500">{season}</button>) })}</div>
                       <div id="episodes-container">
                           {Object.keys(seasons).map((season, idx) => { 
                               const safeId = season.replace(/\s+/g, '-'); 
                               return (
                                   <div id={`ep-grid-${safeId}`} class="season-content hidden mt-2 bg-slate-800/50 rounded-2xl border border-white/5 p-4 animate-fade-in">
                                       <div class="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-60 overflow-y-auto custom-scroll pr-1">
                                           {seasons[season].map(ep => (
                                               <button onclick={canWatch ? `loadPlayer('${ep.url.trim()}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)` : (moviePrice > 0 ? `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')` : `openVipModal()`)} class="srv-btn bg-slate-700 hover:bg-slate-600 text-white text-[11px] py-3 rounded-lg border border-white/5 transition-colors font-bold truncate">{canWatch ? "" : "🔒 "} {ep.name}</button>
                                            ))}
                                       </div>
                                   </div>
                               ) 
               ...
               <div class="bg-white/5 p-5 rounded-2xl border border-white/5 mb-8">
                   <h3 class="text-gray-400 text-xs font-bold uppercase mb-2">Synopsis</h3>
                   <p class="text-sm text-gray-200 leading-relaxed whitespace-pre-wrap">{movie.description}</p>
               </div>

               {related.length > 0 && (
                   <div class="pt-6 border-t border-white/10"><h3 class="font-bold text-white mb-4 text-lg">You May Also Like</h3><div class="h-scroll-section custom-scroll">{related.map(m => (<a href={`/movie/${m.id}`} class={`h-scroll-item block relative rounded-xl overflow-hidden flex-shrink-0 movie-card group glass-panel ${m.category === "All Uncensored" || m.category === "Myanmar and Asian" || m.category === "4K Porns" ? 'wide' : 'w-28'}`}><div class={`${m.category === "All Uncensored" || m.category === "Myanmar and Asian" || m.category === "4K Porns" ? "aspect-video" : "aspect-[2/3]"} w-full relative overflow-hidden img-skeleton`}><img src={m.category === "All Uncensored" || m.category === "Myanmar and Asian" || m.category === "4K Porns" ? (m.coverUrl || m.posterUrl) : m.posterUrl} loading="lazy" decoding="async" onload="window.imgLoaded(this)" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700 img-fade" /></div><div class="p-2"><h3 class="text-[11px] font-bold truncate text-white">{m.title}</h3></div></a>))}</div></div>
               )}
           </div>
        </div>
      </Layout>
      ...
      <Layout hideNav={true} lang={lang} activeTab="me">
        <div class="min-h-[80vh] flex items-center justify-center p-4">
            <div class="w-full max-w-sm relative">
                <div class="absolute -top-20 -left-20 w-40 h-40 bg-purple-600/30 rounded-full blur-[80px]"></div>
                <div class="absolute -bottom-20 -right-20 w-40 h-40 bg-pink-600/30 rounded-full blur-[80px]"></div>
                
                <h1 class="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 mb-10 text-center italic tracking-tighter drop-shadow-lg">GOLD FLIX</h1>
                <form action="/login" method="post" class="glass-panel p-8 rounded-3xl space-y-6 relative z-10">
                    <h2 class="text-2xl font-bold text-white text-center mb-2">{t.login}</h2>
                    <div class="space-y-4">
                    ...
                    <label class="flex items-center text-gray-400 text-xs cursor-pointer">
                        <input type="checkbox" name="remember" class="mr-2 accent-purple-500 w-4 h-4 rounded" /> {t.remember}
                    </label>
                    <button class="btn-primary w-full shadow-lg shadow-purple-900/30">{t.login}</button>
                    <div class="text-center space-y-3 pt-2">
                        <p class="text-xs text-gray-400">{t.no_acc} <a href="/signup" class="text-purple-400 font-bold hover:text-purple-300 transition">{t.create_acc}</a></p>
                        <a href="/forgot-password" class="block text-xs text-gray-500 hover:text-white transition">{t.forgot_pass}</a>
                    </div>
                </form>
            </div>
        </div>
...
app.post("/login", async (c) => { 
    ...
    const lang = getLang(c);
    const t = i18n['my'];
    if (await isIpBanned(ip)) return c.html(<Layout hideNav={true} title="Access Denied" lang={lang}><div class="min-h-screen flex items-center justify-center p-6"><div class="glass-panel p-8 rounded-3xl border-red-500/30 text-center max-w-sm w-full relative overflow-hidden"><div class="absolute inset-0 bg-red-600/10 blur-xl"></div><div class="relative z-10"><i class="fa-solid fa-ban text-6xl text-red-500 mb-6 drop-shadow"></i><h1 class="text-2xl font-black text-white mb-2 uppercase tracking-widest">{t.access_denied}</h1><p class="text-gray-400 text-sm mb-6 leading-relaxed">{t.ip_banned} <span class="font-mono text-red-400 bg-red-900/20 px-2 py-1 rounded">{ip}</span></p></div></div></div></Layout>);
    
    if (!await checkLoginRateLimit(ip)) return c.html(<Layout hideNav={true} title="Security Alert" lang={lang}><div class="min-h-screen flex items-center justify-center p-6"><div class="glass-panel p-8 rounded-3xl border-red-500/30 text-center max-w-sm w-full"><div class="w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-red-500/20"><i class="fa-solid fa-shield-halved text-4xl text-red-500 animate-pulse"></i></div><h1 class="text-xl font-bold text-white mb-2">Too Many Attempts</h1><p class="text-gray-400 text-sm mb-6">Password မှားယွင်းမှု များနေပါသည်။</p><div class="bg-black/30 rounded-xl p-4 border border-white/5 mb-6"><p class="text-yellow-500 text-xs font-bold uppercase tracking-wider">{t.wait}</p><p class="text-white font-mono text-lg font-bold">15 Minutes</p></div><a href="/login" class="block w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-xl transition">Back to Login</a></div></div></Layout>);
...
app.get("/signup", (c) => {
    ...
    <Layout hideNav={true} lang={lang} activeTab="me">
        <div class="min-h-[80vh] flex items-center justify-center p-4">
            <div class="w-full max-w-sm relative">
                <div class="absolute -top-20 -left-20 w-40 h-40 bg-purple-600/30 rounded-full blur-[80px]"></div>
                <h1 class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 mb-8 text-center italic tracking-tighter">GOLD FLIX</h1>
                <form action="/signup" method="post" class="glass-panel p-8 rounded-3xl space-y-6 relative z-10">
                    <h2 class="text-xl font-bold text-white text-center mb-2">{t.create_acc}</h2>
                    <div class="space-y-4">
                        <input name="username" placeholder={t.username} required class="input-box" />
                        <input type="password" name="password" placeholder={t.password} required class="input-box" />
                        <div class="space-y-2 pt-2">
                             <label class="text-xs text-purple-400 font-bold uppercase tracking-wide pl-1">{t.sec_q}</label>
                             <select name="question" class="input-box bg-black/40 text-sm text-gray-300">
                                 {SECURITY_QUESTIONS.map(q => <option value={q}>{q}</option>)}
                             </select>
                             <input name="answer" placeholder={t.sec_a} required class="input-box" />
                        </div>
                    </div>
                    <button class="btn-primary w-full shadow-lg shadow-purple-900/30">{t.signup}</button>
                    <p class="text-xs text-gray-500 text-center mt-2">{t.has_acc} <a href="/login" class="text-purple-400 font-bold hover:text-purple-300 transition">{t.login}</a></p>
                </form>
            </div>
        </div>
    </Layout>
...
app.post("/signup", async (c) => { 
    ...
    const lang = getLang(c);
    const t = i18n['my'];
    if (await isIpBanned(clientIp)) return c.html(<Layout hideNav={true} title="Access Denied" lang={lang}><div class="min-h-screen flex items-center justify-center p-6"><div class="glass-panel p-8 rounded-3xl border-red-500/30 text-center max-w-sm w-full"><i class="fa-solid fa-ban text-6xl text-red-500 mb-6"></i><h1 class="text-2xl font-black text-white mb-2">{t.access_denied}</h1><p class="text-gray-400 text-sm">{t.ip_banned}</p></div></div></Layout>);
    ...
    return c.html(
        <Layout hideNav={true} lang={lang}>
            <div class="min-h-screen flex items-center justify-center p-4">
                <div class="w-full max-w-sm glass-panel p-8 rounded-3xl relative overflow-hidden">
                    <h2 class="text-xl font-bold text-white mb-6 text-center">{t.forgot_pass}</h2>
                    <form action="/forgot-password" method="post" class="space-y-4 relative z-10">
                        <input name="username" placeholder={t.username} required class="input-box" />
                        <button class="btn-primary w-full">{t.next}</button>
                    </form>
                    <a href="/login" class="block text-center text-xs text-gray-500 mt-6 hover:text-white">{t.back_login}</a>
                </div>
            </div>
        </Layout>
    );
        ...
        <Layout hideNav={true} lang={lang}>
            <div class="min-h-screen flex items-center justify-center p-4">
                <div class="w-full max-w-sm glass-panel p-8 rounded-3xl">
                    <h2 class="text-xl font-bold text-white mb-2 text-center">{t.sec_q}</h2>
                    <div class="bg-purple-600/10 border border-purple-500/20 p-4 rounded-xl mb-6 text-purple-300 font-bold text-sm text-center shadow-inner">{user.securityQ}</div>
...
app.get("/profile", async (c) => { 
    ...
    return c.html(
      <Layout user={user} lang={lang} activeTab="me">
        <div class="p-4 max-w-2xl mx-auto space-y-6 pb-24 pt-4">
            <div class="relative w-full aspect-[1.9/1] rounded-3xl overflow-hidden p-6 text-white shadow-2xl group">
                ...
                <div class="absolute top-0 right-0 p-6 opacity-20"><i class="fa-solid fa-crown text-8xl transform rotate-12"></i></div>
                
                <div class="relative z-10 h-full flex flex-col justify-between">
                     <div class="flex justify-between items-start">
                         <div class="flex items-center gap-4">
                             <div class="w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-3xl font-black border border-white/30 shadow-inner">{user.username[0].toUpperCase()}</div>
                             <div><h2 class="text-2xl font-black tracking-tight leading-none drop-shadow-md">{user.username}</h2><span class="text-[10px] font-bold bg-black/30 px-3 py-1 rounded-full text-white/90 inline-block mt-1.5 uppercase tracking-widest border border-white/10 backdrop-blur-sm">{statusText}</span></div>
                         </div>
                     </div>
                     <div class="font-mono"><p class="text-[10px] uppercase opacity-70 font-bold mb-1 tracking-widest">Membership ID</p><div class="flex items-center gap-3"><p class="text-xl font-bold tracking-widest">{memberId}</p><button onclick={`copyToClip('${memberId}')`} class="w-8 h-8 flex items-center justify-center bg-white/10 rounded-lg hover:bg-white/20 transition text-sm backdrop-blur"><i class="fa-regular fa-copy"></i></button></div></div>
                </div>
            </div>

            <div class="grid grid-cols-3 gap-4">
                 <a href="/favorites" class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-white/5 transition active:scale-95"><div class="w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center text-red-400 text-lg shadow-inner"><i class="fa-solid fa-heart"></i></div><span class="block text-2xl font-black text-white leading-none">{favCount}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Saved</span></a>
                 <div class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2"><div class="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 text-lg shadow-inner"><i class="fa-solid fa-coins"></i></div><span class="block text-2xl font-black text-white leading-none">{userCoins}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Kyats</span></div>
                 <div class="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-2"><div class={`w-10 h-10 rounded-full flex items-center justify-center text-lg shadow-inner ${premium ? 'bg-green-500/20 text-green-400' : 'bg-gray-700/20 text-gray-500'}`}><i class="fa-solid fa-clock"></i></div><span class={`block text-2xl font-black leading-none ${premium ? 'text-white' : 'text-gray-500'}`}>{premium ? finalDays : "0"}</span><span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Days Left</span></div>
            </div>

            <div class="glass-panel p-6 rounded-3xl relative overflow-hidden group">
                 <div class="absolute -right-6 -top-6 opacity-10 group-hover:opacity-20 transition duration-500"><i class="fa-solid fa-ticket text-9xl"></i></div>
                 <h3 class="font-bold text-gray-300 text-sm mb-4 flex items-center gap-2 relative z-10 uppercase tracking-wide"><i class="fa-solid fa-gift text-purple-500"></i> Redeem Code</h3>
                 <form action="/profile/redeem" method="post" class="relative z-10 flex gap-3">
                    <input name="key" placeholder="Enter VIP or Coin Code..." required class="input-box font-mono tracking-wide" />
                    <button class="bg-purple-600 text-white font-bold px-6 rounded-xl hover:bg-purple-500 transition shadow-lg shadow-purple-900/20"><i class="fa-solid fa-arrow-right"></i></button>
                 </form>
            </div>
            
            {/* 🔥 KPAY/WAVE UI */}
            <div class="glass-panel p-6 rounded-3xl relative overflow-hidden">
                 <h3 class="font-bold text-blue-400 text-sm mb-5 flex items-center gap-2 uppercase tracking-wide"><i class="fa-solid fa-money-bill-transfer"></i> Manual Top-up</h3>
                 <form action="/profile/topup" method="post" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <label class="cursor-pointer relative group">
                            ...
                            <div class="p-4 rounded-2xl bg-black/40 border border-white/5 peer-checked:border-orange-500 peer-checked:bg-orange-500/10 transition text-center hover:bg-white/5 group-active:scale-95 h-full flex flex-col justify-center items-center">
                                <i class="fa-solid fa-wallet text-orange-500 mb-2 text-2xl"></i>
                                <span class="font-bold text-xs text-gray-400 peer-checked:text-orange-500">KBZ Pay</span>
                            </div>
                            <div class="absolute top-3 right-3 text-orange-500 opacity-0 peer-checked:opacity-100 transition"><i class="fa-solid fa-circle-check"></i></div>
                        </label>
                        <label class="cursor-pointer relative group">
                            <input type="radio" name="method" value="wave" class="peer sr-only" />
                            <div class="p-4 rounded-2xl bg-black/40 border border-white/5 peer-checked:border-yellow-400 peer-checked:bg-yellow-400/10 transition text-center hover:bg-white/5 group-active:scale-95 h-full flex flex-col justify-center items-center">
                                <i class="fa-solid fa-money-bill-wave text-yellow-400 mb-2 text-2xl"></i>
                                <span class="font-bold text-xs text-gray-400 peer-checked:text-yellow-400">Wave Pay</span>
                            </div>
                            <div class="absolute top-3 right-3 text-yellow-400 opacity-0 peer-checked:opacity-100 transition"><i class="fa-solid fa-circle-check"></i></div>
                        </label>
                    </div>
                    <input type="number" name="amount" placeholder="Amount (Ks)" required class="input-box" />
                    <input name="transactionId" placeholder="Transaction ID / Slip Link" required class="input-box" />
                    <button class="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:brightness-110 text-white font-bold py-4 rounded-xl transition text-xs shadow-lg shadow-blue-900/30 active:scale-95 uppercase tracking-wider">Submit Slip</button>
                 </form>
            </div>

            <div>
                <h3 class="font-bold text-white mb-4 flex items-center gap-2 text-sm uppercase tracking-wider text-gray-500 pl-2"><i class="fa-solid fa-gem text-blue-500"></i> Plans</h3>
                <div class="space-y-4">{plans.map(p => (<div class={`relative glass-panel p-5 rounded-2xl flex flex-col justify-between transition active:scale-95 ${p.popular ? 'border-yellow-500/50 shadow-[0_0_20px_rgba(234,179,8,0.1)]' : ''}`}>{p.popular && <div class="absolute -top-3 left-1/2 -translate-x-1/2 bg-yellow-500 text-black text-[9px] font-black px-3 py-1 rounded-full shadow-lg uppercase tracking-widest">Best Value</div>}<div class="text-center mb-4"><h4 class="text-gray-400 font-bold text-xs uppercase mb-1 tracking-wide">{p.name}</h4><span class="text-white font-black text-2xl tracking-tight drop-shadow-md">{p.price}</span></div><div class="space-y-2 mb-5 px-2">{p.features.slice(0,2).map(f => (<div class="flex items-center gap-2 text-[11px] text-gray-400 justify-center"><div class="w-4 h-4 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 text-[8px]"><i class="fa-solid fa-check"></i></div> {f}</div>))}</div><a href="https://t.me/LuGyiandYoteshinMovies" target="_blank" class={`block text-center py-3 rounded-xl font-bold text-xs transition uppercase tracking-wide ${p.popular ? 'bg-yellow-500 text-black hover:bg-yellow-400 shadow-lg' : 'bg-slate-700 text-white hover:bg-slate-600'}`}>Buy Now</a></div>))}</div>
            </div>

            {purchasedMovies.length > 0 && (
                <div class="pt-6 border-t border-white/10">
                    <h3 class="font-bold text-green-400 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider pl-2">
                        <i class="fa-solid fa-cart-arrow-down"></i> My Collection ({purchasedMovies.length})
                    </h3>
                    <div class="max-h-[400px] overflow-y-auto custom-scroll pr-1">
                        <div class="grid grid-cols-2 gap-4 pb-2">
                            {purchasedMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="block rounded-xl overflow-hidden movie-card group relative glass-panel">
                                    <div class="aspect-video relative overflow-hidden">
                                        <img src={m.coverUrl || m.posterUrl} loading="lazy" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-700" />
                                        <div class="absolute top-1 right-1 bg-green-600 text-white text-[8px] font-bold px-2 py-0.5 rounded shadow backdrop-blur-md">Owned</div>
                                    </div>
                                    <div class="p-3"><h3 class="text-[10px] font-bold truncate text-white">{m.title}</h3></div>
                                </a>
                            ))}
                        </div>
                    </div>
            ...
            <a href="/logout" class="flex items-center justify-center w-full gap-2 text-red-400 font-bold text-xs py-4 rounded-xl border border-red-500/20 bg-red-500/5 hover:bg-red-500 hover:text-white transition active:scale-95 mt-6 uppercase tracking-wider"><i class="fa-solid fa-arrow-right-from-bracket"></i> Sign Out</a>
        </div>
      </Layout>
    ); 
});
...
app.get(ADMIN_ROUTE, (c) => c.html(<Layout hideNav={true}><div class="min-h-screen flex items-center justify-center bg-slate-900"><form action={ADMIN_ROUTE + "/login"} method="post" class="glass-panel p-8 rounded-2xl w-80 shadow-2xl border border-white/10"><h2 class="font-bold text-center mb-6 text-blue-400 text-xl tracking-widest">ADMIN ACCESS</h2><input type="password" name="password" placeholder="Enter Secure Key" class="input-box mb-6 text-center tracking-widest" /><button class="bg-blue-600 text-white w-full py-3 rounded-xl font-bold hover:bg-blue-500 transition shadow-lg shadow-blue-900/20">Unlock Dashboard</button></form></div></Layout>));
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-slate-900 min-h-screen font-sans text-sm text-gray-200">
                <div class="flex justify-between items-center mb-6 bg-slate-800 p-4 rounded-xl border border-white/5 shadow-sm"><h1 class="font-bold text-blue-400 text-lg flex items-center gap-2"><i class="fa-solid fa-shield-cat"></i> Dashboard</h1><div class="flex gap-2"><a href="/admin/backup" class="bg-slate-700 hover:bg-slate-600 text-white px-3 py-1.5 rounded-lg text-xs border border-white/10 font-bold"><i class="fa-solid fa-download"></i> Backup</a><form action="/admin/restore" method="post" enctype="multipart/form-data" class="inline"><label class="bg-slate-700 hover:bg-slate-600 text-white px-3 py-1.5 rounded-lg text-xs border border-white/10 cursor-pointer font-bold"><i class="fa-solid fa-upload"></i> Restore<input type="file" name="file" class="hidden" onchange="this.form.submit()" /></label></form></div></div>
                <div class="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
                    <button id="btn-stats" onclick="openTab('stats')" class="tab-btn active px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Analytics</button>
                    <button id="btn-movies" onclick="openTab('movies')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Movies</button>
                    <button id="btn-drafts" onclick="openTab('drafts')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5 flex items-center gap-2">Drafts <span class="bg-yellow-500 text-black px-1.5 rounded-full text-[9px]">{drafts.length}</span></button>
                    <button id="btn-keys" onclick="openTab('keys')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">VIP/Coins</button>
                    <button id="btn-topups" onclick="openTab('topups')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5 flex items-center gap-2">Top-ups <span class="bg-blue-500 text-white px-1.5 rounded-full text-[9px]">{topups.filter(t=>t.status==='pending').length}</span></button>
                    <button id="btn-users" onclick="openTab('users')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Users</button>
                    <button id="btn-requests" onclick="openTab('requests')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Requests</button>
                    <button id="btn-logs" onclick="openTab('logs')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Logs</button>
                    <button id="btn-config" onclick="openTab('config')" class="tab-btn px-5 py-2.5 bg-slate-800 rounded-full text-xs font-bold text-gray-400 hover:text-white transition whitespace-nowrap border border-white/5">Config</button>
                </div>

                <div id="tab-stats" class="tab-content active">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl">
                            <h2 class="text-white font-bold mb-4">Content Distribution</h2>
                            <canvas id="contentChart"></canvas>
                        </div>
                        <div class="bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl flex flex-col justify-center items-center">
                            <h2 class="text-white font-bold mb-4">Quick Stats</h2>
                            <div class="grid grid-cols-2 gap-6 w-full text-center">
                                <div class="bg-black/20 p-4 rounded-xl border border-white/5"><p class="text-3xl font-black text-white">{totalUsers}</p><p class="text-xs text-gray-500 uppercase">Users</p></div>
                                <div class="bg-black/20 p-4 rounded-xl border border-white/5"><p class="text-3xl font-black text-yellow-500">{movies.length}</p><p class="text-xs text-gray-500 uppercase">Movies</p></div>
                                <div class="bg-black/20 p-4 rounded-xl border border-white/5"><p class="text-3xl font-black text-blue-500">{topups.filter(t=>t.status==='approved').reduce((a,b)=>a+b.amount,0).toLocaleString()}</p><p class="text-xs text-gray-500 uppercase">Total Income (Ks)</p></div>
                                <div class="bg-black/20 p-4 rounded-xl border border-white/5"><p class="text-3xl font-black text-green-500">{logs.length}</p><p class="text-xs text-gray-500 uppercase">Actions</p></div>
                            </div>
                        </div>
                    </div>
                    <script dangerouslySetInnerHTML={{__html: `
                ...
                <div id="tab-movies" class="tab-content">
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-slate-800 border-white/5'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title (Leave empty for 4K Auto)" value={editMovie?.title} class="input-box w-full bg-black/40 border-white/10 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black/40 border-white/10">{["Movies","Series","4K Movies","Animation","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-24 text-center flex-shrink-0 bg-black/40 border-white/10" /></div></div>
                                <input name="posterUrl" placeholder="Poster URL (Portrait)" value={editMovie?.posterUrl} required class="input-box w-full bg-black/40 border-white/10" /><input name="coverUrl" placeholder="Cover URL (Landscape)" value={editMovie?.coverUrl} required class="input-box w-full border-yellow-500/30 bg-black/40" />
                                <div class="grid grid-cols-3 gap-2"><input name="fileSize" placeholder="File Size (e.g. 1.2 GB)" value={editMovie?.fileSize} class="input-box w-full bg-black/40 border-white/10" /><input name="duration" placeholder="Duration (e.g. 25m)" value={editMovie?.duration} class="input-box w-full bg-black/40 border-white/10" /><input type="number" name="price" placeholder="Price (Ks)" value={editMovie?.price || 0} class="input-box w-full bg-black/40 border-yellow-500/50 text-yellow-500 font-bold" /></div>
                                <div class="p-3 bg-black/40 rounded-xl border border-white/10 space-y-2"><label class="text-[10px] text-gray-500 uppercase font-bold">Main Stream</label><select name="linkType" class="input-box text-xs w-full py-2 bg-slate-900"><option value="direct" selected={editMovie?.linkType==="direct"}>Direct Link (Auto-Resolve)</option><option value="embed" selected={editMovie?.linkType==="embed"}>Embed Code / Iframe</option></select><input name="streamUrl" placeholder="Stream URL (or Episode 1)" value={editMovie?.streamUrl} class="input-box w-full bg-slate-900" /></div>
                                <div class="bg-slate-700/50 p-3 rounded-xl mb-4 border border-white/10"><div class="flex justify-between items-center mb-2"><label class="text-[10px] text-green-400 uppercase font-bold"><i class="fa-solid fa-wand-magic-sparkles"></i> Magic Tools</label><button type="button" onclick="clearEpisodes()" class="text-[10px] text-red-400 hover:text-white underline">Clear All</button></div><div class="flex gap-2 mb-3"><input id="gen-season" placeholder="Season (e.g. 1)" value="1" class="input-box text-[10px] bg-black/40 border-white/10 h-8 w-1/4 text-center" /><input id="gen-start" type="number" placeholder="Start Ep (1)" value="1" class="input-box text-[10px] bg-black/40 border-white/10 h-8 w-1/4 text-center" /></div><div class="border-t border-white/10 pt-2 mb-3"><p class="text-[9px] text-gray-400 mb-1">Tool 1: Sequence (Use *** or $$)</p><div class="flex gap-2"><input id="gen-url" placeholder="Link template..." class="input-box text-[10px] bg-black/40 border-white/10 h-8 flex-grow" /><input id="gen-end" type="number" placeholder="End Ep" class="input-box text-[10px] bg-black/40 border-white/10 h-8 w-16 text-center" /><button type="button" onclick="generateEpisodes()" class="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold px-3 rounded transition">Gen</button></div></div><div class="border-t border-white/10 pt-2"><p class="text-[9px] text-gray-400 mb-1">Tool 2: Bulk Extractor (Paste Database/Messy text)</p><textarea id="bulk-text" placeholder="Paste text here..." class="input-box text-[10px] bg-black/40 border-white/10 h-16 w-full mb-2"></textarea><button type="button" onclick="extractLinks()" class="w-full bg-green-700 hover:bg-green-600 text-white text-xs font-bold py-2 rounded transition">Force Extract Links</button></div><script dangerouslySetInnerHTML={{__html: `function generateEpisodes() { const urlTpl = document.getElementById('gen-url').value; const season = document.getElementById('gen-season').value; const start = parseInt(document.getElementById('gen-start').value) || 1; const end = parseInt(document.getElementById('gen-end').value); const textarea = document.getElementsByName('episodeList')[0]; if (!urlTpl || !end) { alert("URL & End Number Required!"); return; } let placeholder = ""; if (urlTpl.includes("***")) placeholder = "***"; else if (urlTpl.includes("$$")) placeholder = "$$"; else if (urlTpl.includes("XX")) placeholder = "XX"; if (!placeholder) { alert("Use *** or $$ in URL!"); return; } let result = textarea.value ? textarea.value + "\\n" : ""; for (let i = start; i <= end; i++) { const numStr = i < 10 ? '0' + i : '' + i; const finalUrl = urlTpl.replace(placeholder, numStr); result += \`Season \${season} | Ep.\${i} | \${finalUrl}\\n\`; } textarea.value = result.trim(); } function extractLinks() { try { let rawText = document.getElementById('bulk-text').value; const season = document.getElementById('gen-season').value; let epCount = parseInt(document.getElementById('gen-start').value) || 1; const textarea = document.getElementsByName('episodeList')[0]; if (!rawText) { alert("Paste some text first!"); return; } rawText = rawText.replace(/[\\x00-\\x1F\\x7F]/g, "\\n"); const urlRegex = /(https?:\\/\\/[^\\n\\r]+?\\.(?:txt|mp4|mkv|m3u8|avi|mov|flv|webm|ts|mpg|mpeg))/gi; const foundLinks = rawText.match(urlRegex); if (!foundLinks || foundLinks.length === 0) { alert("No valid file links found!"); return; } let processedLinks = foundLinks.map(link => { try { return decodeURIComponent(link).trim(); } catch (e) { return link.trim(); } }); const uniqueLinks = [...new Set(processedLinks)]; uniqueLinks.sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' })); let result = textarea.value ? textarea.value + "\\n" : ""; let addedCount = 0; uniqueLinks.forEach(url => { let cleanUrl = url; if (cleanUrl.endsWith('.')) cleanUrl = cleanUrl.slice(0, -1); if (cleanUrl.length < 15) return; result += \`Season \${season} | Ep.\${epCount} | \${cleanUrl}\\n\`; epCount++; addedCount++; }); textarea.value = result.trim(); document.getElementById('bulk-text').value = ''; alert(\`Success! Extracted \${addedCount} links!\`); } catch (e) { alert("Error: " + e.message); } } function clearEpisodes() { if(confirm('Clear all episodes?')) { document.getElementsByName('episodeList')[0].value = ''; } }`}} /></div>
                                <div class="p-3 bg-black/40 rounded-xl border border-white/10"><label class="text-[10px] text-yellow-500 uppercase font-bold mb-1 block">Series Episodes</label><textarea name="episodeList" placeholder="S1 | Ep.1 | https://..." rows={3} class="input-box w-full font-mono text-xs whitespace-pre overflow-x-auto bg-[#111]">{epString}</textarea></div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2"><input name="downloadUrl" placeholder="DL Link 1" value={editMovie?.downloadUrl} class="input-box w-full text-xs border-green-900/30 focus:border-green-500 bg-black/40" /><input name="downloadUrl2" placeholder="DL Link 2" value={editMovie?.downloadUrl2} class="input-box w-full text-xs border-green-900/30 focus:border-green-500 bg-black/40" /></div>
                                <div class="p-3 bg-black/40 rounded-xl border border-white/10"><label class="text-[10px] text-gray-500 uppercase font-bold">Backup Server</label><input name="streamUrl2" placeholder="Stream URL 2" value={editMovie?.streamUrl2} class="input-box w-full mt-1 bg-[#111]" /></div>
                                <div class="relative"><div class="flex justify-between items-end mb-1"><label class="text-[10px] text-gray-500 uppercase font-bold">Synopsis / Description</label><button type="button" onclick="generateAIDesc()" class="text-[10px] bg-purple-600 hover:bg-purple-500 text-white px-2 py-1 rounded flex items-center gap-1 transition"><i class="fa-solid fa-robot"></i> Auto Gen (AI)</button></div><textarea id="desc-box" name="description" placeholder="Description..." class="input-box w-full h-24 bg-black/40 border-white/10">{editMovie?.description}</textarea><div id="ai-loader" class="absolute inset-0 bg-black/80 hidden items-center justify-center rounded-lg z-10"><div class="text-purple-500 font-bold text-xs animate-pulse">Thinking...</div></div></div><script dangerouslySetInnerHTML={{__html: `async function generateAIDesc() { const title = document.getElementsByName('title')[0].value; const type = document.getElementsByName('category')[0].value; const year = document.getElementsByName('year')[0].value; const box = document.getElementById('desc-box'); const currentDesc = box.value; if(!title) { alert("Please enter Movie Title first!"); return; } const loader = document.getElementById('ai-loader'); loader.classList.remove('hidden'); loader.classList.add('flex'); try { const res = await fetch('/admin/api/generate-desc', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'title=' + encodeURIComponent(title) + '&type=' + encodeURIComponent(type) + '&year=' + encodeURIComponent(year) + '&currentDesc=' + encodeURIComponent(currentDesc) }); const data = await res.json(); if(data.error) { alert(data.error); } else { box.value = data.desc; } } catch(e) { alert("Error generating description"); } finally { loader.classList.add('hidden'); loader.classList.remove('flex'); } }`}} /><label class="flex items-center gap-2 p-3 rounded-xl bg-slate-700/50 border border-white/10 cursor-pointer hover:bg-slate-700 transition my-2"><input type="checkbox" name="updateTime" class="accent-yellow-500 w-4 h-4" /><span class="text-xs font-bold text-gray-300">Move to Top / ထိပ်ဆုံးသို့ရွှေ့မည်</span></label><div class="grid grid-cols-2 gap-3 pt-2"><button type="submit" name="saveType" value="draft" class="py-3 rounded-xl font-bold text-xs bg-slate-700 text-gray-300 hover:bg-slate-600 border border-white/10 transition"><i class="fa-solid fa-floppy-disk"></i> Save Draft</button><button type="submit" name="saveType" value="publish" class="py-3 rounded-xl font-bold text-xs bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-900/20 transition">{editMovie ? "Update Live" : "Publish Now"}</button></div>
                            </form>
                        </div>
                        <div class="lg:col-span-2 bg-slate-800 p-5 rounded-2xl border border-white/10 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw] shadow-xl">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <input oninput="filterMovies(this.value)" placeholder="Search..." class="bg-black/40 border border-white/10 rounded-lg px-4 py-2 text-xs w-48 outline-none" />
                            </div>
                            <details class="mb-4 p-3 bg-slate-900 rounded-xl border border-white/10 cursor-pointer text-xs">
                                ...
                                <form action="/admin/movie/bulk-import" method="post" class="mt-3">
                                    <textarea name="json" placeholder='[{"title":"Movie1","posterUrl":"...","streamUrl":"..."},{"title":"Movie2"}]' class="w-full h-32 bg-black/40 border border-white/10 rounded-lg p-2 font-mono text-xs"></textarea>
                            ...
                            <form action="/admin/movie/bulk-delete" method="post" id="bulk-form">
                                ...
                                <div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scroll h-[calc(100%-80px)]">
                                    {movies.map(m => (
                                        <div class="movie-item flex gap-4 p-3 rounded-xl items-center border bg-black/20 border-white/5 hover:border-white/20 transition" data-title={m.title}>
                ...
                <div id="tab-topups" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl">
                        ...
                        <div class="space-y-4">
                            {topups.map(t => (
                                <div class={`p-4 rounded-xl border flex justify-between items-center ${t.status === 'pending' ? 'bg-black/40 border-blue-500/30' : 'bg-slate-900 border-white/5 opacity-60'}`}>
                                    <div>
                                        <div class="flex items-center gap-2 mb-1">
                                            <span class="font-bold text-white text-lg">{t.amount} Ks</span>
                                            <span class="text-xs bg-slate-700 px-2 py-0.5 rounded text-gray-400 uppercase">{t.method}</span>
                ...
                <div id="tab-users" class="tab-content"><div class="max-w-4xl mx-auto"><div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6"><div class="bg-gradient-to-br from-blue-900/20 to-black border border-blue-500/20 p-6 rounded-2xl flex items-center gap-5 shadow-lg"><div class="w-14 h-14 bg-blue-500/10 rounded-full flex items-center justify-center text-blue-400 text-3xl shadow-inner"><i class="fa-solid fa-users"></i></div><div><h3 class="text-xs uppercase text-blue-400 font-bold tracking-widest mb-1">Total Users</h3><p class="text-4xl font-black text-white tracking-tight">{totalUsers}</p></div></div><div class="bg-slate-800 border border-white/5 rounded-2xl overflow-hidden h-64 flex flex-col shadow-lg"><h3 class="bg-black/50 px-5 py-3 text-xs font-bold text-gray-400 border-b border-white/5 flex justify-between backdrop-blur-sm"><span>User Control</span><span>IP / Action</span></h3><div class="overflow-y-auto custom-scroll p-3 space-y-2">{userList.map(u => { const isVip = u.expiryDate && new Date(u.expiryDate) > new Date(); return (<div class="flex justify-between items-center bg-black/40 p-2.5 rounded-xl border border-white/5 hover:border-white/20 transition"><div class="flex items-center gap-3"><div class={`w-2.5 h-2.5 rounded-full shadow ${u.isBanned ? 'bg-red-600 shadow-red-500/50' : (isVip ? 'bg-green-500 shadow-green-500/50' : 'bg-gray-500')}`}></div><div><span class={`font-bold block text-xs ${u.isBanned ? 'text-red-500 line-through' : 'text-gray-300'}`}>{u.username}</span><span class="text-[9px] text-gray-600 font-mono">{u.lastLoginIp || u.ip || "Unknown"}</span></div></div><div class="flex items-center gap-2"><span class={isVip ? "text-yellow-500 font-black text-[9px] bg-yellow-500/10 px-1.5 py-0.5 rounded" : "hidden"}>VIP</span><form action="/admin/user/toggle-ban" method="post"><input type="hidden" name="username" value={u.username} /><button class={`px-3 py-1 rounded-lg text-[9px] font-bold border transition ${u.isBanned ? 'bg-green-500/10 text-green-500 border-green-500/50 hover:bg-green-500 hover:text-black' : 'bg-red-500/10 text-red-500 border-red-500/50 hover:bg-red-500 hover:text-white'}`}>{u.isBanned ? "UNBAN" : "BAN"}</button></form></div></div>) })}</div></div></div><div class="grid md:grid-cols-2 gap-6"><div class="bg-slate-800 p-6 rounded-2xl border border-yellow-500/20 shadow-xl h-fit relative overflow-hidden"><div class="absolute top-0 right-0 p-4 opacity-5"><i class="fa-solid fa-crown text-8xl text-yellow-500"></i></div><h2 class="font-bold mb-5 text-yellow-500 text-lg flex items-center gap-2 relative z-10"><i class="fa-solid fa-circle-plus"></i> Manual VIP Top-up</h2><form action="/admin/user/add-vip" method="post" class="space-y-4 relative z-10"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Username</label><input name="username" placeholder="Enter username..." required class="input-box bg-black/40 border-white/10" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Duration (Days)</label><input type="number" name="days" placeholder="e.g. 30" required class="input-box bg-black/40 border-white/10" /></div><button class="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold w-full py-3 rounded-xl hover:brightness-110 transition shadow-lg shadow-yellow-500/20">Add VIP Time</button></form></div><div class="bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl h-fit relative overflow-hidden"><div class="absolute top-0 right-0 p-4 opacity-5"><i class="fa-solid fa-lock text-8xl text-blue-500"></i></div><h2 class="font-bold mb-5 text-blue-500 text-lg flex items-center gap-2 relative z-10"><i class="fa-solid fa-user-lock"></i> Reset Password</h2><form action="/admin/user/reset" method="post" class="space-y-4 relative z-10"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Username</label><input name="username" placeholder="Enter username..." required class="input-box bg-black/40 border-white/10" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">New Password</label><input name="newpass" placeholder="Enter new password..." required class="input-box bg-black/40 border-white/10" /></div><button class="bg-blue-600 text-white font-bold w-full py-3 rounded-xl hover:bg-blue-500 transition shadow-lg shadow-blue-500/20">Reset Password</button></form></div></div></div></div>

                <div id="tab-requests" class="tab-content"><div class="max-w-3xl mx-auto bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl"><h2 class="font-bold mb-6 text-pink-500 flex items-center gap-2"><i class="fa-solid fa-clapperboard"></i> Movie Requests ({requests.length})</h2><div class="space-y-3">{requests.map(r => (<div class="bg-black/40 p-4 rounded-xl flex justify-between items-center border border-white/5 hover:border-white/20 transition"><div><h3 class="font-bold text-lg text-white">{r.movieName}</h3><p class="text-xs text-gray-500 mt-1">Requested by <span class="text-blue-400 bg-blue-400/10 px-1.5 py-0.5 rounded font-bold">{r.username}</span> • {new Date(r.timestamp).toLocaleDateString()}</p></div><form action={`/admin/request/delete/${r.id}`} method="post"><button class="text-red-500 hover:text-red-400 p-2 transition bg-red-500/10 rounded-lg hover:bg-red-500 hover:text-white"><i class="fa-solid fa-check"></i> Done</button></form></div>))}</div></div></div>
                
                <div id="tab-logs" class="tab-content">
                    <div class="max-w-4xl mx-auto bg-slate-800 p-6 rounded-2xl border border-white/5 shadow-xl">
                        <h2 class="font-bold mb-4 text-gray-300">System Logs</h2>
                        <div class="space-y-2 h-[500px] overflow-y-auto custom-scroll p-2 bg-black/40 rounded-xl">
                            {logs.map(l => (
                                <div class="text-xs font-mono border-b border-white/5 pb-2 mb-2">
                ...
                <div id="tab-config" class="tab-content"><div class="max-w-xl mx-auto bg-slate-800 p-6 rounded-2xl border border-white/5 space-y-8 shadow-xl"><div class="bg-red-900/10 border border-red-500/30 p-4 rounded-xl flex justify-between items-center"><div><h3 class="font-bold text-red-500 flex items-center gap-2"><i class="fa-solid fa-screwdriver-wrench"></i> Maintenance Mode</h3><p class="text-[10px] text-gray-400">Turn ON to hide website from users.</p></div><form action="/admin/config/maintenance" method="post"><input type="hidden" name="status" value={config.maintenanceMode ? "off" : "on"} /><button class={`px-4 py-2 rounded-lg font-bold text-xs transition ${config.maintenanceMode ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-700 text-gray-400'}`}>{config.maintenanceMode ? "🔴 ON (Hidden)" : "⚪️ OFF (Visible)"}</button></form></div><div><h2 class="font-bold mb-4 text-gray-300 flex items-center gap-2"><i class="fa-solid fa-bullhorn"></i> Announcement</h2><form action="/admin/config" method="post" class="space-y-4"><div><input name="text" placeholder="Enter message..." value={config.announcement} class="input-box bg-black/40 border-white/10" /></div><label class="flex items-center gap-3 p-4 bg-black/20 rounded-xl border border-white/5 cursor-pointer hover:border-white/20 transition"><input type="checkbox" name="show" checked={config.showAnnouncement} class="accent-yellow-500 w-5 h-5" /><span class="font-bold text-sm text-gray-300">Show Announcement Bar</span></label><button class="btn-primary w-full shadow-lg">Save Changes</button></form></div><div class="border-t border-white/10 pt-8"><h2 class="font-bold mb-4 text-yellow-500 flex items-center gap-2"><i class="fa-solid fa-image"></i> Popup Ads / Promotion</h2><form action="/admin/config/popup" method="post" class="space-y-4"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Image URL</label><input name="popupImage" placeholder="https://..." value={config.popupImage || ""} class="input-box bg-black/40 border-white/10" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Message / Description (Optional)</label><textarea name="popupMessage" placeholder="Write your promotion text here..." class="input-box bg-black/40 border-white/10 h-20 text-xs">{config.popupMessage || ""}</textarea></div><div class="grid grid-cols-2 gap-3"><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Button Text</label><input name="popupBtnText" placeholder="e.g. Buy VIP Now" value={config.popupBtnText || "Check it out"} class="input-box bg-black/40 border-white/10" /></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Link URL</label><input name="popupLink" placeholder="/profile" value={config.popupLink || "/profile"} class="input-box bg-black/40 border-white/10" /></div></div><div><label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Target Audience</label><select name="popupTarget" class="input-box bg-black/40 border-white/10"><option value="all" selected={config.popupTarget !== "free"}>Show to EVERYONE (VIP + Free)</option><option value="free" selected={config.popupTarget === "free"}>Show to FREE Users Only (Hide for VIP)</option></select></div><label class="flex items-center gap-3 p-4 bg-black/20 rounded-xl border border-white/5 cursor-pointer hover:border-white/20 transition"><input type="checkbox" name="showPopup" checked={config.showPopup} class="accent-yellow-500 w-5 h-5" /><span class="font-bold text-sm text-gray-300">Active Popup (Enable)</span></label><button class="btn-primary w-full shadow-lg">Save Popup Settings</button></form></div><div class="border-t border-white/10 pt-8"><h2 class="font-bold mb-2 text-green-500 flex items-center gap-2"><i class="fa-solid fa-gift"></i> Global VIP Event</h2><p class="text-xs text-gray-500 mb-4">Give VIP access to ALL users until a specific date.</p><div class="bg-black/20 p-4 rounded-xl border border-white/5 mb-4 flex justify-between items-center"><span class="text-xs font-bold text-gray-500 uppercase">Status</span> <span class="text-lg font-black text-white">{vipDate}</span></div><form action="/admin/config/vip" method="post" class="flex gap-3"><input type="number" name="days" placeholder="Days" required class="input-box bg-black/40 border-white/10 w-24 text-center" /><button class="bg-green-600 text-white font-bold px-4 py-2 rounded-xl hover:bg-green-500 flex-grow shadow-lg shadow-green-900/20">Start Event</button></form><form action="/admin/config/vip-clear" method="post" class="mt-3 text-right"><button class="text-xs text-red-500 hover:text-red-400 font-bold bg-red-500/10 px-3 py-1.5 rounded-lg border border-red-500/20 hover:bg-red-500 hover:text-white transition">End Event</button></form></div><div class="border-t border-white/10 pt-8"><h2 class="font-bold mb-2 text-purple-500 flex items-center gap-2"><i class="fa-solid fa-database"></i> Database</h2><p class="text-xs text-gray-500 mb-4">Re-sync search index if movies are missing.</p><form action="/admin/config/reindex" method="post"><button class="bg-purple-900/30 text-purple-400 border border-purple-500/30 font-bold px-6 py-3 rounded-xl hover:bg-purple-600 hover:text-white w-full transition">Re-Sync Database</button></form></div></div></div>
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    try {
        ...
        if (description) {
            description = description.replace(/&amp;nbsp;/g, " ").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/<[^>]*>?/gm, "").replace(/\s+/g, " ").trim();
        }
        ...
        const movie = { 
            ...body, title, description, id, createdAt: timeToSave, episodes, price, duration 
        }; 
...
app.post("/admin/drafts/publish-all", adminGuard, async (c) => { 
    ...
    for (const m of drafts) { 
        if (m.description) {
            m.description = m.description.replace(/\\*&quot;/g, '"').replace(/\\+"/g, '"').replace(/\\/g, "").replace(/&amp;nbsp;/g, " ").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/<[^>]*>?/gm, "").replace(/\s+/g, " ").trim();
        }
    ...
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=All Drafts Published Sequentially!"); 
});
...
app.get("/admin/backup", adminGuard, async (c) => { 
    const stream = new ReadableStream({ 
        async start(controller) { 
            ...
            try {
                ...
                let first = true; 
                const targetPrefixes = [["config"],["movies"],["users"],["keys"],["drafts"],["requests"],["topups"],["banned_ips"],["counts"],["idx_time"],["idx_cat"]];
    ...
    return new Response(stream, { 
        headers: { "Content-Type": "application/json", "Content-Disposition": `attachment; filename="backup_full_${Date.now()}.json"` } 
    }); 
});
