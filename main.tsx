           ...
           <div class="p-6">
               ...
               {movie.category === "Series" && episodes.length > 0 && premium && ( 
                   <div class="mb-8">
                       ...
                       <div class="flex overflow-x-auto gap-3 pb-2 mb-3 scrollbar-hide">
                           {Object.keys(seasons).map((season, idx) => {
                               const safeId = season.replace(/\s+/g, '-');
                               // 🔥 NOTE: ဘယ် Season မှ Active မပေးထားပါ (အားလုံး မီးခိုးရောင်)
                               return (
                                   <button 
                                       id={`btn-${safeId}`}
                       ...
                       {/* 2. EPISODES GRID (Initially All Hidden) */}
                       <div class="bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3 min-h-[50px] flex items-center justify-center">
                           
                           {/* အစမှာ ဘာမှမရွေးရသေးရင် ဒီစာမပေါ်ဘူး၊ နေရာလွတ်ပဲ ကျန်နေမယ်၊ ခလုတ်နှိပ်မှ Content ပေါ်မယ် */}
                           
                           {Object.keys(seasons).map((season, idx) => {
                               const safeId = season.replace(/\s+/g, '-');
                               return (
                                   // 🔥 NOTE: အားလုံးကို class="hidden" ပေးထားပါတယ်
                                   <div id={`ep-grid-${safeId}`} class="season-content hidden w-full">
