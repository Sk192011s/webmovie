      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.switchSeason = function(seasonId) {
                 ...
                 document.getElementById('ep-grid-' + seasonId).classList.remove('hidden');
           ...
           <div class="p-6">
               ...
               {movie.category === "Series" && episodes.length > 0 && premium && ( 
                   <div class="mb-8">
                       ...
                       {/* 1. SEASON TABS (Horizontal Scroll) */}
                           ...
                           {Object.keys(seasons).map((season, idx) => {
                               ...
                               return (
                                   <button 
                                       ...
                                       class={`season-tab-btn flex-shrink-0 px-5 py-2 rounded-full text-xs font-bold transition border border-zinc-800 ${isActive ? 'bg-yellow-500 text-black' : 'bg-zinc-800 text-gray-400 hover:text-white'}`}
                                   >
                                       {season}
                                   </button>
                               )
                       ...
                       {/* 2. EPISODES GRID (Scrollable Box) */}
                       <div class="bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3">
                           {Object.keys(seasons).map((season, idx) => {
                               const safeId = season.replace(/\s+/g, '-');
                               const isActive = idx === 0;
                               return (
                                   <div id={`ep-grid-${safeId}`} class={`season-content ${isActive ? '' : 'hidden'}`}>
