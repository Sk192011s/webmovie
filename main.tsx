...
app.get("/api/mediafire-stream", async (c) => {
    ...
    try {
        // ၁။ ဝင်လာတဲ့ Link မှာ proxy.avotc.tk ပါနေရင် ခဏဖယ်မယ် (Link အမှန်ကို အရင်ယူမယ်)
        let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();

        // ၂။ Deno Server ကနေ MediaFire စာမျက်နှာကို လှမ်းဖတ်မယ် (Direct Link ရှာဖို့)
        // Deno Server တွေက US မှာရှိလို့ ဒီအဆင့်မှာ VPN မလိုပါဘူး
        ...
        // ဝင်လာတဲ့ Link က Video File တန်းဖြစ်နေရင် (ဥပမာ .mp4 နဲ့ဆုံးရင်)
        const contentType = res.headers.get("content-type") || "";
        if (contentType.includes("video") || contentType.includes("octet-stream")) {
            // proxy.avotc.tk ခံပြီး User ဆီ ပြန်လွှတ်လိုက်မယ်
            return c.redirect("https://proxy.avotc.tk/" + cleanUrl, 302);
        }

        // ၃။ Page ဖြစ်နေရင် HTML ထဲက Direct Download Link ကို ရှာမယ်
        ...
        if (match && match[1]) {
            ...
            // ၄။ အရေးကြီးဆုံးအဆင့် - ရလာတဲ့ Direct Link ကိုမှ proxy.avotc.tk ခံပြီး Redirect လုပ်မယ်
            // ဒါမှ VPN မလိုဘဲ တန်းကြည့်လို့ရမှာပါ
            return c.redirect("https://proxy.avotc.tk/" + directLink, 302);
        } else {
            // ရှာမတွေ့ရင် မူရင်း Link ကိုပဲ Proxy ခံလွှတ်မယ် (ကံသေကံမ)
            return c.redirect("https://proxy.avotc.tk/" + cleanUrl, 302);
        }

    } catch (e) {
        console.error(e);
        // Error တက်ရင်လည်း Proxy ခံပြီး ပြန်လွှတ်မယ်
