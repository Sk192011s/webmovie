...
async function checkRateLimit(ip: string): Promise<boolean> {
  ...
  const res = await kv.get<{ count: number }>(key);
  const count = res.value?.count || 0;
  if (count >= 50) return false; // Increased limit for live search
...
// =======================
// 4. UI LAYOUT (MODERN SEARCH & CLEAN UI)
// =======================
                         ...
                         try {
                             ...
                             if(data.results.length === 0) {
                                 ...
                                 resultsContainer.innerHTML = data.results.map(m => \`
                                     <a href="/movie/\${m.id}" class="block rounded-xl overflow-hidden glass-panel aspect-[2/3] relative group active:scale-95 transition">
                                         ...
                                         <div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/90 to-transparent">
                                             <p class="text-[10px] font-bold text-white truncate">\${m.title}</p>
                                         </div>
                                     </a>
                                 \`).join('');
                             }
