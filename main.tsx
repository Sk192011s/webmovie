...
// 🔥 AI Description Generator (IMDb/TMDB Based)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    const { title, type, year } = await c.req.parseBody(); // 🔥 year လက်ခံမယ်
    const apiKey = Deno.env.get("GEMINI_API_KEY");

    if (!apiKey) return c.json({ error: "Server Error: GEMINI_API_KEY not set." });
    if (!title) return c.json({ error: "Title is required" });

    // 🔥 PROMPT အသစ် (IMDb ကို အခြေခံပြီး ဘာသာပြန်ခိုင်းခြင်း)
    const prompt = `
    Act as a professional movie critic and translator.
    1. Identify the specific ${type || "movie"} titled "${title}" released in ${year || "recent years"}.
    2. Retrieve its official plot summary from reliable sources like IMDb or TMDB.
    3. Translate that summary into natural, engaging Burmese (Myanmar Unicode).
    4. Ensure the translation matches the actual story of the movie "${title} (${year})".
    5. Do not include spoilers.
    6. Output ONLY the Burmese translation (no English text).
    `;
