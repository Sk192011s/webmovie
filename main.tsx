                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                {/* TOOL 2: BULK EXTRACTOR (IMPROVED FOR DATABASE FILES 🔥) */}
                                    ...
                                    <script dangerouslySetInnerHTML={{__html: `
                                        ...
                                        // Tool 2: Bulk Extractor (IMPROVED REGEX 🚀)
                                        function extractLinks() {
                                            ...
                                            // 🔥 အဓိက ပြင်လိုက်တဲ့နေရာ (Binary/Symbols တွေကို ရှောင်ပြီး Link သန့်သန့်ပဲ ယူမယ်)
                                            // http (သို့) https နဲ့စမယ်... ပြီးရင် အင်္ဂလိပ်စာလုံး၊ ဂဏန်း၊ အစက်၊ မျဉ်းစောင်းတွေပဲ ပါရမယ်
                                            // သင်္ကေတအထူးအဆန်းတွေ တွေ့တာနဲ့ Link ဆုံးပြီလို့ သတ်မှတ်မယ်
                                            ...
                                            // Link တူတာတွေပါနေရင် ဖယ်ထုတ်မယ် (Duplicate Remove)
                                            const uniqueLinks = [...new Set(foundLinks)];

                                            let result = textarea.value ? textarea.value + "\\n" : "";

                                            uniqueLinks.forEach(url => {
                                                // Link အဆုံးမှာ အစက် (.) ပါနေရင် ဖြုတ်ပစ်မယ် (SQLite မှာ တခါတလေ ပါလာတတ်လို့)
                                                let cleanUrl = url;
                                            ...
                                            alert(\`Success! Extracted \${uniqueLinks.length} unique links.\`);
                                        }
