...
// LOGIN ROUTE (UI ပြင်ဆင်ပြီး)
app.post("/login", async (c) => { 
    // Rate Limit Check
    const ip = c.req.header("x-forwarded-for") || "unknown";
    
    // 🔥 ဒီနေရာမှာ UI လှလှလေး ပြင်လိုက်တာပါ 🔥
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-zinc-900 min-h-screen">
                ...
                {/* TAB 3: USERS (🔥 UPDATED WITH STATS 🔥) */}
                <div id="tab-users" class="tab-content">
                    <div class="max-w-4xl mx-auto">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                             ...
                             {/* User List Box */}
                             <div class="bg-[#1f1f1f] border border-zinc-700 rounded-xl overflow-hidden h-32 flex flex-col">
                                 <h3 class="bg-black/50 px-4 py-2 text-xs font-bold text-gray-400 border-b border-zinc-800">Latest Users</h3>
                                 <div class="overflow-y-auto custom-scroll p-2 space-y-1">
                                     {userList.map(u => {
                                         const isVip = u.expiryDate && new Date(u.expiryDate) > new Date();
                                         return (
                                            <div class="flex justify-between items-center bg-black/50 p-1.5 rounded text-xs">
                                                <span class="text-gray-300 font-bold truncate">{u.username}</span>
                                                <span class={isVip ? "text-yellow-500 font-bold" : "text-gray-600"}>{isVip ? "VIP" : "FREE"}</span>
                                            </div>
                                         )
                                     })}
                                 </div>
