    ...
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 HYBRID SMART PLAYER (Resolve URL -> Proxy Check -> Force Iframe or Video)
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 3. Resolve URL (Ignore if Proxy)
                if (type === 'direct' && !content.includes('proxy.avotc.tk')) { 
                    try { 
                        const res = await fetch('/api/resolve-url?token=' + content); 
                        const data = await res.json(); 
                        if (data.url) finalUrl = data.url; 
                    } catch (e) { console.error("Link Error", e); } 
                } else if (type === 'direct' && content.includes('proxy.avotc.tk')) {
                     // Proxy Link - Extract ID if stored as token or use directly if it's already a URL
                     if(!content.startsWith('http')) finalUrl = '/stream/' + content;
                }

                // 4. Force Iframe for Proxy/Embed
                if (type === 'embed' || finalUrl.includes('<iframe') || finalUrl.includes('proxy.avotc.tk')) { 
                    ...
                    // Standard Video Tag with Fallback
