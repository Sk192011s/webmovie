...
const i18n: any = {
    en: {
        ...
        see_all: "See All", views: "Views", dl_help: "Download Guide",
        server1: "Server 1", server2: "Server 2", share: "Share",
        unlock: "Unlock VIP", vip_only: "VIP Only",
        latest_users: "Latest Users", total_users: "Total Users",
        access_denied: "Access Denied", ip_banned: "Your IP is banned.",
        security_alert: "Security Alert", wait: "Please Wait"
    },
    my: {
        home: "ပင်မ", saved: "သိမ်းဆည်း", request: "တောင်းဆို", login: "အကောင့်", me: "ပရိုဖိုင်",
...
function getLang(c: any) {
    return getCookie(c, "app_lang") || "en"; // 🔥 Default English
}
...
// 6. UI LAYOUT
// =======================
const Layout = (props: { children: any; title?: string; user?: User | null; hideNav?: boolean; announcement?: string; isAdmin?: boolean; coverUrl?: string; lang?: string }) => {
    ...
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        .h-scroll-item.wide { width: 240px; }
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                document.querySelectorAll('.srv-btn').forEach(b => {
                    b.classList.remove('bg-red-600', 'text-white', 'border-transparent');
                    b.classList.add('bg-white', 'text-black');
                });
                if (btnElement) {
                    btnElement.classList.remove('bg-white', 'text-black');
                    btnElement.classList.add('bg-red-600', 'text-white', 'border-transparent');
                }
                ...
                // Direct Link ဖြစ်ရင် Token ဖြည်မယ်
                if (type === 'direct') { 
                    try { 
                ...
                if (type === 'embed' || finalUrl.includes('<iframe')) { 
                    ...
                    // 🔥 HTML5 Player (Auto-Resume ထည့်ထားသည်)
                ...
                if(video) { 
                    ...
                    // ၂။ Error တက်သွားရင် (Connection ပြတ်ရင်) Auto ပြန်ဆက်မယ်
                    video.addEventListener('error', async () => {
                        console.log("Video Error! Attempting to reconnect...");
                        const currentTime = video.currentTime; // ကြည့်လက်စ အချိန်မှတ်မယ်
                        video.load(); // ပြန် Load လုပ်မယ်
                        video.currentTime = currentTime; // နေရာဟောင်း ပြန်ရွှေ့မယ်
                        try { await video.play(); } catch(e) {} // ပြန် Play မယ်
                    });
      ...
      {!props.hideNav && (
        <nav class="sticky top-0 z-40 bg-black/95 border-b border-white/10 px-4 py-3 shadow-lg">
          <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="flex gap-4 items-center text-xs font-bold text-gray-400">
              {props.isAdmin ? (
                 <>
                 ...
                 <>
                   <a href="/" class="hover:text-white">{t.home}</a>
                   <a href="/favorites" class="hover:text-red-500">{t.saved}</a>
                   <a href="/request" class="hover:text-yellow-500">{t.request}</a>
                   {props.user ? <a href="/profile" class="text-white">{t.me}</a> : <a href="/login">{t.login}</a>}
                   
                   {/* Language Switcher */}
                   <a href={l === 'en' ? '/lang/my' : '/lang/en'} class="bg-zinc-800 text-white px-2 py-1 rounded border border-zinc-700 hover:bg-zinc-700 transition flex items-center gap-1">
                       <i class="fa-solid fa-earth-asia"></i> {l === 'en' ? 'MY' : 'EN'}
                   </a>
                 </>
              )}
            </div>
...
app.get("/", async (c) => {
  ...
  return c.html(
    <Layout user={user} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang}>
        <div class="p-3 bg-black sticky top-[50px] z-30 shadow-md">
             <form action="/search" method="get" class="relative">
                 ...
                 <input name="q" placeholder={t.search_ph} class="w-full bg-[#1f1f1f] border border-zinc-800 rounded-full py-2 pl-10 pr-4 text-sm text-white focus:border-red-600 outline-none" />
             </form>
        </div>
      {sliderMovies.length > 0 && (<div class="slider-container">{sliderMovies.map((m, idx) => (<div class={`slide ${idx === 0 ? 'active' : ''}`}><img src={m.coverUrl} /><div class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/30"></div><div class="absolute bottom-4 left-4 right-4"><span class="bg-red-600 text-[10px] text-white px-2 py-0.5 rounded font-bold">{t.featured}</span><h1 class="text-xl md:text-3xl font-bold text-white drop-shadow-md truncate mt-1">{m.title}</h1><a href={`/movie/${m.id}`} class="mt-2 inline-flex items-center gap-2 bg-white text-black px-4 py-1.5 rounded font-bold text-sm"><i class="fa-solid fa-play"></i> {t.play}</a></div></div>))}</div>)}
      ...
      <div class="px-3 py-6 space-y-8">
          {sections.map(section => { 
              ...
              if(cat === "All Uncensored") { 
                  return (
                    <div>
                        <div class="flex justify-between items-end mb-3 px-1"><h2 class="text-lg font-bold text-white border-l-4 border-red-600 pl-2">{cat}</h2><a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1">{t.see_all} <i class="fa-solid fa-chevron-right text-[10px]"></i></a></div>
                        <div class="h-scroll-section custom-scroll">
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded overflow-hidden active:scale-95 transition-transform">
                                    <div class="aspect-video w-full relative"><img src={m.coverUrl || m.posterUrl} class="absolute inset-0 w-full h-full object-cover" /></div>
              ...
              return (
                <div>
                    ...
                    <div class="h-scroll-section custom-scroll">
                        {catMovies.map(m => (
                            <a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded overflow-hidden w-28 flex-shrink-0 active:scale-95 transition-transform">
                                {/* 🔥 IMAGE FIX: Forced Aspect Ratio 2:3 & Object Cover */}
                                <div class="aspect-[2/3] w-full relative"><img src={m.posterUrl} class="absolute inset-0 w-full h-full object-cover" /></div>
                                <div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white leading-tight">{m.title}</h3></div>
                            </a>
                        ))}
                    </div>
...
app.get("/movie/:id", async (c) => {
    ...
    return c.html(
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang}>
           ...
           <div class="p-4">
               ...
               {premium && (
                   <div class="flex flex-col gap-2 mb-6">
                        {/* Player Buttons with Color Toggle */}
                        {movie.category !== "Series" && (
                            <div class="flex gap-2 overflow-x-auto">
                                <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken : playerUrl}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn flex-1 bg-white text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-gray-200 whitespace-nowrap border border-zinc-700">
                                    <i class="fa-solid fa-play"></i> {movie.streamUrl2 ? t.server1 : t.play}
                                </button>
                                {movie.streamUrl2 && (
                                    <button onclick={`loadPlayer('${movie.linkType === 'direct' ? playbackToken2 : playerUrl2}', '${movie.linkType}', '${movie.id}', '${movie.title}', '${movie.posterUrl}', this)`} class="srv-btn flex-1 bg-white text-black font-bold py-3 px-4 rounded flex items-center justify-center gap-2 active:scale-95 transition hover:bg-white whitespace-nowrap border border-zinc-700">
                       ...
                       <button onclick="toggleHelp()" class="text-xs text-yellow-500 hover:text-yellow-400 flex items-center gap-1 mt-2"><i class="fa-solid fa-circle-question"></i> {t.dl_help}</button>
               ...
               {related.length > 0 && (<div class="pt-4 border-t border-zinc-800"><h3 class="font-bold text-white mb-4">You May Also Like</h3><div class="h-scroll-section custom-scroll">{related.map(m => (<a href={`/movie/${m.id}`} class="h-scroll-item block relative bg-[#1f1f1f] rounded overflow-hidden w-28 flex-shrink-0"><div class="aspect-[2/3] w-full relative"><img src={m.posterUrl} class="absolute inset-0 w-full h-full object-cover" /></div><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white">{m.title}</h3></div></a>))}</div></div>)}
           </div>
        </div>
      </Layout>
    );
...
app.get("/login", (c) => {
    const lang = getLang(c);
    const t = i18n[lang];
    return c.html(<Layout hideNav={true} lang={lang}><div class="min-h-screen flex items-center justify-center bg-black p-4"><div class="w-full max-w-sm"><h1 class="text-3xl font-black text-red-600 mb-8 text-center italic">GOLD FLIX</h1><form action="/login" method="post" class="bg-[#1f1f1f] p-6 rounded-lg border border-zinc-800 space-y-4 shadow-xl"><h2 class="text-xl font-bold text-white mb-2">{t.login}</h2><input name="username" placeholder="Username" required class="input-box" /><input type="password" name="password" placeholder="Password" required class="input-box" /><label class="flex items-center text-gray-400 text-xs"><input type="checkbox" name="remember" class="mr-2 accent-red-600" /> Remember Me (7 Days)</label><button class="btn-primary w-full mt-2">{t.login}</button><p class="text-xs text-gray-500 text-center mt-4">No account? <a href="/signup" class="text-white font-bold">Sign up</a></p></form></div></div></Layout>);
});
...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    const userList = []; 
    for await (const res of iterUsers) userList.push(res.value);
            ...
            <div class="p-4 bg-zinc-900 min-h-screen">
                ...
                <div id="tab-movies" class="tab-content active">
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-1 bg-[#1f1f1f] p-4 rounded border border-zinc-700 h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden">
                            <h2 class="font-bold mb-3 text-sm text-yellow-500 border-b border-zinc-700 pb-2">{editMovie ? "Edit Movie" : "Add New Movie"}</h2>
                            <form action="/admin/movie/save" method="post" class="space-y-3 text-sm w-full">
                                ...
                                <div class="space-y-2">
                                    <input name="title" placeholder="Movie Title" value={editMovie?.title} required class="input-box w-full" />
                                    <div class="flex gap-2 w-full">
                                        <select name="category" class="input-box flex-grow min-w-0">
                                            {["Movies","Series","Adult","All Uncensored"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}
                                        </select>
                                        <input name="year" value={editMovie?.year || "2025"} class="input-box w-20 text-center flex-shrink-0" />
                                    </div>
                                </div>
                                <input name="posterUrl" placeholder="Poster URL (Portrait)" value={editMovie?.posterUrl} required class="input-box w-full" />
                                <input name="coverUrl" placeholder="Cover URL (Landscape)" value={editMovie?.coverUrl} required class="input-box w-full border-yellow-500/50" />
                                <input name="fileSize" placeholder="File Size (e.g. 1.2 GB)" value={editMovie?.fileSize} class="input-box w-full" />
                                <div class="p-3 bg-black/50 rounded border border-zinc-800 space-y-2">
                                    <label class="text-[10px] text-gray-500 uppercase font-bold">Main Stream</label>
                                    <select name="linkType" class="input-box text-xs w-full py-2">
                                    ...
                                    <input name="streamUrl" placeholder="Stream URL (or Episode 1)" value={editMovie?.streamUrl} class="input-box w-full" />
                                </div>
                                <div class="p-3 bg-black/50 rounded border border-zinc-800">
                                    ...
                                    <textarea name="episodeList" placeholder="S1 | Ep.1 | https://..." rows={5} class="input-box w-full font-mono text-xs whitespace-pre overflow-x-auto">{epString}</textarea>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <input name="downloadUrl" placeholder="DL Link 1" value={editMovie?.downloadUrl} class="input-box w-full text-xs border-green-900/50 focus:border-green-500" />
                                    <input name="downloadUrl2" placeholder="DL Link 2" value={editMovie?.downloadUrl2} class="input-box w-full text-xs border-green-900/50 focus:border-green-500" />
                                </div>
                                <div class="p-3 bg-black/50 rounded border border-zinc-800">
                                    <label class="text-[10px] text-gray-500 uppercase font-bold">Backup Server (Opt)</label>
                                    <input name="streamUrl2" placeholder="Stream URL 2" value={editMovie?.streamUrl2} class="input-box w-full mt-1" />
                                </div>
                                <textarea name="description" placeholder="Movie Synopsis / Description..." class="input-box w-full h-24">{editMovie?.description}</textarea>
                                <button class="btn-primary w-full py-3 text-sm uppercase tracking-widest">{editMovie ? "Update Movie" : "Save Movie"}</button>
                                {editMovie && <a href={ADMIN_ROUTE + "/dashboard"} class="block text-center text-xs text-gray-400 mt-2 py-2">Cancel Edit</a>}
                            </form>
                        </div>
                        <div class="lg:col-span-2 bg-[#1f1f1f] p-4 rounded border border-zinc-700 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw]">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2">
                                <h2 class="font-bold text-sm">Library ({movies.length})</h2>
                                <input oninput="filterMovies(this.value)" placeholder="Search library..." class="bg-black border border-zinc-800 rounded px-3 py-2 text-xs w-full sm:w-48 outline-none focus:border-blue-500" />
                            </div>
                            <div class="space-y-2 flex-1 overflow-y-auto pr-1 custom-scroll">
                                {movies.map(m => (
                                    <div class="movie-item flex gap-3 p-2 bg-black rounded items-center group relative border border-zinc-800 hover:border-zinc-600 transition" data-title={m.title}>
                                        <img src={m.posterUrl} class="w-10 h-14 object-cover rounded flex-shrink-0" />
                                        ...
                                        <div class="flex gap-2 flex-shrink-0">
                                            <a href={`${ADMIN_ROUTE}/dashboard?edit=${m.id}`} class="text-blue-500 text-xs border border-blue-500/30 px-3 py-1.5 rounded hover:bg-blue-500/10">Edit</a>
                                            <form action={`/admin/movie/delete/${m.id}`} method="post" onsubmit="return confirm('Delete this movie?')">
                                                <button class="text-red-500 text-xs border border-red-500/30 px-3 py-1.5 rounded hover:bg-red-500/10">Del</button>
                                            </form>
                                        </div>
                                    </div>
                                ))}
                ...
                <div id="tab-users" class="tab-content">
                    <div class="max-w-4xl mx-auto">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                             <div class="bg-gradient-to-br from-blue-900/40 to-blue-900/10 border border-blue-500/30 p-6 rounded-xl flex items-center gap-4">
                                 <div class="w-12 h-12 bg-blue-500/20 rounded-full flex items-center justify-center text-blue-400 text-2xl"><i class="fa-solid fa-users"></i></div>
                                 <div><h3 class="text-xs uppercase text-blue-400 font-bold tracking-wider">Total Users</h3><p class="text-3xl font-black text-white">{totalUsers}</p></div>
                             </div>
                             <div class="bg-[#1f1f1f] border border-zinc-700 rounded-xl overflow-hidden h-64 flex flex-col">
                                 <h3 class="bg-black/50 px-4 py-2 text-xs font-bold text-gray-400 border-b border-zinc-800 flex justify-between"><span>User Control</span><span>IP / Action</span></h3>
                                 <div class="overflow-y-auto custom-scroll p-2 space-y-1">
                                     {userList.map(u => {
                                         const isVip = u.expiryDate && new Date(u.expiryDate) > new Date();
                                         return (
                                            <div class="flex justify-between items-center bg-black/50 p-2 rounded text-xs border border-zinc-800/50 hover:border-zinc-600 transition">
                                                <div class="flex items-center gap-2">
                                                    <div class={`w-2 h-2 rounded-full ${u.isBanned ? 'bg-red-600' : (isVip ? 'bg-green-500' : 'bg-gray-500')}`}></div>
                                                    <div>
                                                        <span class={`font-bold block ${u.isBanned ? 'text-red-500 line-through' : 'text-gray-300'}`}>{u.username}</span>
                                                        <span class="text-[9px] text-gray-500">{u.lastLoginIp || u.ip || "Unknown"}</span>
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <span class={isVip ? "text-yellow-500 font-bold text-[9px]" : "hidden"}>VIP</span>
                                                    <form action="/admin/user/toggle-ban" method="post">
                                                        <input type="hidden" name="username" value={u.username} />
                                                        <button class={`px-2 py-1 rounded text-[9px] font-bold border ${u.isBanned ? 'bg-green-900/30 text-green-500 border-green-900' : 'bg-red-900/30 text-red-500 border-red-900'}`}>{u.isBanned ? "UNBAN" : "BAN"}</button>
                                                    </form>
                                                </div>
                                            </div>
                                         )
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="bg-[#1f1f1f] p-6 rounded-xl border border-yellow-500/30 shadow-lg h-fit">
                                ...
                                <form action="/admin/user/add-vip" method="post" class="space-y-4">
                                    ...
                                    <button class="bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold w-full py-3 rounded-lg hover:brightness-110 transition shadow-lg shadow-yellow-500/20">Add VIP Time</button>
                                </form>
                            </div>
                            <div class="bg-[#1f1f1f] p-6 rounded-xl border border-zinc-700 shadow-lg h-fit">
                                ...
                                <form action="/admin/user/reset" method="post" class="space-y-4">
                                    ...
                                    <button class="bg-blue-600 text-white font-bold w-full py-3 rounded-lg hover:bg-blue-700 transition shadow-lg shadow-blue-600/20">Reset Password</button>
                                </form>
                            </div>
                        </div>
                    </div>
