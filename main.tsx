...
import { z } from "npm:zod"; // 🔥 SECURITY: Input Validation
...
const TopupSchema = z.object({
    ...
    transactionId: z.string().min(4, "Invalid Transaction ID"),
    method: z.string(),
    purpose: z.string()
});
...
interface User {
    ...
    securityQ?: string;
    securityA?: string;
    notifications?: { id: string; msg: string; date: number; read: boolean }[]; // 🔥 NOTIFICATIONS
}
...
// 🔥 OPTIMIZATION: wsrv.nl သုံးပြီး WebP ပြောင်းမည်၊ Size ချုံ့မည်
function optimizeImg(url: string, width: number = 400) {
    if (!url) return "";
    url = url.trim();

    // 1. Data URL (Base64) သို့မဟုတ် ကိုယ့်ဟာကိုယ် Proxy လုပ်ပြီးသားဆိုရင် ဒီအတိုင်းထားမယ်
    if (url.startsWith("data:") || url.includes("wsrv.nl")) return url;

    // 2. wsrv.nl ကို သုံးပြီး ပုံကို Optimize လုပ်မယ်
    // w = width (ဆိုဒ်အကျယ်)
    // q = quality (ပုံအရည်အသွေး 80%)
    // output = webp (WebP ဖောင်းပြောင်းမယ်)
...
async function checkAdminRateLimit(ip: string): Promise<boolean> {
    ...
    if (count >= 5) return false; // 5 attempts per 15 mins
    return true;
}
...
// 🔥 DATA: SOFT DELETE IMPLEMENTATION
async function softDeleteMovieDB(id: string) {
    ...
    // Remove from active indexes so it doesn't show in search/home
...
async function restoreMovieDB(id: string) {
    ...
    const m = res.value;
    m.isDeleted = false;
    await saveMovieDB(m); // Re-index everything
}
...
async function getLatestMovies(limit: number = 20) {
    ...
    const iter = kv.list<MovieSummary>({ prefix: ["idx_time"] }, { reverse: true, limit: limit + 10 }); // Fetch slightly more to filter
    const movies = []; 
...
async function searchMoviesDB(query: string) {
    ...
    const iter = kv.list({ start: startKey, end: endKey }, { limit: 100 }); // Limit Memory Usage
...
async function reIndexDatabase() {
    ...
    // 2. Category အဟောင်းစာရင်းတွေကို အရင်ဖျက်မယ်
    ...
    // 3. ဇာတ်ကားတွေကို Batch နဲ့ ပြိုင်တူပြန်စီမယ် (Timeout မဖြစ်အောင်)
    const iter = kv.list<Movie>({ prefix: ["movies"] });
    
    const tasks = [];
    const BATCH_SIZE = 50; // တစ်ခါလုပ်ရင် ကား ၅၀ ပြိုင်တူလုပ်မယ်

    for await (const res of iter) {
        if (!res.value.isDeleted) {
            // Task ထဲ အရင်ထည့်ထားမယ်
            tasks.push(saveMovieDB(res.value));

            // ၅၀ ပြည့်ရင် တပြိုင်တည်း Run မယ်
            if (tasks.length >= BATCH_SIZE) {
                await Promise.all(tasks);
                tasks.length = 0; // ပြီးရင် စာရင်းပြန်ရှင်းမယ်
            }
        }
    }

    // ကျန်နေတဲ့ ကားတွေကို လက်စသတ်မယ်
    if (tasks.length > 0) {
        await Promise.all(tasks);
    }
}
  ...
  <html lang={l}>
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        // 🔥 1. ဒီနေရာမှာ Proxy Protection ကုဒ်ကို စထည့်ပါ
        document.addEventListener('error', function(e) {
            if (e.target.tagName === 'IMG' && e.target.src.includes('wsrv.nl')) {
                // wsrv.nl ပျက်ရင် မူရင်း Link ပြန်ယူမယ်
                try {
        ...
        // 👇 မူရင်းကုဒ်က ဒီအောက်ကစပါတယ် (မဖျက်ရ)
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // 🔥 UX: PLAYER USING GENERATOR LOGIC
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                ...
                // 🔥 MEDIAFIRE: Generator Logic ဆီ ပို့မည်
                if (finalUrl.includes('mediafire.com')) {
                    // Browser က ဒီ Link ကိုဖွင့်ရင် Deno က Direct Link ဆီ Redirect လုပ်ပေးလိမ့်မယ်
...
// 🔥 FIX: Token ဖြေပြီးမှ MediaFire ဖြစ်နေရင် Generator ဆီ ထပ်ပို့မည်
if (finalUrl.includes('mediafire.com')) {
    finalUrl = '/api/mediafire-stream?link=' + encodeURIComponent(finalUrl);
}
...
console.log("Playing:", finalUrl);
                ...
                // Error တက်ရင် Download Link ပြပေးမယ်
                ...
                if(video) { 
                    // 🔥 FALLBACK SYSTEM: Proxy ပျက်ရင် Direct Link ပြောင်းဖွင့်မည်
                    video.addEventListener('error', function() {
                        // အကယ်၍ ပြန်မစမ်းရသေးရင် (First Fail)
                        if (!video.dataset.retried && finalUrl.includes('/api/mediafire-stream')) {
                            console.log("Proxy failed! Switching to Direct Link (VPN Required)...");
                            
                            // User ကို စာတန်းထိုးပြမယ်
                            showToast("Connection Slow. Switching to Direct Line (VPN Needed)", "error");
                            
                            video.dataset.retried = "true"; // ပြန်စမ်းနေကြောင်း မှတ်ထားမယ်
                            
                            // Server ကို "Proxy မခံနဲ့တော့" လို့ လှမ်းပြောပြီး ပြန်ဖွင့်မယ်
                            video.src = finalUrl + "&noproxy=true";
                            video.play();
                        } else {
                            // Direct Link ပါ ဖွင့်မရတော့မှ Error စာပြမယ်
...
app.use("*", async (c, next) => {
    ...
    if (currentCount > 100) return c.text("Too Many Requests! Please wait a minute.", 429);
...
// 🔥 MEDIAFIRE STREAMER (Proxy Smart Fix)
app.get("/api/mediafire-stream", async (c) => {
    const url = c.req.query("link");
    const noProxy = c.req.query("noproxy"); 

    if (!url) return c.text("Missing Link", 400);

    // 1. Proxy နဲ့ MediaFire မူရင်း Link ကို သန့်ရှင်းရေးလုပ်မယ်
    let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();
    
    // Proxy URL အပြည့်အစုံ (Fallback အတွက်)
    const proxyPageUrl = "https://proxy.avotc.tk/" + cleanUrl;

    try {
        // 2. MediaFire Page ကို ဖတ်မယ် (Browser အစစ်ယောင်ဆောင်ပြီး)
        ...
        // 3. Direct Link ကို Regex အစုံနဲ့ ရှာမယ်
        ...
        if (match && match[1]) {
            ...
            // 🔥 LOGIC:
            // noproxy=true ပါရင် (Error တက်ပြီး ပြန်စမ်းတာဆိုရင်) Link အစစ်ပေးမယ် (VPN လိုမယ်)
            if (noProxy) {
                return c.redirect(directLink, 302);
            }

            // ပုံမှန်ဆိုရင် Proxy ခံပြီး Direct Link ပေးမယ် (Auto Play/Download ဖြစ်ဖို့)
            return c.redirect("https://proxy.avotc.tk/" + directLink, 302);
        } else {
            console.log("Extraction Failed. Redirecting to Proxied Webpage.");
            
            // 🔥 FIX: Link ရှာမတွေ့ရင် Error မပြတော့ဘဲ 
            // "Proxy ခံထားတဲ့ Webpage" ဆီ ပို့လိုက်မယ်။
            // ဒါဆို User က VPN မလိုဘဲ Webpage မြင်ရပြီး ကိုယ်တိုင်နှိပ်ဒေါင်းလို့ရမယ်။
            return c.redirect(proxyPageUrl, 302);
        }
    } catch (e) {
        console.error("Stream API Error:", e);
        // Error တက်ရင်လည်း Proxy Webpage ဆီပဲ ပို့မယ် (Safe Landing)
        return c.redirect(proxyPageUrl, 302); 
    }
});
...
app.get("/api/list", async (c) => { 
    ...
    const movies = all.slice(start, start + limit).map(m => ({
        ...m,
...
// 🔥 OPTIMIZED MOVIE DETAIL ROUTE (No Waiting for Resolve)
                               ...
                               return (
                                   <div id={`ep-grid-${safeId}`} class="season-content hidden mt-3 bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3 animate-fade-in">
                                       <div class="grid grid-cols-4 gap-2 max-h-60 overflow-y-auto custom-scroll pr-1">
                                           {eps.map((ep, i) => {
                                               // Find next ep URL for Series Logic
...
app.post("/profile/topup", async (c) => {
    ...
    if (!parsed.success) return c.redirect("/profile?error=" + parsed.error.issues[0].message);

    const { amount, method, transactionId, purpose } = body; 
    ...
    return c.html(
        <Layout title="Admin Dashboard" isAdmin={true}>
            <div class="p-4 bg-black min-h-screen font-sans text-sm">
                ...
                {/* 6. Tab Content: Keys (Includes COIN Gen) */}
                ...
                {/* 11. Tab Content: Config (Full Features) */}
...
app.post("/admin/movie/bulk-import", adminGuard, async (c) => {
    try {
        ...
        // 🔥 SORTING LOGIC
        // Jav နဲ့ USA (name/photo1 ပါသော Format) ဆိုရင် JSON အောက်ဆုံးကောင် ထိပ်ဆုံးရောက်ရမယ် (Do nothing)
        // Myanmar (title ပါသော Format) ဆိုရင် JSON အပေါ်ဆုံးကောင် ထိပ်ဆုံးရောက်ရမယ် (Reverse)
        if (movies.length > 0) {
            const firstSample = movies[0];
            const isJavOrUSA = firstSample.name || firstSample.photo1 || firstSample.dapp;
            
            if (!isJavOrUSA) {
                // Myanmar Format ဖြစ်လို့ Reverse လှန်မယ်
                movies.reverse();
            }
        }
        ...
        for (let i = 0; i < movies.length; i += BATCH_SIZE) {
            ...
            await Promise.all(chunk.map(async (m, idx) => {
                ...
                let finalLink = "", category = "";

                // (A) JAV / USA Format (Uses 'name', 'photo1', 'dapp')
                if (m.name || m.photo1 || m.dapp) {
                    ...
                    // 🔥 JAV vs USA ခွဲခြားခြင်း
                    // Jav Code တွေက SSIS-123, IPX-001 ပုံစံရှိတတ်တယ် (စာလုံး၂လုံးထက်ပိုမယ် - ဂဏန်း၃လုံးထက်ပိုမယ်)
                    const javRegex = /[a-zA-Z]{2,}-[0-9]{3,}/;

                    if (javRegex.test(title)) {
                        category = "Jav"; // Code ပါရင် Jav
                    } else {
                        category = "USA"; // Code မပါရင် USA
                    }

                    // Link ရှာခြင်း
                    if (m.dapp && m.dapp.includes("mediafire.com")) {
                        finalLink = m.dapp;
                    } else if (m.download && m.download.includes("mediafire.com")) {
                        finalLink = m.download;
                    }
                } 
                // (B) MYANMAR Format (Uses 'title')
                else if (m.title) {
                    title = m.title || "";
                    poster = m.image || "";
                    cover = m.image || "";
                    ...
                    if (m.link && m.link.includes("mediafire.com")) {
                        finalLink = m.link;
                    }
                }

                // Title သို့မဟုတ် Link မရှိရင် ကျော်မယ် (MediaFire မပါရင် Auto ပယ်သည်)
                if (!title || !finalLink) return;

                // 🔥 BlueTV to Gold Flix (Description ထဲမှာ ရှာပြင်မယ်)
                if (desc) {
                    desc = desc.replace(/(bluetv\s*pro|bluetv)/gi, "Gold Flix");
                }

                const uniqueKey = finalLink; 
                const id = await createUniqueId(uniqueKey);
                
                // Server 1 & Download 1 (Proxy & Direct API)
                const proxyStreamLink = "https://proxy.avotc.tk/" + finalLink; 
                const directDlLink = "/api/mediafire-stream?link=" + encodeURIComponent(finalLink);
                ...
                const movieData: Movie = {
                    ...
                    // Streaming
                    streamUrl: proxyStreamLink,   // Proxy
                    streamUrl2: finalLink,        // Original
                    
                    // Downloading
                    downloadUrl: directDlLink,    // Auto Direct Extract
                    downloadUrl2: finalLink,      // Original Backup
        ...
        return c.redirect(ADMIN_ROUTE + "/dashboard?success=Imported " + totalCount + " Movies (Auto Jav/USA Sorted)");
...
// 🔥 DELETE ALL USERS (Admin Access မထိခိုက်ပါ)
...
// 🔥 DELETE ALL MOVIES IN 'MYANMAR AND ASIAN'
app.get("/admin/danger/nuke-myanmar", adminGuard, async (c) => {
    const iter = kv.list<Movie>({ prefix: ["movies"] });
    let count = 0;
    for await (const res of iter) {
        // Category တူရင် အကုန်ဖျက်မည်
        if (res.value.category === "Myanmar and Asian") {
            await kv.delete(res.key); // Main Data ဖျက်မည်
            // Index တွေပါ လိုက်ဖျက်ချင်ရင် deleteMovieDB(res.value.id) သုံးရမယ်၊ 
            // ဒါပေမဲ့ အမြန်လိုချင်ရင် ဒီလိုဖျက်ပြီး Re-index ပြန်လုပ်လိုက်ပါ။
            count++;
        }
    }
    return c.text(`Deleted ${count} movies from Myanmar and Asian. Now please go to Admin > Config > Re-Sync Database.`);
});
// 🔥 MYANMAR GHOST FIXER: Myanmar Category တစ်ခုတည်း သီးသန့်ရှင်းရန်
app.get("/admin/fix-myanmar-ghosts", adminGuard, async (c) => {
    const targetCat = "Myanmar and Asian";
    
    // 1. Myanmar Category ထဲက List အကုန်လိုက်စစ်မယ်
    const iter = kv.list({ prefix: ["idx_cat", targetCat] });
    let removedCount = 0;

    for await (const res of iter) {
        const summary = res.value; // List ထဲက အချက်အလက်
        const movieId = summary.id;

        // 2. Data အစစ် ရှိ/မရှိ လှမ်းစစ်မယ်
        const movieData = await kv.get(["movies", movieId]);

        // 3. Data အစစ်မရှိရင် (Null ဖြစ်နေရင်) ဒါ Ghost ဖြစ်လို့ Index တွေကို လိုက်ဖျက်မယ်
        if (!movieData.value) {
            // Category Index ဖျက်မယ်
            await kv.delete(res.key);

            // Latest Movies (Time) Index ဖျက်မယ်
            await kv.delete(["idx_time", summary.createdAt, movieId]);

            // Search Index တွေ လိုက်ဖျက်မယ်
    ...
    // 4. Cache ရှင်းမယ် (ချက်ချင်း အာနိသင်ပြအောင်)
    RAM_CACHE.latestMovies = { data: [], timestamp: 0 };
    
    return c.text(`Scanned '${targetCat}'. Found and removed ${removedCount} ghost images.`);
});
// 🔥 FORCE FIX COUNTS: အရေအတွက်တွေကို အကုန်ပြန်ရေပြီး အသစ်ထည့်မည်
app.get("/admin/force-fix-counts", adminGuard, async (c) => {
    // 1. အဟောင်းတွေ ဖျက်မယ် (Reset)
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    for(const cat of cats) await kv.delete(["counts", cat]);

    // 2. လက်ရှိကားတွေကို လိုက်ရေမယ် (RAM ထဲမှာပဲ ရေမို့ အရမ်းမြန်တယ်၊ Timeout မဖြစ်ဘူး)
    const iter = kv.list({ prefix: ["movies"] });
    const tally = {}; // မှတ်စုစာအုပ်
    let totalMovies = 0;

    for await (const res of iter) {
        const m = res.value;
        // ဖျက်ထားတဲ့ကား မဟုတ်ရင် ရေမယ်
        if (m && !m.isDeleted) {
            const cat = m.category || "Other";
            // ရှိပြီးသားဆို ၁ ပေါင်း၊ မရှိသေးရင် ၁ စ
            tally[cat] = (tally[cat] || 0) + 1;
            totalMovies++;
        }
    }

    // 3. ရေထားတာတွေကို Database ထဲ ပြန်ထည့်မယ်
    let msg = `✅ Fixed Counts for ${totalMovies} Total Movies:\n\n`;
    for (const [cat, count] of Object.entries(tally)) {
        // Deno KV Count Format အတိုင်း ပြန်သိမ်းမယ်
...
// 🔥 EMERGENCY REBUILD: Home Page ပျောက်သွားရင် ဒါကို run ပါ
app.get("/admin/emergency-rebuild", adminGuard, async (c) => {
    
    // 1. ဇာတ်ကားအရေအတွက် (Counts) တွေကို အရင် Reset ချမယ်
    const cats = ["Movies","Series","4K Movies","USA","Jav","All Uncensored","Myanmar and Asian","4K Porns"];
    for(const cat of cats) await kv.delete(["counts", cat]);
    ...
    // 3. ရှိသမျှ ဇာတ်ကားတွေကို ပြန်ဖတ်ပြီး Index ပြန်ဆောက်မယ် (Batch စနစ်)
    const iter = kv.list({ prefix: ["movies"] });
    let count = 0;
    const BATCH_SIZE = 20; // တစ်ခါလုပ် 20 ကား (Timeout မဖြစ်အောင်)
    let tasks = [];

    for await (const res of iter) {
        const movie = res.value;
        if (movie && !movie.isDeleted) {
            // saveMovieDB က Index တွေရော၊ Counts တွေရော အကုန် auto ပြန်ဆောက်ပေးတယ်
...
// 🔥 MIGRATION: USA ကားတွေကို Series နဲ့ Movies ထဲ ခွဲထည့်ပေးမည့် ကုဒ်
app.get("/admin/fix-usa-category", adminGuard, async (c) => {
    ...
    for await (const res of iter) {
        const m = res.value;
        // Usa Category ဖြစ်မှ အလုပ်လုပ်မယ်
        if (m && !m.isDeleted && m.category === "USA") {
            
            // Episodes ရှိရင် (သို့) Season ပါရင် -> Series ထဲရွှေ့မယ်
            if ((m.episodes && m.episodes.length > 0) || m.title.toLowerCase().includes("season")) {
                m.category = "Series";
                toSeries++;
            } 
            // Episodes မရှိရင် (တစ်ကားထဲဆို) -> Movies ထဲရွှေ့မယ်
            else {
                m.category = "Movies";
                toMovies++;
            }

            // Database ထဲ ပြန်သိမ်း
            await saveMovieDB(m);
        }
    }
