...
app.use("*", compress());
// 404 Not Found (လမ်းမှားရောက်ရင်)
app.notFound((c) => {
  return c.html(
...
// 500 Server Error (ဆာဗာ Error တက်ရင်)
app.onError((err, c) => {
  console.error(err); // Admin ကြည့်ဖို့ Console မှာ Error ထုတ်မယ်
  return c.html(
...
if (!SALT || !ADMIN_PASS) {
  console.error("❌ ERROR: You MUST set 'SECRET_SALT' and 'ADMIN_PASSWORD' in Environment Variables.");
  Deno.exit(1); 
}
...
interface Movie {
  ...
  category: "Movies" | "Series" | "Animation" | "Jav" | "All Uncensored"; 
...
async function deleteMovieDB(id: string) {
    ...
    try {
        ...
        console.log("Counter error ignored (Movie deleted successfully).");
    }
}
...
async function searchMoviesDB(query: string) {
    ...
    // ၁. ရှာမည့်စာသားကို သန့်ရှင်းရေးလုပ်မည် 
    // (စာလုံးအသေးပြောင်းမည်၊ a-z နဲ့ 0-9 ကလွဲရင် ကျန်တာအကုန်ဖြုတ်မည်)
    // ဥပမာ User က "Spider-Man" ရိုက်ရင် "spiderman" ဖြစ်သွားမယ်
    const cleanQuery = query.toLowerCase().replace(/[^a-z0-9]/g, "");

    // ဘာမှမရိုက်ထားရင် အလွတ်ပဲ ပြန်မယ်
    if (!cleanQuery) return [];

    for await (const res of iter) {
        const m = res.value;
        
        // ၂. Database ထဲက Title နဲ့ Tags ကိုလည်း အလားတူ သန့်ရှင်းရေးလုပ်မယ်
        const cleanTitle = m.title.toLowerCase().replace(/[^a-z0-9]/g, "");
        const cleanTags = m.tags ? m.tags.toLowerCase().replace(/[^a-z0-9]/g, "") : "";

        // ၃. သန့်ရှင်းပြီးသားအချင်းချင်း တိုက်စစ်မယ်
        if (cleanTitle.includes(cleanQuery) || cleanTags.includes(cleanQuery)) {
            results.push(m);
            // Result ၅၀ ပြည့်ရင် ဆက်မရှာတော့ဘူး (Server ပေါ့ပါးစေရန်)
...
`;
  ...
  const t = i18n[l];
  const active = props.activeTab || "home"; // လက်ရှိရောက်နေတဲ့ Tab ကို စစ်မယ်

  // 🔥 1. DAYS CALCULATION (ရက်တွက်တဲ့အပိုင်း)
  let daysLeft = 0;
  ...
  <html lang={l}>
    ...
    el.className = 'movie-card block bg-[#1f1f1f] rounded overflow-hidden mb-4'; 
                        el.innerHTML = '<img src="'+m.coverUrl+'" loading="lazy" decoding="async" class="aspect-video object-cover w-full" /><div class="p-3"><h3 class="text-sm font-bold truncate text-white">'+m.title+'</h3></div>'; 
                    } else { 
                        el.className = 'movie-card block bg-[#1f1f1f] rounded overflow-hidden'; 
                        el.innerHTML = '<img src="'+m.posterUrl+'" loading="lazy" decoding="async" class="aspect-[2/3] object-cover w-full" /><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white">'+m.title+'</h3></div>'; 
                    } 
                    grid.appendChild(el); 
                }); 
        ...
        <>
            <header class="top-header">
                ...
                <div class="flex items-center gap-3">
                    ...
                    {/* 🔥 2. DISPLAY LOGIC: Home tab + Not Admin ဆိုမှ ပေါ်မယ် */}
...
// SITEMAP ROUTE (Google SEO အတွက်)
app.get("/sitemap.xml", async (c) => {
    // နောက်ဆုံးကား ၁၀၀၀ လောက်ထိ ယူမယ်
    ...
    return new Response(xml, {
        headers: {
            ...
            "Cache-Control": "public, max-age=3600" // ၁ နာရီ Cache မှတ်မယ်
        }
    });
});
...
// 🔥 DATA MIGRATION: Adult -> Jav
app.get("/api/fix-categories", async (c) => {
    // Admin Password မှန်မှ အလုပ်လုပ်မယ် (သူများလာမနှိပ်နိုင်အောင်)
    ...
    for await (const res of iter) {
        ...
        // "Adult" ဖြစ်နေရင် "Jav" လို့ ပြောင်းမယ်
        if (movie.category === "Adult") {
            movie.category = "Jav"; 
            
            // saveMovieDB က အဟောင်း(Adult)ကိုဖျက်ပြီး အသစ်(Jav)နေရာမှာ Auto ပြန်စီပေးပါလိမ့်မယ်
            await saveMovieDB(movie); 
            count++;
        }
    }
    
    // Adult စာရင်းဟောင်းကို ဖျက်မယ်
            ...
            <div class="p-4">
                ...
                <div class="grid grid-cols-3 gap-3">
                    {results.map(m => {
                        ...
                        return (
                            <a href={`/movie/${m.id}`} class={`block bg-[#1f1f1f] rounded-xl overflow-hidden movie-card group ${isUncensored ? 'col-span-3' : ''}`}>
                                <div class={`${isUncensored ? "aspect-video" : "aspect-[2/3]"} relative overflow-hidden`}>
                                    ...
                                    {isUncensored && <div class="absolute top-2 right-2 bg-red-600 text-white text-[9px] font-bold px-2 py-0.5 rounded">18+</div>}
                                </div>
...
app.post("/report", async (c) => {
    ...
    // Request စာရင်းထဲမှာပဲ [REPORT] ဆိုပြီး ခေါင်းစဉ်တပ် သိမ်းလိုက်မယ် (Admin Panel မှာ Request Tab မှာပဲ မြင်ရမယ်)
    const reqId = crypto.randomUUID();
    const reportData: UserRequest = { 
        id: reqId, 
        username: user ? user.username : "Guest", 
        movieName: `[BROKEN LINK] ${movieName}`, // Request Tab မှာ ဒီလိုပေါ်နေမယ်
        timestamp: Date.now() 
    }; 
...
app.get("/logout", async (c) => {
    // ၁. လက်ရှိ Cookie ကို လှမ်းယူမယ်
    const authCookie = getCookie(c, "auth_session");
    if (authCookie) {
        // ၂. Username ကို ခွဲထုတ်ပြီး User Data ကို ဆွဲမယ်
        const [username] = authCookie.split(":");
        const user = await getUser(username);
        
        // ၃. Database ထဲက Session ID ကို ဖျက်မယ် (Reset ချမယ်)
        if (user) {
            user.sessionId = ""; // Session ဖြတ်လိုက်ပြီ (နောက်တစ်ခါ ဒီ Session ID နဲ့ ဝင်မရတော့ဘူး)
            await kv.set(["users", username], user);
        }
    }
    // ၄. ပြီးမှ Browser Cookie ကို ဖျက်မယ်
            ...
            <div class="p-4 bg-black min-h-screen font-sans text-sm">
                ...
                <div id="tab-movies" class="tab-content active">
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        {/* FORM SECTION (Save as Draft ခလုတ် ထည့်ထားသည်) */}
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                        ...
                                        function extractLinks() {
                                            try {
                                                ...
                                                let processedLinks = foundLinks.map(link => {
                                                    try { return decodeURIComponent(link).trim(); } catch (e) { return link.trim(); }
                                                });
                                ...
                                {/* MAGIC TOOLS & EPISODE LIST (Same as before - Shortened for brevity) */}
                                ...
                                <textarea name="description" placeholder="Description..." class="input-box w-full h-20 bg-black border-zinc-700">{editMovie?.description}</textarea>
                                {/* 👇 CHECKBOX (ဒီနေရာမှာ ထည့်ပါ) 👇 */}
                                        ...
                                        <div class="flex flex-col justify-between flex-grow">
                                            ...
                                            <div class="flex gap-2 mt-3">
                                                ...
                                                <form action="/admin/movie/save" method="post" class="flex-1">
                                                    ...
                                                    {/* Note: Complex objects like episodes need handling, but for single publish usually 'Edit' then 'Publish' is safer. 
                                                        Let's keep it simple: Use 'Edit' to publish single items. */}
                                                </form>
                ...
                {/* TAB: KEYS, USERS, REQUESTS, CONFIG (Existing Tabs...) */}
                ...
                <div id="tab-config" class="tab-content">
                    ...
                    <div class="max-w-xl mx-auto bg-[#111] p-6 rounded-2xl border border-zinc-800 space-y-8 shadow-xl">
                        ...
                        {/* 2. POPUP ADS (UPDATED WITH TARGET & MESSAGE) */}
...
// ၁. ဇာတ်ကားသိမ်းသည့်စနစ် (Draft ရော Publish ရော ပေါင်းထားသည်)
// ၁. ဇာတ်ကားသိမ်းသည့်စနစ် (FIX: Edit လုပ်ရင် နေရာမရွေ့တော့ပါ)
app.post("/admin/movie/save", adminGuard, async (c) => { 
    const body = await c.req.parseBody(); 
    const saveType = body["saveType"]; // 'draft' or 'publish'
    const isMoveToTop = body["updateTime"] === "on"; // Checkbox စစ်ခြင်း
    ...
    // Live Database မှာ ရှိပြီးသားလား စစ်မယ်
    const existingLive = await kv.get(["movies", id]);
    
    // ID Overwrite Protection (အသစ်ဖန်တီးတာပေမယ့် ID တူနေရင် အသစ်ထုတ်ပေးမယ်)
    if (body["mode"] === "create" && existingLive.value && saveType === "publish") {
        id = crypto.randomUUID(); 
    }

    // အချိန်သတ်မှတ်ခြင်း (Logic ပြင်လိုက်ပါပြီ)
    let timeToSave = Number(body["createdAt"]) || Date.now();

    if (saveType === "publish") {
        // (က) Live မှာ မရှိသေးဘူး (အသစ် သို့မဟုတ် Draft ကလာတာ) ဆိုရင် => ထိပ်တင်မယ်
        // (ခ) သို့မဟုတ် Checkbox ခြစ်ထားရင် => ထိပ်တင်မယ်
        if (!existingLive.value || isMoveToTop) {
            timeToSave = Date.now();
        }
        // ကျန်တဲ့အခြေအနေ (Edit လုပ်တာ၊ Checkbox မခြစ်တာ) ဆိုရင် timeToSave က အဟောင်းအတိုင်း ဖြစ်နေပါလိမ့်မယ်
    }
...
// ၂. Draft တွေအကုန်လုံးကို တပြိုင်နက်တင်မည့် ခလုတ် (Publish All)
app.post("/admin/drafts/publish-all", adminGuard, async (c) => {
    const iter = kv.list<Movie>({ prefix: ["drafts"] });
    let count = 0;
    for await (const res of iter) {
        const m = res.value;
        // အချိန်ကို လက်ရှိအချိန်ပြောင်း (Latest ဖြစ်အောင်)
        m.createdAt = Date.now() + count; // (count ပေါင်းထည့်တာက အစီအစဉ်မကျော်အောင်)
        
        await saveMovieDB(m); // Main DB ထဲထည့်
        await kv.delete(["drafts", m.id]); // Draft ထဲကဖျက်
        count++;
    }
    return c.redirect(ADMIN_ROUTE + "/dashboard?success=All Drafts Published Successfully!");
});

// ၃. Draft တစ်ခုချင်းစီ ဖျက်ချင်ရင်
