...
// 🔥 ROBUST DELETE FUNCTION (Plan B ပါဝင်သည်)
async function deleteMovieDB(id: string) {
    try {
        ...
        // ကြိုးစားမှု (၁) - Counter ရော ဇာတ်ကားရော ဖျက်မယ်
        const op = kv.atomic();
        op.delete(["movies", id]);
        op.delete(["idx_time", m.createdAt, id]);
        op.delete(["idx_cat", m.category, m.createdAt, id]);
        op.sum(["counts", m.category], -1n); // အရေအတွက်လျှော့

        const result = await op.commit();

        // အကယ်၍ (၁) မအောင်မြင်ခဲ့ရင် (Result Not OK)
        if (!result.ok) {
            console.log("⚠️ Normal delete failed. Trying Force Delete...");
            
            // ကြိုးစားမှု (၂) - ဇာတ်ကား Data သီးသန့် အတင်းဖျက်မယ် (Counter ကို လှစ်လျူရှုမယ်)
            const forceOp = kv.atomic();
            forceOp.delete(["movies", id]);
            forceOp.delete(["idx_time", m.createdAt, id]);
            forceOp.delete(["idx_cat", m.category, m.createdAt, id]);
            await forceOp.commit();
        }
