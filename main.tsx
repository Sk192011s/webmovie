...
app.get(ADMIN_ROUTE + "/dashboard", adminGuard, async (c) => { 
    ...
    if (adminQuery) {
        movies = await searchMoviesDB(adminQuery); // This now includes soft-deleted logic check inside searchMoviesDB if you want, but admin usually wants to see everything.
        // Let's modify searchMoviesDB slightly or just filter here. 
        // Current implementation of searchMoviesDB filters out deleted. 
        // For admin, we might want to see them, but for simplicity let's stick to active ones in search.
    } else {
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class={`lg:col-span-1 p-5 rounded-lg border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title (Leave empty for 4K Auto)" value={editMovie?.title} class="input-box w-full bg-black border-zinc-700 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">{["Movies","Series","4K Movies","USA","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-24 text-center flex-shrink-0 bg-black border-zinc-700" /></div></div>
    ...
    <h2 class="font-bold text-sm text-green-500 flex-shrink-0"><i class="fa-solid fa-circle text-[8px] animate-pulse"></i> Live Movies ({movies.length})</h2>
    
    <!-- 🔥 REAL DATABASE SEARCH FORM -->
    <form action={ADMIN_ROUTE + "/dashboard"} method="get" class="flex gap-2 w-full md:w-auto">
        <input type="hidden" name="tab" value="movies" />
        <input name="q" value={adminQuery} placeholder="Search Database..." class="bg-black border border-zinc-700 rounded-lg px-4 py-2 text-xs w-full md:w-48 outline-none focus:border-blue-500 transition" />
        <button class="bg-blue-600 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-500 transition"><i class="fa-solid fa-magnifying-glass"></i></button>
        {adminQuery && (
            <a href={ADMIN_ROUTE + "/dashboard?tab=movies"} class="bg-red-500/20 text-red-500 border border-red-500/30 px-3 py-2 rounded-lg text-xs font-bold hover:bg-red-500 hover:text-white transition flex items-center"><i class="fa-solid fa-xmark"></i></a>
        )}
    </form>
</div>
    ...
    {/* 🔥 ADMIN: UPDATED MOVIE LIST WITH SOFT DELETE UI */}
    <div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scroll">
        {movies.map(m => (
            <div class={`movie-item flex gap-4 p-3 rounded-xl items-center border transition ${m.isDeleted ? 'bg-red-900/20 border-red-500/50 opacity-70' : 'bg-black border-zinc-800/50 hover:border-zinc-600'}`} data-title={m.title}>
    ...
    <div class="max-w-4xl mx-auto bg-[#111] p-6 rounded-lg border border-zinc-800 shadow-xl">
        <div class="flex justify-between items-center mb-6">
            ...
            <form action="/admin/topup/clear-history" method="post" onsubmit="return confirm('Clear all Approved/Rejected history?')">
