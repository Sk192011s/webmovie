...
// 4. DB FUNCTIONS
// =======================
// 🔥 MANUAL SAVE FUNCTION (Stable & Error Free)
async function saveMovieDB(movie: Movie) {
    ...
    // ၂. Category ပြောင်းလဲမှုရှိရင် Counter တွေကို အရင်ရှင်းမယ်
    if (old && old.category !== movie.category) {
        // (က) Index အဟောင်းကို ဖျက်မယ် (ဒါမှ ဟိုဘက် Category မှာ မပေါ်တော့မှာ)
        await kv.delete(["idx_cat", old.category, old.createdAt, old.id]);

        // (ခ) Counter တွေ ချိန်ညှိမယ် (Try Catch ခံထားလို့ Error တက်လည်း ရုပ်ရှင်ပျောက်မသွားဘူး)
        try {
            // အဟောင်းထဲက ၁ နှုတ်
            ...
            console.log("Counter update failed (But movie saved).");
        }
    } else if (!old) {
        // ဇာတ်ကားအသစ်ဆိုရင် Counter ၁ တိုးမယ်
        try {
            const newKey = ["counts", movie.category];
            const newCnt = await kv.get(newKey);
            let val = newCnt.value ? Number(newCnt.value) + 1 : 1;
    ...
    // ၃. ဇာတ်ကား Data အသစ်ကို သိမ်းမယ် (Sequential Writes)
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    // 🔥 အဓိက ပြင်ဆင်လိုက်သော နေရာ 🔥
    // ၁။ ဇာတ်ကား အဟောင်းလား အသစ်လား အရင်စစ်မယ်
    const existing = await kv.get(["movies", body["id"]]);
    
    let timeToSave;
    if (!existing.value) {
        // (က) အသစ်တင်တာဆိုရင် - Save နှိပ်တဲ့ လက်ရှိအချိန်ကို ယူမယ် (ဒါမှ ထိပ်ဆုံးရောက်မှာ)
        timeToSave = Date.now();
    } else {
        // (ခ) ပြင်ဆင်တာ(Edit) ဆိုရင် - မူလတင်ခဲ့တဲ့ အချိန်အဟောင်းကိုပဲ ပြန်သုံးမယ် (နေရာမရွှေ့အောင်လို့)
    ...
    const movie = { 
        ...body, 
        id: body["id"], 
        createdAt: timeToSave, // ပြင်ဆင်ထားသော အချိန်ကို ထည့်မယ်
        episodes 
    }; 
