...
// ဒီကုဒ်ကို Deno.serve(app.fetch); အပေါ်နားမှာ ထည့်ပါ
app.get("/admin/fix-database-now", adminGuard, async (c) => {
    const iter = kv.list({ prefix: ["movies"] });
    let count = 0;
    for await (const res of iter) {
        const m = res.value;
        // Delete old indexes first to be safe
        await kv.delete(["idx_time", m.createdAt, m.id]);
        
        // Save again (triggers new indexing)
        await saveMovieDB(m);
        count++;
    }
    return c.html(`<h1>✅ Fixed ${count} Movies Successfully!</h1><p>Now go to Admin Dashboard and check again.</p><a href="/admin_panel_secure/dashboard">Go to Dashboard</a>`);
});
