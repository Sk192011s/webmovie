...
// *** EMERGENCY RESCUE ROUTE (MUST BE BEFORE IP CHECK) ***
app.get("/rescue-me", async (c) => {
    // Delete all banned IPs
    const iter = kv.list<BannedIP>({ prefix: ["banned_ips"] });
    for await (const res of iter) {
        await kv.delete(res.key);
    }
    return c.text("SUCCESS: All IP Bans have been cleared. You can access Admin now.");
});

// GLOBAL IP CHECKER (Runs on everything EXCEPT Admin/Rescue)
app.use('*', async (c, next) => {
    const path = new URL(c.req.url).pathname;

    // Safety Bypass List
    if (path.startsWith("/admin") || path.startsWith("/login") || path.startsWith("/rescue") || path.startsWith("/static") || path.startsWith("/manifest") || path.startsWith("/service-worker")) {
        await next();
        return;
    }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.body.addEventListener('click', (e) => {
                ...
                if (link) {
                    ...
                    // Ignore admin, rescue, and special links from loader
                    if (href && href.startsWith('/') && !href.includes('logout') && !href.includes('#') && !href.includes('admin') && !href.includes('rescue') && target !== '_blank') {
...
// =======================
// 6. ADMIN & AUTH ROUTES (ORDER MATTERS - ADMIN FIRST)
// =======================
...
// =======================
// 7. PUBLIC PAGES (HOME, SEARCH, ETC) - MUST BE LAST
// =======================
