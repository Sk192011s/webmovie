...
// =======================
// 4. UI LAYOUT (MODERN SEARCH ICON & OVERLAY)
// =======================
  ...
  <html lang={l}>
    <head>
      ...
      <style>{`
        ...
        * { user-select: none; -webkit-user-select: none; }
        ...
        .bottom-nav { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); width: 90%; max-width: 400px; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.5); display: flex; justify-content: space-around; padding: 12px 6px; z-index: 50; }
        ...
        .top-header { position: fixed; top: 0; left: 0; width: 100%; z-index: 40; padding: 12px 20px; display: flex; justify-content: space-between; items-center; background: rgba(15, 23, 42, 0.8); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.05); }
        ...
        /* 🔥 SEARCH OVERLAY */
        #search-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.98); backdrop-filter: blur(20px); z-index: 100; transform: translateY(-100%); transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1); display: flex; flex-direction: column; padding: 20px; }
        #search-overlay.open { transform: translateY(0); }
        
        /* 🔥 LOADERS */
        #page-loader { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.9); z-index: 99999; display: none; justify-content: center; align-items: center; backdrop-filter: blur(5px); }
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.toggleSearch = function() {
                 ...
                 if(overlay.classList.contains('open') && input) setTimeout(() => input.focus(), 100);
             }
        });
    ...
    <body>
      ...
      <div id="search-overlay">
          ...
          <form action="/search" method="get" class="relative mb-8">
              <i class="fa-solid fa-magnifying-glass absolute left-4 top-4 text-gray-400"></i>
              <input id="search-input-main" name="q" placeholder="Type movie name..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white text-lg focus:border-purple-500 outline-none" />
          </form>
          <div>
              <h3 class="text-xs font-bold text-gray-500 uppercase mb-4 tracking-wider">Quick Links</h3>
              <div class="flex flex-wrap gap-2">
                  {["Action", "Romance", "Horror", "18+", "Myanmar", "4K"].map(tag => (
                      <a href={`/search?q=${tag}`} class="px-4 py-2 rounded-full bg-white/5 border border-white/5 text-sm text-gray-300 hover:bg-purple-600 hover:text-white hover:border-purple-500 transition">{tag}</a>
                  ))}
              </div>
          </div>
      </div>
                 ...
                 <form action="/profile/topup" method="post" class="space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer relative group">
                            ...
                            <div class="absolute top-2 right-2 text-orange-500 opacity-0 peer-checked:opacity-100 transition"><i class="fa-solid fa-circle-check"></i></div>
                        </label>
                        <label class="cursor-pointer relative group">
                            ...
                            <div class="absolute top-2 right-2 text-yellow-400 opacity-0 peer-checked:opacity-100 transition"><i class="fa-solid fa-circle-check"></i></div>
                        </label>
                    </div>
                    ...
                    <button class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl transition text-xs shadow-lg shadow-blue-900/20 active:scale-95">Submit Slip</button>
                 </form>
            </div>
