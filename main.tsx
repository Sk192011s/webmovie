                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="bg-zinc-800 p-3 rounded-xl mb-2 border border-zinc-700">
                                    ...
                                    {/* JAVASCRIPT LOGIC (WITH AUTO-SORT FIX ✅) */}
                                    {/* JAVASCRIPT LOGIC (TOOL 1 + TOOL 2 WITH MKV SUPPORT ✅) */}
                                    <script dangerouslySetInnerHTML={{__html: `
                                        // FUNCTION FOR TOOL 1
                                        ...
                                        // FUNCTION FOR TOOL 2 (MKV SUPPORTED 🚀)
                                        function extractLinks() {
                                            try {
                                                ...
                                                // 1. Clean Binary (Keep spaces)
                                                rawText = rawText.replace(/[\\x00-\\x1F\\x7F]/g, " ");

                                                // 2. Extract Links (Added mkv, ts, webm, etc.)
                                                // http နဲ့စမယ်... ကြားထဲမှာ Space ပါမယ်...
                                                // အဆုံးမှာ extension တစ်ခုခုနဲ့ ဆုံးမှ ယူမယ်
                                                const urlRegex = /(https?:\\/\\/[a-zA-Z0-9.\\-_~:/?#\\[\\]@!$&'()*+,;=% ]+\\.(?:txt|mp4|mkv|m3u8|avi|mov|flv|webm|ts|mpg|mpeg))/gi;
                                                
                                                const foundLinks = rawText.match(urlRegex);

                                                if (!foundLinks || foundLinks.length === 0) {
                                                    alert("No valid file links found! (Links must end with .mp4, .mkv, .txt, etc.)");
                                                    return;
                                                }
                                                ...
                                                alert(\`Success! Extracted \${addedCount} links (including MKV)!\`);
