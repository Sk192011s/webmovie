...
// 🔥 FIXED SAVE ROUTE (With Auto-Cleaner)
app.post("/admin/movie/save", adminGuard, async (c) => {
    const body = await c.req.parseBody();
    
    // Server 2 Logic
    let s2 = String(body.streamUrl2 || "");
    let dl1 = String(body.downloadUrl || "");
    let dl2 = String(body.downloadUrl2 || "");

    const movie: Movie = {
        id: String(body.id) || crypto.randomUUID(),
        
        // 🔥 Title ကိုပါ သန့်လိုက်တယ်
        title: cleanText(String(body.title)),
        
        posterUrl: String(body.posterUrl),
        
        // Cover မရှိရင် Poster ယူမယ်
        coverUrl: String(body.coverUrl) || String(body.posterUrl),
        
        category: String(body.category) as any,
        
        // 🔥 Description မှာ အမှိုက်သန့်စင်စနစ် ခံလိုက်ပါပြီ
    ...
    if(epText.trim()){
        movie.episodes = epText.split("\n").map(line => {
            ...
            if(parts.length >= 2) {
                return { 
                    name: cleanText(parts[0]), // 🔥 Episode နာမည်ကိုပါ သန့်လိုက်ပြီ
                    url: parts[1] 
                }; 
            }
            return null;
