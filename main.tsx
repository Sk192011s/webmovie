...
// CONFIG & ENV
// =======================
const SALT = Deno.env.get("SECRET_SALT") || "GOLD_FLIX_SECURE_SALT_2025";
...
// CACHING SYSTEM
// =======================
let MOVIE_CACHE: Movie[] | null = null;
function clearCache() { MOVIE_CACHE = null; }
...
async function getMovies() {
  if (MOVIE_CACHE) return MOVIE_CACHE;
  const iter = kv.list<Movie>({ prefix: ["movies"] });
  const movies = [];
  for await (const res of iter) movies.push(res.value);
  MOVIE_CACHE = movies.sort((a, b) => (Number(b.createdAt) || 0) - (Number(a.createdAt) || 0));
  return MOVIE_CACHE;
}
async function getMovie(id: string) { 
  if (MOVIE_CACHE) return MOVIE_CACHE.find(m => m.id === id);
  const res = await kv.get<Movie>(["movies", id]); return res.value; 
}
...
async function resolveRedirect(url: string) {
  ...
  try { const res = await fetch(url, { method: "HEAD", redirect: "follow", signal: controller.signal }); clearTimeout(timeoutId); return res.url; } catch { return url; }
}
  ...
  <html lang="my">
    <head>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.copyToClip = function(text) {
                 if(navigator.clipboard) {
                     ...
                     const el = document.createElement('textarea'); el.value = text; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); showToast('Copied!', 'success');
                 }
             }

             // Player & Other Logic (Retained)
        ...
        // Infinite Scroll Logic for User side
        let page = 1; let isLoading = false; let hasMore = true;
        async function loadMoreMovies(category) { if(isLoading || !hasMore) return; isLoading = true; document.getElementById('loading-indicator').style.display = 'block'; page++; try { const res = await fetch('/api/list?cat=' + category + '&page=' + page); const data = await res.json(); if(data.movies.length === 0) { hasMore = false; document.getElementById('loading-indicator').style.display = 'none'; return; } const container = document.getElementById('movie-grid'); data.movies.forEach(m => { const el = document.createElement('a'); el.href = '/movie/' + m.id; if(category === "All Uncensored") { el.className = 'block bg-[#1f1f1f] rounded overflow-hidden mb-4'; el.innerHTML = '<img src="'+m.coverUrl+'" class="aspect-video object-cover w-full" /><div class="p-3"><h3 class="text-sm font-bold truncate text-white">'+m.title+'</h3></div>'; } else { el.className = 'block bg-[#1f1f1f] rounded overflow-hidden'; el.innerHTML = '<img src="'+m.posterUrl+'" class="aspect-[2/3] object-cover w-full" /><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white">'+m.title+'</h3></div>'; } container.appendChild(el); }); } catch(e) { console.error(e); } isLoading = false; document.getElementById('loading-indicator').style.display = 'none'; }
...
app.get("/.well-known/assetlinks.json", (c) => c.json([{
  ...
  "target": { "namespace": "android_app", "package_name": "dev.deno.goldflix_stream.twa", "sha256_cert_fingerprints": ["29:7D:1A:43:86:09:03:FE:02:F9:69:46:5A:F8:B7:C0:9A:14:75:10:F6:F3:07:4F:2E:CF:0E:F1:3E:D4:5F:7D"] }
}]));

app.get("/service-worker.js", (c) => c.text(`
  self.addEventListener('install', (e) => { self.skipWaiting(); });
  self.addEventListener('activate', (e) => { e.waitUntil(self.clients.claim()); });
  self.addEventListener('fetch', (e) => { if (e.request.mode === 'navigate') { e.respondWith(fetch(e.request).catch(() => caches.match(e.request))); } else { e.respondWith(caches.match(e.request).then((res) => res || fetch(e.request))); } });
      ...
      <div class="px-3 py-6 space-y-8">
        {sections.map(cat => {
            ...
            if(cat === "All Uncensored") {
                return (
                    <div>
                        <div class="flex justify-between items-end mb-3 px-1"><h2 class="text-lg font-bold text-white border-l-4 border-red-600 pl-2">{cat}</h2><a href={`/category/${cat}`} class="text-xs font-bold text-gray-400 flex items-center gap-1">See All <i class="fa-solid fa-chevron-right text-[10px]"></i></a></div>
                        <div class="h-scroll-section custom-scroll">
                            {catMovies.map(m => (
                                <a href={`/movie/${m.id}`} class="h-scroll-item wide block relative bg-[#1f1f1f] rounded overflow-hidden active:scale-95 transition-transform">
                                    <img src={m.coverUrl || m.posterUrl} /><div class="p-1.5"><h3 class="text-[10px] font-bold truncate text-white leading-tight">{m.title}</h3></div>
                                </a>
                            ))}
                        </div>
                    </div>
...
app.get("/movie/:id", async (c) => {
    ...
    if(episodes) {
        episodes.forEach(ep => { if(ep.season) { if(!seasons[ep.season]) seasons[ep.season] = []; seasons[ep.season].push(ep); } else { ungrouped.push(ep); } });
    }

    let playerUrl = "";
    let secureDownloadUrl = "";
    let secureDownloadUrl2 = "";
    let playerUrl2 = "";
    let playbackToken = "";
    let playbackToken2 = "";
           ...
           <div class="p-4">
               ...
               {premium && movie.category !== "Series" && (
                   <div class="flex flex-col gap-2 mb-6">
                        ...
                        <div class="flex gap-2">{secureDownloadUrl && (<a href={secureDownloadUrl} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs"><i class="fa-solid fa-download"></i> DL {movie.downloadUrl2 ? "1" : ""}</a>)}{secureDownloadUrl2 && (<a href={secureDownloadUrl2} target="_blank" class="flex-1 bg-zinc-800 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 border border-zinc-700 active:scale-95 transition hover:bg-zinc-700 whitespace-nowrap text-xs"><i class="fa-solid fa-download"></i> DL 2</a>)}</div>
...
// =======================
const adminAuth = async (c: any, next: any) => { const session = getCookie(c, "admin_session"); if (session === ADMIN_PASS) await next(); else return c.redirect(ADMIN_ROUTE); };
...
app.post(ADMIN_ROUTE + "/login", async (c) => { 
    ...
    if (password === ADMIN_PASS) { 
        setCookie(c, "admin_session", String(password), { path: "/", httpOnly: true, secure: !c.req.url.includes("localhost"), sameSite: "Lax" }); 
        return c.redirect(ADMIN_ROUTE + "/dashboard"); 
    } 
    return c.redirect(ADMIN_ROUTE); 
});

app.get(ADMIN_ROUTE + "/dashboard", adminAuth, async (c) => { 
    ...
    return c.html(
        <Layout title="Admin" isAdmin={true}>
            <div class="p-4 bg-zinc-900 min-h-screen">
                {/* Header Actions */}
                <div class="flex justify-between items-center mb-6 bg-black p-4 rounded border border-zinc-800">
                    <h1 class="font-bold text-blue-500 text-lg">Dashboard</h1>
                    <div class="flex gap-2">
                        <a href="/admin/backup" class="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1 rounded text-xs border border-zinc-700"><i class="fa-solid fa-download"></i> Backup</a>
                        <form action="/admin/restore" method="post" enctype="multipart/form-data" class="inline">
                             <label class="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1 rounded text-xs border border-zinc-700 cursor-pointer"><i class="fa-solid fa-upload"></i> Restore<input type="file" name="file" class="hidden" onchange="this.form.submit()" /></label>
                        </form>
                    </div>
                </div>
                ...
                <div class="flex gap-2 mb-4 overflow-x-auto pb-2">
                    <button id="btn-movies" onclick="openTab('movies')" class="tab-btn active px-4 py-2 bg-[#1f1f1f] rounded text-sm font-bold text-gray-400 hover:text-white transition">Movies</button>
                    <button id="btn-keys" onclick="openTab('keys')" class="tab-btn px-4 py-2 bg-[#1f1f1f] rounded text-sm font-bold text-gray-400 hover:text-white transition">VIP Keys</button>
                    <button id="btn-users" onclick="openTab('users')" class="tab-btn px-4 py-2 bg-[#1f1f1f] rounded text-sm font-bold text-gray-400 hover:text-white transition">Users</button>
                    <button id="btn-requests" onclick="openTab('requests')" class="tab-btn px-4 py-2 bg-[#1f1f1f] rounded text-sm font-bold text-gray-400 hover:text-white transition">Requests</button>
                    <button id="btn-config" onclick="openTab('config')" class="tab-btn px-4 py-2 bg-[#1f1f1f] rounded text-sm font-bold text-gray-400 hover:text-white transition">Config</button>
                </div>

                {/* TAB 1: MOVIES */}
                <div id="tab-movies" class="tab-content active">
                    <div class="grid lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-1 bg-[#1f1f1f] p-4 rounded border border-zinc-700 h-fit sticky top-4">
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-3 text-sm">
                                ...
                                <div class="flex gap-2"><select name="category" class="input-box">{["Movies","Series","Adult","All Uncensored"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-20" /></div>
                                ...
                                <div class="p-2 bg-black rounded border border-zinc-800"><label class="text-xs text-yellow-500 mb-1 block">Series Episodes (Season | Ep | URL)</label><textarea name="episodeList" placeholder="S1 Ep1 | https://...&#10;S1 Ep2 | https://..." rows={5} class="input-box font-mono text-xs whitespace-nowrap overflow-x-auto">{epString}</textarea></div>
                                <div class="grid grid-cols-2 gap-2"><input name="downloadUrl" placeholder="Download Link 1" value={editMovie?.downloadUrl} class="input-box text-xs border-green-900/50 focus:border-green-500" /><input name="downloadUrl2" placeholder="Download Link 2" value={editMovie?.downloadUrl2} class="input-box text-xs border-green-900/50 focus:border-green-500" /></div>
                                <div class="p-2 bg-black rounded border border-zinc-800"><label class="text-xs text-gray-500">Server 2 (Optional)</label><input name="streamUrl2" placeholder="Stream URL 2" value={editMovie?.streamUrl2} class="input-box mt-2" /></div>
                                <textarea name="description" placeholder="Description" class="input-box">{editMovie?.description}</textarea>
                                <button class="btn-primary w-full">{editMovie ? "Update Movie" : "Save Movie"}</button>
                                {editMovie && <a href={ADMIN_ROUTE + "/dashboard"} class="block text-center text-xs text-gray-400 mt-2">Cancel Edit</a>}
                            </form>
                        </div>
                        <div class="lg:col-span-2 bg-[#1f1f1f] p-4 rounded border border-zinc-700 flex flex-col h-[80vh]">
                            ...
                            <div class="space-y-2 flex-1 overflow-y-auto pr-2 custom-scroll">
                                {movies.map(m => (
                                    <div class="movie-item flex gap-3 p-2 bg-black rounded items-center group relative border border-zinc-800 hover:border-zinc-600 transition" data-title={m.title}>
                                        <img src={m.posterUrl} class="w-10 h-14 object-cover rounded" />
                                        <div class="flex-grow min-w-0"><div class="font-bold text-sm truncate text-gray-200">{m.title}</div><div class="text-[10px] text-gray-500">{m.category} • {m.year}</div></div>
                                        <div class="flex gap-2">
                                            <a href={`${ADMIN_ROUTE}/dashboard?edit=${m.id}`} class="text-blue-500 text-xs border border-blue-500/30 px-3 py-1 rounded hover:bg-blue-500/10">Edit</a>
                                            <form action="/admin/movie/delete/${m.id}" method="post" onsubmit="return confirm('Delete this movie?')"><button class="text-red-500 text-xs border border-red-500/30 px-3 py-1 rounded hover:bg-red-500/10">Del</button></form>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div id="tab-keys" class="tab-content">
                    <div class="max-w-2xl mx-auto space-y-4">
                        <div class="bg-[#1f1f1f] p-6 rounded border border-zinc-700 shadow-lg">
                            <h2 class="font-bold mb-4 text-yellow-500"><i class="fa-solid fa-key mr-2"></i> Generate VIP Key</h2>
                            <form action="/admin/key/create" method="post" class="flex gap-2">
                                <input type="number" name="days" placeholder="Days (e.g. 30)" required class="input-box flex-grow" />
                                <button class="btn-primary w-32">Generate</button>
                            </form>
                        </div>
                        <div class="bg-[#1f1f1f] p-6 rounded border border-zinc-700">
                            <h2 class="font-bold mb-4 text-gray-300">Active Keys</h2>
                            <div class="space-y-2 max-h-[500px] overflow-y-auto custom-scroll">
                                {keys.map(k => (
                                    <div class="flex justify-between items-center p-3 bg-black rounded border border-zinc-800">
                                        <div class="flex items-center gap-3">
                                            <span class="text-yellow-500 font-mono font-bold text-lg">{k.code}</span>
                                            <span class="text-xs bg-zinc-800 px-2 py-1 rounded text-gray-400">{k.days} Days</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <button onclick={`copyToClip('${k.code}')`} class="text-xs bg-blue-900/30 text-blue-400 px-3 py-1.5 rounded hover:bg-blue-900/50"><i class="fa-solid fa-copy"></i> Copy</button>
                                            <form action={`/admin/key/delete/${k.code}`} method="post"><button class="text-xs bg-red-900/30 text-red-400 px-3 py-1.5 rounded hover:bg-red-900/50"><i class="fa-solid fa-trash"></i></button></form>
                                        </div>
                                    </div>
                                ))}
                                {keys.length === 0 && <p class="text-center text-gray-500 py-4">No active keys.</p>}
                            </div>
                        </div>
                    </div>
                </div>

                {/* TAB 3: USERS (RESET PASSWORD) */}
                <div id="tab-users" class="tab-content">
                    <div class="max-w-xl mx-auto bg-[#1f1f1f] p-8 rounded border border-zinc-700 shadow-lg">
                         <h2 class="font-bold mb-6 text-blue-500 text-xl"><i class="fa-solid fa-users-gear mr-2"></i> User Management</h2>
                         <div class="bg-black/50 p-6 rounded border border-zinc-800">
                             <h3 class="font-bold text-white mb-4">Reset User Password</h3>
                             <form action="/admin/user/reset" method="post" class="space-y-4">
                                 <div>
                                     <label class="block text-xs text-gray-400 mb-1">Target Username</label>
                                     <input name="username" placeholder="Enter username..." required class="input-box" />
                                 </div>
                                 <div>
                                     <label class="block text-xs text-gray-400 mb-1">New Password</label>
                                     <input name="newpass" placeholder="Enter new password..." required class="input-box" />
                                 </div>
                                 <button class="bg-blue-600 text-white font-bold w-full py-3 rounded hover:bg-blue-700 transition">Reset Password</button>
                             </form>
                             <p class="text-xs text-gray-500 mt-4 text-center">Note: This will override the user's existing password immediately.</p>
                         </div>
                    </div>
                </div>
                ...
                <div id="tab-requests" class="tab-content">
                    <div class="max-w-3xl mx-auto bg-[#1f1f1f] p-6 rounded border border-zinc-700">
                        <h2 class="font-bold mb-4 text-pink-500">Movie Requests ({requests.length})</h2>
                        <div class="space-y-3">
                            {requests.map(r => (
                                <div class="bg-black p-4 rounded flex justify-between items-center border border-zinc-800">
                                    <div>
                                        <h3 class="font-bold text-lg text-white">{r.movieName}</h3>
                                        <p class="text-xs text-gray-500">Requested by <span class="text-blue-400">{r.username}</span> • {new Date(r.timestamp).toLocaleDateString()}</p>
                                    </div>
                                    <form action={`/admin/request/delete/${r.id}`} method="post">
                                        <button class="text-red-500 hover:text-red-400 p-2"><i class="fa-solid fa-check"></i> Done</button>
                                    </form>
                                </div>
                            ))}
                            {requests.length === 0 && <div class="text-center py-10 text-gray-500">No pending requests.</div>}
                        </div>
                    </div>
                </div>
                ...
                <div id="tab-config" class="tab-content">
                    <div class="max-w-xl mx-auto bg-[#1f1f1f] p-6 rounded border border-zinc-700">
                        <h2 class="font-bold mb-4 text-gray-300">App Configuration</h2>
                        <form action="/admin/config" method="post" class="space-y-4">
                            <div>
                                ...
                                <input name="text" placeholder="Enter message..." value={config.announcement} class="input-box" />
                            </div>
                            <label class="flex items-center gap-2 p-3 bg-black rounded border border-zinc-800 cursor-pointer">
                                <input type="checkbox" name="show" checked={config.showAnnouncement} class="accent-yellow-500 w-5 h-5" />
                                <span class="font-bold text-sm">Show Announcement Bar</span>
                            </label>
...
// Admin Actions
app.post("/admin/config", adminAuth, async (c) => { const body = await c.req.parseBody(); await kv.set(["config"], { announcement: body['text'], showAnnouncement: body['show'] === 'on' }); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Settings Saved"); });
app.post("/admin/request/delete/:id", adminAuth, async (c) => { await kv.delete(["requests", c.req.param("id")]); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=requests"); });
app.post("/admin/movie/save", adminAuth, async (c) => { 
    const body = await c.req.parseBody(); 
    const epString = body["episodeList"] as string; 
    const episodes: Episode[] = []; 
    if(epString && epString.trim().length > 0) { epString.split('\n').forEach(line => { const parts = line.split('|'); if(parts.length === 3) { episodes.push({ season: parts[0].trim(), name: parts[1].trim(), url: parts[2].trim() }); } else if(parts.length === 2) { episodes.push({ season: "", name: parts[0].trim(), url: parts[1].trim() }); } }); } 
    const movie = { ...body, id: body["id"], createdAt: Number(body["createdAt"]), episodes }; 
    await kv.set(["movies", movie.id as string], movie); 
    clearCache(); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Saved"); 
});
app.post("/admin/movie/delete/:id", adminAuth, async (c) => { await kv.delete(["movies", c.req.param("id")]); clearCache(); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Deleted"); });
app.post("/admin/key/create", adminAuth, async (c) => { const { days } = await c.req.parseBody(); const code = "VIP-" + Math.random().toString(36).substring(2, 7).toUpperCase(); await kv.set(["keys", code], { code, days: parseInt(String(days)) }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=keys"); });
app.post("/admin/key/delete/:code", adminAuth, async (c) => { await kv.delete(["keys", c.req.param("code")]); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=keys"); });
...
app.post("/admin/restore", adminAuth, async (c) => { try { const body = await c.req.parseBody(); const file = body['file']; if (file instanceof File) { const text = await file.text(); const data = JSON.parse(text); for (const item of data) { await kv.set(item.key, item.value); } clearCache(); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Data Restored"); } } catch(e) { return c.redirect(ADMIN_ROUTE + "/dashboard?error=Restore Failed"); } });
