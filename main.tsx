                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        ...
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                {/* Script for AI (Updated with Year) */}
                                <script dangerouslySetInnerHTML={{__html: `
                                    async function generateAIDesc() {
                                        const title = document.getElementsByName('title')[0].value;
                                        const type = document.getElementsByName('category')[0].value;
                                        const year = document.getElementsByName('year')[0].value; // 🔥 Year ကိုပါ ယူလိုက်ပြီ
...
// 🔥 AI Description Generator (Smart Translate & Generate)
app.post("/admin/api/generate-desc", adminGuard, async (c) => {
    ...
    // 🔥 LOGIC: အကွက်ထဲမှာ စာရှိရင် "ဘာသာပြန်" ခိုင်းမယ်၊ မရှိရင် "ရှာခိုင်း" မယ်
    if (currentDesc && String(currentDesc).trim().length > 10) {
        // Translation Mode
        prompt = `
        Task: Translate the following movie plot summary into professional, engaging Burmese (Myanmar Unicode).
        Target Audience: Movie streaming website users.
        Tone: Dramatic, Suspenseful, and Exciting (Movie Synopsis Style).
        
        Original Text to Translate:
        "${currentDesc}"
        
        Instructions:
        1. Do NOT just translate word-for-word. Rewrite it to sound like a natural Myanmar movie synopsis.
        2. Output ONLY the Burmese text.
        `;
    } else {
        // Generation Mode (Old way)
        prompt = `
        Task: Write a plot summary for the ${type || "movie"} "${title}" (${year || "recent"}).
        Instructions: Retrieve facts from IMDb/TMDB. Write a dramatic, engaging summary in Burmese (Myanmar Unicode). Do not include spoilers.
        `;
    }
