...
// 🔥 MEDIAFIRE STREAMER (Universal IP Fix - No VPN Needed)
// 🔥 MEDIAFIRE STREAMER (Fast Track Optimization)
// 🔥 MEDIAFIRE STREAMER (Stable Tunneling - No VPN Needed)
app.get("/api/mediafire-stream", async (c) => {
    const url = c.req.query("link");
    if (!url) return c.text("Missing Link", 400);

    // Cache မမှတ်ရန် (အရေးကြီးသည်)
    c.header("Cache-Control", "no-cache, no-store, must-revalidate");

    try {
        // 1. Input Link ထဲက Proxy တွေ ပါလာရင် ဖယ်ထုတ်မယ် (သူငယ်ချင်းပြောတဲ့ အပိုင်း)
        let cleanUrl = decodeURIComponent(url).replace("https://proxy.avotc.tk/", "").trim();

        // 2. Deno ကနေ တိုက်ရိုက်မသွားဘဲ Proxy ကနေတဆင့် လှမ်းချိတ်မယ်
        // (ဒါမှ Link ထုတ်တဲ့ IP နဲ့ User ကြည့်မယ့် IP တူပြီး MediaFire က လက်ခံမှာပါ)
        const proxyUrl = "https://proxy.avotc.tk/" + cleanUrl;

        const res = await fetch(proxyUrl, {
            method: "GET",
            headers: {
                // Mobile User Agent သုံးမယ် (စာမျက်နှာ Load တက်တာ ပိုမြန်အောင်)
                "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36",
                "Referer": "https://www.mediafire.com/"
            },
            redirect: "follow" // Redirect လုပ်ရင် လိုက်သွားမယ်
        });

        // 3. Premium Link ဟုတ်မဟုတ် စစ်မယ် (Content-Type ကြည့်မယ်)
        const contentType = res.headers.get("content-type") || "";

        // Video ဖိုင်အတိုင်း တန်းရနေရင် (Premium Link)
        if (contentType.includes("video") || contentType.includes("application/octet-stream")) {
            // Proxy Link ကိုပဲ ပြန်ပေးလိုက်မယ် (Server က ဖတ်လို့ရနေပြီမို့လို့)
            return c.redirect(proxyUrl, 302);
        }

        // 4. Standard Link ဆိုရင် HTML ဖတ်ပြီး ခလုတ်ရှာမယ်
        ...
        if (match && match[1]) {
            ...
            // 5. ရလာတဲ့ Link ကို Proxy ပြန်ခံပေးမယ် (IP Match ဖြစ်အောင်)
            if (!directLink.includes("proxy.avotc.tk")) {
                 directLink = "https://proxy.avotc.tk/" + directLink;
            }
            return c.redirect(directLink, 302);
        } else {
            // ရှာမတွေ့ရင် Proxy Page ကိုပဲ ပြန်ပို့မယ် (လက်နဲ့ဒေါင်းလို့ရအောင်)
            return c.redirect(proxyUrl, 302);
        }

    } catch (e) {
        console.error("Tunnel Error:", e);
        // Error တက်ရင်လည်း Proxy ခံပြီး ပြန်ပို့မယ်
        return c.redirect("https://proxy.avotc.tk/" + url.replace("https://proxy.avotc.tk/", ""), 302);
    }
});
