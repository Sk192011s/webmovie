...
app.use('*', async (c, next) => {
    ...
    // Bypass for Admin, Login, Rescue, Assets
    if (path.startsWith("/admin") || path === "/login" || path === "/rescue-me" || path.startsWith("/static") || path === "/manifest.json" || path === "/service-worker.js") {
        await next();
        return;
    }
...
app.get("/rescue-me", async (c) => { const iter = kv.list({ prefix: ["banned_ips"] }); for await (const res of iter) { await kv.delete(res.key); } return c.text("All IPs Unbanned."); });
