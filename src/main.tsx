...
interface User { 
    ...
    coins?: number; // 🔥 လက်ကျန်ငွေ
    purchased?: string[]; // 🔥 ဝယ်ပြီးသား ကားများ
}
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             window.loadPlayer = async function(content, type, movieId, title, poster, btnElement) {
                document.querySelectorAll('.srv-btn').forEach(b => { b.classList.remove('bg-yellow-500', 'text-black', 'border-transparent'); b.classList.add('bg-zinc-800', 'text-white'); });
                if (btnElement) { btnElement.classList.remove('bg-zinc-800', 'text-white'); btnElement.classList.add('bg-yellow-500', 'text-black', 'border-transparent'); }
                ...
                if(video) { 
                    ...
                    video.addEventListener('error', async () => { const currentTime = video.currentTime; video.load(); video.currentTime = currentTime; try { await video.play(); } catch(e) {} });
...
app.get("/movie/:id", async (c) => {
    ...
    const [movieRes, user, config] = await Promise.all([getMovie(id), getCurrentUser(c), getConfig()]);
    ...
    let playerUrl = "", secureDownloadUrl = "", secureDownloadUrl2 = "", playerUrl2 = "", playbackToken = "";
    ...
    if (canWatch) {
        ...
        playbackToken = token; 
        if (movie.streamUrl2) { const token2 = crypto.randomUUID(); await kv.set(["stream_tokens", token2], movie.streamUrl2, { expireIn: 3600 * 3 }); playerUrl2 = `/stream/${token2}`; }
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <>
              ...
              <div id="video-player" class="w-full h-full hidden"></div>
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center bg-zinc-900 z-30">
                      <img src={displayImage} class="absolute inset-0 w-full h-full object-cover opacity-30 blur-sm" />
                      <div class="relative z-40 text-center p-6 bg-black/80 rounded-2xl border border-zinc-800 backdrop-blur-md">
                          {moviePrice > 0 ? (
                             <>
                                <i class="fa-solid fa-cart-shopping text-5xl text-blue-500 mb-4 drop-shadow-lg"></i>
                                <h2 class="text-xl font-black text-white mb-1">Premium Content</h2>
                                <p class="text-gray-400 text-xs mb-4">This movie requires separate purchase.</p>
                                <div class="text-2xl font-black text-yellow-500 mb-6">{moviePrice} Coins</div>
                                {user ? (
                                    <form action="/api/buy-movie" method="post">
                                        <input type="hidden" name="movieId" value={movie.id} />
                                        <button class="bg-blue-600 text-white font-bold py-3 px-8 rounded-full hover:bg-blue-500 transition shadow-lg flex items-center gap-2 mx-auto">{user.coins && user.coins >= moviePrice ? "Confirm Purchase" : "Not Enough Coins"}</button>
                                        {(!user.coins || user.coins < moviePrice) && (<a href="/profile" class="block text-xs text-yellow-500 mt-3 hover:underline">Top-up Coins</a>)}
                                    </form>
                                ) : (
                                    <a href="/login" class="btn-primary inline-block text-sm">Login to Buy</a>
                                )}
                             </>
                          ) : (
                             <>
                                <i class="fa-solid fa-lock text-5xl text-yellow-500 mb-4 drop-shadow-lg"></i>
                                <h2 class="text-2xl font-black text-white mb-2">{t.vip_only}</h2>
                                <p class="text-gray-400 text-sm mb-6">Join VIP to watch this content.</p>
                                <a href={user ? "/profile" : "/login"} class="btn-primary inline-block text-sm">{t.unlock}</a>
                             </>
                          )}
                      </div>
                  </div>
           ...
           <div class="p-6">
               ...
               <div class="flex items-center gap-3 text-xs text-gray-400 mb-6">
                   ...
                   <span class="text-yellow-500 font-bold border border-yellow-500/30 bg-yellow-500/10 px-3 py-1 rounded-full">{movie.category}</span>
                   {moviePrice > 0 && <span class="bg-blue-900/30 text-blue-400 px-3 py-1 rounded-full border border-blue-500/30 font-bold">{moviePrice} Coins</span>}
               </div>
               {canWatch && (
               ...
               {canWatch && (movie.category === "Series" || movie.category === "Animation") && episodes.length > 0 && (
                   <div class="mb-8"><h3 class="font-bold text-white mb-3 flex items-center gap-2"><i class="fa-solid fa-layer-group text-yellow-500"></i> Seasons & Episodes</h3><div class="flex overflow-x-auto gap-3 pb-2 scrollbar-hide">{Object.keys(seasons).map((season, idx) => { const safeId = season.replace(/\s+/g, '-'); return (<button id={`btn-${safeId}`} onclick={`switchSeason('${safeId}')`} class="season-tab-btn flex-shrink-0 px-5 py-2 rounded-full text-xs font-bold transition border border-zinc-800 bg-zinc-800 text-gray-400 hover:text-white hover:border-zinc-600">{season}</button>) })}</div><div id="episodes-container">{Object.keys(seasons).map((season, idx) => { const safeId = season.replace(/\s+/g, '-'); return (<div id={`ep-grid-${safeId}`} class="season-content hidden mt-3 bg-[#1f1f1f] rounded-xl border border-zinc-800 p-3 animate-fade-in"><div class="grid grid-cols-4 gap-2 max-h-60 overflow-y-auto custom-scroll pr-1">{seasons[season].map(ep => (<button onclick={`loadPlayer('${ep.url.trim()}', '${movie.linkType}', '${movie.id}', '${movie.title.replace(/'/g, "\\'")}', '${movie.posterUrl}', this)`} class="srv-btn bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] py-3 rounded-lg border border-zinc-700 transition-colors font-bold truncate">{ep.name}</button>))}</div></div>) })}</div></div> 
               )}
...
app.post("/api/buy-movie", async (c) => {
    ...
    if (user.purchased && user.purchased.includes(id)) return c.redirect(`/movie/${id}`);
    if (!user.coins || user.coins < movie.price) return c.redirect(`/movie/${id}?error=Not enough coins! Please top up.`);
...
app.post("/signup", async (c) => { 
    ...
    const newUser: User = { username: String(username), passwordHash, expiryDate: null, favorites: [], sessionId: "", ip: clientIp, lastLoginIp: clientIp, isBanned: false, coins: 0, purchased: [] }; 
                    ...
                    <div class="flex flex-col lg:grid lg:grid-cols-3 gap-6">
                        <div class={`lg:col-span-1 p-5 rounded-2xl border h-fit lg:sticky lg:top-4 z-10 w-full max-w-[100vw] overflow-hidden shadow-xl transition-colors ${editMovie ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[#111] border-zinc-800'}`}>
                            ...
                            <form action="/admin/movie/save" method="post" class="space-y-4 text-sm w-full">
                                ...
                                <div class="space-y-2"><input name="title" placeholder="Movie Title" value={editMovie?.title} required class="input-box w-full bg-black border-zinc-700 focus:border-yellow-500" /><div class="flex gap-2 w-full"><select name="category" class="input-box flex-grow min-w-0 bg-black border-zinc-700">{["Movies","Series","4K Movies","Animation","Jav","All Uncensored", "Myanmar and Asian", "4K Porns"].map(o => <option selected={editMovie?.category===o}>{o}</option>)}</select><input name="year" value={editMovie?.year || "2025"} class="input-box w-24 text-center flex-shrink-0 bg-black border-zinc-700" /></div></div>
                                ...
                                <div class="grid grid-cols-2 gap-2"><input name="fileSize" placeholder="File Size (e.g. 1.2 GB)" value={editMovie?.fileSize} class="input-box w-full bg-black border-zinc-700" /><input type="number" name="price" placeholder="Price (Coins) - 0 for Free" value={editMovie?.price || 0} class="input-box w-full bg-black border-yellow-500/50 text-yellow-500 font-bold" /></div>
                        ...
                        <div class="lg:col-span-2 bg-[#111] p-5 rounded-2xl border border-zinc-800 flex flex-col h-[600px] lg:h-[80vh] w-full max-w-[100vw] shadow-xl"><div class="flex justify-between items-center mb-4"><h2 class="font-bold text-sm text-green-500"><i class="fa-solid fa-circle text-[8px] animate-pulse"></i> Live Movies ({movies.length})</h2><input oninput="filterMovies(this.value)" placeholder="Search..." class="bg-black border border-zinc-700 rounded-lg px-4 py-2 text-xs w-48 outline-none" /></div><div class="space-y-3 flex-1 overflow-y-auto pr-1 custom-scroll">{movies.map(m => (<div class="movie-item flex gap-4 p-3 rounded-xl items-center border bg-black border-zinc-800/50 hover:border-zinc-600 transition" data-title={m.title}><img src={m.posterUrl} class="w-10 h-14 object-cover rounded-lg flex-shrink-0" /><div class="flex-grow min-w-0"><div class="font-bold text-sm truncate text-gray-200">{m.title}</div><div class="text-[10px] text-gray-500">{m.category} • {m.year}</div></div><div class="flex gap-2"><a href={`${ADMIN_ROUTE}/dashboard?edit=${m.id}`} class="text-blue-500 text-xs border border-blue-500/20 bg-blue-500/10 px-3 py-1.5 rounded hover:bg-blue-500 hover:text-white transition">Edit</a><form action={`/admin/movie/delete/${m.id}`} method="post" onsubmit="return confirm('Delete?')"><button class="text-red-500 text-xs border border-red-500/20 bg-red-500/10 px-3 py-1.5 rounded hover:bg-red-500 hover:text-white transition">Del</button></form></div></div>))}</div></div>
                    </div>
                </div>
...
app.post("/admin/config/popup", adminGuard, async (c) => { const body = await c.req.parseBody(); const current = await getConfig(); await kv.set(["config"], { ...current, popupImage: body['popupImage'], popupMessage: body['popupMessage'], popupBtnText: body['popupBtnText'], popupLink: body['popupLink'], popupTarget: body['popupTarget'], showPopup: body['showPopup'] === 'on' }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Popup Settings Updated"); });
app.post("/admin/config/maintenance", adminGuard, async (c) => { const body = await c.req.parseBody(); const current = await getConfig(); await kv.set(["config"], { ...current, maintenanceMode: body['status'] === 'on' }); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=config&success=Maintenance Mode Updated"); });
...
app.post("/admin/movie/save", adminGuard, async (c) => { 
    ...
    if (body["mode"] === "create" && existingLive.value && saveType === "publish") { id = crypto.randomUUID(); }
    let timeToSave = Number(body["createdAt"]) || Date.now();
    if (saveType === "publish") { if (!existingLive.value || isMoveToTop) { timeToSave = Date.now(); } }
    const movie = { ...body, id: id, createdAt: timeToSave, episodes, price: parseInt(String(body["price"] || "0")) }; 
    if (saveType === "draft") { await kv.set(["drafts", id], movie); return c.redirect(ADMIN_ROUTE + "/dashboard?tab=drafts&success=Saved to Drafts"); } else { await kv.delete(["drafts", id]); await saveMovieDB(movie as Movie); return c.redirect(ADMIN_ROUTE + "/dashboard?success=Movie Published Live!"); }
});

app.post("/admin/drafts/publish-all", adminGuard, async (c) => { const iter = kv.list<Movie>({ prefix: ["drafts"] }); const drafts: Movie[] = []; for await (const res of iter) drafts.push(res.value); drafts.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0)); let count = 0; for (const m of drafts) { m.createdAt = Date.now() + (count * 1000); await saveMovieDB(m); await kv.delete(["drafts", m.id]); count++; } return c.redirect(ADMIN_ROUTE + "/dashboard?success=All Drafts Published Sequentially!"); });
...
app.get("/admin/backup", adminGuard, async (c) => { const stream = new ReadableStream({ async start(controller) { const encoder = new TextEncoder(); controller.enqueue(encoder.encode("[")); let first = true; for await (const entry of kv.list({ prefix: [] })) { if (!first) controller.enqueue(encoder.encode(",")); const item = JSON.stringify({ key: entry.key, value: entry.value }); controller.enqueue(encoder.encode(item)); first = false; } controller.enqueue(encoder.encode("]")); controller.close(); } }); return new Response(stream, { headers: { "Content-Type": "application/json", "Content-Disposition": `attachment; filename="backup_${Date.now()}.json"` } }); });
