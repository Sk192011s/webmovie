...
interface User { 
    ...
    coins?: number; // 🔥 Balance in Ks
    purchased?: string[]; // 🔥 Purchased Movie IDs
}
...
app.get("/movie/:id", async (c) => {
    ...
    // Logic: ဈေးရှိရင် ဝယ်ပြီးမှကြည့်ရမယ် (VIP ဖြစ်နေလည်း ဝယ်ရမယ်)
    // ဈေးမရှိရင် (0) ဆိုရင် VIP သက်တမ်းရှိရင် ကြည့်လို့ရမယ်
    if (moviePrice > 0) {
    ...
    // canWatch မှသာ Link တွေ ထုတ်ပေးမယ်
    if (canWatch) {
    ...
    return c.html(
      <Layout user={user} title={movie.title} coverUrl={movie.coverUrl} announcement={config.showAnnouncement ? config.announcement : undefined} lang={lang} activeTab="home">
        ...
        {/* 🔥 Purchase Confirmation Modal (အသစ်ထည့်ထားသော Box) */}
        <div id="buy-modal" class="fixed inset-0 z-[100] bg-black/90 hidden items-center justify-center backdrop-blur-sm p-4">
             <div class="bg-[#1f1f1f] p-6 rounded-2xl border border-yellow-500/30 w-full max-w-sm text-center relative shadow-2xl modal-enter">
                  ...
                  <div class="flex gap-3">
                      <button onclick="closeBuyModal()" class="flex-1 py-3 rounded-xl bg-zinc-800 text-white font-bold hover:bg-zinc-700 transition border border-zinc-700">Cancel</button>
                      
                      {user ? (
                          <form action="/api/buy-movie" method="post" class="flex-1">
                              <input type="hidden" name="movieId" value={movie.id} />
                              <button class="w-full py-3 rounded-xl bg-gradient-to-r from-yellow-600 to-yellow-500 text-black font-bold hover:brightness-110 transition shadow-lg">
                                  Buy Now
                              </button>
                          </form>
                      ) : (
                          <a href="/login" class="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-500 transition flex items-center justify-center">Login</a>
                      )}
                  </div>
             </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto">
           ...
           <div class="w-full aspect-video bg-black relative shadow-lg group">
                {/* canWatch ဖြစ်မှ Player ပြမယ်၊ မဟုတ်ရင် Cover ပုံပဲပြမယ် (Overlay အမည်းကြီးမရှိတော့ပါ) */}
                {canWatch ? (
                    <>
           ...
           <div class="p-6">
               ...
               {/* Info Tags - Price ကို Ks နဲ့ပြမယ် */}
               <div class="flex items-center gap-3 text-xs text-gray-400 mb-6">
                   ...
                   {moviePrice > 0 ? (
                       isPurchased ? 
                       <span class="bg-green-900/30 text-green-400 px-3 py-1 rounded-full border border-green-500/30 font-bold"><i class="fa-solid fa-check"></i> Purchased</span> :
                       <span class="bg-blue-900/30 text-blue-400 px-3 py-1 rounded-full border border-blue-500/30 font-bold">{moviePrice} Ks</span>
                   ) : (
                       <span class="bg-zinc-800 px-3 py-1 rounded-full">Free</span>
                   )}
               </div>
                       ...
                       <div class={`grid gap-2 ${movie.streamUrl2 ? "grid-cols-2" : "grid-cols-1"}`}>
                            ...
                            <button 
                                onclick={canWatch 
                                    ...
                                    : `openBuyModal('${moviePrice}', '${movie.title.replace(/'/g, "\\'")}')`} 
                            ...
                            >
                                <i class={`fa-solid ${canWatch ? 'fa-play' : 'fa-lock'}`}></i> {t.server1} {canWatch ? "" : "(Buy)"}
                            </button>
